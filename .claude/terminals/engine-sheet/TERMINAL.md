...
- **Never offer a hand rewrite of the world as the fix for an engine defect** — no rebase, resize, backfill or "one-time correction" of citizens, hoods or any lived number so the code reads cleaner (SIM_DOCTRINE §1, §8, §16). What the engine produced is canon. The fix is the rule going forward, and a wrong state is corrected by causes acting over cycles, designed with the builder. Offered 2026-09-20 on the four small hoods (engine.249) and rejected as canon-breaking; do not re-offer it in any wrapper.
