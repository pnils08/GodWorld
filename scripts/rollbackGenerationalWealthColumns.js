/**
 * ============================================================================
 * ROLLBACK: GENERATIONAL WEALTH & INHERITANCE (Week 2)
 * ============================================================================
 *
 * Removes columns added by addGenerationalWealthColumns.js migration.
 *
 * WARNING: This will delete all wealth and inheritance data.
 * Use only if you need to completely roll back the Week 2 enhancement.
 *
 * Columns removed:
 * - Simulation_Ledger: WealthLevel, Income, InheritanceReceived, NetWorth,
 *   SavingsRate, DebtLevel (6 columns)
 * - Household_Ledger: HouseholdWealth, HomeOwnership, HomeValue,
 *   MortgageBalance, SavingsBalance, DebtBalance (6 columns)
 * - Family_Relationships: InheritanceAmount, InheritanceCycle (2 columns)
 *
 * Usage:
 *   node scripts/rollbackGenerationalWealthColumns.js
 *   node scripts/rollbackGenerationalWealthColumns.js --dry-run
 *
 * ============================================================================
 */
...
async function main() {
  ...
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════════════╗');
  console.log('║   ROLLBACK: GENERATIONAL WEALTH & INHERITANCE (Week 2)           ║');
  console.log('╚════════════════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  if (dryRun) {
    ...
    console.log('⚠️  WARNING: This will DELETE all wealth and inheritance data!');
  ...
  Object.keys(results).forEach(sheetName => {
    console.log(`  - ${sheetName}: ${results[sheetName].removed}`);
  });

  if (dryRun) {
    console.log(`\n🔍 DRY RUN: Would remove ${totalRemoved} columns`);
    console.log('\nRun without --dry-run to apply rollback');
  } else {
    console.log(`\n✅ Rollback complete!`);
    console.log('\nNext steps:');
    console.log('1. Remove generationalWealthEngine.js from Apps Script');
    console.log('2. Remove processGenerationalWealth_() call from Phase 05');
    console.log('3. Restore householdFormationEngine.js income estimates');
