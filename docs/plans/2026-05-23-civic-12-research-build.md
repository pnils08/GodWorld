...
| **G-R4** agent-memory permission | Investigation work: are `.claude/agent-memory/*/MEMORY.md` writes intentionally blocked (safety boundary) or accidentally restricted? If intentional, remove memory-write step from RULES.md across 7+ voice agents; if accidental, fix permission setting. Either path is broader than this session's scope. |
| **G-PREP1** auto-investigate move | Needs deeper design rethink: is the right fix (a) post-`/city-hall` audit step, (b) gate on `/city-hall` completion, (c) reframe "no current-cycle entry" disposition? Each has different downstream effects on the disambiguation logic civic.11 closed (S216). Worth more thought before changing. |
