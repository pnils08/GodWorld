...
(function () {
  ...
  ok(K({ stage: '' }).clock === null && K({ blocked: 'no-delivering-gate' }).reason === 'gate-no-delivering-gate' && K({ blocked: 'no-delivering-reader' }).clock === null, 'stall: no clock on a legacy row, an unbuilt exit or an unreadable one (ruling 3)');
  ...
  ['', 'Proposed', 'Funded', 'Standing', 'Delivering', 'Bogus'].forEach(function (st) { ['operational', 'vote-ready', 'stalled', 'blocked', ''].forEach(function (ph) { [null, 'no-delivering-gate'].forEach(function (bl) { [[100, 100], ['', 100], [110, 100], ['', '']].forEach(function (w) { [100, 105, 106, 112, 113, 130].forEach(function (cy) {
    ...
    var rv = { stage: st, phase: ph, priorPhase: w[0] ? 'x' : '', stallCycle: w[1], lastWorkCycle: cy };
  ...
  fireAt(safe, 130);
  ok(safe[ix('ImplementationPhase')] === 'dispatch-live', 'stall: a Standing row with no delivering gate runs no clock (ruling 3 as written — under ruling e this exempts neglect too; recorded)');
  ...
  ok(delivered[ix('ImplementationPhase')] === 'stalled' && delivered[ix('Stage')] === 'Delivering' && dh.st === 128 && dh.first === 115 && dh.obs === 127, 'stall: a neglected Delivering row stalls at the 13th untended Cycle; Stage, first-delivered and the counted observation survive in the same cell');
