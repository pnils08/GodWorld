...
POOLKEY SELECTION (sandbox-proven S336): eligible ledger rows retain their total effective mass against hardcoded entries, but that mass is divided evenly across eligible PoolKeys immediately before each draw; row weights still rank entries within a PoolKey. Bench 0720 groundhog C103 held the row cap at 40 and drew 101 distinct rows across 995 total ledger draws. No added RNG or event-volume change. PoolKeys remain additive-only and do not suppress hardcoded blocks.
REMAINING DEPTH: aggregate trajectory conditions and ledger-native Baylight/Tribune pools are designed in [[2026-07-01-persistence-seams-content-ledger]] Task 12 but not built; exclusive/migrate policy still needs an exact hardcoded replacement target.
