...
- **PRE-C93 ENGINE HANDOFF: civic phase-vocabulary parity (engine-sheet terminal) — S170, no reference doc.** Three small fixes surfaced by the C92 city-hall close. None are blocking E92 publication; all must land before C93 engine runs or civic voice signal degrades quietly for one cycle.
  - **ADD `'vote-ready': 0.15` to `PHASE_INTENSITY` in `phase02-world-state/applyInitiativeImplementationEffects.js:114`.** Origin: C92 Transit Hub (INIT-003) became the first initiative in 88 cycles to advance phase — Elena Soria Dominguez certified 7/8 CBA requirements met, 8th closing Oct 31, Vega calendared Nov 8 vote, so the voice agent flipped `vote-scheduled → vote-ready` in the trackerUpdate and `applyTrackerUpdates.js 92 --apply` wrote it live. The corresponding `PHASE_SENTIMENT` key (sentiment aggregator in `scripts/applyTrackerUpdates.js`) was added in the same S170 patch at 0.3. The engine's `PHASE_INTENSITY` dict never got the parallel key, so Transit's flip scores `undefined → 0` intensity next cycle. Not actively harmful (vote-scheduled was also 0), but the flip should mean something — a certified, calendared vote is structurally different from a vote deferred again. 0.15 is the suggested value (between `vote-scheduled: 0` and the real-work phases; smaller than `visioning-complete: 0.15` would also be defensible — pick the calibration you like, they're both small). One-line add. Verify against `PHASE_SENTIMENT` in `scripts/applyTrackerUpdates.js:178` to keep the two dicts aligned.
  - **HYGIENE: reconcile `pilot_evaluation` (underscore) vs the hyphen-convention in both PHASE dicts.** Initiative_Tracker row 3 stores INIT-002 OARI's ImplementationPhase as `pilot_evaluation`. Neither `PHASE_INTENSITY` nor `PHASE_SENTIMENT` has that key (they use `pilot-active`). The C92 sentiment aggregator's current-phase fallback read it, found no match, didn't score it — so OARI's full-throated C92 decision set (Rivers motion, Tran condition, DA legal items, framework published with C94/C95 clocks) contributed 0 to civic sentiment. Either add `'pilot_evaluation'` as an alias in both dicts, or normalize the tracker column to `pilot-active` (one-cell sheet edit + whoever writes this phase value in the engine needs the matching change). Low priority — OARI is still rippling through the tracker-read channel (Channel 1); only the sentiment-compound bonus (Channel 2, +25% on civic-media alignment) is missing.
  - **HYGIENE: INIT-007 Youth Apprenticeship has blank ImplementationPhase.** C92 is the first cycle anything moved on it — Mayor tied it to NBA arena CBA, OPP filed implementation legislation, `applyTrackerUpdates.js` wrote the MilestoneNotes + NextScheduledAction + NextActionCycle=93 but the Phase column stays empty. Voice agents didn't propose a phase value because the initiative was previously `(none) — announced`. Engine-sheet decision: should `announced` be the default for INIT-007 going forward (scores 0 in both dicts), or does this initiative need a distinct phase name like `legislation-filed`? If the latter, add to both dicts. If the former, engine-sheet can set row 8 ImplementationPhase to `announced` directly on the sheet.
  - **Why it's in this doc and not a plan file:** Three small parity touches, scoped to two files (one engine, one sheet column, optionally one dict). No design question, no architecture decision — just the follow-through from the civic terminal's C92 work. If engine-sheet wants to batch with other post-E92 repair, fine; if they do it as a one-off before C93 boots, also fine. Removed from this list once landed.
  - **Related files:** `output/production_log_city_hall_c92.md` Tracker Updates Applied section (lists the same flags); `output/civic_sentiment_c92.json` (shows `decisionsScored: 4` of 6 — the 2 missing are INIT-002 and INIT-007); `output/city-civic-database/clerk_audit_c92.json` (Clerk's re-audit notes the same three items as informational flags).
