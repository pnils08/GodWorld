...
// loadRecentReflections REMOVED S340. It read `### Nightly Reflection` blocks
// out of JOURNAL.md, which froze to archive at S300 — so it returned entries
// that stopped growing on 2026-07-11 while Mags kept reflecting on her citizen
// page. Its last live consumer, mags-discord-bot.js, repointed at the page
// itself (loadPageReflections → citizenPage.recentPage_(POP-00005)) in that same
// freeze, leaving this with zero callers. The page is the source now; anything
// wanting her recent inner life uses citizenPage or scripts/magsPageRecall.js.

/**
 * Env-tag check: warn (don't die) if base_context.json was built from a
 * different sheet than this process targets — sandbox run clobbering the
 * prod file (S306 mags-bot incident). Bot keeps running on stale context.
 * @param {Object} raw - Parsed base_context.json
 */
function warnIfForeignBaseContext(raw) {
  var fileSheet = raw && raw.source && raw.source.sheetId;
  var envSheet = process.env.GODWORLD_SHEET_ID;
  if (fileSheet && envSheet && fileSheet !== envSheet) {
    console.warn('[mags] WARN: base_context.json was built from sheet ' + fileSheet +
      ' but GODWORLD_SHEET_ID is ' + envSheet + ' — world state may be sandbox data.');
  }
}
...
function loadWorldState() {
  // engine.76 W3: output/world_state.json is the single cron artifact — built
  // every cycle by scripts/buildWorldState.js, deterministic orientation block.
  // Preferred over parsing the world_summary markdown head (format drifts).
  try {
    var wsJson = JSON.parse(fs.readFileSync(path.join(ROOT, 'output', 'world_state.json'), 'utf-8'));
    var o = wsJson.orientation || {};
    if (o.cycle) {
      var wl = [];
      wl.push('## Oakland right now — Cycle ' + o.cycle + ' (orientation — look deeper if you want)');
      wl.push('Season: ' + (o.season || '?') +
        (o.holiday && o.holiday !== 'none' ? ' (' + o.holiday + ')' : '') +
        ' | Weather: ' + ((o.weather && o.weather.type) || '?') +
        ((o.weather && o.weather.temp != null) ? ' ' + o.weather.temp + 'F' : '') +
        ' | Sentiment: ' + (o.citySentiment != null ? o.citySentiment : '?') +
        (o.cycleWeight && o.cycleWeight !== 'normal' ? ' | Weight: ' + o.cycleWeight : ''));
      if (o.cycleWeightReason) wl.push('Why: ' + o.cycleWeightReason);
      if (o.sportsSeason) wl.push('Baseball: ' + o.sportsSeason + '.');
      var cn = wsJson.canon || {};
      if (cn.statusAlerts && cn.statusAlerts.length > 0) {
        wl.push('Alerts' + (cn.stale ? ' (as of c' + cn.sourceCycle + ')' : '') + ': ' +
          cn.statusAlerts.map(function (a) { return a.name + ' (' + a.title + ') — ' + a.status; }).join('. '));
      }
      if (cn.pendingVotes && cn.pendingVotes.length > 0) {
        wl.push('Pending' + (cn.stale ? ' (as of c' + cn.sourceCycle + ')' : '') + ': ' + cn.pendingVotes.map(function (v) {
          return v.name + ' — Cycle ' + v.voteCycle + ' (' + v.projection + ')';
        }).join('. '));
      }
      return wl.filter(function (l) { return l; }).join('\n');
    }
  } catch (wsJsonErr) {
    // fall through to world_summary head
  }

  // Fallback 1: live per-cycle world summary (build-world-summary writes
...
function loadCitizenKnowledge() {
  ...
  try {
    var canon = {};
    if (fs.existsSync(contextPath)) {
      var ctxRaw = JSON.parse(fs.readFileSync(contextPath, 'utf-8'));
      warnIfForeignBaseContext(ctxRaw);
      canon = ctxRaw.canon || {};
    }
...
/**
 * Current sim year — the ENGINE calendar's arithmetic (engine.148, S412, builder-direct):
 * cycles 1–52 = 2040, 53–104 = 2041, 105–156 = 2042; the year steps at cycle 53, 105, 157…
 * Same formula as simYearFromCycle_ in phase01-config/advanceSimulationCalendar.js.
 * (The S252 version, 2041 + floor(cycle/52), sat one year ahead of the engine on every
 * cycle — 2043 vs 2042 at C105 — so every Discord age read a year old.)
 * Cycle read from the newest world_summary_c{NN}.md.
 * @returns {number} the in-world year (>= 2040)
 */
function currentSimYear() {
  try {
    var wsFiles = fs.readdirSync(path.join(ROOT, 'output'))
      .filter(function (f) { return /^world_summary_c\d+\.md$/.test(f); });
    if (!wsFiles.length) return 2040;
    var maxCycle = Math.max.apply(null, wsFiles.map(function (f) { return parseInt(f.match(/c(\d+)/)[1], 10); }));
    return 2040 + (Math.ceil(Math.max(1, maxCycle) / 52) - 1);
  } catch (e) {
    return 2041;
  }
}
...
function searchDisk(query, maxFiles) {
  ...
  var files = [];
  var snippetPat = terms[0];
  try {
    // engine.92 (S349) parity with MCP disk_search: exact-phrase first
    // (fixed-string, so punctuated names aren't regex), AND-of-terms fallback.
    // MCP gained the AND fallback at S345 "same semantics as searchDisk";
    // this closes the drift from the other side.
    if (terms.length > 1) {
      try {
        var phraseOut = execFileSync('grep', [
          '-rIliF', '--include=*.json', '--include=*.jsonl', '--include=*.md',
          '--include=*.txt', '--', q, 'output', 'docs', 'editions'
        ], { cwd: ROOT, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
        files = phraseOut.split('\n').filter(Boolean);
        if (files.length) snippetPat = q;
      } catch (e) {
        files = []; // no exact-phrase hits — fall through to AND-of-terms
      }
    }

    if (files.length === 0) {
      // First term search (case-insensitive)
      var out = execFileSync('grep', [
        '-rIli', '--include=*.json', '--include=*.jsonl', '--include=*.md',
        '--include=*.txt', '--', terms[0], 'output', 'docs', 'editions'
      ], { cwd: ROOT, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
      files = out.split('\n').filter(Boolean);

      // AND filter for subsequent terms
      for (var i = 1; i < terms.length; i++) {
        if (files.length === 0) break;
        try {
          var nextOut = execFileSync('grep', ['-Ili', '--', terms[i]].concat(files),
            { cwd: ROOT, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
          files = nextOut.split('\n').filter(Boolean);
        } catch (e) {
          files = []; // one of the keywords had no match in the remaining files
          break;
        }
      }
    }
  ...
  files.sort(function (a, b) { 
    ...
    // If same rank, sort by extracted cycle/edition number (descending)
    var ca = a.match(/_c?(\d+)(?:_|\.)/);
    var cb = b.match(/_c?(\d+)(?:_|\.)/);
    var numA = ca ? parseInt(ca[1], 10) : 0;
    var numB = cb ? parseInt(cb[1], 10) : 0;
    if (numA !== numB) return numB - numA;
    
    return b.localeCompare(a); // Fallback descending alphabetical
  });
  ...
  top.forEach(function (rel) {
    try {
      var ex = execFileSync('grep', ['-m1', '-niF', '--', snippetPat, rel],
        { cwd: ROOT, encoding: 'utf-8', maxBuffer: 1024 * 1024 }).trim();
      var ageNote = '';
      var bm = ex.match(/"BirthYear"\s*:\s*"?(\d{4})"?/);
      if (bm) { var yr = currentSimYear(); ageNote = ' [AGE ' + (yr - parseInt(bm[1], 10)) + ' — born ' + bm[1] + '; it is ' + yr + ' in Oakland]'; }
...
function searchSupermemory(query, limit, timeoutMs) {
  ...
  // shape against the 'mags' container, shared chunk-joining shaper, ''
  // fail-soft. Signature and defaults unchanged.
  if (!SUPERMEMORY_KEY) return Promise.resolve('');
  return require('./supermemory').searchContext(query, {
...
module.exports = { ROOT, MAGS_DIR, LOG_DIR, FAMILY_POP_IDS, currentSimYear, loadIdentity, loadJournalTail, loadWorldState, loadCitizenKnowledge, loadArchiveKnowledge, loadEditionBrief, loadLatestEdition, searchDisk, loadNotesToSelf, loadTodayConversationDigest, getCentralDate, searchSupermemory, saveToSupermemory, loadFamilyData };
