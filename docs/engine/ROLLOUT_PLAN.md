...
6. **Phase 39.2–39.7** — Three-lane scope redesign + Final Arbiter. **PLAN WRITTEN S146** as §§13–20 of `docs/engine/PHASE_39_PLAN.md`. MIA three-lane prompts + Microsoft UV process/outcome split + two-pass hallucination evidence pattern, all verbatim-grounded. Six sub-items: Rhea narrows to Sourcing Lane only (5 checks, weight 0.3); cycle-review narrows to Reasoning Lane only (3 checks, weight 0.5); Mara narrows to Result Validity Lane (3 checks, weight 0.2) via her claude.ai system prompt; process/outcome + controllable/uncontrollable fields added to all four lane outputs; two-pass hallucination inside Rhea (deferred until Anthropic API key wired); Final Arbiter agent + script consume all four lanes plus capability gate, output single A/B verdict with blame attribution, plug into write-edition Step 5.5 before publish. Replay against E91 stays the validating acceptance: full chain still catches Temescal miss + new fabricated-stat fixture flushes through two-pass detection. **READY TO BUILD `(research-build terminal)`** — all skill/agent edits, two new JSON-shaping scripts, one new agent. No engine code changes; engine terminal stays focused on spine step 5 (Phase 38.2/3/4).
