...
- **Mechanism — two shapes, pick one before building:**
  1. **Direct-mint dice roll (recommended).** At the same feeder pass that mints new Generic Citizens each cycle (`phase05-citizens/generateGenericCitizens.js`), run a small-probability, capped-per-cycle roll (same shape as `DRIP_CAP_PER_CYCLE` in `processAdvancementIntake.js` — 1-2/cycle, never a flood) over the pool of ownerless private-commercial businesses, weighted by a wealth-tier score (`Avg_Salary` × `Employee_Count`, or `Avg_Salary` alone — pick one, see open questions). A hit skips the GC waiting room entirely and queues the winning citizen straight onto `Advancement_Intake1` — the exact route Task 10/11's six founders just used (Tier 3-4, EmployerBizId set, `Key_Personnel` backfilled post-mint) — under a new bounded exception alongside engine.109's "the family is the reason": **"the business is the reason."** This is the only shape that actually satisfies "probably definitely should" — deterministic and immediate, not a maybe-someday promotion.
  2. **Latent GC tag (rejected as primary, name for completeness).** Tag a newly-minted GC (Tier-5 waiting-room) citizen with an `OwnerCandidateBizId`, weighted the same way; if and only if that specific citizen later promotes via the normal EmergenceCount-3 ladder (media coverage), the promotion also backfills the owner link. Organic, but most GC citizens never get covered enough to promote — Mike's own doctrine 5 (`docs/plans/2026-08-16-new-life-intake.md` §2, "of the two documented promotion routes, media coverage cannot fire at all" pre-engine.108, and even post-fix it's a lottery, not a guarantee) makes this the wrong tool for a "should have an owner" claim. Worth naming so a future session doesn't re-propose it as new.
- **Why this reuses, not duplicates:** the actual income/wealth mechanics need zero new code — `applyOwnerDraw_` (Task 10) already pays any Tier 3-4, non-heritage citizen resolved via `Key_Personnel` regardless of how they arrived. The only new surface is the SELECTION + ROUTING step (which business, which cycle, which citizen), not a second payment mechanism.
- **Open questions (deliberately unresolved — this is a "maybe," not a build order):**
  - [ ] Weighting formula: `Avg_Salary` alone, or a composite with `Employee_Count`/`Annual_Revenue`? The 7-business cluster above is consistent under either, but the cutoff for business #8 onward differs.
  - [ ] Per-cycle cap and overall target: how many of the 104 (or the top-N tier) should ever get an owner? Uncapped eventually reaches all 104, which re-opens the "should every corner shop have a named owner" question Mike explicitly set aside 2026-09-08.
  - [ ] Heritage eligibility: does a dice-selected owner ever roll into a heritage line (Task 11's mechanism), or do they start off-heritage only, same as tonight's six?
  - [ ] Sector exclusion list: confirm the ~60 institutional/public businesses are excluded by an explicit Sector allowlist/denylist, not by income alone (a low-paying but still-institutional Sector shouldn't accidentally qualify, and a high-paying private one shouldn't accidentally get excluded).
- **Status:** [ ] NOT STARTED — design captured 2026-09-08, awaiting Mike's go before Task 5/6/7's engine-sheet builds this.
