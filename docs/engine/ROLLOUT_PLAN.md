...
| engine.133 | `engine-wiring` subagent — one target in, verified WIRING CARD out (callers, phase position, S fields, write path, tabs, rows, git log); headless via runEngineAgent.js; rule in engine rules + guest change protocol. Acceptance + cost in plan §9. | done-pending-archive | engine-sheet | [[../plans/2026-08-22-engine-agent-fleet]] §9 |
