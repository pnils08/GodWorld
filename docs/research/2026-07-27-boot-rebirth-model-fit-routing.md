...
- **Three mechanisms, and every rule needs the right one → classify before consolidating.** S335 produced clean evidence for each:
  1. **Action-necessary → a pointer is genuinely sufficient.** `plans/TEMPLATE.md` and `research/TEMPLATE.md` were both opened unprompted, because a template cannot be filled without opening it. The task requires the file, so the pointer self-enforces. This is the only category where pointers work.
