...
## Next Session Priorities (S432 → next, engine-sheet)

0. **C106 fired live on PROD @64 (2026-09-06 20:31): 133/133 phases, 0 errors, 197.5s** — the whole @46–@64 stack smoked in one fire. engine.90 archive live (54 moved, audit 0/0/0); engine.148 wave minted 8 across 6 hoods; engine.163 intake notes clean. Three HIGH rows out of the C106 gap log: **engine.164** (sim-year literal 2041 sweep — cut first, it's one year wrong at every age site since C105), **engine.165** (C105 sentiment inflation — sift must not run C106's 19 hood decays as stories), **engine.166** (two doubled incomes). Cards from archive done (54 written). Civic seats re-modelled `500e8e25` — Sunday chain is the proof for gemini/llama on the number gate. RUN-CYCLE C106 GAP LOG — S432 (engine-sheet), 53 mechanical + 10 judgment, 3 HIGH mechanical (all the sawtooth) + 3 HIGH judgment.
1. ~~**Live smoke of PROD @58** at the builder's next fire~~ DONE C106 (see 0). Original text kept for the record: **Live smoke of PROD @58** at the builder's next fire — thirteen deployments (@46–@58) ride it, all bench-proven on live-synced C105. engine.148 expectations: World_Config +6 rows self-armed; 6 'migration wave' ledger rows in the emptiest hoods, women first; feeder adds up to 8 women; 22 Crime_Metrics rows refreshed; no throw on Neighborhood_Map AE–AI (all cells read back); a label-pool texture line on a ten-hood citizen (P3). Earlier waves' expectations on their rows. **Scenes behavioural proof** waits for the first First Friday / hosted-holiday cycle: KONO in the arts spotlight, crowd boosts on tagged hoods, an arts/festival bond in a tagged hood.
2. ~~**engine.148 follow-through**~~ DONE — swept to ROLLOUT_ARCHIVE at the S432 close on the C106 smoke; the plan's Findings (wave roles price at 60000 in employer-less hoods) stays filed, not a task.
...
| # | Item | State | Terminal | Pointer |
|---|------|-------|----------|---------|
| engine.164 | Sim-year literal 2041 swept to `simYearOf_` at 17 engine sites (`5337fbff`, PROD @65) and the readers now follow the calendar too (`c0f5c4c5`, builder: "it's 2042 in the sim now"); bench C107/C109 clean; live smoke = the builder's C107 | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @65 + @67; C106 gap log G-EC59 |
| engine.165 | Sawtooth = the New Year (+0.4 city term at C105 reaching every hood); Neighborhood_Map Sentiment now clamped to [-1, 1] on save (`a1b4e9d5`); bench C108 clean; PROD @66. C105 up / C106 down = the holiday ending, not decay | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @66; C106 gap log G-EC58 |
| engine.174 | The wave could only surface who waited in a hood's own pool line, empty where the map was thin, while the feeder skipped on the citywide floor. Feeder now refills per short hood, one queue for both floors. Bench C116: Glenview 0→4, East Oakland 1→3; PROD @72 | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @72; civic.21 remainder |
| engine.173 | Move-up gate 2.5× → 1.5× the hood median (`MISFIT_INCOME_RATIO`). At 2.5× only 9 of 699 working adults ever cleared it; at 1.5× ~54 enter the lane and the fit score already points them at the empty mid-priced hoods. Builder: "I'll take your advice." Bench C114; PROD @71 | done-pending-archive | engine-sheet | [[../research/2026-09-07-economy-true-up]] §3; [[../reference/DEPLOY_HISTORY]] §PROD @71 |
| engine.172 | The life places the citizen in the job's band: years (spine), credential vs the field's expected, drive, Tier-3, employer growth, seeded ±5% (`77653f25`). Two plumbers never make the same money and the reason is on their row. Bench C113 25/25 at the predicted figure; PROD @70 | done-pending-archive | engine-sheet | [[../research/2026-09-07-economy-true-up]] §3; [[../reference/DEPLOY_HISTORY]] §PROD @70 |
| engine.169 | Employer floor now applies the company average only to jobs in its own sector; an out-of-sector job floors at the lower of its band and the employer (never past either signal). Forward-only: the 88 already above band stay (raise-only). Bench C111; PROD @69 | done-pending-archive | engine-sheet | [[../research/2026-09-07-economy-true-up]] §2–3; [[../reference/DEPLOY_HISTORY]] §PROD @69 |
| engine.170 | Matcher was already sector-gated (S336/E3); the gap was a field-change hire keeping its old RoleType. A hire into a new field now takes that field's entry job, tag and band. Ledger job/employer mismatches are seeded, not the matcher. Bench C111; PROD @69 | done-pending-archive | engine-sheet | [[../research/2026-09-07-economy-true-up]] §3; [[../reference/DEPLOY_HISTORY]] §PROD @69 |
...
| engine.168 | The catalog listed owners/professionals, not the jobs the engine mints (256/676 rows matched); unlisted roles priced at an owner-field median (a 101k janitor). 108 working roles added at the catalog's low bands (`27f9c662`); coverage 495/676; bench C110; PROD @68 | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @68 |
| engine.166 | Builder ruling 2026-09-07: pay comes from the JOB, never the hood. `jobReferencePay_` prices from the economic catalog (`e5b56b0a`), hood-reference pay retired; the two doubled incomes were the old floor reading a stale tag. Bench C109; PROD @67. Live cells at C104 values | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @67; C106 gap log G-EC60 |
...
| engine.107 | ARC milestone EventText not person-readable — absorbed into engine.38 B3 (S272 reversal; `stampPromotion_` person-readable S327). State cell repaired S436: the absorption note sat in the state column and the sweep skipped the row | done-pending-archive | research-build | [[archive/ENGINE_REPAIR]] Row 32 (migrated S371) |
...
| civic.21 | Civic edge truth migration — ten-hood emergence remainder. Closed by engine.174 (the feeder refills per short hood) on top of the engine.148 wave: the GC→ledger door is open and its line is refilled where the map is thin | done-pending-archive | engine-sheet | [[../plans/2026-08-15-civic-edge-truth-migration]] §11.3b |
