...
- **engine.141 opened** — `monitorStorylineHealth_` still reads the dead tracker and publishes `S.storyHooks` + `S.storylineHealth` into ctx. The one live path still carrying that tab's data into the world; needs its own caller-graph pass. **CLOSED S419 (2026-09-04): retired, not repointed.** Live `Storyline_Tracker` read directly: 239 rows, Status=abandoned on every one (types active/new/thread/…, 79 high/urgent priority, `LastMentionedCycle` blank throughout). `loadActiveStorylinesForHealth_` filters to Status=active → the monitor returned at 0 processed every cycle; `S.storylineHealth` had zero readers in engine/lib/scripts. `storyHook.js` `loadStorylines_` fed four hook kinds (dormant/priority/mystery/density) — all gated on status=active/dormant or type mystery/question, none of which exist → 0 storyline-* hooks. Repointing at `Storyline_Ledger` (23 open rows, FirstCycle/LastCycle/Desks/Citizens/Hoods) would duplicate what `buildWorldSummary` v1.2 `openThreads` already hands the desks newsroom-side, so the engine channel is closed instead. Cut: `storylineHealthEngine.js` (13 fns) + `updateStorylineStatusv1.2.js` (4 fns, 0 callers, `generateStorylineBriefingSection_` included) deleted; `storyHook.js` 130-line tracker block removed (its `S.activeStorylineCount`/`S.dormantStorylineCount` writes had no reader; `applyStorySeeds.js:726` still sets the former); `Phase8-StorylineHealth` slot commented at both entry points with the reason; 168 → 166 files; collisions 0; ctxMap 0 undeclared. Docs trued: ENGINE_MAP, EVENT_SYSTEM_MAP, SHEETS_MANIFEST (§tab + §9 rows), TRUTH_MAP, COUPLING_MAP, CODEBASE_MAP, SPREADSHEET.md; four `scripts/` labels. Out of scope, noted: the tracker's legacy WRITERS (`storylineWeavingEngine` intent append, `applyStorySeeds` reads at :170/:545, `mediaRoomIntake` tab-create) still touch the discontinued tab — a separate row if the tab is ever to be frozen for real.
...
**Landed:** every one of the 29 carries a first-line `// @cycle-status:` header naming what invokes it or when and why it was retired; ctxMap reads the tag and now reports **UNDECLARED** dead files as the signal (0 today) with the declared set listed for the record. A new dead file is a real question again. Deletions filed as **engine.142** (six files, zero live references) — the cost is ~15 truth-doc rows, not the rm, so it is its own row. **DONE S419 (2026-09-04):** the six deleted (`eventArcEngine.js`, `processIntakeV3.js`, `parseMediaIntake.js`, `processArcLifeCyclev1.js`, `mediaRoomBriefingGenerator.js`, `v3LedgerWriter.js`; 36 functions, 174 → 168 live files). Pre-cut proof: bare-identifier caller graph over phase/utilities/lib/scripts/.claude — 0 external references; `auditFunctionCollisions` 0 (no deleted name was shadowing a live definition); `ctxMap` 0 undeclared. One truth-doc lie caught on the way: ENGINE_MAP listed live `Phase10-BondLedger` under `v3LedgerWriter.js` — the writer is `bondEngine.js:2621`, row corrected. 16 docs trued (ENGINE_MAP, TRUTH_MAP, COUPLING_MAP, SHEETS_MANIFEST §9, CODEBASE_MAP ×2, GODWORLD_REFERENCE, TIER_MOBILITY, DOCUMENTATION_LEDGER, SIMULATION_LEDGER writer column: `processIntakeV3` → `processIntake_`), STUB_MAP/REVERSE + COL_MAP regenerated, the two `godWorldEngine2.js` disable notes updated. Left for their owners: `.claude/agents/engine-validator/IDENTITY.md:58` (illustrative example names the deleted arc file), `.claude/skills/pre-mortem/SKILL.md:121` (already says retired). Deploy: bench @42 → C114 → PROD (see DEPLOY_HISTORY). `updateStorylineStatusv1.2.js` stays with engine.141; `generateChicagoCitizensv1.js` stays for the frozen pool.
...
## Changelog
- 2026-09-05 (engine-sheet, S423) — **Row recheck: both open items were already closed by the record.** (1) G-PF33 clear-branch fix went LIVE at PROD @15 (S410 late) and v2.1 at @16 — DEPLOY_HISTORY line 33; the row's "push after the live C106 smoke" was stale. (2) S-B wake acceptance MET: 15 `output/cron-compare/*.state.json` carry `openThreads`, and eight articles dated 09-02..09-04 render `STORYLINE: <slug> | advanced` (Carmen Delaine, Lila Mezran, Luis Navarro, Elliot Graye, Kai Marston, Sharon Okafor, Hal Richmond, P Slayer). No `closed` yet — reachable, not yet earned. The Saturday 16:00 cron's Storyline_Ledger write is the remaining unattended proof and is not a gate. Row → done-pending-archive.
- 2026-09-01 (engine-sheet, S410) — **G-PF33 v2.1 + a correction to §S-A amendment 1.** (a) Builder call: `passed`+`signed` rows are now held BEFORE the v1.7 skip (`applyEngineClockHold_`, shared with the main seam) — bench C107: INIT-002/INIT-006 stamped `[ENGINE-CLOCK n=1 from=C107]`, clocks 105 → 108, all six rows held. Live at PROD @16 after the C108 bench read. (b) The "expiry cycle itself is unprotected" claim above is **wrong on the evidence**: `civicInitiativeEngine_` writes `Initiative_Tracker` own-tab in Phase 5 (`:529 setValues`, MANIFEST §9), and `updateCivicApprovalRatings_` reads the sheet after it, so a held clock IS read the same cycle. Bench arithmetic this session: C106 Santana 69 → 55 = held 001/003 sitting (−2 −1) + un-held 002/006 silence (−6 −3) + 005/007 sitting (−1, 0) + decay −1 = −14 exactly; C107 55 → 50 = six rows sitting (−2 −1 −1) + decay −1 = −5 exactly. The earlier "silence ×6 in the same cycle as n=1" reading came from the clear-branch bug (amendment 3), not from write ordering.
