...
- **Determinism intact:** classification is input-side (outside the cycle, same class as media intake); only the categorical tag → `applyTaggedEvent_` pure lookup touches dials. Prose never touches the engine. **GATE still holds: no write-back wired until the Phase-1 daily audit signs off.**
