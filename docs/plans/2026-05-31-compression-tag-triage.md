...
- **Phase 0 — Exhaustive writer/tag inventory + uniform-tagging (prerequisite, read-then-fix).**
  Complete the per-writer tag sweep (S250 grep got the shape: ~40–50 tags incl. the unmapped health cluster; some tags are concatenated, not literal → needs care). Fix the 3 raw-line writers (promotion / arrival / career `lifeOut`) and any others to emit a uniform `[Tag]` so every event routes. Map every emitted tag → one of the 5 categories. *(Absorbs old T1 route-unmapped + T2 dead-mappings + T3 tag→category.)*
