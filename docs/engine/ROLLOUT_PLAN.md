...
| engine.184 | DemographicMarker is scored per hood, not once for the city: `v3NeighborhoodWriter` reads `S.neighborhoodMigration[hood].drift` inside the hood loop; `applyMigrationDrift` per-hood thresholds become ratios of the cycle's own cross-hood median (engine.38 B2 pattern) and the city term drops from `drift/8` to `drift/25` — BENCH-PROVEN S441 on 0908 @8 / C111 — MigrationFlow un-pinned (was 5x19) and 6 distinct labels across 22 hoods (was 1) | done-pending-archive | engine-sheet | [[../plans/2026-09-09-health-slice-freshness]] §criterion 4 |
| engine.185 | **Sentiment is an inert control signal.** Live city sentiment has never gone below −0.02 across 37 recorded cycles (C69–C106, min −0.02 / max 1.0), but 62 comparison sites across 37 engine files gate on `sentiment <= -0.2 … -0.5` — every downside pathway (crisis spikes, faith events, world events, promotions, bonds, relationships, education, household, career, neighborhood, evening media, nightlife, story hooks, texture triggers) sits behind a door that has never opened. The mirror holds too: `sentiment >= 0.2` gates (applyStorySeeds:1311, chicagoSatellite:392) are always-true against a live min of 0.23. Sentiment triggers nothing down and everything up. **ROOT CAUSE FOUND — it is not the 62 thresholds, it is the one signal feeding them.** `phase02-world-state/applyCityDynamics.js` carries 54 upward sentiment pushes totalling **+10.26** against 29 downward totalling **−2.10** — 4.89x more upward force. The clamp allows −1; the forces never get there. Largest single downward push in the whole engine is −0.20; New Year's Eve alone is +0.50. Adversity is priced at nothing: unemployment >12% = −0.15, crime>=3 = −0.12, economic mood <=30 (a depression) = −0.10, shocks>=3 = −0.08, a business struggling = −0.04, hood illness >=10% = −0.04. A holiday outweighs twelve epidemics. BUILDER DIRECTION 2026-09-10: the world can have bad days — prosperity-era bars imported real-world Oakland cynicism, not adversity ([[project_real-life-is-not-real-world-oakland]]). FIX SHIPPED `738256d0` + fix-up (S441): repriced onto severity tiers; first pass OVER-SHOT because `shocks>=3`/`crime>=3` were always-on activity counts (true 19/19 cycles) — now relative to their own 6-cycle baseline, at-or-below costs nothing; hood sentiment `Math.max(0)` floor removed (v3NeighborhoodWriter:433). Real-condition tiers (unemployment/sick/mood) behaved correctly on the bench. The 62 `<= -0.3` branches are REACHABLE now (depression+epidemic = -0.62) but UNPROVEN — no real crisis has landed on a bench. Scan S441 | in-progress | engine-sheet | ROLLOUT row (scan output S441) |
| engine.186 | **Arcs peak but never close — the chain has no end or aftermath.** Measured on live `Event_Arc_Ledger` (654 rows, C69–C105, 38 distinct arcs): 95% reach `peak` (36/38), **3% reach `resolved` (1/38)**; phase rows early 361 / peak 252 / rising 36 / **resolved 5**. Arcs oscillate peak<->decline (~6.6 peak-transitions per arc) instead of completing. CAUSE — asymmetric thresholds in `generateCrisisBuckets.js`: onset needs `>= ONSET_CHANNELS (2)` bad channels, but resolution needs **zero** bad channels on two consecutive cycles (`:246-262`). A hood that is chronically slightly-bad (1 channel) bleeds tension but never gets the clean cycle, so it never closes. ALSO starving onset: the sentiment channel (`abs <= 0.15` vs live min 0.23) is dead and retail (`abs <= 5.0`) fires for 1 of 22 hoods — 2 of 7 channels cannot contribute, and onset needs 2. Only ONE new arc since C100. Aftermath machinery is READY and unused: the ledger carries `ResolutionType`/`ResolutionCycle`/`ArcAge` back to C69 and `scripts/buildDeskPackets.js` feeds arcs to media — but almost nothing closes, so nothing becomes 'that event'. SHIPPED `738256d0` + carry fix `6db160c6` (consecutiveGood was stripped by compactCrisisArcs_ — the rule's carry contract). **BENCH-PROVEN C119: five arcs resolved in one cycle (Rockridge/Ivy Hill/Baylight/San Antonio/Brooklyn), ledger 6 -> 11 resolved across all history; Brooklyn onset C113 -> resolved C119 predicted before the fire.** Start/peak/END work. REFERENCE still open: arcs seed as `CRISIS-<cycle>-<HOOD>` with a metric-dump summary, no name column in the ledger's 32, and no arc has ever reached published copy — builder asked about naming 2026-09-10. BUILDER FRAME: start, peak, end, aftermath, both directions | done-pending-archive | engine-sheet | ROLLOUT row (scan S441) |
| engine.187 | **Shock detector is stuck-on.** `S.shockFlag` = `shock-flag` on 19 of 19 cycles measured (C95–C113). `applyShockMonitor.js:186-192` event-spike gate is already relative (engine.38 style), so one of its OTHER gates (severity spike, …) fires unconditionally. Downstream readers that inherit the stuck flag: v3NeighborhoodWriter 'Shock event zone' override, applyCycleWeight, applyCityDynamics `shocks` (now gated relatively as a workaround — engine.185 fix-up — and should read S.shockFlag once this is unstuck). Found S441 during the 185 over-shoot diagnosis | needs-info | engine-sheet | ROLLOUT row |
| engine.188 | **Sentiment top end — the resting level climbs toward 1.0 once the tax came off.** After engine.185 removed the always-on −0.48/cycle, bench hood mean went 0.094 (C113) -> 0.68 (C116) -> 0.893 (C119), max pinned at 1.00; city series 0.45 0.59 0.80 still rising. Force tally now: upward 54 sites +10.26, downward 30 sites −4.12, always-on downward 0 — the upward vocabulary was tuned to overcome a tax that no longer exists (equilibrium under 30% momentum carry rose ~0.69). Same defect as 185, other end: no dynamic range at the top; crons/media would read a euphoric city for no reason. FIX: the upward side gets the engine.185 treatment — which of the 54 pushes are always-on, and relative gating for the count-fed ones. **Gates the PROD push of 185/186.** Found S441 | needs-info | engine-sheet | ROLLOUT row |
