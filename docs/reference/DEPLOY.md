...
- Web-app deployment `AKfycbxbfP2Wu6haP4HXUGLk9T4wlW4ixWPsnfZJ51G5wVFTbkhESbT4teJG2xvRowP3BygIkw` — @1 S405 (engine.137 + G-PF18), @2 S409 wave 1, @3 S409 wave 2 = the live @14 tree, **@4 S409 wave 3 = @14 + `civicInitiativeEngine.js` (G-PF33 hold clear-branch fix), bench-proven C108–C110, NOT yet live** (2026-09-01). **WARNING: any valid-token GET fires a FULL cycle — no ping mode.**
- Bench is at C110 after the re-sync (C106 token-fired replica, C107 builder editor run = the execution log, C108–C110 wave-3 proof). Its properties carry C110 memory now — the next replica needs the three-blob delete again.
