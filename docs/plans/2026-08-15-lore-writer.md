...
- 2026-09-06 (S429, research-build) — pipeline.61 closed: the Friday cron is proven unattended (Aug 28 Vinnie Keane → passed + ingested `be3e579c`; Sep 4 Hal Richmond → Gemini 429 ×8, cursor held, retries Sep 11). The Mags Corliss deep-dive was first FAILED on the "engine numbers on sight" rule, then the builder ruled lore is open-book (accurate ledger numbers are the point) — rule rewritten in `lore-ingest` §3, piece re-graded on accuracy (every figure, age, hood and household matched; Rose Delgado POP-00503 is real) and PASSED at Y3C1 into bay-tribune + the staging lane. NotebookLM leg came back degraded (no source id) — re-run that leg when the bridge is healthy.
