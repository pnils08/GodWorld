...
> **⚠ Superseded by deployments (verified 2026-08-04, kimi):** Do NOT cite §1/§3 as current state. By S318–S328 the code shipped past this doc: births mint real Tier-4 child rows (`createChildRow_`, engine.57 P6, reversing S248); dice-weddings are retired in favor of bond-driven `marryCitizens_` (engine.59); `SpouseId` exists (SL col P); the legacy married-without-spouse gap is handled by tier doctrine, not a new rule — family-match drip lottery (engine.66 S324) + GC courtship/marriage materialization (engine.66b), i.e. off-ledger family is BY DESIGN (tier 5 = Generic_Citizens waiting room, materialized on need). engine.5 plan archived at `docs/engine/archive/LEDGER_REPAIR_HOUSEHOLDS.md` is likewise behind code. Real open gaps as of C102: zero romantic bonds live, wakes read bonds but not direct family columns, no bond write-back from interactions, per-engine active-citizen filters still divergent.
