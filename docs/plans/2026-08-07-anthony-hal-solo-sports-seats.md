...
- **Status:** [x] DONE. Mike's call: both are Oaks reporters, dedicated solo seats. Built S429 (research-build) — voice files, agent packages, persona-map, approach-map, wake packages staged under `_pending-*` keys; chicago-desk retired. Engine-sheet found (S430) NO MINT was needed — Selena POP-00591 + Talia POP-00592 were already on the ledger (Talia's RoleType re-beat live). Activated S431 (research-build): pending keys renamed, popids filled, IDENTITY ledger lines corrected, voice-file neighborhoods corrected to the ledger (Selena Chinatown not Jack London; Talia Jack London not West Oakland), test fixture updated, both voices dry-run verified.
