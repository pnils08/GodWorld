...
2. **Rewrite your terminal's `NEXT[<terminal>]:` line** — one line of **resumption-state**: where the work is + what's decided + the next move, with a `(claude-mem: <hook>)` pointer when the thread is rich (a design conversation, a multi-step build). NOT a task stub ("Phase 3" loses the thread; "mid-X, decided Y, next Z (claude-mem: …)" recovers it). claude-mem is the shared backing store — NEXT is this terminal's entry point into it. Identical form soft or hard. Don't touch other terminals' NEXT lines. This *replaces* the old STATUS paragraph (what shipped → git log; open work → ROLLOUT).
