...
| pipeline.62 | Coverage-ratings channel is silently mis-domaining since the edition reformat — `rateEditionCoverage.js` finds C104's 7 articles but resolves every one to `(unknown)` → COMMUNITY and exits 0 (no section headers left in the edition; detector needs a `SECTION_NAMES` line). The v2.1 fail-loud gate only catches articles==0, so it passes. Sheet has no rows since C102 and the script has no cron. Scope: widen the gate to 0-domains-resolved, re-source domain (reporter→domain is already parsed), add the civic-chain-health step to `preflightInputCheck.js` (G-PF9). FIXED 2026-08-31 (S403, Mike-direct: key domain off the reporter): resolution is now section header -> reporter beat -> default, with the beat read from `scripts/persona-map.json` (`beatDomain`, the same file cron-desk-run uses to resolve a byline) rather than a list invented in the rater; ACCOUNTABILITY aliases to CIVIC (only beatDomain that is not a Domain_Tracker column) and any future non-column beat exits 2 rather than writing a domain the engine would silently give DEFAULT_RULE. New domain-resolution gate fails the run when articles resolve to nothing — the case the articles==0 gate missed. Backward compatibility proven empirically: C102 re-rates byte-identical to what the old code wrote (CIVIC -2/2/Navarro, HEALTH 1/1/Mezran, SPORTS 3/2/Raines), all 5 via the section path. 16/16 in `scripts/rateEditionCoverage.test.js`. **C103 (7 domains) + C104 (3 domains) backfilled and verified live; pre-flight for C105 is now clean READY, zero warnings.** Residual CLOSED 2026-08-31: wired as `stepCoverage` in `cron-saturday-run.js`, straight after `stepPublish` (hung off the edition artifact, not a second cron line, so it cannot fire before the file exists or drift onto another cycle); non-fatal by design since publish has already ingested canon, with `preflightInputCheck.js` as the independent backstop. Also fixed there: `arg()` read only spaced flags, so `--step=x` fell through to "run every step" — one keystroke from `--step=publish --apply` running the whole chain against live canon (G-PF13). OPEN residual: `rateEditionCoverage.test.js` is intermittent under the full suite (2 fail / 1 pass; always green in isolation and filtered) — logged G-PF14 for a fresh look, code under test separately proven. | in-progress | engine-sheet | [[../../output/production_log_run_cycle_c105_gaps]] §G-PF7, §G-PF9 |
...
| engine.138 | **This week's chase list** — every anomaly the C105 live fire surfaced, annotated on the execution log itself (`output/execution_log_c105.md`, Drive original `1ewlOWp74NySp-Yll4r7DXzcz-O1a5kTH`). 12 items across engine / civic / editorial, every one riding out on an `Info` line inside a run that logged **0 engine errors** and 130 `ok:true` phases. Verified head of the list: **civic voice sentiment has never reached the live world** — `applyInitiativeImplementationEffects.js:55-64` reads its input through `require('fs')`, which does not exist in Apps Script, so the loader can only ever work under a Node harness; the C104 hearing scored 0.41 across 20 statements and the world felt 0. Carrier must move to a sheet like every other channel. Next: the Mayor lost 13 approval points penalised "sitting, nothing free" on six initiatives during the same cycle four of them ADVANCED (audit counted those four as improvements); `civicInitiativeEngine` processed 0 of 6 live initiatives; all 9 storylines abandoned in one pass with 0 concluded, leaving sift an empty slate; `runCareerEngine` moved 0 businesses across a **0 denominator**; 256 stagnant with 0 education/career/income and adulthood settled 0. Instrumentation debt: 6 identical CIVIC priority clamps, 210/915 commuters unresolved, fractional totalPopulation off a silent fallback source. | ready | engine-sheet | [[../../output/execution_log_c105]] §Annotations |
| engine.137 | Pre-mortem §3 made deterministic, and what it found. `ctxMap.js` gained an ordering pass (read-before-write by phase, orchestrator and same-file reads excluded, fallback-vs-undefaulted flagged) — §3 was "the most important check" and had no tooling, so it was never really run. It immediately surfaced unreachable mechanics: `S.civicLoad` is written P06 (`applyCivicLoadIndicator.js:373`, sole writer) but gates rivalry-arc escalation at `eventArcEngine.js:219-226` in P04; `S.cycleWeight` is written P09 but read at `eventArcEngine.js:143,145` (P04) and `bondEngine.js:710` (P05). `===` against undefined, so nothing throws — the escalation branches simply never take. **Design call needed:** either these want the previous cycle's value (carry-forward must then seed them at Phase 1 — nothing does) or the consumers move after their writers. Long-standing, not a regression (no read-side file changed since 2026-08-19). Also fixed here: pre-mortem scan 2 read its carve-out list from a boot doc that no longer carries the table, so the exception set was empty and all 41 direct writers flagged — repointed at SHEETS_MANIFEST §9, 41 → 0. | ready | engine-sheet | [[../../output/production_log_run_cycle_c105_gaps]] §G-PF15, §G-PF16, §G-PF17 |
| engine.136 | Silent end-of-cycle flush. Root cause was wider than the row said: every write in `sheetCache.flush()` ran bare, so one transient threw out of the whole function and abandoned the rest of the queue for every remaining tab — and `stats.errors` only ever carried "sheet not found", so the loss was invisible (bench C110 lost World_Config.cycleCount that way). Fix: per-write isolation in flush (writes + appends), both call sites report `stats.errors`, `verifyCycleCountPersisted_` reads the counter back off the sheet, and `repairCycleCount_` rewrites it when the stall signature is exact — nothing guards a re-run, so a stalled counter re-runs a cycle over a world already past it. 46/46 node proofs (`utilities/sheetCache.test.js`, `phase01-config/godWorldEngine2.cycleCount.test.js`); bench-proven at SANDBOX 0827 @19 / C115 (cycleCount 114→115 landed, Engine_Errors empty — no false positive from the guard), shipped PROD @11. | done-pending-archive | engine-sheet | [[../plans/2026-08-29-employment-system-cascade]] §Phase E |
...
| civic.26 | Civic Sunday chain halts at mayor-open, so nothing has applied since C104 — `mistralai/mistral-large` returns HTTP 429 (`upstream_provider_shared_pool`, model still listed and valid) and the caller retries twice ~0.4s apart with no backoff and no fallback seat (`scripts/civic-office-map.json:36`). `close_c104.json` stays `applied:false`; INIT-001/005/007 stuck at `NextActionCycle=C104` — the "3 past-due" pre-flight warning. Second, independent blocker on the same apply: `gate_c104.json` fails `engine-verbiage` (council_d3 quoted "0.42 sentiment"), so clearing the 429 alone will not let C104 apply. FIXED 2026-08-30 (S403): cross-family fallback chain + backoff in `callVoice` (a provider 429 takes every slug it serves, so fallbacks are other families, ordered least-used-first to protect voice distinctiveness); provider errors now carry HTTP code + upstream text; `metric-decimal` narrowed to engine-internal vocabulary per Mike-direct — sentiment/approval/severity/tension are metrics every city tracks and must never fail canon. **C104 applied: gate PASS, 6 initiatives written to Initiative_Tracker, verified live, no past-due rows left.** | done-pending-archive | engine-sheet | [[../../output/production_log_run_cycle_c105_gaps]] §G-PF8, §G-PF10 |
...
| infrastructure.7 | Gemini Notebook autonomous deep-research → Drive → daily brief | wontfix | engine-sheet | [[../plans/2026-08-02-notebooklm-deep-research-integration]] — CLOSED 2026-08-20 Mike-direct; canon-contamination route, unverified CLI premise, superseded by pipeline.51 verified local branching |
