...
- 2026-08-13 (codex) — Replacement proofs confirmed the fail-closed interview contract: Trevor preserved one of three attempted direct reactions, Simon preserved one of one, and unsupported Anthony/Tanya responses were excluded from quote-free recaps. All four passed Packet-aware Rhea and deterministic contamination checks; story-linked lived-exposure testimony remains a separate unproved gate.
- 2026-09-06 (research-build, pipeline.57) — Closed the unproved gate: `story-linked lived-exposure testimony` was unreachable in production because no builder ever populated `packet.exposure.evidence` — `hasLivedEvidence()` was permanently false, so every interview fell back to direct reaction regardless of how directly a citizen was tied to the event. The addressable source already existed (`story.ref`, itself a specific sheet+row/StorylineId citation from `buildWorldSummary.js`'s desk-signal builder) and was flowing into `known` claims, just never into `exposure`. Fix landed in `livedExperiencePacket.js` (`evidenceFor`), reaching the live LEP/2 contract unchanged since `livedExperiencePacketV2.js` clones V1's packet. A candidate earns an evidence row unless reached only through the `neighborsFromLedger` proximity fallback (`why: same-hood-ledger|ledger-resident`) — matching this document's own "same-neighborhood... is not lived exposure" line. Verified end-to-end against a real C105 desk-signal anomaly.
