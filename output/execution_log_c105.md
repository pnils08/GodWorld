...
> **RESOLVED S409 (chase §S-E, `b128ec05`) — it was hiding one, and a worse one than the noise.** 11.70 = CIVIC 9 × coverage-crisis 1.3 on six identical seeds. The T2.4 divide-by-1.5 was non-monotone: CIVIC-in-crisis 7.8 ranked *below* plain CIVIC 9.0, HEALTH-in-crisis 13→8.67 below plain HEALTH 10 — the crisis boost was a penalty for every top-tier domain. Now a hard cap at 10 (order-preserving). In-engine nothing sorts on `priorityScore`, but the engine-auditor's `routePatternSeeds.js` orders the pattern-seed deck by it (Node-side, live now): under the old divide INFRA HIGH 7.0 sat below ENVIRONMENT HIGH 9.0 there. Ties at 10 now break on domain weight at the sort site. Expected on the bench: the six lines read `final=10.00`; expected at the next `/engine-review`: HIGH top-domain seeds lead the deck in domain order.
...
> **DESIGN SUPERSEDED SAME SESSION — G-PF34 (engine.139, builder-direct).** The ruling above stands: the three systems were never in conflict. But "the scoring stands" did not survive the day. Approval has four inputs and three could only subtract; the fourth paid only at a `complete` nothing has ever reached, and the media arm's `<= -3` gate **never fired once** in 15 cycles of live CIVIC ratings (range -2..+3). Every clamp in that path was set while the media channel was dark. Worse, the C103→95 inflation bug was never fixed — a row parked at `complete` still paid its owner +3 *every cycle*, dormant only because nothing finishes. Scoring is now **positives are EVENTS, negatives are CONDITIONS**: transitions pay once, terminal states pay nothing, media is symmetric, and `sitting`/`advanced` get mirrored width ladders. Under the new rules C105 costs the mayor **-5**, not -13 — and C104, where four initiatives advanced, would have been **0** instead of -13.
