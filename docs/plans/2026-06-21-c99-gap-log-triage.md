...
- **Status:** [→] ROUTED — research-build design (DONE: cycle-form not calendar) + **engine-sheet build** (the RULES edit); **NOT civic** (corrected S267, Mike: civic builds nothing — it only *runs* the agent). Determined empirically: `scripts/assembleDecisions.js` only *assembles* voice-file output — it does NOT synthesize dates. The real-calendar gate dates ("Jan 14 2027 / Feb 18 / March 10 2027") live in the `MilestoneNotes` field the **civic-office-baylight-authority agent** emits. The fix = the agent emitting cycle-form gate references ("gate 1 @ C{n}"), a `.claude/agents/civic-office-baylight-authority/RULES.md` change. Per the four-terminal split (S264) a behavior change to an agent is **research-build-designed + engine-sheet-built**; civic only executes. Design is settled (cycle-form), so engine-sheet can make the small RULES edit — pending (offered, not yet done).
