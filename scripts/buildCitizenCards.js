...
async function wipeOldCitizenCards(citizens) {
  ...
  for (var k = 0; k < matches.length; k++) {
    ...
    for (var attempt = 0; attempt < 4 && !ok; attempt++) {
      ...
      if (ok) break;
      // engine.112: 404 means the document is already absent, which satisfies
      // the caller's intent. Previously it fell through to "non-recoverable" and
      // was tallied as a failure — the reason a clean run could look broken.
    ...
    if (ok) deleted++;
      ...
      failed++;
      failures.push({ id: matches[k].id, popId: matches[k].popId, status: lastStatus });
  ...
  // engine.112: the failure count reached a log line and nothing else, so a
  // partial wipe was indistinguishable from a clean one and the write pass ran
  ...
  return {
    ...
    deleted: deleted, alreadyGone: alreadyGone, failed: failed
  };
}
...
async function main() {
  ...
  if (FROM_ARCHIVE) {
    try {
      ...
      var arRows = arRes.data.values || [];
      if (arRows.length && JSON.stringify(arRows[0].slice(0, rows[0].length)) !== JSON.stringify(rows[0])) {
        console.error('[buildCitizenCards] Citizen_Archive header does not start with the Simulation_Ledger header — refusing the union');
        process.exit(1);
      }
      var iExitA = arRows.length ? arRows[0].indexOf('ExitCycle') : -1;
      var latest = {};
      for (var ai = 1; ai < arRows.length; ai++) {
        var apop = (arRows[ai][0] || '').trim();
