...
**Simulation_Ledger rows:** 906 (S262 live count) | **Columns:** 49 (A–AW; SMPageId added at AW S262 — citizen-loop Phase 2 per-citizen Supermemory-page pointer, lazy-filled; DialState at AV S256, engine.31) | **Sim Year:** 2041
...
**Canonical column definitions:** `schemas/SCHEMA_HEADERS.md` under `## Simulation_Ledger` — auto-generated header list, refreshed via Apps Script `exportAndPushToGitHub`. Live sheet is **49 columns** as of S262 — the A–AT table below covers 46; columns AU (Gender, confirmed S146), AV (DialState, added S256 — engine.31 per-citizen 8-dial JSON machine state; TraitProfile col R is the derived face), and AW (SMPageId, added S262 — citizen-loop Phase 2 per-citizen Supermemory-page pointer; lazy-filled, blank until a citizen is first woken) are not yet in §Column Reference. SCHEMA_HEADERS reports 49 (regenerated S262). When the sheet drifts from this doc, SCHEMA_HEADERS is ground truth. *(Phase 41.6 backlink, S156.)*
