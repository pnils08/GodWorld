...
- **Design direction — SESSION_CONTEXT as on-demand doc, not auto-boot primitive (S243 Mike).** Supersedes the crude "make `--rotate-history` default-on" fix for G-SE5/G-SS5. Mike's reframe: the *capture* of a session-span is warranted, but *auto-injecting* it every boot is mostly noise — relevance to the next session is contingent on what that session is (continuation vs pivot), unknowable at write-time. Target model:
  - **Unit = span**, not session: soft closes append to the live `SESSION_CONTEXT.md`; hard close snapshots the span to a numbered, wiki-referenceable `SESSION_CONTEXT_S<##>.md` (## = hard-close session number) and resets the live file to Priority + a one-line pointer to the snapshot (not empty).
  - **Span is shared across terminals** — both research-build + engine-sheet append to the live span; hard close snapshots the shared episode. (This is the unified-close coordination, made default — S243 the two terminals hit exactly this.)
  - **Boot drops the 80-line SESSION_CONTEXT read.** Auto-inject keeps only the always-true mechanical slice already present: `<godworld-state>` (terminal/cycle/session) + Shipped Last Session block (git log) + Next-Priority pointer (ROLLOUT is canonical) + a one-line "last span: `<pointer>`" so a *continuing* session can pull the on-demand doc.
  - **Open questions (resolve before build):** (a) does auto-boot drop the SESSION_CONTEXT read entirely or keep a ≤10-line Priority slice? (b) numbering when a span crosses many sessions — key on hard-close session # or span range? (c) how does a fresh-but-continuing session reliably discover + pull the span doc (greeting pointer vs "resume" convention in CLAUDE.md)? (d) bloat is *bounded by span length*, not eliminated — relies on the "≥3 chained soft closes → hard close" rule actually triggering.
  - **ADR when locked:** flipping SESSION_CONTEXT from always-load → on-demand reverses a premise in CLAUDE.md ("Read this file at the start of every session") + all four TERMINAL.md Always-Load tables + `session-startup-hook.sh`. Hard-to-reverse + surprising-without-context → earns an ADR recording the decision + rejected alternatives (keep-auto-boot, rotate-default-only). Concept stays here until Mike locks direction; then ADR + the cascade edit lands as its own boot-arch sub-phase.
