...
- **Builder go given 2026-09-20 16:39 — 7 of 9 applied, read back 7/7** (BIZ-00078, 79, 80, 81, 82, 84, 85 → the INSTITUTIONS.md canon names; record `output/engine-sheet/2026-09-20-business-ledger-faith-canon-subs.json`; beat dump refreshed, 0 legacy hits). **2 HELD — the canon org already exists as its own row:** BIZ-00083 `Cathedral of Christ the Light` (emp 30) beside BIZ-00088 `Cathedral of the Living Word` (emp 8, key Bishop Antoine Vermeer POP-00755); BIZ-00086 `Beth Jacob Congregation` (emp 7) beside BIZ-00092 `B'nai Tikvah Synagogue` (emp 6, Sector `Synagogue`). No ledger citizen is employed at any of the four. A rename makes two rows share a name; the clean end is retiring the two legacy rows to Business_Archive — a row removal, builder's word. Also seen: BIZ-00079 Key_Personnel reads `Imam Yusuf Kareem` while the canon leader is Imam Aziz Rahimi (POP-00762, whose EmployerBizId is BIZ-00028).
- **Builder word 16:43 "retire and repair" — DONE, every write read back:** BIZ-00083 and BIZ-00086 copied to `Business_Archive` (`canon-duplicate-of:BIZ-00088` / `:BIZ-00092`, ExitCycle 108) by the engine's own archive-then-remove order, then removed; ledger 177 → 175 rows, 0 duplicate or blank BIZ_IDs; both had 0 ledger employees and only historical references (C102–C107 seeds, one C103 ripple). The economic slice excludes the faith sector from closures, so neither reads as a shutdown. BIZ-00079 Key_Personnel → `Imam Aziz Rahimi (POP-00762)`. Same names found and fixed on three more live sources the desks or the engine read: `Community_Programs` 6 program names, `Event_Content_Ledger` row 166 (active pool line "back pew at Allen Temple"), and 10 cells across C108 `Story_Seed_Deck` rows. Beat dump refreshed: 0 legacy names on Business_Ledger, Community_Programs and C108 seeds. Records: `output/engine-sheet/2026-09-20-*-faith-canon-subs.json`, `…-canon-duplicate-retirements.json`.
- **Builder go 16:55 — life-history residue swapped:** 49 `Simulation_Ledger.LifeHistory` cells (47 "Allen Temple", 2 "Islamic Center of Oakland") + 51 `LifeHistory_Log.EventText` lines → canon names; only those two columns touched; read back 0 mismatches, 0 legacy names left on either tab; ledger 952 rows / 0 duplicate POPIDs after; snapshot refreshed. Reason: citizen wakes read the life-history tail, so the old name could be repeated into a wake and from there to a desk. Pre-write values: `output/engine-sheet/2026-09-20-lifehistory-faith-canon-subs.backup.json`.
- **Left as history:** `Faith_Ledger` C78–C93, hooks ≤C93, seeds ≤C107. **Left as code:** `utilities/ensureFaithLedger.js` seed literals. Both ride engine.248 — the builder's direction that faith orgs get a role in the sim, not window dressing for media.
- **Original note:** apply the other 9 ruled substitutions to Business_Ledger. Any sighting/venue draw can surface them into seeds the desk crons read.
...
- **Status: FIXED same day — engine.247, PROD @105 `d5738907`.** Bench C116 @75 4/4 predictions (slips 1.41/2.59 vs the live-C108 control 1.44/2.52). Live column H set to Plain text, 14 cells restored, read back 0 numeric. Pre-flight now warns on date-stored records and runs the engine's WeekRecord validator (C108 row 226 would have read NOT READY). Live confirmation of the effect: all 12 C108 `sports:as` slips carry the 3-0 price.
- **Original fix note (code, engine-sheet):** in `readOaklandFeedEntries_` and `processFeedSheet_`, a Date in the record column becomes `(month)-(day)`; bench with a row holding a real date serial. **Sheet side (builder's tab):** format the Team Record column as plain text and re-enter the 14 cells. **Pre-flight:** fail on any numeric-stored Team Record cell — the script reads display strings today and cannot see it.
...
- **RULED 2026-09-20 17:06 (builder):** scale the applied migration to the table's share of the city, split by hood size × the hood's own MigrationFlow, arrivals take the hood's own age mix — filed engine.249, `ready`, not cut this session. **Rejected: the rebase offer, not the problem.** This seat proposed resizing the four small hoods in one hand write; the builder called it what it was — rewriting the world so the code works better, against the doctrine. The size inversion itself (Lake Merritt 532 people / 102 tracked citizens; Glenview 2,110 / 4) stays OPEN and has to be solved through the sim, by causes that move people over cycles, designed with the builder. This seat first mis-recorded the ruling as "item 4 dropped"; corrected same session. Extra evidence gathered for the ruling: every engine reader of this table uses ratios, never the headcount; the hook deck carried "Lake Merritt seeing N% population growth" at C105–C108 (18/17/15/13%); Laurel and KONO read outflow on the map while growing here.
...
- **Status:** RULED 2026-09-22 (builder, via research-build): owning and renting are separate progression vehicles; a home gates family formation; ownership never flips silently. **FIXED same day (engine-sheet):** `processRelocations_` anchors every household unit that is not `rented` (owned or unknown to the ledger) and logs the count; `updateHouseholdLedgerMove_` refuses an owned row outright. A change of home stays the wealth engine's sale/purchase. Bench-pending with the housing lever build.

### G-EC71 — every rented household's MonthlyRent is its hood's median stamp; within a hood the burden varies only by income [judgment] [scenery] [MED]
- **Source:** phase05-citizens/householdFormationEngine.js `estimateRent_` (hood market median at formation); every later writer (relocation `updateHouseholdLedgerMove_`, purchase) re-stamps the destination hood's median.
- **Evidence:** live C108 dump, active rented households: distinct MonthlyRent values per hood — West Oakland 2 ($3,621 / $3,625 across 31 rows), Fruitvale 1, Laurel 1, Piedmont Ave 1, KONO 1, East Oakland 1; no hood above 4. Rent burden inside a hood is therefore an income ranking, not a housing fact.
- **Status:** OPEN (found 2026-09-22 during engine.255 design; agy review F2 arithmetic). §16: a column that never moves per row is scenery. Fix shape is a sim call — a lease drawn around the hood median by unit size / tenure length / move-in cycle, drifting on renewal — filed for the builder, not this plan's.

### G-EC72 — the household stress flag is near-silent because the savings buffer counts whole net worth [judgment] [gate-never-fires] [HIGH]
- **Source:** phase05-citizens/householdFormationEngine.js:1013 (`totalSavings += money.netWorth`), :1269 (`householdSavings >= monthlyCost × 12` skips stress); migrationTrackingEngine.js:256 same 12-month `buffered`.
- **Evidence:** live C108: 392 active renters, 37 at burden ≥ 0.40, 13 at ≥ 0.50, but only 7 remain flagged after the buffer (2 in West Oakland); renter savings p50 = 165 months of rent (NetWorth p50 $369k on income p50 $98k); nothing debits HouseholdSavings but a casino loss. Beverly Hayes POP-00772 (the Stabilization Fund's canon face) NetWorth $939,915, no household row, and her ledger role differs from `.claude/agents/civic-project-stabilization-fund/IDENTITY.md` (home health aide).
- **Status:** OPEN — engine.255 plan §Data reality, builder call 8 (which number moves: a liquid-savings share vs net worth, or money-out-is-the-fact). Wealth-layer row when ruled. The Beverly Hayes role mismatch is the civic lane's to reconcile.
