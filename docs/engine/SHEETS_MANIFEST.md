...
- **`Casino_Ledger`**: Gated 4b wager tab (review build 2026-08-31, grok). Missing tab = no-op in `processCasinoLedger_`. Append-only citizen slips + one `HOUSE` float row. Engine-sheet arms the tab on bench; this file does not insert it. Sports settlement reads the first `Oakland_Sports_Feed` `game-result` whose `Streak` is `W#`/`L#` — no new sports columns.
