...
sources:
  ...
  - docs/plans/PLAN_TEMPLATE.md — sibling plan template; research template mirrors its discipline
...
pointers:
  ...
  - "[[plans/PLAN_TEMPLATE]] — sibling artifact; research is the standing library, plan is the project"
...
**Pointers:**
- Prior work: docs/plans/PLAN_TEMPLATE.md (the sibling shape this mirrors)
...
**Acceptance criteria:**
1. `docs/research/RESEARCH_TEMPLATE.md` exists with the eight-section shape (incl. a distinct **Applications (living)** reuse-index, separate from the Changelog) + verdict enum + verdict-not-state rule + "research never archives" lifecycle note. A fresh session can copy it and produce a uniform research file without reconstructing context.
...
- **Files:**
  - `docs/research/RESEARCH_TEMPLATE.md` — create
- **Steps:**
  1. Copy the frontmatter + how-to shape from `docs/plans/PLAN_TEMPLATE.md` (sibling discipline: self-contained, fresh-session-executable, cite-don't-recall).
...
- **Steps (section triage — RESEARCH.md is NOT homogeneous, advisor-flagged):**
  1. Add a banner under the title: this file is the **frozen legacy learning log** (S99–S248 chronological findings + early-project AI-literacy research). New deliberate research → per-topic files in `docs/research/` using `RESEARCH_TEMPLATE.md`. Existing sections are a frozen snapshot; migrate opportunistically, not in a sweep.
...
- **Steps:**
  1. Add: research is filed as a per-topic file from `docs/research/RESEARCH_TEMPLATE.md` (not appended to the frozen RESEARCH.md log).
...
- **Files:**
  ...
  - `.claude/terminals/research-build/TERMINAL.md` — modify (add `docs/research/RESEARCH_TEMPLATE.md` to the Research table)
  - `docs/SCHEMA.md` — modify (§7 folder map: note `docs/research/` uses `RESEARCH_TEMPLATE.md` + a sub-index — the parent-spec back-link for the template)
