...
- **Status:** **DONE S183** (engine-sheet). **Single-citizen verification (S182):** Beverly POP-00772 1 dual-tagged write. **Bulk apply (S183):** 408 wd-citizens written (404 + 4 retries) — 278 thin citizens hit the line-515 quality gate. **Cold-start fix (S183 follow-up):** added `--no-quality-gate` flag — when set, skips the gate AND drops the wipe idempotency filter so already-tagged docs get re-wiped. Used `--apply --wipe-old --no-quality-gate` for full rebuild: wiped all 408 prior wd-citizens + wrote all 686 fresh = **686 wd-citizens** final (682 in bulk + 4 targeted retries). Final substrate: **843 world-data docs, 100% domain-tagged, 0 orphans, 0 cold-start-trap citizens**.
...
| world-data total | **843** | Full R1 retrofit + cold-start fix. 100% domain-tagged, zero orphans. |
| `wd-citizens` tagged | **686** | Full ledger ingested (760 rows minus 73 blank/incomplete + 1 hard miss = 686 matched). Cold-start trap closed by `--no-quality-gate` rebuild — every citizen with a POPID + first name is now in the lookup, including the 278 previously skipped by the quality gate. |
...
Fresh-terminal pickup state: **all S183 plan tasks complete + cold-start trap closed.** Substrate is structurally clean — every world-data doc carries exactly one domain tag, and every ledger citizen is now retrievable via wd-citizens (no more 278-citizen blind spot). Five new writers + R1 + R2 retrofits + M1-M4 MCP tools + DELETE-401/429 retry patch + `--wipe-only` recovery flag + `--no-quality-gate` cold-start fix all shipped. Open follow-ups: NM↔SL neighborhood-name reconciliation (W4 surfaced); future automation — wire R1 / W2-W5 / R2 into the post-cycle or post-publish hook so they re-run automatically as canon evolves (currently manual, fine for now since substrate is fresh).
...
5. Task R1 (citizen retrofit) — proves the retrofit + POPID-content-scoped wipe pattern. **DONE S183** (Beverly verified S182; bulk apply S183 + `--no-quality-gate` rebuild closed cold-start trap). 686 wd-citizens written (full ledger). Final state: 843 world-data docs, 100% domain-tagged.
...
- 2026-04-28 — S183 R1 cold-start fix. Engine-sheet identified that the line-515 quality gate (skip if no appearances + no trait + no bio) creates a cold-start trap: a thin citizen never gets discovered → never gets named in an edition → never gets traits/bio backfilled → stays thin → keeps being skipped. The 278 silent-skipped citizens were structurally unreachable. Added `--no-quality-gate` flag to `scripts/buildCitizenCards.js` that (a) skips the line-515 gate, (b) drops the wipe idempotency filter so already-tagged docs get re-wiped — combined effect is a clean rebuild from scratch. Ran `--apply --wipe-old --no-quality-gate`: wiped all 408 prior wd-citizens, wrote 686 fresh (682 in bulk + 4 targeted retries on rate-limited Broderick Mitchell / Travis Paiz / Dak Leo / Lisa Tanaka). **Final wd-citizens = 686** (full ledger). Substrate now 843 world-data docs, 100% domain-tagged, 0 orphans, 0 cold-start-trap citizens. Reasoning: thin cards are a discovery surface for "who lives in X / who works as Y" queries, not pollution — hybrid search ranks by similarity, so they only surface on direct name/role/neighborhood queries (exactly what discovery looks like). If retrieval gets noisy later, filter at query time (in MCP tool), not ingest time.
