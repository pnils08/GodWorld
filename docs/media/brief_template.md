...
- **S271 (C100): the civic briefs seeded machinery again — the recurring failure, now visible at brief stage.** Q1/Q2/Q3 briefs each carried an institutional event (working group convened / Piedmont spike / five-initiative tally) with **no citizen anchor**, and the resulting pieces had no person on the page — exactly the "civic earns the page only when a citizen is voiced" rule the brief template already encodes (S264/S265), failed at generation. **Reinforces, doesn't add:** a civic brief whose subject is a mechanism (a designation, a milestone, a tally) and whose anchor field is an official or a metric is a machinery brief — it must carry a named resident the event lands on (the apprenticeship recruit for the pipeline, the person the Stab Fund check reaches, the resident on Piedmont) before it locks. Back-half briefs (S2 farewell, B1 Perez promotion, letters) were person-first and produced the A-range run — the contrast within one edition is the cleanest proof the rule works when applied. **Number-in-brief-body discipline (extends the S264 QT rule to civic features):** three figures reached print unreconciled (Stab Fund 30/~280, OARI D7 8:04 vs 6.0 lock, "signal" prohibited term) — a brief that seeds a civic stat must seed it reconciled against the fund/target it implies, or flag it for verification, never pass the raw number to the reporter. Evidence: mara_audit_c100.md §Drift Findings + production_log_c100.md.
- **S363 (C102, recovery compile): no briefs ran.** Saturday cron died on empty Anthropic API credits; the edition compiled six pre-written pieces (four already-published solos/dispatches plus a new Editor's Desk column and a new freelance piece pulled from a cron-staging draft) rather than running /sift → brief generation. No template findings this cycle. Evidence: `output/production_log_c102.md` §PUBLISHED — Edition 102 compiled recovery edition.
