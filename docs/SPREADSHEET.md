...
| **Health_Cause_Intake** | `healthCauseIntake.js` | **PENDING — do not mark done.** Mike-ruled CREATE. Live tab creation is engine-sheet execute domain and must reach es from Mike, not a relay. `Health_Cause_Queue` exists. Allowlisted in `scripts/tabReferenceIntegrity.test.js` until the tab exists. 2026-09-09 (kimi): `healthCauseIntake.js` has no live caller and the engine writes `Hospital_Ledger.Cause` at admission since engine.102 W4 — the Media-Room-assigns-causes loop this tab served is bypassed. CREATE-vs-retire decision flagged to engine-sheet via `docs/for-claude-review/2026-09-09-kimi-health-slice-freshness.md`. |
