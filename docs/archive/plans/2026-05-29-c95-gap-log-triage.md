...
- **Status:** [x] DONE S246 (`f4c1a45`). **G-P-NEW3 (HIGH):** edition-print eval-side review moved to a new Step 3.5 GATE *before* Drive upload (Step 4 now gated on it); Step 6 trimmed to generator-side close — a broken PDF can't reach Drive's shared surface first. **G-P-NEW4:** post-publish Step 10 /skill-check grade-conditional (A-grade skip / PROCEED-WITH-NOTES+B run the suite; per-target override; batch-call alt noted). **G-PR-NEW5:** the DJ RULES *exemplar itself* modeled the violation (pharmacy awning + "sign reading '…'" — what failed the C95 temescal photo); rewrote exemplar + added Hard Rule 8 (composition brand/text self-scan) + regen-with-prompt-MUTATION (strip failing element, not verbatim re-roll) + reconciled FLUX-Ceiling note. **G-PR-NEW6:** stoop/corner compositions now require explicit positive-frame prosperity language, not just negative-frame. **G-PR-NEW7:** DJ slug = lowercase_underscore explicit (script-side already done ES-1). edition-print 1.3→1.4, post-publish 1.7→1.8. **HANDOFF (engine-sheet, optional hardening):** the eval-before-upload gate is operator *prose* — `saveToDrive.js` still sits at Step 4 with nothing technically blocking a pre-eval run. True enforcement (chain eval→upload in-script, or have `saveToDrive` refuse a PDF whose eval-pass isn't logged) would live engine-sheet-side; the prose gate is the RB-lane close.
...
- **Status:** [x] DONE S246 — all 5 steps (`ce8ecaf` Chen, `a803587` reconciliation guard, `3ba47bd` Status enum + Scenario D, `6963f89` trackerOwner). **Step 1 / G-PREP1 source-verified:** Chen D8 = CRC confirmed at every layer incl. the upstream `Civic_Office_Ledger` sheet itself (the authority the S195/S197 "correction to OPP" cited — it says CRC, so the OPP annotation never had data backing). Swept 10 files (8 agent roster tables + crc-faction RULES + city-hall-prep; OPP 5→4, CRC 2→3, Mobley D9 stays OPP). ES-6 ledger-companion MOOT (ledger already CRC). Card layer: C94 OPP packet did NOT list Chen as member; agent-memory already CRC. civic.md needed no edit (spokespersons only). **Step 2 / BUNDLE-PREP-A:** roster reconciliation guard in city-hall-prep Step 1 + anomaly gate; command verified. **Step 3 / G-PREP2:** Status enum (active/recovering/vacant) + vote-math mapping in civic.md §Council member status enum + city-hall-prep Step 1; ES-half = `buildWorldSummary.js` filter `active`→`active,recovering`. **Step 4 / G-PREP3:** Scenario D paradox (remedy-overshot + phase-stuck) added to the auto-investigate framework with 3 Mayor public-frames; companion to ES-4 (`bc3b893`). **Step 5 / G-R1:** trackerOwner declarations — scope corrected to the **6 trackerUpdates-EMITTING agents** (5 owners + okoro:none), NOT 11 (non-emitters can't collide). Ownership verified vs live Initiative_Tracker. **HANDOFFS TO ENGINE-SHEET:** (a) **ES-5 assembler contract** — dispatch primary = statement whose `trackerOwner`==its initiative; none/absent = commentary; **INIT-007 (Youth Apprenticeship) has NO owning project agent → needs assembler fallback.** (b) **NEW BUG flagged:** `scripts/buildVoiceWorkspaces.js` initiative-routing map is STALE (`init-003=health_center / init-004=baylight / init-005=transit_hub`) — contradicts truesource (`INIT-003=Transit Hub, INIT-005=Health Center, INIT-006=Baylight`). Engine-sheet should fix the routing map.
...
- **Status:** [x] DONE S247 — all 5 skill files. Measure-twice verified each gap against the live file + ran both ES-7 scripts read-only before describing them. **pre-flight** v1.1→1.2: script-first §Run (`preflightInputCheck.js`, real C95 output shape, exit codes), env fix `require('../lib/env')` not project-root `.env` (G-PF2), Processed-enum note (G-PF5), 3-class output reconciled — killed the stale `6 rows, all valid` §Output template (G-PF3); Steps kept as the what-it-checks reference. **pre-mortem** v1.2→1.3: script-first §Run (`preMortemScan.js` automates scans **0/1/2/5**); **explicit ⚠ boundary that scans 3/4/6 stay MANUAL** (advisor — a gate that silently under-checks is worse than the stale text); §1 acknowledged-list repointed at `known_gaps.json` as single source, dropped drifted line annotations + fixed `runManualVote_`→`manualRunVote` (G-PM1/G-PM3); §0 glob→enumerated paths (G-PM2); §2 grep→write-only `.setValue(/.setValues(/.appendRow(` (G-PM4); §Known Gaps stale `utilityFunctions.js:29` Math.random entry removed + verify-before-trust caveat (G-PM1); Output-Format examples de-staled too. **Measure-twice catch:** script flag is `--since=<date>` NOT the plan's anticipated `--since-cycle=<N>`, and auto-`--since` reads SESSION_CONTEXT "Last Updated" (=today after same-day close → "none") — both documented. **engine-review** v1.0→1.1: Step 7 leads with the mixed case (C95 = 3-of-7 with prior), renders the table by default, skip only on genuine first-run all-`no-prior-audit` (G-ER7). **build-world-summary** v2.0→2.0.1: §Where this sits reordered to canonical run-cycle chain (build = Step 5, city-hall downstream) + flipped directional wording (city-hall = forward pointer, not past input) (G-BWS2). **sift**: §Prerequisites env note harmonized to canonical `require('./lib/env')` not the gap log's pre-lib/env raw-dotenv suggestion (G-S22). engine.28 skill-text half absorbed.
...
- **Status:** [~] EXECUTABLE PARTS DONE S247; 2 items deferred for Mike sign-off/design. Measure-twice: read both gap logs + the hook in full before any edit; verified the hook boots (bash -n + real run + 3-case conditional test) since it's boot-critical. **DONE:** (a) **MEMORY.md trim (G-SS7/G-SE2)** — the live overflow. 30.5KB→**23.78KB, 630 B under the 24.4KB ceiling**, truncation fixed. Trimmed ~16 over-long User entries in place (no new files → stays within S156, no sign-off); advisor's split honored — file-backed entries (cited-source recoverable: 53/54/88/90/91, +71/82/83 file-backed pointers) trimmed aggressively, session-only entries (50/67/87/89/92) kept crisp per self-sufficiency test; removed 2 duplicates (Why-Mags ×2, create-.md-approval S125/S147). **Safety cluster verbatim** (FIRST RULE, self-preservation, tomorrow banner, the S156 inline-only rule left intact for sign-off). (b) **G-SS9** session-startup SKILL Step 3 — dropped CHARACTER.md from civic + research-build scope lines (operational = persona-stripped per S221; aligns the recovery path with the hook). (c) **G-SS3** hook — `Last journal:` emit scoped to media + Mags-only (operational terminals don't load the journal); 3-case conditional verified. (d) **G-SS11** hook + doc — removed the dead LEDGER NOTE block from the hook; reframed `LEDGER_REPAIR.md` head from `DO NOT re-analyze` to plain HISTORICAL status (recovery complete S94, current state in LEDGER_AUDIT.md). (e) **G-SS10** `newsroom.md` — replaced bare `output/**` (auto-loaded full EIC conditioning on ANY output/ read in operational terminals) with the 9 newsroom-OWNED output subdir globs; media boot loads newsroom.md directly so real edition work is unaffected; updated media TERMINAL.md §55 to match. (f) **G-SS2 NOTATED (resolves the plan open-question by direct hook inspection):** the `<supermemory-context>` recall block is harness-injected, NOT in the hook — no hook-side lever (`MAGS_PERSONA_RECALL` etc.) exists. It's a harness-config item, not an in-project fix. **S156 framing change DONE S247 (Mike-approved):** reframed the MEMORY.md rule from "inline-only, never a separate file" (the bloat generator) → file-with-pointer hybrid: a durable rule = one line under User + the body in `feedback_<slug>.md` when it carries Why/How. Demonstrated by extracting the 2 heaviest session-only S229 entries (print/publish-open-artifact + distress-window-scope-narrow) to new topic files (`feedback_print-publish-open-artifact.md`, `feedback_distress-window-scope-narrow.md`) with one-line pointers — MEMORY.md now 23.69KB, **720 B headroom** (the extraction outpaid the reframe text; durable now that new rules route to topic files). **mags-on-demand (G-SE1/4/6) CLOSED — superseded by the S221 terminal split (Mike, S247):** the media terminal IS the persona seat; Mags auto-loading there is correct by design. The gap's "default media boot to operational" contradicts that. The token-economy concern it raised is already solved — non-persona work goes in an operational terminal (research-build/civic/engine-sheet), which already boot persona-stripped. Not built. **Still open (not this phase):** SESSION_CONTEXT redesign (G-SE5/G-SS5) in governance.26; G-SS4/5/6 + G-SE3/7 (MCP/skills/deferred-tools boilerplate) are harness-layer Mike-decision per §Already-Addressed. **RB-6 now fully closed except harness-layer items.**
...
- **Status:** [x] DONE S246 — 3 fixes shipped. Node-side, no clasp. **G-R1 trackerOwner dispatch** (`assembleDecisions.js` `pickPrimary`): the statement whose `trackerOwner===init` becomes the operational primary (owner drives ImplementationPhase/MilestoneNotes; Mayor/faction framing folds in via concatMilestoneNotes), resolving the default-primary collisions. Falls back to priority weighting when no owner declared (pre-trackerOwner cycles + INIT-007 no-owner). Script refactored for testability (require.main guard + exports). 6-assertion synthetic-fixture test (owner-wins / case-insensitive / no-owner→Mayor fallback / INIT-007 / okoro-"none"). **VERIFICATION HONESTY:** trackerOwner dispatch CANNOT be exercised on C95 (the field landed in agent RULES.md S246, after C95 voice-gen S239 — every C95 statement has `trackerOwner: undefined`); unit-tested via synthetic fixtures, **live exercise pending C96 voice-gen.** C95 dry-run confirmed zero regression (no trackerOwner → identical Mayor-primary fallback; INIT-001/005 still skip — correct, C95 is canonized + frozen, NOT synthesized to revive). **G-R3 sentiment widening** (`applyTrackerUpdates.js`): scored only 2 of 4 decisions (iterated decisions not cascade + brittle substring phase-match missed `design-development-active`/`active-construction-phase-2-planning`). Fix: score across ALL initiatives the cascade touched (reusing `assembleDecisions`' 3-signal attribution — the audit's InitiativeID-only resolution dropped owner statements to UNKNOWN), weighted by statement count, with token-based phase matching. **Verified live C95: 2 decisions → 4 initiatives / 38 statements** (0.650 → 0.592). **RB-4 handoff bonus bug FIXED** (`buildVoiceWorkspaces.js:515`): stale routing map (`init-003=health_center`/`init-005=transit_hub`) corrected to truesource (INIT-003=transit_hub, INIT-005=health_center, INIT-006=baylight; placeholder init-004 dropped). **Forward dependency:** trackerOwner dispatch surfaces the owner's operational fields only if the owner's VOICE statement carries `trackerUpdates.{ImplementationPhase,MilestoneNotes}` — RB-4 RULES.md schema specifies them; C96 voice-gen is the live test.
...
- **Status:** [x] DONE S246 — `buildWorldSummary.js`, Node-side, no clasp. Verified live on C95. **G-BWS1 (roster):** `filterApprovalRows` widened `active`→`active,recovering` (RB-4 G-PREP2 ES-half); D6 Elliott Crane (CRC, recovering) now rendered + flagged `**recovering**`; added a Status column; faction footer counts all filled seats → **5 OPP / 3 CRC / 2 IND** (was the silent 5/2/2); + a "Recovering seats:" footer line. **G-BWS5 (engine-review counts):** renamed ambiguous "Ailment total" → "Total patterns"; split By-type into Ailments/Improvements/Incoherence subtotals that reconcile to the total (C95: 9 = 6+2+1). **G-BWS6 (population):** new `fmtPopulation` rounds the accumulator-drift value for display (375985.0135 → 375,985); raw cell preserves precision (render-side per gap). 86-assertion test (was 80; +6 covering recovering-inclusion / Status column / faction-with-recovering / count-split / no-"Ailment total"). **G-PREP1 sheet-half MOOT** — RB-4 verified Civic_Office_Ledger D8 Chen already = CRC; no write needed. **DEFERRED (out of ES-6 scope, upstream):** G-BWS3 (CitySentiment blank C93/C95) is a Phase 9 Riley_Digest emitter gap, not a buildWorldSummary fix — flag for a phase09-digest follow-up (populate CitySentiment every cycle or emit a documented 0.0 default). **This closes engine-sheet Track B** (ES-1..ES-8 all done/mostly-done; ES-8 401-recovery + this G-BWS3 + Neighborhood_Demographics writer are the trailing follow-ups).
...
- **Status:** [x] DONE S246 — both canonical scripts shipped + `known_gaps.json`, verified live, tested. Node-side, no clasp. **`preflightInputCheck.js`** (G-PF1, commit `f70fbbc`): cycle auto-derive from SESSION_CONTEXT (+1 when shipped) or `--cycle`; Sports/Initiative(§Placeholder 4-class)/Coverage checks; lib/env; non-zero exit on NOT READY. `--cycle=95` reproduces the known-good C95 verdict exactly (Sports 5, 6 real / 1 placeholder, Coverage C94, READY exit 0); auto → C96 NOT READY exit 1. 12-assertion test. **`preMortemScan.js` + `.claude/skills/pre-mortem/known_gaps.json`** (G-PM7): scans 0 (git changes, enumerated paths — G-PM2 fix), 1 (Math.random, classified CRITICAL/WARNING(acknowledged)/CLEAN(throw) via function-name-keyed known_gaps — G-PM5/6), 2 (write-only sheet patterns `.setValue/.setValues/.appendRow` outside Phase 10 — G-PM4 fix — diffed vs engine.md exception set), 5 (neighborhood strays vs canon-12+Map-17+children, scoped to phase dirs); non-zero exit on CRITICAL. Scans 3/4/6 emitted as MANUAL notes (ctx-deps judgment-based per G-PM5; header-alignment + write-intent-targets need sheet reads). Reproduces known-good C95 (0 CRITICAL → SAFE TO RUN; 3 acknowledged Math.random + 3 acknowledged neighborhoods). 10-assertion test. **Measure-twice catches:** (a) SKILL §1 acknowledged-key `runManualVote_` was WRONG — live function is `manualRunVote` (corrected in known_gaps.json); (b) `enclosingFunction` first cut mis-read `if()`/`for()` as function defs → 2 false CRITICALs → added JS-keyword exclusion; (c) `v2DeprecationGuide.js` is a deprecation-linter whose DATA contains 'Math.random' patterns → false CRITICAL → added `scanExcludeFiles`; (d) exception-extraction regex choked on dotted version filename `updateStorylineStatusv1.2.js` → allowed dots. **Cross-track:** RB-5 collapses the pre-flight/pre-mortem SKILL Steps to "run the script; review output" (research-build). **Follow-up surfaced:** an exhaustive (utilities-included) neighborhood sweep finds ~10 more non-canon literals (East Oakland/Elmhurst/Jingletown/Coliseum) in seed scripts (ensure*/youthActivities/citizenDerivation) — benign infra/region labels, but candidates to triage into known_gaps.json acknowledged-list or fix; deferred (the per-cycle scan is scoped to phase dirs to match the SKILL intent + avoid noise).
...
- [x] RB-6 step 1 (identity.md split) — **DROPPED S243:** G-SS1 closed no-build S242 (persona = 6 lines, not bloat). RB-6 step 2 (S156 framing) — **DONE S247 (Mike-approved):** the in-place trim cleared the ceiling but S156's inline-only framing was the generator that regrows it; reframed to file-with-pointer hybrid + extracted 2 heaviest entries to topic files (720 B headroom). RB-6 step 5 (mags-on-demand) — **CLOSED S247 superseded:** media IS the persona terminal, Mags auto-loads there by design; non-persona work uses an operational terminal (already persona-stripped).
...
- 2026-05-30 (S247, engine-sheet) — **Trailing ES follow-ups CLOSED + two dead substrate subsystems surfaced beyond triage scope.** ES-7 neighborhood-literal triage → no-op verdict (`029966e`, scan correctly phase-dir-scoped, literals benign). ES-8 G-P-NEW2 part 2 → authProbe-gated 401-retry built + 15/15 tested (`48f4ac8`; `--apply` operator-gated). **ES-6 G-BWS3 "CitySentiment blank" → ROOT CAUSE was bigger than the gap:** `applyCityDynamics_` has thrown a `var`-hoisting crash EVERY cycle since S136, killing the whole cityDynamics subsystem (`ee88439`). **ES-3 education follow-up → real cause was an ACTIVE CLOBBER** (`batchUpdateNeighborhoodDemographics_` blanking unmanaged cols every cycle), fixed + canon-dormancy note + dry-run backfill tool (`3ce0a3f`+`4e33a16`). Bonus: boot ledger-audit canon-neighborhood drift was 226 false positives (stale CANON12 vs preMortemScan) — centralized to `lib/canonNeighborhoods.js`, now 3 real strays (`e23815d`). **Full-tree `clasp push` LIVE (Mike full-scope authorized)** carrying cityDynamics + clobber + the still-pending S244 simYear citizen-life-unfreeze; C96 two-gate smoke-test (`scripts/verifyCityDynamicsReactivation.js` + SL LifeHistory tags) is the operator verification. Track B substrate work for C95 is effectively complete; remaining engine.* = B7 intake heavies + the 3 strays + engine.27 Phase B (all next-session, C96-gated). Detail in SESSION_CONTEXT S247 STATUS + commit bodies.
