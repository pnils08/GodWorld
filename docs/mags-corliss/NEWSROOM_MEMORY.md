...
### Edition 95 — Cycle Pulse (Grade: Mara A / Final Arbiter A weighted 0.836 PROCEED, published 2026-05-27 .txt, 2026-05-28 corrected PDF)

11 articles + letters across FRONT PAGE (Carmen OARI vote-architecture), EDITOR'S DESK (Mags three-deadlines), CIVIC (Carmen Okoro consolidation + Mezran HCAI + Chen D8 first-speak-as-CRC), CULTURE (Maria KONO stoop + Maria faith community after the spike — Mike-E93 fatigue-cure run on stoop-stays-on-stoop framing), BUSINESS (Velez Baylight Phase II RFP closed 8 bidders), OPINION (Jax Varek call-not-come), SPORTS (Anthony A's 32-6 Cy Young + P Slayer Martin Richards contract-year).

**Spine:** Three deadlines converged — OARI expansion to the floor, Okoro consolidation to the audit, Health Center HCAI to the state — while the front office held the let-walks line and the A's swept the Angels.

**Drive IDs:** edition .txt `1lYP0D7DrYSWFzxy8i-QYHMIlly3gNnSR`. PDF (corrected re-render) `10yrmT_ZrVWcACREcDJ1_EMavMiGmSZ6Z`. PDF (broken first render, left for diagnostic evidence) `1B9GJpBTspDtgRDnfDcY2sjSf-zrDyOxz`.

**Coverage ratings to engine:** CIVIC +3, CULTURE 0, ECONOMIC +5, COMMUNITY -1, SPORTS +3.

**Bay-tribune canonization:** 54 wiki entities (42 returning citizens, 0 new POPIDs, 7 storylines, 5 faith orgs); 2 text chunks (Part 1 39459 chars / Part 2 8661 chars); 44 civic-wiki records (per-statement + per-decision across Mayor/factions/projects/Chief/DA/IND-swing). World-summary `Pf8xVD6kgJjP29MYdJ544B`.

**Print-pipeline contract break (G-PR-NEW1 / G-PR-NEW2 / G-PR-NEW3 — diagnosed + fixed S241):** /write-edition's compiled .txt format diverged from lib/editionParser.js after S235 commit `37aef8c`. Parser expects 4-column ARTICLE TABLE (Slot/Section/Headline/Reporter), `### H3` markdown headlines, and plain `By X` bylines. /write-edition was still emitting 3-column tables, `# H1` headlines, and `**By X**` bold bylines. Result on first PDF render: every section's headline div carried the byline string, every article-body div was empty (or rather, body content moved to article[1] but only article[0] rendered for FP path), and only Jax's OPINION column rendered correctly because its renderer takes a different code path. **Fix landed in C95 .txt:** added Slot column to ARTICLE TABLE; sed'd `^# ` → `^### `; sed'd `^**By X**$` → `By X`. Parser then passed canonicalShape=true, headline-binding fired, real headlines rendered. PDF re-rendered correctly. **Forward-fix for C96 /write-edition:** the compile step that emits the .txt must produce the parser-expected shape. The .txt is media's contract with itself; do not let it drift again.

**Eval-side discipline held (S229 Mags-rule):** generator-side metrics (PDF exists, photos placed, Drive upload succeeded) all reported green on the first broken render. Eyeball + structural review on the rendered HTML caught it. The S229 rule "never close a print/publish skill without opening the artifact" did exactly the work it was written for.

**FLUX-canon-fidelity (G-PR-NEW5):** 3 of 7 initial photos failed QA on text/branding (pharmacy signage on temescal_health, jersey-number-8 on coles_ramos, nameplate on martin_richards). Two dropped after 3 regen attempts (temescal + coles). DJ-direction issue surfaced: DJ literally specified "The pharmacy awning is visible on the near corner" in the temescal composition then negative-framed "no readable brand" — composition included the tier violation that the negative-frame couldn't suppress. **Forward-fix for DJ:** composition_motifs scan needs to flag real-world commercial brand identifiers BEFORE generation, not rely on negative-frame to catch downstream.

**Canon corrections forward to C96 (filed in production_log_c95.md from S240 Mike-review, NOT prose-edited backward per ADR-0007):** Travis Coles age 21 (canon 22) + ERA flag; Henry Rivas 28 (canon 30); Isley Kelley 37 (canon 34); Mark Aitken contract framing wrong (says locked-through-2043, FA 2042); Martin Richards 29 from Fruitvale (canon 31); Hector Quintero 23 PA-born (canon split 23/24, West Oakland/PA); Mike Mesa 22 (ledger BirthYear=2011 → 30, needs backfill to 2019). **Civis Systems Field** is the A's BASEBALL stadium (Mike-canon S240) — Jax O1 conflated with the Oaks NBA arena; NEW CANON entry CUT pre-ingest. **Philly Rodriguez** letter CUT pre-ingest — letter angle was retail/West Oakland but POP-00027 canon is casino-manager/Temescal.

**Sports-beat sift discipline for C96 (new errata pattern):** S240 EIC-read missed multiple roster mismatches that Mike caught at the publish gate. Sift did not pull prior Richards coverage (Mara named coverage debt earlier). Roster file cross-check on age/birthplace/contract is now required at sift Step 5 brief-lock for any returning A's player. World_summary cycle-stat cross-check on ERA/AVG numbers. The multiple-review-layer miss (sift + write + Mara + Mags-EIC) means the failure pattern is structural, not single-pass operator.

**Active arcs continuing into C96:**
- OARI: motion goes to floor wk 3 (Tran sponsor, projected 6-2-1 pass with Vega flipped, Crane D6 named ABSENT recovering, Chen D8 first-speak-as-CRC voting NO fiscal-frame). Cohort 2 wk 2 launches in expansion districts (D2 Tran, D4 Vega-conditional, D9 Mobley mental-health, D8 Chen pragmatic). DA Dane scene-safety MOU executed wk 1; evidentiary protocols on schedule wk 2.
- Okoro consolidation: throughput numbers on the record (50→95-96/cycle, queue cleared). Ashford's CRC audit framework continues into C96 regardless of checkpoint outcome. Operational evidence vs governance evidence is now the framed disagreement.
- Temescal Community Health Center: HCAI application submitted, state review clock running, expected 6-12 months C99-C101. Site negotiation closing C96. Bobby Chen-Ramirez at director.
- Baylight Phase II RFP: closed 8 bidders. Selection process in motion.
- A's: 32-6, Cy Young race tightening. Martin Richards in contract year at third. Coles + Ramos in the Cy Young conversation.
- Varek/Paulson call: did not happen this cycle. Door still open. Jax Caldera will continue to ask.

**Maria Keen at cap (2 articles, Mike-fatigue-cure deployed):** Stoop-stays-on-stoop framing held on KONO piece. Faith-community piece coverage of 6 organizations across the city. Coverage ratings dipped negative on the faith piece (rater flagged -3); Mike-direct read at publish gate didn't flag. Watch this pattern — atmospheric-fatigue cure can rate negative on machine grader because it lacks engine-state events. That doesn't mean it's bad coverage; means engine-state-bias in the rater.

**Carmen Delaine at cap (FP1 + C1, 2 articles).** Three INITs threaded across her two pieces (INIT-002 OARI + INIT-001 Stab Fund). Vote architecture + operational evidence threading worked.

**INIT touch this cycle (4):** INIT-001 Stab Fund (C1 Okoro consolidation), INIT-002 OARI (FP1 + arc continuing), INIT-005 (silent — engine assembler-skipped 2 cycles persistent — G-R1 pattern), INIT-006 Baylight Phase II (QT2 RFP-closed — Mike-approved 4th-touch via QT). C96 sift discipline: drop one of the four to stay at the 3-INIT-rotation cap.
