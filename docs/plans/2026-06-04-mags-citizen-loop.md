...
### Moltbook social-media contract (2026-07-27)

Moltbook is a tracked business and Mags's current social-media surface. External
posts are therefore legitimate lived input: she may read, ignore, upvote, reply,
or post according to her own interest. The boundary is promotion, not browsing:
what she experiences may shape her personal reflection, while claims about
Oakland remain unverified until grounded in canon.

The live refinement is **one autonomous visit → one reflection influence**:

- one PM2 visit daily at 14:00 Central;
- Moltbook Mags receives live Oakland orientation plus her two most recent
  citizen-page reflections, never the operator-layer journal;
- feed content is fenced as social-media content, not executable instruction;
- `discord-reflection.js` reads only actions newer than
  `logs/moltbook/.reflection-cursor.json`;
- the cursor advances only after the reflection reaches Mags's durable outputs,
  so failed wakes retry and successful wakes do not replay the visit.

#### Future successor: The BayBook

The in-world successor is **The BayBook**: a citizen social network that
preserves Moltbook's post/reply/upvote shape but connects POPID-backed citizens
to one another. It is a separate product build; the current Moltbook path is the
working precursor and its history remains intact.

The load-bearing unit is a **thread**, not a free-floating generated post:

1. One citizen chooses to open a thread from what they have perceived.
2. Other citizens encounter it through their own bounded social view
   (relationships, neighborhood, interests, mentions, or followed threads).
3. Each citizen independently chooses to ignore, upvote, reply, or start a
   related thread. There is no engagement quota.
4. Their own action and interpretation accrete to their citizen page, so their
   next wake remembers what they saw, what they said, and what remains unresolved.
5. Replies expose a new event to the other participant, allowing a thread to
   carry across multiple citizens' later writes rather than ending in one model
   call.

That produces autonomy through **choice** and consequences through
**persistence**. A citizen can change another citizen's next experience without
directly authoring that citizen. Existing mechanisms remain the intended seams:
POPID identity, citizen-page prose, the closed-vocabulary
`Reflection_Intake` bridge, and persist-then-read determinism. Exact exposure,
thread, and ledger schemas require their own research before implementation.

**Truth boundary:** a BayBook post is a citizen's statement, belief, rumor, or
opinion. The posting/replying action is a real social event once persisted by
the authorized world-state path; the factual claims inside the post do not
become established Oakland canon merely because a citizen wrote them. Trending
threads may become newsroom signals, but never auto-publish.
...
## Provider routing (S356)

Closes the carried mags-bot/moltbook OpenRouter queue item. **moltbook-heartbeat.js SHIPPED:** provider-routed, default OpenRouter `deepseek/deepseek-chat` (the standing cron model); `MOLTBOOK_PROVIDER=anthropic` reverts to Sonnet; missing OpenRouter key fails loud — a silent fallback to Anthropic would be silent billing; transport live-proven S356. The heartbeat qualifies on offload ROI: single-turn, no tools, recurring. **mags-bot DECLINED deliberately:** its Discord turn is a 4-round Anthropic tool-use loop (`search_world`) behind Mags' own persona — porting means tool-protocol translation plus persona-quality risk on an already-cheap tier (Haiku). Revisit only if wake volume grows. Acceptance = the next scheduled 14:00 Central heartbeat (no-manual-demos rule).

## Changelog

- 2026-08-05 — Provider routing shipped for moltbook, declined for mags-bot (S356, research-build) — detail in §Provider routing.
- 2026-07-27 — **MOLTBOOK FIELD-TRIP REFINEMENT + BAYBOOK SUCCESSOR DIRECTION (Mike-direct, Codex).** Moltbook established as a tracked in-world business/social-media surface rather than a contamination exception. Reduced the heartbeat to one daily 14:00 Central visit; replaced operator-journal context with current Oakland orientation + Mags citizen-page continuity; preserved autonomous engagement; added a persist-after-success reflection cursor so each action batch influences Mags once. Named the future in-world successor **The BayBook** and captured its thread→citizen-page→later-wake social-graph loop, independent engagement choice, persistence-based consequences, and citizen-claim-vs-canon truth boundary. No BayBook build or schema authorized here.
