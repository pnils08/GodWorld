...
// Bias register readback (B1 read path, seams Task 7) — NOT a recall candidate:
// an opinion joins the prompt ONLY when today's perception actually mentions the
// target (case-insensitive name match, no scoring — Design B1 read 1: carried
// history surfacing, not a recital). Reads MemoryRegisters .biases (Task-6 fold);
// defensive parse per the DialState pattern. s==0 (undecided) stays silent.
// Sentiment bands -> plain second-person phrasing; challenged opinions carry
// their doubt, well-reinforced ones their certainty. Never touches dials.
function biasPhrase(b) {
  const s = Number(b.s) || 0;
  const t = String(b.t);
  const qualifier = (Number(b.c) || 0) >= 1 ? " — though lately you're less sure"
    : (Number(b.r) || 0) >= 2 ? ' — and nothing has changed your mind yet'
    : '';
  if (s <= -2) return `You've never trusted ${t}${qualifier}.`;
  if (s < 0) return `You don't much care for ${t}${qualifier}.`;
  if (s >= 2) return `You think highly of ${t}${qualifier}.`;
  return `You've got a soft spot for ${t}${qualifier}.`;
}
function biasReadback(memRegistersJson, perceptionText, cap) {
  let reg = null;
  try { reg = JSON.parse(String(memRegistersJson || '')); } catch (e) { return ''; }
  if (!reg || !Array.isArray(reg.biases)) return '';
  const hay = String(perceptionText || '').toLowerCase();
  if (!hay) return '';
  const lines = [];
  for (const b of reg.biases) {
    if (!b || !b.t || !Number(b.s)) continue;
    if (hay.indexOf(String(b.t).toLowerCase()) < 0) continue;
    lines.push(biasPhrase(b));
    if (lines.length >= (cap || 2)) break;
  }
  return lines.join(' ');
}
...
module.exports = { selectMemories, markRecalled, memKey, tensionCandidates, unlivedCandidates, biasReadback, _internals: { tokenize, contextMatch, staleness, affectWeight, W, STALENESS_CAP, biasPhrase } };
