...
console.log('Test 3: citizen name resolution — G-PF22, no Name column on the sheet');
{
  const rows = [
    { POPID: 'POP-00081', First: 'Carmen', MaidenName: '', Last: 'Mesa' },
    { POPID: 'POP-01078', First: 'Pablo', Last: 'Almanzar' },
    { POPID: 'POP-00999', First: 'Carmen', Last: 'Mesa-Ortiz' },
    { POPID: 'POP-00001', First: '', Last: '' },
  ];
  assert('citizenFullName joins First+Last', sheets.citizenFullName(rows[0]) === 'Carmen Mesa');
  assert('citizenFullName tolerates blanks', sheets.citizenFullName(rows[3]) === '');
  assert('citizenFullName tolerates null', sheets.citizenFullName(null) === '');
  const exact = sheets.matchCitizensByName('carmen  mesa', rows);
  assert('exact match wins over substring (1 row, not 2)', exact.length === 1 && exact[0].POPID === 'POP-00081', JSON.stringify(exact.map(r => r.POPID)));
  const sub = sheets.matchCitizensByName('Mesa', rows);
  assert('substring fallback returns every hit', sub.length === 2);
  assert('no match -> empty, never undefined', Array.isArray(sheets.matchCitizensByName('Nobody Here', rows)) && sheets.matchCitizensByName('Nobody Here', rows).length === 0);
  assert('empty query -> empty', sheets.matchCitizensByName('', rows).length === 0);
  assert('exports findCitizensByName', typeof sheets.findCitizensByName === 'function');
}
