...
- 2026-08-27 (grok) — BART is a train, not a leak. Removed `\bBART\b` from `REAL_OAKLAND` / `REAL_WORLD`. Ogawa and the rest of the landmark list stay. Not pushed.
- 2026-08-29 (grok) — Pre-Rhea word tests (`skipRhea` at `cron-desk-run.js` L2489, writer `phase-lede`, LEP `"I've heard"`) are a unit test on copy, not a newspaper. Successor plan [[2026-08-29-rhea-is-the-gate]] (pipeline.62) for Claude land. Empty W2 stays.
- 2026-08-21 (engine-sheet) — Angle stage now resamples once before failing (`cron-desk-run.js`, W1 generate+validate wrapped in a 2-attempt loop). Grok's `c3a08738` made `chaseReplacesAssignment` fair by widening the token blob to `story.angle/label/hookLine` + `known[]`; this closes the other half of the same failure — the stage was straight-line, so ONE bad draw threw and took report and write with it ("no angle artifact"), which is how Hal's single W1 miss became three red rows. Deliberately a plain resample on the same prompt: feeding the validation error back would teach the model to keyword-stuff whatever the checker greps for, which is the drift the checker exists to catch. Fails closed unchanged — the second attempt's error propagates verbatim, so a genuinely off-assignment plan still routes to flagged/. Loop semantics proven 4/4 (first-try success takes no extra call; bad-then-good recovers; bad-twice rethrows the original error; never exceeds 2 calls). Real acceptance test is the next unattended 06:15 fanout — no hand-run.
