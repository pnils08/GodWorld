...
| `phase05-citizens/civicInitiativeEngine.js` | Initiative_Tracker | schema-setup + own-tab | tab is `requireTab_` on the cycle path (engine.119); `seedInitiativeTracker()` is the menu creator. civic.38 Task 4 step 1: `ensureInitiativeStageColumns_` appends the six stage columns (Stage, StageBaseline, LastStageChangeCycle, LastWorkCycle, LastWorkSeat, PriorPhase) to the header, ≤1× per column, called from `runCivicInitiativeEngine_` before the grid read — a real cycle-path caller, so it self-arms on live |
