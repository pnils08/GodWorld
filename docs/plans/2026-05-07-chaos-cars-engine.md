...
- **Status:** [x] DONE S423 (research-build) — **emitter already existed, unattributed to this plan.** `emitChaosCars()` was built + wired into the summary output on 2026-07-06 (`2ae2464d`) under a different initiative tag (`ES-3 / governance.46 T5` — "no read-side consumer ingested Chaos_Cars"), not credited against this plan's T5.2 and carrying zero test coverage. Verified this session against the T6.1 dry-run's real synthetic Tier-1 fixture (`output/chaos_cars_tier1_fixture.json`) rather than rebuilt: renders a table of ALL cycle events (vehicle / dice outcome / target+tier / metric / magnitude / floor-fired flag) rather than a Tier-1-only "scandal entry" list — broader than the plan's original spec, satisfies the acceptance criterion (named entity present, `ConsequenceFloorFired` visible per row) and arguably better (full context, not just scandals). Gap found + closed: function existed but was not in `module.exports`, so it had zero test coverage — exported it and added Test 9b to `scripts/buildWorldSummary.test.js` (11 assertions, all against the real fixture). Gap left open, deliberately: `TargetId` renders as POPID/BIZ-ID, not a resolved name — `buildWorldSummary.js` does not load `Simulation_Ledger` anywhere in its current read set (checked; the only name-resolver present, `resolveFeedNames_`, runs name→POPID for sports-feed rows, the opposite direction) — adding a full ledger read for this cosmetic case is out of scope (FIX not ADD); POPID is the canonical identifier and downstream consumers resolve it. `.claude/skills/build-world-summary/SKILL.md` §What the script reads/emits updated (doc-sync).
...
- **Status:** [x] BUILT + UNIT-TESTED S423 (research-build) — NOT YET LIVE-VERIFIED. Both pieces confirmed absent before starting (grepped both target files, zero "chaos" hits). Built: `scripts/dumpChaosCascade.js` (read helper — filters `Chaos_Cars` for the cycle where `ConsequenceFloorFired=TRUE`, resolves POPID→name by reading `output/simulation_ledger_snapshot.jsonl` directly — POPID/Name are plain columns on the same row, no formatter round-trip — renders perception-only text per the skill's engine→in-world translation contract) + its test (`dumpChaosCascade.test.js`, 12 assertions, stubs only `lib/sheets`; name resolution runs against the REAL snapshot on disk, not a hand-typed stub, per a second advisor-pass finding — the first version stubbed `canon-name-check.profilesForPopids` with a fabricated display string and reverse-parsed it with a regex, testing this script's model of that formatter rather than the real path; fixed same session, before it could go stale silently). **Caught before declaring done (advisor pass): `.claude/skills/city-hall-prep/SKILL.md` carries `disable-model-invocation: true` and last ran interactively at C94/C95 (per session memory) — the actual unattended path is `scripts/cron-civic-run.js --stage=prep`, a DETERMINISTIC NODE PORT of city-hall-prep that never reads the SKILL.md at all.** The SKILL.md edit alone would never fire on a real cron cycle. Fixed: `cron-civic-run.js` now requires `dumpChaosCascade.js`, computes `chaosHits` once before the Step-3 packet-writing loop, and prepends the `**[CHAOS CASCADE]**` block to every office's packet (via `rows[0].title + ' ' + rows[0].holder` as `{voice}`) right before the existing `## City This Cycle` injection point — same per-office loop pattern as the citywide-digest/roster-status blocks already there. `.claude/skills/city-hall-prep/SKILL.md` v1.12 documents the rule for the interactive path (still real, just not the path that runs unattended). Verified: `node -c` syntax-clean, module loads without error; NOT live-fired (correctly — zero Tier-1 hits exist to fire against; forcing one would mean hand-editing live `Chaos_Cars` state, which is not how this closes). **Third advisor-pass finding, closed same session:** the block's telemetry-leak test was a hand-guessed regex (same "model of the checker" class as the two fixes above) — replaced with an actual call to `scripts/lintCivicPackets.js`'s `lintText()`, the real linter `cron-civic-run.js` runs on every packet ("fails loud on any leak" per its own header). Returns `[]` (clean) against the real block text. Also checked: `cron-civic-run.js`'s only decision-counting mechanism is a literal `^## DECISION ` markdown-header regex (line ~440, used for week-carry insertion-point + numbered topic blocks) — the chaos block's "Decision required:" is inline prose, never a `## DECISION` header, so it does not inflate the hearing-gate's decision count. **Ordering note, load-bearing:** `chaosHits` is computed AFTER Step 1.5's ledger-snapshot freshness refresh, so `dumpChaosCascade.js`'s POPID→name lookup always reads a snapshot current for the working cycle — do not move the chaos-cascade check above that refresh in a future edit. **Live acceptance criterion is NOT claimed — it closes only on the next unattended `cron-civic-run.js --stage=prep` run that processes a real Tier-1 hit; manual demo ≠ acceptance per this project's rules.** **Bench proof, engine-sheet, same session (S423).** Seeded a synthetic Tier-1 row on SANDBOX 0831 (bench-only, never-replay per DEPLOY.md): CycleId 110, ambulance/citizen/POP-00003 (Mark Aitken, Tier 1)/medical_emergency, `ConsequenceFloorFired=TRUE`. Ran the real chain against it: `readTier1Hits('110')` (sheet id overridden to bench) → 1 hit, name resolved via the live ledger snapshot; `reactionBlockFor` → the real `**[CHAOS CASCADE]**` block text; `lintText(block)` → `[]` (passes `lintCivicPackets` clean). Control: `readTier1Hits('109')` → `[]`. **This proves the reader→block→lint chain end-to-end — the part that had never executed.** What it does NOT prove: the actual packet write inside `cron-civic-run.js --stage=prep`. Engine-sheet correctly stopped short of running `--stage=prep` against bench C110 — `runPrep()` hard-requires `output/world_summary_c110.md` + `output/engine_audit_c110.json` (live-cycle products of `/engine-review`/`/build-world-summary` that don't exist for a bench cycle), and the stage has no `--output-root`/`--sheet-id` flag, so a run would write C110 artifacts into the LIVE `output/cron-civic/` tree. **Remaining gap, now narrow and named:** the packet-write step closes on the next live cycle with a real hit, or on a deliberately-built scratch harness (synthetic world_summary/engine_audit + an isolated output root) if that's ever worth building. Not filed as a task — flag only.

**Visibility fix (Mike-asked, S423): "how will we know when the next cycle runs?"** Before this fix, the only signal was a `console.log` line inside `runPrep()`, captured to `logs/civic-cron.log` by the crontab redirect — real but not discoverable (nobody reviews a raw log file per cycle). Fixed: `cron-civic-run.js`'s existing `## /city-hall-prep (AUTO — cron-civic-run.js)` section — written into `output/production_log_c{XX}.md` every single prep run, hit or not, alongside Pressure/Ledger-snapshot/Packets/Lint — now carries a `- Chaos cascade (engine.11 T5.3):` line stating either `none this cycle` or the hit count + citizen name + vehicle. **Answer: open `output/production_log_c{XX}.md` §/city-hall-prep for any cycle — it will say explicitly, every time, whether a Tier-1 chaos hit fired that cycle.** No grepping raw logs, no waiting to notice.
...
- **Status:** [x] DONE S423 (research-build). `scripts/chaosCarsDryRun.js` created — runs the REAL `runChaosCarsEngine_` (not a re-implementation) against a 300-citizen fixture whose tier proportions mirror the live 968-row snapshot (7/18/45/230 for Tier 1-4). 5 cycles, all within [3,15]. Row-shape gotcha caught before writing: the engine unit test's `writeChaosCarsRow_` stub captures the raw camelCase orchestrator payload, which is the WRONG shape for T5.2/T5.3 consumers (they read PascalCase sheet-objects via `getSheetAsObjects`). This script wires the REAL `phase10-persistence/saveChaosCars.js` writer instead and zips its queued row against `CHAOS_CARS_HEADERS` — verified byte-shape-identical to a live `Chaos_Cars` row (checked live sheet 2026-09-05: 66 rows, `ConsequenceFloorFired` comes back as the *string* `'TRUE'`/`'FALSE'`, not boolean; header L is `CycleStamp` not the plan's original `TimestampUtc` — schema drifted during the engine-sheet build, T3.9-era rename). Live sheet has zero Tier-1 hits yet (expected, ~0.15 statistically at 66 rows) — a dedicated seed-sweep (mirrors the unit test's Test 6 pattern) found one at seed 90084 after 84 attempts and wrote it + its cycle's 14 sibling events to `output/chaos_cars_tier1_fixture.json` for T5.2/T5.3 to author+verify against. Report: `output/chaos_cars_dryrun_report.md`.
...
- **Status:** [x] DONE, SCOPED S423 (research-build) — **not built to spec verbatim; checked live data availability first, scoped to what's real.** `scripts/chaosCarsMagnitudeReport.js`. Neighborhood scope IS measurable — the plan assumed no `Neighborhood_Map` cell history exists (true, §S265), but `buildWorldSummary.js`'s City State neighborhood table snapshots the 4 movable columns every cycle it runs, and `output/world_summary_c{103,104,105}.md` exist — that IS cross-cycle history. Parsed those tables against 30 live `Chaos_Cars` neighborhood rows: 14 events had both cycle-N and cycle-N+1 snapshots available (16 skipped, no overlap — C100-102/C105 gaps). **Caught before declaring done (advisor pass): the first version classified events by raw up/down direction, but `DECAY_RULES` (§S265 T4.1) key on GOOD-vs-BAD direction per metric — CrimeIndex inverts (down is good, reverts fast; up is bad, lingers). Raw-direction classing lumped 5 CrimeIndex-down "good" swings (correctly reverting) in with Sentiment-down "bad" swings and reported "9 down, 1 persisted," reading as asymmetry failing when most of it was working.** Fixed: `GOOD_DIRECTION` map reclassifies by metric semantics, and a citywide-median-delta confound check flags any event where the whole-city move (e.g. the c104→c105 Sentiment surge, ~+0.4 to +0.5 across every neighborhood) is bigger than the chaos magnitude itself. Corrected result: 7 good swings measured, 7 reverted (100%); 1 bad swing measured, 0 persisted (small sample, real counter-example, reported honestly — not forced to fit); 6 of the original 14 flagged confounded (all c104 Sentiment) and excluded from the tally rather than misread as decay failure. **Business scope NOT measured — explicit gap, not silently skipped:** no historical `Business_Ledger` snapshot exists, `buildWorldSummary.js` emits no business table; would need a new per-cycle dump script (`dumpLedger.js` pattern) to ever measure `applyChaosDecay_`'s effect on `Annual_Revenue`/`Employee_Count`. Citizen scope has no decay row by design. Report: `output/chaos_cars_magnitude_report.md`.
...
All four Phase-5 cascade outputs and all three Phase-6 validators are now built. T5.1/T5.4/T5.5 (engine-sheet, shipped earlier) + T5.2 (credited: pre-existing `emitChaosCars`, verified+tested this session) + T5.3 (built this session, then corrected: the real wiring point is `scripts/cron-civic-run.js --stage=prep`, NOT `city-hall-prep/SKILL.md` — that skill carries `disable-model-invocation:true` and doesn't run on the unattended path at all; the SKILL.md documents the rule for the interactive path, the cron script is what actually fires it) = the full cascade, code-complete. T6.1 (dry-run + synthetic Tier-1 fixture) / T6.2 (magnitude report, scoped to real `world_summary` history, then corrected to classify good/bad swing per metric instead of raw up/down — CrimeIndex inverts) / T6.3 (frequency validator, PASS on live C100-C105) all shipped this session. **Only remaining gate: T5.3's live acceptance** — the next unattended `cron-civic-run.js --stage=prep` run that processes an actual Tier-1 hit, showing the CHAOS CASCADE block in a real packet. Zero live Tier-1 hits so far (66 rows, C100-C105 — expected, ~0.15 statistically). Everything below this line is the prior S265/S274 design history, unchanged.
...
- 2026-09-05 — **Advisor-pass fixes (S423, research-build)** — T5.3 real wiring moved to `cron-civic-run.js`; T6.2 asymmetry reclassified good/bad-per-metric + confound flag (see status logs).
