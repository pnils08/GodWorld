...
// phase already down; a Standing row whose exit is unbuilt or unreadable
// (ruling 3 — `blocked` from stageRequirement). Strict `>`: a row may sit the
// full limit; the Cycle after that it stalls. cycle == reference reads 0, so a
// row never stalls the fire it changed stage.
// input  { stage, phase, blocked, cycle, lastWorkCycle, lastStageChangeCycle, stallCycles, untendedStallCycles }
...
// stuck paying -2 with no handle. Revival is NOT an advance (approval's revival
// guard) and carries no business lift.
// input  { stage, phase, priorPhase, stallCycle, lastWorkCycle }
