...
const Y = 2041; // engine.164: citizenExitDefects_ takes the calendar year; these fixtures are 2041-era
...
assert('a clean row has no defects', A.citizenExitDefects_(H, row(clean), Y).length === 0, JSON.stringify(A.citizenExitDefects_(H, row(clean), Y)));
assert('the live classes: SchoolQuality 5 / blank, CareerStage spelling, MigrationIntent set, employer blank with a role',
  JSON.stringify(A.citizenExitDefects_(H, row(Object.assign({}, clean, { SchoolQuality: '5', CareerStage: 'early-career', MigrationIntent: 'staying', EmployerBizId: '' })), Y)) === JSON.stringify(['schoolQualityUnusable', 'careerStageSpelling', 'employerBlankWithRole', 'migrationIntentSet']));
assert('blank CareerStage is its own class; NetWorth / Income blank; BirthYear out of range', JSON.stringify(A.citizenExitDefects_(H, row(Object.assign({}, clean, { CareerStage: '', NetWorth: '', Income: '', BirthYear: 1889 })), Y)) === JSON.stringify(['careerStageBlank', 'netWorthBlank', 'birthYearOOB', 'incomeBlank']));
assert('a role-less row with no employer is not an employer defect', A.citizenExitDefects_(H, row(Object.assign({}, clean, { RoleType: '', EmployerBizId: '' })), Y).length === 0);
console.log('citizenArchiveRow_ ArchiveNote');
const meta = A.CITIZEN_ARCHIVE_META_HEADERS;
const ar = A.citizenArchiveRow_(H, row(Object.assign({}, clean, { SchoolQuality: '', MigrationIntent: 'staying' })), 'traded-away', 110, Y);
...
assert('a clean exit leaves ArchiveNote blank', A.citizenArchiveRow_(H, row(clean), 'deceased', 110, Y)[H.length + meta.indexOf('ArchiveNote')] === '');
