...
### Two refinements (Mike-approved S260)

**1. The session counter is a boot odometer, not a span marker.** The prior model ("soft close appends to the open session; hard close ends it") depends on a close-boundary judgment Mike doesn't reliably make — he soft-closes for days, so the number froze or drifted (the 259/260 mess). Resolution: **the Session number bumps once per genuine fresh boot, mechanically, zero judgment. Soft vs hard close governs *only ritual depth* (journal write, sweeps) — never the number; both bump per the boot that follows.** Soft-close-for-days then advances the number honestly each boot; it's an instance odometer ("which boot of me is this"), not a work-span claim. **Cycle** stays the meaningful sim-clock (engine-run-driven); Session is just an instance label for commit tags + journal anchors. Increment guard: bump on `SessionStart source=startup` (real new boot), **not** on `/clear` or compaction within one working context. Trade-off accepted: the number inflates with reboot frequency — a mechanically-always-correct counter beats a semantically-tidy-but-usually-wrong one. **Implemented by the governance.35 remnant** (engine-sheet auto-increments the PIN counter at boot from `.claude/state/session-counter.txt`); until then session-end hand-bumps the PIN +1 every close, soft or hard.

**2. NEXT is resumption-state, not a task stub.** NEXT[terminal] answers "what's needed to resume this terminal's in-flight work" — and the work isn't uniform (a design conversation like this S260 thread, or a coding task). So NEXT carries **where the work is + what's decided + the next move**, one line, with a `(claude-mem: <hook>)` pointer when the thread is rich. **claude-mem is the shared backing store** (every terminal boots the same claude-mem); NEXT is this terminal's *entry point* into it. That keeps NEXT one line (no regrowth into a STATUS paragraph) while making it genuinely resumable across both work modes — a task-stub ("Phase 3") loses the conversation; a resumption line + claude-mem hook recovers it.
...
- 2026-07-01 — Extension §minimal-handoff (S283, research-build, Mike-direct). SESSION_CONTEXT hardened to a minimal AI→AI handoff: one header line + PIN + one NEXT line per terminal, NOTHING else — no prose, no tables, no sections. Hard caps mechanically enforced FATAL at close (`sessionEndMechanical.js` `subContextMinimalGuard`: NEXT ≤ 350 chars, PIN ≤ 450, any other content fails). Maintenance-Rule prose + Key Tools table + Current Work section removed (duplicated CLAUDE.md/STACK.md/ROLLOUT); pre-trim NEXT lines archived to SESSION_HISTORY.md (2026-07-01 rotation block). Division of record restated: claude-mem saves the session, git history shows the work, ROLLOUT carries open work. Companion edits: `/session-end` SKILL Step 2 + Step 3, 4× TERMINAL.md carried-set lines.
