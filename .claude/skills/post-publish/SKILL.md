...
**Verification gate:** for each CUL-ID in source NAMES INDEX, `lookup_cultural(name)` returns a card with `last_appeared` reflecting this cycle. If no CUL-IDs in NAMES INDEX, substep skipped with stdout "0 cultural-only entries — substep N/A this artifact." Plan reference: [[archive/plans/2026-04-30-dispatch-gap-followups]] Task E6 (closes the Brody Kale unrefreshed gap from S188).
...
Where `<N>` = the **parser's total parsed-row count** that `ingestPublishedEntities.js` reports — every NAMES INDEX row it parsed. Use that total directly; do **NOT** reconstruct it by summing the resolution buckets (`matched + candidates + ambiguous + phantom + appended`). That sum **omits the canon-drift bucket**, which IS a parsed row (an existing-in-bay-tribune / missing-in-Sim_Ledger citizen, per [[../../../docs/adr/0007-cross-layer-canon-authority-precedence|ADR-0007]]); bucket-summing under-counts by the number of canon-drift rows and the gate then false-fails on the off-by-one against the independent NAMES INDEX row count. (RB-4, C98 G-P-C98-2.) The helper independently counts NAMES INDEX rows in the source `.txt`. Exit code 0 = counts match AND section present; exit code 1 = mismatch OR `--strict` and NAMES INDEX section absent (FAIL LOUDLY, block publish).
...
- 2026-07-10 — v1.10 (S310, research-build). research.23 Task 7 — **Step 1c NotebookLM push added** (all types; `--audio` on edition only). Matrix row + step block + Parallel-OK grouping updated. Non-blocking by contract: `scripts/notebooklmPush.js` exits 0 on runtime bridge failures (auth rot / rate limits / API drift); verification gate accepts `complete` OR `(non-blocking)` warning. Plan: [[plans/2026-07-10-notebooklm-bridge-deploy]].
