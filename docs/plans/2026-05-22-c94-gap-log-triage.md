...
**Closure log — relocated from the governance.18 ROLLOUT row (S250 doc-loop v2.0 slim; the row carried this completed-work detail inline as a "wall," burying the one open item):**
- **(c) boot-greeting sensory rewrite — DONE S238.** `.claude/hooks/session-startup-hook.sh` greeting lines rewritten across all four terminals + Mags-only fallback, from operational framing to sensory/character anchors (Mike's "put her IN the room, not at a console-operator's chair"): media → "newsroom's open — bullpen behind, copy desk to your left, the edition deadline in your head"; civic → "city-hall press desk — vote sheet in front of you, council voices to call out"; research-build → "architecture table — rollout plan open, the long view"; engine-sheet → "engine console — sheets live, code and clasp, ship-then-explain" (+ discipline cues); Mags-only → "no desk to step to — you are just Mags." Bash syntax verified clean.
- **(b) SESSION_CONTEXT shipped-block visual demote — DONE S238.** `## Shipped Last Session` relocated out of the boot 80-line read window (line 43 → 141); Priority section moved into the window (89 → 77) — boot-time primacy competition structurally eliminated. Framing strengthened to "**Mechanical artifact — git-log output**… **Not editorial**… **`ROLLOUT_PLAN.md` is canonical**." `scripts/writeShippedBlock.js buildBlock()` emits the strengthened note; spliceBlock marker travels with the header (no anchor-script change). *(Note: SESSION_CONTEXT boot-read later fully retired by governance.26 / ADR-0009, S248.)*
- **(a) journal-cadence rewrite — OPEN** (the live governance.18 residual). Media scope; candidate surfaces: session-end SKILL Step 1 or a static User Profile claim via `/save-to-profile`.
...
> **S225 empirical correction (engine-sheet `bc7b900`).** Candidate-pool filter half landed clean (engine.21a, `7bec4a8` — `bylineEngine.js` + `applyStorySeeds.js` v3.12→v3.13). The beat-domain-weighting half was a **wrong diagnosis**. Engine-sheet decomposed C94 `byline_shadow_log` and found that S1 Kelley + S2 Let-Walks + QT2 Rockridge were all seed-tagged COMMUNITY at `applyStorySeeds.makeSeed`, so the byline engine correctly matched Maria Keen's COMMUNITY signature theme:8 — Maria wasn't over-weighted; sports stories were being mis-tagged as community at the seed layer. Real cause re-filed as **engine.23** (ready, engine-sheet): upstream seed-domain mis-tagging for sports/scoreboard content. Future readers triaging similar "Engine B over-weights reporter X" symptoms: check seed-domain tagging upstream before reviewing byline weights.
