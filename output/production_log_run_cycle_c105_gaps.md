...
- **Suggested action:** OPEN, unowned — **surfaced, not chased** (S-A scope guard; one cluster per session). Three shapes worth weighing, none decided here: (a) let the engine re-stamp `NextActionCycle` when it observes a phase it cannot advance, so a quiet cycle is neutral rather than accusatory; (b) distinguish "overdue because nobody acted" from "overdue because nothing could act" in `classifyInitiativeMotion_`; (c) treat it as pipeline cadence and make the civic chain's non-application loud at pre-flight instead. (c) is the smallest surface and (a) is the one that changes canon, so it needs a builder ruling.
