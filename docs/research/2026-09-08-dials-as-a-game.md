...
- 2026-09-08 (kimi, review) — **Verdict: adopt stands; the six-cut order B → A → C → D → E → F is endorsed, with B and A in the same bench cycle as filed.** Reviewed against the code, not just the cards: `utilities/citizenMemory.js`, `utilities/citizenDialMap.js`, `utilities/compressLifeHistory.js` (fold, `serializeDialState_` :1184, `foldAgedOutEntries_` :1373, per-row loop :462-601), `lib/citizenDials.js`, `lib/wakePerception.js`, `phase05-citizens/bondEngine.js` (:641-846, :1911-1920, :2396-2563), `casinoLedgerEngine.js:680-701`, `runCivicElectionsv1.js:278-296`, `runCareerEngine.js:159-169`, `loadNeighborhoodState.js:36-51`, `utilities/neighborhoodPulseMap.js`. The C106 measure replays clean off `output/simulation_ledger_snapshot.jsonl`: 546/911 all-neutral, every band table exact, negative tags exact including zero ordinary-bad (Neighborhood count 3,085 vs the doc's 3,086 — a one-line bracket-parse nuance, immaterial).

  **The four asks:**
  1. **Cut D contest curve (`p = 0.5 + 0.08×gap`, clamp 0.2–0.8) — hold it.** Bands are −2..+2 (`BAND_SIGNED`, citizenMemory.js:54), so with weight 1 per dial and two named dials per contest the gap spans ±8 and pins at the 0.8 clamp from gap ≥ 3.75 — the fully-polar mismatch caps at 4:1, the dice always speak (§2), and the 0.2 floor is exactly the bench target's underdog bound. Three conditions: (a) pin weights at 1 per dial and exactly two dials per contest in the plan, else an unnormalized weight set silently moves the clamp engagement point; (b) "credential against drive + composure" needs credential converted onto the same −2..+2 scale before it enters the gap — it is an ordinal level today, and unconverted it dominates the gap arithmetic; (c) the "underdog wins 20–40%" bench measure is only meaningful on a post-A/B population — on today's converged dials most contest gaps are 0–2 (p ≈ 0.50–0.66, near coin flips, and "underdog" is undefined at gap 0), so D's bench must sit on the A/B bench state and should log the gap distribution alongside the scores and outcome it already plans to log.
  2. **Warmth-as-maintenance (Cut C) vs §11 — no conflict; one correction.** §1 explicitly hands the engine "how fast bonds grow," and a maintenance term on intensity drift reinforces §11's establish → maintain → mature → marry shape rather than shortcutting it. The correction: the bond engine is **not** dial-blind today, contra the doc's 22-engine list — `bondWarmthFactor_` (bondEngine.js:1911-1914) already scales FRIENDSHIP growth ×0.75–1.25 off pair warmth, and `bondFamilyFactor_` (:1917-1920) rides the GC chance — both read the TraitProfile *face* via `bondTraitOf_` regex, not the band surface. Cut C must **consolidate**: one warmth term sourced from `getCitizenDialBands_`, replacing the face-regex read, not stacked beside it — otherwise friendship carries warmth twice (growth × maintenance). Note for scope: the face read means today's warmth factor tracks the cadence-lagged face, not live base.
  3. **Ambient retune to ±1 (Cut B) — the size is right.** Arithmetic under per-cycle fold + `MOOD_DECAY` 0.8: a steady +1/cycle one-line drip equilibrates mood near 5 and hardens ≈ 1.2 base per 3 cycles — a band crossing in ~15+ cycles of sustained one-direction living, versus today's single-fold +3.6 saint jump from three Neighborhood lines landing in one batch. Roughly an order of magnitude slower texture, big events (Promotion +8, Divorce −8) still dominate — that is the intended shape. The residual risk is not magnitude but **sign**: post-retune the ambient floor is still all-positive, so the negative pole rests entirely on the new collision emitters. Set a bench sign-ratio target (negative share of net nudges — the doc gives none) so "the feed is one-signed" has a measurable exit, and expect the 12-cycle all-neutral fall to come from the collision tags + persisted mood (the wake gate reads base + mood — `lib/wakePerception.js:401` via `currentDials`) more than from hardened base.
  4. **Hood-sign tint double-count (Cut B) — no conflict with anything live; the real double-count is inside Cut B itself.** Verified: no existing path feeds hood pressure into citizen dials or mood — RentShare's citizen footprint is rent → burden → `DisplacementRisk` → `MigrationIntent` (migrationTrackingEngine.js:246-267), `citizenDialMap.js` has no hood read, and `S.neighborhoodPulse` is ctx-local. What does double-count is Cut B's own two channels on one cause: the same rent pressure emits **Friction** (−2 composure, at the collision) *and* flips that citizen's Neighborhood lines to −1 composure (the ambient tint). Make the tint exclusive — a citizen who took a pressure-collision tag this cycle does not also get the tint — and source the hood sign from the **persisted** Neighborhood_Map pressure fields, not the same-cycle `S.neighborhoodPulse` accumulator: the pulse is built from the same citizens' events (`neighborhoodPulseMap.js:110-125`), so keying the tint off it is a same-cycle circular edge.

  **Doctrine check:** §10 fencing confirmed — the family-match and GC marriage lotteries stay two reels and a miss; note for the plan that the live GC chance already carries the Mike-ruled engine.59 fitness and family factors (`bondEngine.js:2455-2458`), so the fence reads as *no new character terms and no character scan*, not a relitigation of engine.59. §2 holds: the curve's 0.2 floor and the kept need-sort ("poorest first is the cause" — hireSlotOrder_ comment at runCareerEngine.js:160 already concedes credential never decides today) are causes-then-dice in the right order. No ninth dial (engine.157) and no raw floats in prompts (research.22) — Cut E's goal/posture/tags-as-words surface is compliant.

  **Hazards found beyond the doc's own list:**
  - **Cut A silently kills engine.38 B3's unlived capture unless it is carried.** `regs.unlived` rides `foldAgedOutEntries_` "fold-once by construction" (:1383-1393); a watermark fold that retires the trim-path fold must move the unlived capture to arrival time or it stops capturing with no error. Same cut owes a comment sweep: `serializeDialState_` header "mood … never stored" (:1181), the S269 "SUPERSEDED … zeroMood_ wipes mood" rationale on `accreteReflectionsIntoBase_` (citizenMemory.js:116-125), and the fold/trim agreement header (:1360) all describe the pre-A world.
  - **Cut A watermark needs a stated rule for legacy entries with no parseable cycle stamp** — fold-once-via-legacy-path or skip, chosen explicitly; a silent default is silent canon drift on the archive-era lines that are 75% of the problem.
  - **Cut B has a ratchet, not just a pole.** A below-median-income citizen in an expensive hood fails the Friction test *every cycle indefinitely*; at −2/cycle with streak hardening, composure bakes to the 0 floor in ~15 cycles with no designed recovery. Keep "no citizen pins at 0 or 100" as a hard bench **fail** criterion, and consider recurrence decay (first breach Friction −2, ongoing breach Strain −1) so chronic pressure wears rather than breaks — §3 wants the world allowed to hurt people, not a mechanical monthly grind to the floor.
  - **Cut E write-back ordering.** The one-notch posture push must apply *after* Phase5-Maneuver's per-cycle recompute or it is overwritten unread, and it must not lift a citizen out of a cause-held retreat (DebtLevel ≥ the bar): a wake sentiment overriding an engine cause is the cron steering a fate — the exact thing §13's fact/color line and the doctrine test forbid. Bounded as filed (one cycle, then decays) it is safe only with that clamp.
  - **Cut C casino composes with engine.157, doesn't bypass it.** The drive read at `casinoLedgerEngine.js:694` already flows through the posture factor when the maneuver phase ran (:693-695); the new integrity-refusal and tilt-stake bands should gate *around* that override, not re-read raw drive beside it.
