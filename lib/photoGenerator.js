 ...
 *
 * Provider: Together AI (FLUX.2 pro — per-image billing; engine.37 bump from FLUX.1 schnell)
...
var OAKLAND_SCENES = {
  ...
  // engine.99: key was 'Piedmont Avenue' — ledger spells 'Piedmont Ave', so the
  // exact-match lookup at buildPrompt never hit and Piedmont Ave articles silently
  // lost their setting line. Prose value keeps the street's real name.
  'Piedmont Ave': 'Piedmont Avenue, boutique shops, Fentons Creamery, neighborhood charm',
 ...
 */
function extractScene(text, headline, beat, sectionName) {
  var lower = (text + ' ' + headline + ' ' + (sectionName || '')).toLowerCase();
  var scenes = [];
  var sportsScenes = [];

  // Tech / startup scenes — varied by beat so supplementals don't get 5 identical photos
  var hasTechKeywords = lower.includes('tech') || lower.includes('startup') || lower.includes('software') ||
      lower.includes('developer') || lower.includes('engineer') || lower.includes('artificial intelligence') ||
      lower.includes('venture') || lower.includes('coworking') || lower.includes('co-working') ||
      lower.includes('ecosystem') || lower.includes('digital') || lower.includes(' ai ') ||
      lower.includes('anthropic') || lower.includes('discord') || lower.includes('biotech');

  if (hasTechKeywords) {
    if (beat === 'front-page') {
      scenes.push('Modern tech office in a converted Oakland warehouse, exposed brick walls, developers at standing desks with large monitors, whiteboards with diagrams, natural light through industrial windows, collaborative open floor plan');
    } else if (beat === 'general' && (lower.includes('citizen') || lower.includes('neighbor') || lower.includes('resident') || lower.includes('voice'))) {
      scenes.push('Diverse group of Oakland residents gathered on a neighborhood sidewalk in conversation, mixed ages, casual and animated discussion, residential street with mature trees, warm afternoon light');
    } else if (beat === 'general') {
      scenes.push('Aerial view of Oakland waterfront at golden hour, modern office buildings along the estuary, cranes and construction in the distance, city skyline with hills behind');
    } else if (beat === 'civic') {
      scenes.push('Oakland City Hall exterior at dusk with modern office towers visible behind it, civic architecture meeting tech industry skyline, warm evening light, pedestrians on the plaza');
    } else if (beat === 'opinion') {
      scenes.push('Oakland street corner with a historic brick building alongside a modern glass-fronted tech office, contrast between old and new Oakland, afternoon light, diverse pedestrians, neighborhood character');
    } else if (beat === 'letters') {
      scenes.push('Diverse group of Oakland residents gathered on a neighborhood sidewalk in conversation, mixed ages, casual and animated discussion, residential street with mature trees');
    } else {
      scenes.push('Aerial view of Oakland waterfront at golden hour, modern office buildings along the estuary, cranes and construction in the distance, city skyline with hills behind');
    }
  }
  ...
  // Sports scenes — kept separate so we can deprioritize when beat isn't sports
  if (lower.includes('stadium') || lower.includes('coliseum') || lower.includes('ballpark') || lower.includes('baseball') ||
      lower.includes("a's") || lower.includes('batting') || lower.includes('pitcher') ||
      lower.includes('baseman') || lower.includes('inning') || lower.includes('home run')) {
    sportsScenes.push('Baseball stadium during a game, players on field, crowd in the stands, green grass diamond under lights, warm summer evening atmosphere, dugout visible');
  }
  if (lower.includes('basketball') || lower.includes('warriors') || lower.includes('arena') || lower.includes('bulls') ||
    ...
    sportsScenes.push('NBA basketball arena during a game, players on court, crowd energy, arena lighting');
  }
  ...
  // Only include sports scenes if beat is actually sports, or if no other scenes matched
  if (beat === 'sports') {
    // Sports beat: sports scenes go first
    scenes = sportsScenes.concat(scenes);
  } else if (scenes.length === 0) {
    // Non-sports beat but nothing else matched: use sports scenes as fallback
    scenes = sportsScenes;
  }
  // Otherwise: non-sports beat with other matches — sports scenes are dropped
...
// ---------------------------------------------------------------------------
// Together AI Provider (FLUX.2 pro — per-image billing)
// ---------------------------------------------------------------------------

/**
 * Generate an image using Together AI's FLUX.2 pro model.
 *
 * @param {string} prompt - The image generation prompt
 * @param {Object} options - Generation options
 * @param {number} [options.width=1344] - Image width
 * @param {number} [options.height=768] - Image height
 * @param {number} [options.steps=28] - Inference steps (FLUX.2 pro default 28, up to 50)
...
function generateWithTogether(prompt, options) {
  ...
  var modelId = options.model || 'black-forest-labs/FLUX.2-pro';
...
async function savePhoto(result, outputDir, filename) {
  ...
  if (result.url) {
    return new Promise(function(resolve, reject) {
      var download = function(url, redirects) {
...
module.exports = {
  ...
  generateWithTogether: generateWithTogether,
  savePhoto: savePhoto,
