...
| engine.250 | Initiative effects bus is emptied in phase 2 (`applyCityDynamics.js:1536`) before its phase-3 school-drift reader and phase-5 business-drift reader run — education initiatives never move school quality, businesses never feel an initiative; tests seed the field directly and mask it. Sibling bus, same fold: `S.approvalNeighborhoodEffects` is written in phase 5 (`updateCivicApprovalRatings.js:930-942`) into a summary rebuilt fresh every cycle (`godWorldEngine2.js:248`) and read in phase 2 — live Ripple_Ledger has 6 initiative-fold rows (C103–C108) and 0 approval-fold rows ever, so an official's approval has never reached a hood (engine.93's open live proof fails) | ready | engine-sheet | wiring card `output/agent_engine-wiring_2026-09-20T19-28-00.md`; blocks civic.38 Task 4 stage-3 for education + economic domains |
