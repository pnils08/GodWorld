...
**engine.178 (S438): dials CLOSE DOORS — band gates at outcome rolls, not frequency knobs.** openness −2 refuses the misfit relocation lane / +2 lowers its income ratio (`migrationTrackingEngine.js` `processRelocations_`); integrity +2 does not sit at the casino, composure −2 draws the tilt stake band (`casinoLedgerEngine.js`, around the engine.157 posture factor); the bond engine reads warmth/family as signed bands off live DialState (`bondDialBand_`, face fallback) and warmth scales the stale-bond decay — maintenance, not formation (`bondEngine.js`); an owner's composure moves the closure streak bar ±1 and drive +2 multiplies a positive drift (`applyBusinessDynamics.js` `bizOwnerBands_`). The owner reader treats an explicit POPID as authoritative: an absent ID or an ID with no DialState never falls back to a name; duplicate full names resolve null even when one namesake has blank DialState, while a later valid owner entry may still resolve. Its per-function caches are private and it adds no RNG or cell mutation. This reader is separate from wealth's `resolveOwnerRow_`, which requires name agreement. An officeholder's integrity band scales the approval-ceiling scandal chance (`updateCivicApprovalRatings.js` `holderIntegrityBand_`). Every site passes the row's DialState string — `ctx.citizenLookup` is built late in Phase 5.

**engine.179 (S438): contests resolved by character.** `contestRoll_` (`utilities/citizenMemory.js`) — two band terms, `p = clamp(0.5 + 0.08·gap, 0.2, 0.8)`, `ctx.rng`, tallied in `S.contests`. Sites: the last job slot when the left-out candidate shares the need band (drive + credential, `runCareerEngine.js` matcher); the heritage storefront stake among members over the $100k floor (drive + integrity, `generationalWealthEngine.js` `updateHeritage_`); the civic challenger nomination between two weighted draws (sociability + integrity, `runCivicElectionsv1.js`); the romantic triangle tick (warmth + sociability, `bondEngine.js` `romanceRivalOf_`). Fenced by §10: the family-match and GC marriage lotteries never contest.
...
| Ambient (tints, engine.176) | `Neighborhood` sociability:1 · `Daily` composure:1 · `Personal` openness:1 · `Background` +1 · `Micro-Event` +1 · `PrevEvening` outabout:1 · `Sports`/`Holiday`/`FirstFriday`/`CreationDay` outabout:1 — ±1 by rule (S438; Neighborhood was +3 across 3,086 live lines) |
| Pressure (engine.176, from causes) | `Friction` -2 (first breach: rent burden over `dialFrictionRentBurden`, DebtLevel ≥ `maneuverRetreatDebt`, hood at `dialHoodPressureBar`/`dialHoodCrimeBar`) · `Stumble` -2,drive:-1 (first cycle unmatched in the hire windows) · `Strain` -1 (ongoing) · adapted after 6 consecutive cycles (nothing emitted until a clean cycle) · one pressure tag per citizen per cycle via `emitPressureTag_` (`utilities/citizenDialMap.js`) · `Career-Layoff` -5,drive:-2 · `[Casino]` loss text -3 / `[Setback]` at ≥ `dialSetbackLossPct` of net worth |
...
- **`updateExistingBonds_`:** intensity moves by mutual-activity (rivalry +1.5/alliance +0.5/...), cycleWeight/shock/sentiment, **calendar** (festival/sports-season/family-holiday/community-holiday/FirstFriday/CreationDay), arc-proximity (+0.5, +0.5 at peak), decay (no-neighborhood/off-season, and **engine.201b neglect = a cycle the pair did not share**: one party active −0.5, neither −0.7, × (2 − pair warmth); the old `bondAge > 15 && lastUpdate > 5` clock is gone — CycleCreated was reset to C102+ on live and lastUpdate is a bloat stamp, not contact), family/professional/neighbor +0.15 × warmth on a shared cycle, clamp 0–10. Status active↔dormant (≤1/≥3); **tension escalates→rivalry (≥6) or settles→professional (≤2)**. `lastUpdate` stamped only on meaningful change (v2.7 ledger-bloat fix).
