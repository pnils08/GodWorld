...
| engine.187 | Shock detector stuck-on: `S.shockFlag` = shock-flag 19/19 cycles (C95–C113). Event-spike gate is relative; another gate in `applyShockMonitor.js` fires unconditionally. Find with `scripts/deadBranchScan.js`. Readers inherit it: v3 'Shock event zone', applyCycleWeight | needs-info | engine-sheet | ROLLOUT row (S441) |
| engine.188 | Sentiment TOP end got the 185 treatment. Row's premise corrected: the C114-C119 'climb' is the calendar (cy 6-9 Winter -> cy 10-15 Spring, + StPatricks/Equinox/Easter/2x First Friday). Real load, measured live C101-C106: boost echo +0.26, sports phase +0.20, events/seeds +0.08, seed cluster +0.07, unemp<5% +0.05, season net +0.05. Six cuts, one file. Resting Spring 0.94 -> 0.42. Commit `8db9ee1b`, test `sentimentRestingLevel` 23/23 | in-progress | engine-sheet | ROLLOUT row (S443) |
...
| engine.193 | Adversity tiers sit behind doors the world never opens: unemployment `> 0.08` / `> 0.12` fire 0 of 22 hoods (live range 2.19-5.92%, median 4.45%); sickness `> 0.10` epidemic tier fires 0 of 22 (range 2.51-6.23%). engine.185's headline repricings are unreachable. Fitting thresholds to the range fits the gate to the data — the question is why the generators produce no depression. SIM: needs Mike | needs-info | engine-sheet | commit `8db9ee1b` |
