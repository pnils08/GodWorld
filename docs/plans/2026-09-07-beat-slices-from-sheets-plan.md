...
- 2026-09-10 (kimi, captured by research-build S442 from the kimi pane — kimi hit its weekly quota wall mid-lane and wrote nothing to disk) — **Education lane audit (Angela Reyes, `buildSchoolsSlice.js`), diagnosis only, no fixes applied.** (1) **The school table is frozen backfill:** nothing in the engine drifts `SchoolQualityIndex` / `GraduationRate` / `CollegeReadinessRate` / `TeacherQuality` / `Funding` on Neighborhood_Demographics — backfilled once by script, no phase touches them; half of Angela's cycles are "weakest school" stories off static data. (2) **Routing is faith-class broken twice:** `storyHook.js` deskMap sends EDUCATION hooks to an "Education Desk" no roster carries; those hooks are `hookType: 'demographic'`, mapped to the community signal before the education signal is consulted, so from C84 they pre-match Sharon Okafor, never Angela; DROPOUT_WAVE alerts bypass `makeHook` and land with no desk and no journalist, re-alerting identically every cycle (West Oakland 62% forever). (3) **What actually moves:** per-hood `Students` counts drift every cycle (population engine) and the slice does not use them; age-18 graduation/dropout credentials mint onto ledger Education fields and are never surfaced; citizen education LifeHistory texture (≤10 events/cycle) is never read. (4) **Ledger:** OUSD is BIZ-00016 (5,201 employees) with `Annual_Revenue: "-15"`; nobody joins district data to the per-hood Funding column; the roster is mis-mapped — 12 of 29 active BIZ-00016/00035 rows are off-role: POP-00040 City Manager, POP-00198/00267/00649 line cooks, POP-00217 Plumber, POP-00226 Bakery worker, POP-00239 Mover, POP-00532/00631 Youth Basketball Coach, POP-00539 Actor, POP-00641 Taxi Driver, POP-00743 Grade Schooler (BirthYear 2030, CareerStage student). All `MappingLayer: existing` — `EmployerBizId` was already on the sheet; 9/12 carry SkillTags `Education`, so the field tag, not the job, placed them. Filed **engine.191**. Also canon.5 still holds the BIZ-00016 rename (OUSD → Oakland City Schools). (5) Slice `desk: 'civic'` vs rosterLookup `culture` — kimi's own question, unruled. **Kimi's proposed course:** scripts-side — `buildSchoolsSlice.js` v2 consumes EDUCATION + DROPOUT_WAVE hooks by domain, leads with Students deltas vs `prev/`, adds an OUSD district card, widens the people pool to educators citywide; engine-side — deskMap EDUCATION to a desk that exists + the demographic→community short-circuit, DROPOUT_WAVE normalization carries desk/journalist. Engine routing cuts wait on the same shape of pass as engine.189/190/191.
- 2026-09-10 (Mike-direct, S442) — **Ruled: drift, not static backdrop.** School table gets a drift pass — filed **engine.192**, doctrine captured [[../SIM_DOCTRINE]] §16 ("A column that never moves is scenery, not simulation"). Standing direction, not just this table: **gravitate every system toward drift** — a value that moves per-cycle off its own causal inputs, over a one-time backfill that only ever gets read. Applied case-by-case as the lane program finds instances (Watch List carries the project-wide scan, not a build yet).
- 2026-09-14 (kimi) — **Education lane scripts-side landed** (builder rulings 2026-09-10, this lane's session: desk question SKIP — domain routing covers it; frozen school table — PROPOSE drift engine; scope FULL COURSE). `buildSchoolsSlice.js` v SCHOOLS-SLICE-2: enrollment movement (Students deltas vs prev/, the one live education number) leads; the frozen quality table states itself ("unchanged since C<N>", self-retires when a drift engine lands); district card by BIZ-00016 ID (canon.5 rename-safe, `-15` revenue artifact not printed); EDUCATION/DROPOUT_WAVE/SCHOOL_QUALITY_CRISIS hooks consumed by domain; people pool falls back to citywide educators named as such. Verified on the live C107 dump (prev=C106 rotation active). Engine cuts + drift-engine options filed: `docs/for-claude-review/2026-09-14-kimi-education-lane.md`. Health_Cause_Queue drop (ruling 2) NOT done — C107 dump shows 3 rows, the "stays empty" gate failed.
- 2026-09-15 (kimi) — **Culture lane landed** (builder-directed lane program; scope FULL COURSE ruled 2026-09-14). `Cultural_Ledger` joins the dump as an OPTIONAL tab (17 tabs; 51 rows live). Three per-seat slices RIDE ALONGSIDE the evening pack (the pack keeps nightlife/TV texture; the slice leads the typed packet — `selectTypedSlice` reads beatSlice first): `buildArtsSlice.js` (Kai — fame movers rising/new/fading off Cultural_Ledger, universe-linked figures resolve to ledger citizens, culture seeds, ARTS/CULTURE/NIGHTLIFE/CULTURAL/FESTIVAL hooks by domain), `buildLifestyleSlice.js` (Sharon — fading-first fame file, CELEBRITY/FAME_WATCH hooks), `buildNeighborhoodSlice.js` (Maria — Community_Programs by rotating hood with founders, hood population movement vs prev/, NEIGHBORHOOD_*/CITIZEN_RELOCATED/RENT_BURDEN hooks, capped 8). `beatSliceKit.domainHooks()` is the shared name+domain fallback. Wired: fanout BEAT_BUILDERS += 3 (evening enrich auto-skips beat-sliced seats; pack texture persists via the write-stage inject); cron-desk-run BEAT_BUILDERS/BEAT_NAME_RE/beatSlugForName += 3 + an after-chain beat load for evening seats. Live-verified C107 all three. Engine cuts filed (`docs/for-claude-review/2026-09-15-kimi-culture-lane.md`): **raw-carried hook normalization (storyHook.js:1384-1394) drops desk+journalist — 38 desk-empty rows at C106-107 across FAME_WATCH/NEIGHBORHOOD_*/CITIZEN_RELOCATED/RENT_BURDEN/DROPOUT_WAVE; one cut heals four lanes**; deskMap holes ARTS/CELEBRITY/FESTIVAL + phantom desk strings ("Culture Desk"/"Education Desk" match no roster desk); trajectory/migration ripples land on the business lane (Celeste Tran material). NOTE: the Cultural_Ledger wiring-card run died on an OpenRouter 403 (key limit) — contract verified by direct code read instead; card rerun noted for the reviewer.
