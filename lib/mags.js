...
function loadWorldState() {
  ...
  try {
    ...
    if (wsFiles.length > 0) {
      ...
      // Cap at ~8000 chars (~2K tokens) — the top sections (city state,
      // neighborhoods, events) carry the current-state juice.
      return '## Oakland Right Now — live world state\n\n' +
        (wsRaw.length > 8000 ? wsRaw.slice(0, 8000) + '\n…(truncated)' : wsRaw);
    }
  } catch (wsErr) {
...
module.exports = { ROOT, MAGS_DIR, LOG_DIR, FAMILY_POP_IDS, loadIdentity, loadJournalTail, loadWorldState, loadCitizenKnowledge, loadArchiveKnowledge, loadEditionBrief, loadLatestEdition, searchDisk, loadNotesToSelf, loadTodayConversationDigest, getCentralDate, searchSupermemory, saveToSupermemory, loadFamilyData };
