...
* **Antigravity (`agy`)** — runs on a Gemini Pro account. Was gated/proposal-only at S332; now actively building under review (e.g. pipeline.56 lore-writer spec, quarantine + Rhea gated) — the gate moved from "can't write project files" to "reviewed and committed by an authorized Claude session before it ships," not removed.
