...
**Current bench pointer:** SANDBOX 0908 web app **@36 = repo `e31399b9` (PROD @87 + engine.223/223b carry save guard + engine.219 hood economic carry) + bench-only diag217 + bench-only diag219 (`output/engine-sheet/engine219-diag-overlay.js`, never ships).** Isolated build from `git archive e31399b9` + both overlays, sandbox `.clasp.json` written last and grep-verified, version 36, `clasp deploy -i -V 36`, read back @36. **RESYNCED FROM LIVE C107 2026-09-14 ~22:50 (S461, third typed resync of the session; the bench's PropertiesService still carried the @35 run's C109 stamps, so the first fire logged engine.119 ghost-skips → sheet recoveries for the three original keys — expected, not a finding).** C108 + C109 FIRED CLEAN: ok:true 195 s / 157 s, 0 Engine_Errors, 133/133 both. engine.219 PROVEN: C108 wrote `PREV_HOOD_ECON_JSON` (338 chars, 22 hoods; range 54.14–56.96 after Phase 6, 54–57 after migration; `PREV_CYCLE_STATE_JSON` unchanged at 8,244 chars); C109's Phase 2 opened on that set 22/22 with no `carryForward` events. engine.223: no `prop-too-large` (blob under the cap; the 9/9 vm test is the proof). Economy identical to @35 (55.83/56 → 53.63/54). Readback `output/engine-sheet/bench-readback-219-c108-c109.md`. Bench cycle now **C109**; BENCH-ONLY since this sync: C108–C109 and everything they wrote. SHIPPED as PROD @88 (version 77) the same session. Was @35 = `8efb8ec4` + diag217 (engine.222 proof; shipped as PROD @87). Was @34 = `a05aa6b1` + diag217 (engine.221 proof, C108–C109 on the session's first C107 resync: 59.01 → 55.95/56 → 53.78/54; shipped as PROD @86). Was @33: SANDBOX 0908 web app **@33 = PROD `c37d85ea` + engine.217 (`9189addf` v3Integration only) + engine.218 (`3e6a8ea1`, four hospital files) + bench-only diag217 emit (`output/codex/engine217-diag-overlay.patch`); engine.214 EXCLUDED byte-for-byte (`applyCityDynamics.js` == PROD). Isolated build from `git archive c37d85ea` + overlays in a scratch dir; versions 31 (217 only, never fired) → 32 (217+218, never fired) → 33; pinned with `clasp deploy -i -V 33`, `clasp deployments` read back, pull-back 168 files byte-identical. RESYNCED FROM LIVE C107 2026-09-14 (S459, codex-run `syncSandboxFromLive.js --apply` twice — the second after codex fixed the script to typed values `91ca74bc`; 82 tabs / 53,053 rows, read-back OK). C108 + C109 FIRED CLEAN on codex's go: ok:true 206 s / 130 s, 0 Engine_Errors, 133/133 phases ok at C109 (C108's per-phase list lost to a helper truncation — capture gap, on record). engine.217 PROVEN: `diag217.economyRuns` 1, `modules` 5/5 both fires; money loop read the carried mood both times (`diag61.mood` 59.01 at C108 = live C107 slot, 48 at C109 = C108's persisted post-migration value). engine.218 SMOKE ONLY: both hospitalized rows recovered at C108 before a new hit was eligible, 0 state lines, incomes unchanged, CareerState cells 15 → 17 none lost — the 39-case regression is the causal proof. Mood actuals: Phase-6 48.81 → 48.59, persisted 48 → 49 (the C108 step from 59.01 did not repeat; Phase-6 start value under codex's trace). Readback: `output/codex/bench-readback-c108-c109.md`. Bench cycle now **C109**; BENCH-ONLY since the S459 sync: C108–C109 and everything they wrote (10 feeder mints, ring slots 108/109). **Codex final ruling 02:35: engine.217 ACCEPTED (two-Cycle proof), engine.218 ACCEPTED as regression + bench smoke only (activation unproven), diag217 BENCH ONLY (never ships); further bench fires, LIVE deploy/fire and push HELD by codex — acceptance does not lift those gates.** New source findings for the next repair (codex, in run-cycle plan Task 7 + HANDOFF): C108 `FACTORY_CLOSURE −20` ripple came from a road-closure decision (`economicRippleEngine.js:533-534`); calendar writes `S.simMonth`, economy reads `S.month` (seasonal case never matches); city economic mood starts from base 50 at Phase 6 (carry unread by the producer).** Was @30 = `f30b8000` (S458, engine.214 sheet-derived cluster membership; codex LAND `output/codex/engine214-confirm.md`; never fired — superseded by the S459 rebuild, engine.214 rejected by the builder). Was @29 (`c37d85ea`, engine.213c+d: mood absolute + smoothed). Fires C108 (@27), C109 (@28), C110–C111 (@29) all ok:true, 0 Engine_Errors, ten `approval*` keys present. **What the bench showed:** hood sentiment flipped +0.46 → −0.14 at C108 with zero world events and stayed negative (engine.165 sawtooth), so every seat slid to ~50 by C111 — the poll faithfully reports an unhappy city; the input is the defect. Bench cycle now **C111**; BENCH-ONLY since the S451 sync: C107–C111 and everything they wrote. Proof in [[DEPLOY_HISTORY]] §PROD @84. The following prior Bench paragraph is historical/superseded and does not describe the current bench.
...
**Live:** PROD **@88** (2026-09-14 ~23:18 Chicago, engine.219 hood economic carry + engine.223 carry save guard) = repo commit `e31399b9`; pushed from an isolated `git archive` stage + version 77 + `clasp deploy -i -V 77`, read back @77; pull-back byte-identical on every engine dir, 0 test files, no diag overlays. Bench SANDBOX 0908 @36 C108–C109 clean on a fresh live-C107 resync (hood range 54–57 recorded, C109 Phase 2 opened on C108's set 22/22). Live remains C107; Sunday's C108 is the smoke for @86 + @87 + @88 (expect: one `PREV_HOOD_ECON_JSON missing` diag then a hood row on the ring, Phase-6 mood ≈ 55–56, doldrums in the ring, Month 1 on WorldEvents rows, 0 Engine_Errors). Full details: [[DEPLOY_HISTORY]] §PROD @88 / @87 / @86. Was **@87** (2026-09-14 ~22:45, engine.222) = `8efb8ec4`, version 76; **@86** (engine.221) = `a05aa6b1`, version 75; **@85** (engine.217 + 218) = `3006ba67`, version 74.
