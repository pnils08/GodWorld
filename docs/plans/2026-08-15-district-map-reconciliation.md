...
|---|---|---|---|
| 1 | ~~`Montclair` → writer C map as D6; seed `Neighborhood_Map` row~~ **WITHDRAWN S423 (builder 2026-09-05: not a tracked hood — no map row, no citizens, no businesses; the "ruled aboard for D6" line had no ruling behind it)**. Node mirror stripped to 22; ghost `Crime_Metrics` row is a hand delete | engine-sheet | withdrawn |
