...
- [ ] **Streak-gated event eligibility (Mike-direct 2026-09-25, idea stage — not built):** after a pilot holds a certain streak/standing, they'd become eligible to get pulled into other engine event types (which ones — evening events, exchange 1-on-1s, civic perception — undecided; Mike's words: "would have to decide on something"). Blocked on the identity-join fix (item e) landing first — no point wiring streak-eligibility on top of a credit pipe that doesn't reach the real pilot.
- [ ] **Deeper mission capability (Mike-direct 2026-09-25, idea stage — not built):** give the pilot agent more instruction/ability in the actual SpaceMolt game rather than the fixed mine-sell-refuel script, so it can be "good at it" — more of the game's real surface (upgrades, modules, varied objectives) instead of one template. Mike's framing: "just ideas nothing that's likely built yet." Sequenced after the item (e) fix per the same session's sequencing call.
...
- **(a′) `Undocked_Draw` tab contract — PUBLISHED 2026-09-08 (engine-sheet, S438; builder go). Build target for both halves.**
  ...
  - **Truth docs:** `SPREADSHEET.md` + `SHEETS_MANIFEST.md` §8/§9 updated in the commit that landed the engine writer (S438b).
  - **Status 2026-09-08 (S438b):** engine side CUT — `undockedDrawCast_` (draw + tab append + `S.undockedNextCast`), `casinoResolveUndocked_` aggregated pilot-cycle resolve with cycle guards (before = carry, after = void), placement keys `c<N+1>:<POPID>`, legacy episode-keyed slips still resolve. Unit: casino suite 51 checks green. **BENCH-PROVEN (SANDBOX 0831 @81, C119–C121):** C119 wrote the first draw (6 rows for c120) but the casino threw — the resolve rewrite had dropped `casinoResolveSports_`; fixed `33cf0e61` with a main-entry sports test. C120: draw for c121 + 9 show slips (`credits_sign` ×7, `night_winner` ×2) all against the drawn cast, 13 sports, 0 errors. Seeded 4 bench-only feed rows for c121 (POP-00637 40 and −100 with a mishap; POP-00700 250; POP-00833 windowed). C121 settled 9/9 as predicted before the fire: 637-neg ×3 win, 637-pos loss, 700-pos ×2 win, 833 void-gate, night_winner 700 win / 833 loss; float 250000→246970; draw for c122 landed; 0 errors. **Deploy path:** engine code goes to PROD with the dials wave (not cherry-pickable onto @74); the live `Undocked_Draw` tab is created only when kimi's (1)+(2) can fly a fresh cast. Kimi's (3) was exercised against SANDBOX 0831's rows. **LIVE `Undocked_Draw` CREATED 2026-09-08 19:08 (S438b) — headers only, 82 tabs — once kimi's (1) live mint proved (Desai POP-00946, `628d0e4c`). It stays empty until engine.175 is on PROD (dials wave); kimi's orchestrator falls back to the local manifest while it has no rows, so tonight's flight is unchanged. The first PROD fire after deploy draws the next cycle's cast and opens the show market. SANDBOX 0908 deliberately does NOT carry the tab — the dials acceptance run measures dials, and engine.175 is already proven on 0831.**
...
- **(e) Fame/mobility loop target (Mike-restated 2026-09-08).** The top-ranked Undocked contestant should reach A's-player fame — a reality star: Nia Rook's coverage pushes Citizen_Media_Usage on strong performers, and performance should make more events and crons aware of the contestants. `Undocked_Standings` (scripts/undockedStandings.js) is the accumulation layer and already recomputes nightly, but NOTHING promotes on those numbers yet (§2.4 build split: the mobility engine gets its own daylight design). This restatement is that design's stated target — engine-sheet territory, alongside the per-cycle reseed wiring in (a). **Engine-sheet review 2026-09-08 (S438): no design doc needed — the loop exists end to end** (climb ladder 3/6/9, decay, engine.118 fame permanence LIVE @27/@28 S412; Nia's recaps stage and the Saturday sweep ingests ALL staged into `Citizen_Media_Usage`). The one dead wire: `Undocked_Standings` has zero readers — `buildNiaSlice.js` reads the feed pack + recaps only, so coverage never knows who leads. Action (research-build, small): carry rank / CyclesLed / CurrentStreak into the Nia slice; the ladder does the rest. ~~Fame target awaits builder confirmation in a Claude seat (relayed).~~
  ...
  **Builder confirmation landed 2026-09-25 (Mike-direct, research-build session):** "it's likely not adding them to events or really giving the fame the ability to grow" — UNDOCKED is the sim's most-watched show and the leaderboard/Nia coverage is helpful for surfacing citizens, but the fame isn't actually growing them. Verified same session against live ledger/sheet state, and the diagnosis is worse than the dead-wire framing above — **the loop is not just unlit, it's broken at the identity join, and this has never actually worked for a draw-1 cast member:**

  - POP-00962 (Marcus Walker, the plan's own "random dishwasher becomes a city figure" example) is still **Tier 4, `UsageCount` blank/0** after 7+ episodes since 2026-08-16 and three staged Nia recaps naming him directly (c104, c105, c106 — confirmed in `output/spacemolt-show/recaps.json` and `output/production_log_c10{4,5,6}.md`). `Citizen_Media_Usage` has **zero rows for POP-00962**, ever.
  - Root cause found live in the c105 cycle: the recap article body correctly says *"Marcus Walker's recent UNDOCKED episode..."*, but the wake-2 citizen-quote/crediting pass for that same `undocked_c105_nia-rook` packet attributed the quote to **"Rick Walker"** — a name with **zero matches anywhere in `Simulation_Ledger`**. Same fabricated name then recurs as the quoted/mentioned citizen across unrelated civic/culture/sports packets the same week (C105–C107), meaning `buildNiaSlice.js`'s documented contract ("story.popids carries the pilot, wake-2 interviews the pilot themselves via citizenVoice," lines 17-19) is not what's actually running for this feed — the pilot's own POPID is not making it into `story.popids` intact, so citizenVoice resolves/invents someone else instead of the real cast member. The article names the right citizen; the crediting pipe that's supposed to turn that into `UsageCount` picks a phantom.
  - This means the mobility promise has never been tested successfully on a low-tier cast member — Dane (POP-00143, already Tier 2 DA) and Jumper (POP-00688, Tier 2) show `UsageCount` 7 each, but they started high-profile; nothing confirms the ladder actually reaches a Tier-4 draw through the show, and the one draw that should prove it (Walker) shows the join is broken, not just unpromoted.
  - **CORRECTION (research-build, same session, ~30 min later): re-assigned to research-build, not engine-sheet.** Traced one hop further before close: `buildNiaSlice.js:110` correctly sets `popids: [e.POPID]` on the lane entry it hands off, and `newsroom-fanout.js:220` (`storyFromSeed`) correctly carries that into `story.popids`. The pilot's real POPID *is* intact through both of those. The break is further downstream, in whatever resolves `story.popids` into the actual quoted name — `collectQuoteAsks` / the citizenVoice call chain in `newsroom-fanout.js` (unread this session). That's Node pipeline/apparatus code, not engine phase code — squarely research-build's own scripts, no handoff needed. **Not traced to root cause yet — next research-build session picks up at `collectQuoteAsks` in `scripts/newsroom-fanout.js`, not at the seed-construction layer** (already cleared). Re-verify a full cycle end-to-end (recap → Citizen_Media_Usage row with the correct POPID → UsageCount increment) once fixed, before touching standings-as-a-lever at all.
  - **Mission-brief variety (Mike floated cost as the likely reason for the narrow mine-only script):** checked live — 34 recorded episodes total **$1.57** (avg $0.046/flight, deepseek-chat, unchanged model/cap). Variety in the brief text (combat, trade, salvage, rescue) is the same token cost on the same 10-minute cap; the adapter (`undockedEpisodeAdapter.js`) already scores combat outcomes, deaths, and mishaps — those fields have simply never fired because every brief to date is the identical mine→sell→refuel→log template. Cost is not the constraint here; brief design is. Open item for a future session, sequenced AFTER the identity-join fix (no point writing richer missions if the credit still doesn't reach the citizen who lived them).
...
- 2026-09-08 (kimi) — (a′) apparatus half LANDED (`e81a0538`): (1) account minting — `output/kimi/spacemolt-phase0/mint-pilot.ts` (bun + @spacemolt/lib: ClerkSource.fetchRegistration → Account.register → commander `sessions/<session>/credentials.json`; never prints/overwrites credentials) behind `scripts/undockedMintAccount.js` (idempotent; username candidates FirstLast / FirstLast+popid-digits; runner still never self-registers); (2) brief generation — `scripts/undockedMissionBrief.js` (draw-row fields → deterministic brief; age = year − BirthYear; preflight gates + cheatsheet + hard stop baked in); (3) `cron-undocked-run.js` reads `Undocked_Draw` at TargetCycle = cycleCount+1, falls to alt-1… on preflight failure (mint-on-demand, brief-on-demand), falls back to the latest local manifest when the tab is absent/unreadable/empty — dry-run verified live: tab missing → manifest draw #1, alts flagged MINT/GENERATE; (4) `undockedStandings.js` writes `output/spacemolt-show/standings.json` at each recompute and `buildNiaSlice.js` names the board leader (Rank/CyclesLed/CurrentStreak) in every recap angle. Tests: undockedMintAccount, undockedMissionBrief, buildNiaSlice suites green. Caveat for engine-sheet: the live register path is validated to the network boundary only — the first real mint (a drawn pilot without creds) is the end-to-end proof.
- 2026-09-08 (kimi) — (a′) piece 1 live proof attempt BLOCKED at the server boundary: `SPACEMOLT_CLERK_API_KEY` is rejected (`GET /api/registration-code` → 401 `invalid token`; key parses clean from the env file). The key that minted the draw-1 accounts (2026-08-16) has expired or rotated. Fix is Mike-side: mint a fresh Clerk API key from the website, update the shared env file. Until then a reseeded cast cannot fly — mint 401s, alternates likewise, `resolvePilot` fails loud by design (no silent manifest fallback once tab rows exist). Piece 2 proven on a real row: LaTrice Desai (POP-00946, alt-1) brief generated; runner preflight correctly refuses her without credentials — fail-closed as contracted.
- 2026-09-08 (kimi) — (a′) piece 1 blocker RESOLVED same day: the Clerk key was never expired — the env value was pasted with literal angle brackets, so `Bearer <ak_…>` 401'd (engine-sheet `ee10da30`: `cleanClerkKey` strips quotes+brackets; kimi's expiry/rotation reading was wrong, corrected here). Live mint proven end-to-end: LaTrice Desai POP-00946 minted (username LaTriceDesai, solarian), brief generated, runner preflight green — alt-1 fully flight-ready. Show canary flipped healthy the same night.
