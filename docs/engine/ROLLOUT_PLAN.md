...
| engine.135 | The other side of engine.133: employment city → hood envelope (unemployment is still `adults × (1−rate)` ±3 copied per hood, `updateNeighborhoodDemographics.js:220`) weighted by hood economics + `Business_Ledger` depth (grok's 72-row six-per-hood fill in the inbox, adopt after Mike's name sign-off); upstream of kimi's casino-ledger stake caps. Wiring cards → test → bench → live, as engine.133. | in-progress | engine-sheet | [[../plans/2026-08-29-employment-system-cascade]] (plan, builder direction 2026-08-29: dial + hood envelope from canon profile + BL fill + wealth-scale realign + quiet the rewriters + placement); grok fill in inbox — A/B1/B2 on SANDBOX 0827 @7 (S397), C–F not started; dial definition = open builder call |
