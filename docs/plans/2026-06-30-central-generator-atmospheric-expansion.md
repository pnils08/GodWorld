...
- **Civic echo** (Mike, S280 — direction set): civic officials LIVE on the ledger, so the echo has a natural suitor — `generateCivicModeEvents` emits the official's own citizen event as their reaction: "spent the evening drafting what to say about the storm," district-conditioned (official's district ∩ event neighborhood). The event reaches the civic layer *as a lived event of the official*, not as a new cross-layer pipe. Engine-sheet buildable once designed; pairs with the reaction-grain rule (S280).
- **Shared-reaction → arc** (Mike, S280 — direction set): many citizens reacting to the same event should become a story arc. Half-built already: crisis buckets spawn arcs at HIGH severity (`spawnCrisisArc`). Missing piece = reaction *density* as an arc input — when N+ citizens in a hood draw reaction lines off one event, spawn/feed an arc (tension bump if arc exists, spawn if not) even at medium severity. Design needs: density counter at emit time (cheap — ctx tally), threshold, anti-spam vs existing `hasActiveArc` guard.
- **Housing texture** (Mike, S280 — direction set): likely a NEW SL column (`HousingType`), backfilled from Neighborhood + Income heuristics (hood housing-stock profile × salary band), reconciled with Household_Ledger where a household row exists. Schema change → measure-twice + SCHEMA_HEADERS regen; then texture conditions like the S280 family/means gates.
- **Event-name vocab → sheet content ledger** (Mike, S280 — direction set): subtype vocab moves to a sheet ledger, and — Mike's lever — a compositional randomizer: instead of fixed strings, weighted word/phrase FRAGMENT pulls (slot-based assembly, ctx.rng-seeded, conditioned on domain/severity/hood). Extends the proven tone-slotter pattern ($VENUE/$CONTACT) from venue-slots to full phrase composition — combinatorial variety from small curated fragment sets, Mike-editable without deploys. Design questions: fragment schema (slot type / fragment / weight / eligibility), grammar-safety of assembled lines, dedup at combinatorial scale, determinism (seeded pulls). This becomes the content ledger's core mechanism, not just its storage.
...
- 2026-07-01 — Depth build items 4/5a/6 SHIPPED in-tree (engine-sheet, S280). Item 4: listening domain (8 anonymous observed-stranger moments → Personal). Item 6: groove domain (5 routine-gravity lines → Micro-Event, lightest ambient). Item 5a: **initiative outcomes reach kitchen tables** — phase order verified (Phase5-Initiatives precedes Phase5-CitizenEvents at both entry points), `S.initiativeEvents` carries same-cycle name+outcome+voteCount → outcome-keyed reaction lines naming the initiative (→ Civic Perception {sociability:2}), cap 3 initiatives. Item 5b/5c DEFERRED with reasons: Tribune-readership needs an edition-headline ctx feed (Phase-2 loader design — seam logged); Baylight is canon-timeline content that would drift hardcoded → content-ledger resident. Seam directions folded (Mike): civic echo via generateCivicModeEvents (officials live on the ledger — their reaction IS the event), shared-reaction density → arc input, HousingType as new SL column backfilled from Neighborhood+Income, vocab ledger with compositional fragment-pull randomizer (tone-slotter pattern generalized).
- 2026-07-01 — Depth build items 1–3 SHIPPED in-tree (engine-sheet, S280) + Mike-direct mid-build refinements: (a) citizen event = REACTION to a city event, never a bulletin — chaos rebuilt as `chaosReaction_` domain/subtype keyword router, heard/lived grains (smoke-test caught + fixed a "business"→"bus" substring mis-route); (b) column-conditioned depth — the 46 ledger columns are the levers: household-shared event memory (deterministic HouseholdId hash → same line across members), MaritalStatus/NumChildren-gated family texture, WealthLevel/DisplacementRisk-aimed money texture; (c) baseDaily → 48 human-domain moments routed to existing ambient dial keys; (d) canon guard reframed: conditioning on columns, not blanket avoidance. Dial routing verified byte-safe (primary-tag DIAL_MAP hit precedes content regexes — proven empirically incl. crisis text containing "hospital"). Items still open *(stale clause — S289 reconcile of seams-plan Finding #1: written mid-build; 4/5a/6 shipped same day per the entry above, 7 shipped `bd71cddd`; genuinely open remainder = 5b Tribune + 5c Baylight, now seams-plan Task 12)*: 4 (listening pool), 5 (initiative/Tribune/Baylight pools), 6 (groove), 7 (static widen + retirement leisure). Coupling map trued same commit. North star noted (Mike): citizens grow, ledgers adapt — multi-session.
