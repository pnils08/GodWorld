...
| engine.185 | **Sentiment is an inert control signal.** Live city sentiment has never gone below −0.02 across 37 recorded cycles (C69–C106, min −0.02 / max 1.0), but 62 comparison sites across 37 engine files gate on `sentiment <= -0.2 … -0.5` — every downside pathway (crisis spikes, faith events, world events, promotions, bonds, relationships, education, household, career, neighborhood, evening media, nightlife, story hooks, texture triggers) sits behind a door that has never opened. The mirror holds too: `sentiment >= 0.2` gates (applyStorySeeds:1311, chicagoSatellite:392) are always-true against a live min of 0.23. Sentiment triggers nothing down and everything up. BUILDER CALL NEEDED: recalibrate the thresholds to the real distribution (engine.184 pattern), or delete the branches as prosperity-doctrine dead weight. Scan S441, method: threshold vs live column range | needs-info | engine-sheet | ROLLOUT row (scan output S441) |
