...
**Status:** [x] verified S427 (research-build) — no code needed on either side. Trace:

- **`/post-publish --type dispatch` handles per-desk artifacts: confirmed, already proven.** It's a mature, format-agnostic convergence point (`.claude/skills/post-publish/SKILL.md`) that has already published at least one real dispatch (S188 KONO Second Song), with known bugs already fixed (S189 E6 cultural-card refresh, E8 count-match verification gate). Nothing fork-specific to build.
- **Supermemory-inject fix: not applicable to the fork.** Traced the actual "monolith" defect Mike meant at S313 (line 25: "buries desk output inside full-edition artifacts") to `ingestEdition.js` under `--type edition`, which chunks the *whole* 9-article edition into one Supermemory document — confirmed empirically tonight (`saturday-run.log`: `[INFO] Split into 1 chunk(s)` for the full 29KB c105 edition). That defect is structural to the edition path's one-big-.txt shape. Dispatch artifacts are one desk's one storyline per file by construction (`editions/cycle_pulse_dispatch_{XX}_{slug}.txt`), so they are already per-article/wiki-taggable with zero fix needed — the fork's whole design point (line 25/36 criterion 4) is satisfied structurally, not by a patch. The edition-path monolith problem itself is real but is out of scope here — it's a `--type edition` defect, not a fork defect; flagging separately rather than folding into T5.
- **engine.46 byline/desk tags: confirmed covered.** `/deep-dispatch` Step 6 already emits the exact `By {journalist} | Bay Tribune {Desk}` line that `ingestPublishedEntities.js` parses to write `byline-published` rows — same convention S188 already exercised live.
- **The one untested link:** `/desk-review` Pass C's footer packaging (NAMES INDEX / BUSINESSES NAMED / CITIZEN USAGE LOG, `docs/media/EDITION_FORMAT_TEMPLATE.txt` — one generic contract, not per-type sub-formats as this doc's Pass C text implied) has never actually been exercised — `/desk-slice`, `/deep-dispatch`, and `/desk-review` have zero output artifacts on disk anywhere. That's T6's job (a real pilot), not a T5 gap — the contract itself is fully specified and already proven by other publishers.
- **pipeline.65 correction (engine-sheet, S427):** initial filing framed the fix as "chunk the edition per article." Wrong direction — `cron-saturday-run.js` `stepSweep` (Saturday Step 5) already writes one Supermemory doc per curated article (`article-c{cycle}-{stem}`, idempotent upsert, popids/hoods/storylines metadata) into `bay-tribune` every Saturday; that's the per-article canon layer already solving the fork's own criterion 4. A chunker on `ingestEdition.js --type edition` would put a THIRD copy of each article body in the container (monolith + sweep + chunk) — the same engine.111 surplus-docs class fixed earlier tonight. The right fix is the opposite: `--type edition` ingest should carry only the masthead + Mags narration frame and stop duplicating article bodies the sweep already owns. `--type dispatch`/`supplemental` are unaffected (no sweep exists for them). Re-scoped in pipeline.65.
...
- 2026-09-06 (S427, research-build) — Task 5 verified, no code needed. Traced /post-publish --type dispatch (mature, S188-proven), the S313 monolith complaint to ingestEdition.js under --type edition specifically (confirmed live tonight, out of T5's scope), and engine.46 byline coverage (confirmed by construction). Open: T6 pilot only.
