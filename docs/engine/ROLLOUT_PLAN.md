...
| pipeline.67 | Oaks beat — Selena Grant + Talia Finch re-beat from the retired Chicago Bureau to two solo Oaks seats (Mike-direct: "like Nia Rook / Anthony / P Slayer"). **CLOSED S431 (research-build): engine-sheet found NO MINT WAS NEEDED — both already on the Simulation_Ledger (Selena POP-00591, Chinatown, T2, married to Ronald Williams POP-01041; Talia POP-00592, Jack London, T4, single), Talia's RoleType re-beat to "Journalist - The Oakland Oaks Ground Reporter" live. Activated: `_pending-*` wake-package keys renamed to `selena-grant`/`talia-finch` with real popids, persona-map filled, IDENTITY.md ledger lines corrected, voice files' invented neighborhoods (Jack London/West Oakland) corrected to the ledger's actual Chinatown/Jack London. `newsroomWakePackages.test.js` golden fixture updated (both seats join the sports rota pool: 20→22 candidates, sports 5→7; DAILY_QUOTAS unchanged at sports:2) — 100% green. Dry-run verified both citizen voices wake with the correct ledger facts (Selena: Chinatown, married, 1 child; Talia: Jack London, single, already has a favorite Oak per her own LifeHistory). Acceptance = first M–F fanout wakes both off a `TeamsUsed: Oaks` row (opening night is C108).** | done-pending-archive | research-build | [[../plans/2026-08-07-anthony-hal-solo-sports-seats]] §Task 6 |
...
| engine.147 | Bloodline ascent — chain cuts 1–9 all live; open: the wake-pack seam. **CLOSED S429 (research-build): the seam is built — `lib/wakePerception.js` `buildPool` reads `Heritage_Ledger` once (header-resolved, 16- and 23-column tabs) and carries `line` + `DialState.maneuver` on every pool entry; `renderStanding` renders the family line (founding/established/prominent/dynasty phrases, tenure, peak, dormant, what it holds) and the posture (climb/retreat with the goal phrase; hold is silence) in plain words — no score, no net worth, no ids. Threaded after the family line in all three voices (citizen-wake, citizen-exchange, citizenVoice; kept out of the no-exposure interview branch). 26/26 tests `lib/wakePerception.test.js`; dry-run voice on Benji Dillon shows the Dillon line. Posture half inert on live until the builder's C106 fire stamps `maneuver`; acceptance = the first live wake after that on a line member shows both halves. Known gap carried: `BusinessesOwned` counts only heritage-minted storefronts (Varek's Civis/Oaks won't read).** | done-pending-archive | research-build / engine-sheet | [[../plans/2026-09-02-bloodline-ascent]] |
...
| engine.107 | ARC milestone EventText is engine bookkeeping, not person-readable — starves citizen-perception continuity; supply side of research.19 T1a' | absorbed → engine.38 B3 (S272 measure-twice reversal: sub-lane (a) tier-text rewrite dropped, no live consumer; sub-lane (b) IS the row; Row 24(b) `stampPromotion_` already person-readable S327) | research-build | [[archive/ENGINE_REPAIR]] Row 32 (migrated S371) |
| engine.163 | Packet stem leaked into citizens' life records — Citizen_Media_Usage `Context` (writer-wake stage stem incl. model id, an ops dedupe key) folded raw into the Advancement_Intake1 / Intake notes → LifeHistory_Log `EventText` (11 live `Advancement` rows, C104–C105). **Fixed at the seam `describeUsageContext_` (mediaRoomIntake.js), bench @66 C110/C111 clean, PROD @64 (S431).** Open: the 11 historical rows (many-row sheet write, builder's go); canon-leak-guard now carries `MODEL_PAT`. | done-pending-archive | engine-sheet | gap log `output/production_log_run_cycle_c105_gaps.md` G-EC57; commit `00d76804` |
...
| engine.117 | Ledger true-up sweep — 348/961 rows carry ≥1 defect; scope is only the classes no open row covers. **Closed S431:** Tasks 2–4 shipped S380, Task 1 resolved by design (SchoolQuality causal for minors, no sweep); audit BirthYear guard now catches non-numeric cells. | done-pending-archive | engine-sheet | [[../plans/2026-08-17-ledger-trueup-sweep]] |
