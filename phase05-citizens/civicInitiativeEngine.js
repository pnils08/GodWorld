...
/**
 * ============================================================================
 * INITIATIVE AUTHORSHIP — createInitiative_ (civic.22, S376)
 * ============================================================================
 *
 * The tracker had five mayoral-action columns and zero proposer columns, so
 * nothing recorded who authored an initiative. Authorship columns were added to
 * Initiative_Tracker by scripts/addInitiativeAuthorshipColumns.js (one-time,
 * engine-sheet). This mints new rows carrying that authorship.
 *
 * Any seated MAYOR-01 or COUNCIL-D1..D9 may author; LeadFaction is taken from
 * the seat rather than assumed. Vacant seats cannot author.
 *
 * Phase 5 write discipline: queueAppendIntent_, committed by Phase 10 — never
 * appendRow from here (.claude/rules/engine.md). Fails loud if the intent
 * helper is absent rather than silently falling back to a direct write.
 *
 * Deliberately does NOT insert columns on cycle fire; the column add is a
 * one-time operator script, not an ensure* that runs every cycle.
 *
 * Authored by grok, reviewed and landed by engine-sheet. Verified before
 * landing: no name collision on createInitiative_ / lookupAuthoringSeat_ in the
 * Apps Script flat namespace; queueAppendIntent_ signature matches
 * (ctx, tab, row, reason, domain); Civic_Office_Ledger carries OfficeId /
 * Holder / Faction / Status; MAYOR-01 + COUNCIL-D1..D9 all exist as OfficeIds.
 */
...
var CIVIC_STAGE_CATALOG_ = {
  ...
  housing:   { playable: false,  stage3Metric: { tab: 'Household_Ledger', column: ['flagged/cohort'], direction: 'down', scope: 'hood' }, disburses: false }  // builder 2026-09-23: no housing program in the sim — off; was engine.255: spends its budget (Task 4); Delivering on the flagged-at-vote cohort (Tasks 6–7, bench pair C109–C113)
};
