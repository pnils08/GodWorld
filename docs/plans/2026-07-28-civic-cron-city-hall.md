...
- 2026-09-06 — S431 (engine-sheet, builder ask): `--stage=status` added to `cron-civic-run.js` + crontab `0 15 * * 0` — one Discord line (DISCORD_WEBHOOK_URL, same channel as the gate alert) saying whether the 14:30 chain wrote the tracker, staged without writing, never started, or had nothing to do. Read-only, exit 0 always. Acceptance = the next unattended Sunday 15:00 run.
