...
- **Engine version:** v3.4 (engine.38 Phase A — full-population life coverage — CLASP-DEPLOYED LIVE S264; smoke-test pending C99)
...
- **Columns:** 49 (A-AW; S262 added SMPageId=col49/AW — citizen-loop per-citizen Supermemory-page pointer, lazy-filled). Past Z: Income=col26, EducationLevel=col31, CareerStage=col33, Gender=col47 (AU), DialState=col48 (AV), SMPageId=col49 (AW). Live rows: 906 (S262).
- **Citizen-loop (research.14) wake-side LIVE S262:** `Reflection_Intake` tab + `citizen-pages` Supermemory container added. Affect tags added to `citizenDialMap` DIAL_MAP (Frustrated/Anxious/Angry/Excited/Content… — negative-pole fix). **S264: dual-tag write-back substrate BUILT + CLASPED LIVE (INERT)** — `nudgesForReflection_` (composer: event non-composure + affect full, additive; composure-as-affect-only scoped to the write-back, DIAL_MAP byte-unchanged) + `applyReflectionDualTag_` in the pushed ES5 utilities; `classifyDualReflection_` ({event, affect, affectFallback} + off-vocab fallback) in `reflectionClassifier.js` (Node). `Reflection_Intake` now carries affect at col H. Gate GREEN (14/14·9/9·4/4), offline 60/60 + composer 20/20, 5-agent verify 4 PASS+1 CONCERN. Still INERT (no cycle caller, no phase reads `Reflection_Intake`) — write-back gated on research-build's Phase-1 re-audit. `scripts/citizen-wake.js` on cron 3×/day (writes citizen pages + AW + dual-tag Reflection_Intake; NO `applyTaggedEvent_`). Modules: `lib/reflectionClassifier.js`, `lib/citizenPage.js`, `lib/citizenDials.js`; gate `scripts/classifierGate.js`.
...
- **S256 engine.33 (citizen↔neighborhood texture loop): DEPLOYED LIVE S257 (Mike go-call post-C97).** 12/12 tasks committed per `docs/archive/plans/2026-06-10-engine33-neighborhood-citizen-loop.md`; integration harness `scripts/testNeighborhoodLoop.js` 20/20 green re-run on tree immediately pre-push (loader fidelity, pulse+texture locality, fold caps, hood-grain conduct, closed-loop determinism). Code spans phase02 loader (`loadNeighborhoodState.js`) / phase04 faith pulse + micro-event microclimate / phase05 state+weather+faith pools + conduct crime blend / phase08 pulse fold + `utilities/neighborhoodPulseMap.js`. **Smoke-test pending C98** — pulse fold + microclimate texture (heavy-rain/fog/warm-pocket lines locality, displacement/retail texture) verify at the first cycle run after deploy. ROLLOUT engine.33 = done-pending-archive.
- **S256 neighborhood-roster alignment: DEPLOYED LIVE S257 (rode the C97 clasp window with engine.33).** Live set-difference found 205 citizens in untracked hoods — citizen-residence (CANON_12) and the map writer roster had drifted. Added Lake Merritt (98)/Uptown (89) + KONO profile (12, was in roster but profileless → silently skipped every cycle = ES-2 G-S6 structural) + Baylight District (INIT-006 Coliseum redevelopment, forward-looking) to `phase08 v3NeighborhoodWriter.js`; reconciled `lib/canonNeighborhoods.js` (MAP_17→MAP_NEIGHBORHOODS, 21). Live Neighborhood_Map stays 17 rows until **phase08 runs** — now DEPLOYED S257, so the 4 new rows (Lake Merritt/Uptown/KONO/Baylight) populate at **C98** (first cycle run after deploy); verify row count 17→21 + demographics rows for the new hoods at C98. 6 stray col-T ledger values reassigned to canon LIVE (0 strays remain). **DISTRICT ANCHORS DONE S256 (`31f9fbb`)** — Mike set the frame (prosperity Oakland using Oakland geography as scaffold; district logic delegated to engine + sim build docs) and the anchors were canonized: Baylight→D5 (Rivers; Coliseum-site successor per Baylight Authority LENS — sim map already anchors Coliseum+Elmhurst in D5), Lake Merritt→D8 (Chen; lake-ring), Uptown→D9 (Mobley; legacy ratified). Recorded in INSTITUTIONS §Neighborhoods + added to phase08 writer `NEIGHBORHOOD_DISTRICT_MAP` (KONO=D7 was already present). Also fixed the **KONO D2→D7 civic-map drift** (`9f21cb7`) — `lib/districtMap.js` + `scripts/buildCivicVoicePackets.js` both had stale KONO=D2 vs canon D7 (INSTITUTIONS:330, S215). **Demographics silent-skip DONE (`31f9fbb`)** — `utilities/ensureNeighborhoodDemographics.js` DEMO_NEIGHBORHOODS roster was missing KONO + the 3 new hoods → no demographic row written each cycle (G-S6 class); added all 4 to roster + profiles (thriving register). **REMAINING (lower priority):** the ~30 hardcoded-literal surfaces beyond demographics — `phase02 applyWeatherModel.js` + `applyCityDynamics.js` are lookup-with-fallback (benign, default behavior for new hoods); phase05 citizen engines (household/career/neighborhood/civic) NOT confirmed as silent-skip — leave until a fallback check proves otherwise. engine.33 CORE already data-driven (0 hardcoded). **Incident (repaired):** first stray-reassign pass wrote to col U (HouseholdId) not T — `updateRangeByPosition` startCol is 0-indexed, passed 1-based; caught by post-write verify, HouseholdId restored from Household_Ledger (invariant validated), redo guarded on column-letter assertion. Deploy: rides the C97 clasp window with engine.31/.33.
...
- **Function count:** 159 engine files / 948 functions per S256 STUB_MAP regen (post-engine.33; adds `utilities/neighborhoodPulseMap.js` + `phase02-world-state/loadNeighborhoodState.js`). Regen via `node scripts/stubEngine.js` when authoritative number needed; history of S185→S256 deletes lives in git log.
- **Last deploy:** **S273 — clasp push (Mike explicit go): G-EC33 sentiment-lockstep fix `82b7f2eb` + full undeployed manifest now LIVE (engine v3.4, no version bump — within-version fix).** Files changed vs live: `phase08-v3-chicago/v3NeighborhoodWriter.js` (G-EC33: per-hood sentiment base = `S.neighborhoodDynamics[hood].sentiment` + frozen profile offset + `CITY_SENTIMENT_NUDGE=0.15` citywide shade; breaks the citywide-scalar lockstep). **Manifest measure-twice (full-replace push): G-EC33 [behavioral·sentiment] + 5 INERT riders (research.14 drain unwired / engine.38 B3 step-1 vocab passive DIAL_MAP / engine.39 temperament tags authoring-only / tier1EssenceEvents 0-callers) + S271 in-world-dates + chaos-stamps [behavioral·EventText stamping, cosmetic, never-run-live] — none touch sentiment, G-EC33's C101 read stays attributable.** "Script is already up to date" idempotent re-push confirm. **C101 SMOKE (REQUIRED):** does live per-hood `Neighborhood_Map.Sentiment` delta spread WIDEN vs C100's uniform +0.16-0.18 lockstep (G-EC33 re-compressed at C100 per `engine_review_c100.md`)? Dry-run gate `testNeighborhoodLoop.js` C6 24/24 (synthetic divergence proven; live magnitude depends on real per-cluster movement — un-dry-runnable). **engine.38 Phase B / B3 (ordinary-bad content) UNBLOCKS once C101 confirms divergence.** Clasp backlog now clear except Row 8 migration + 2 manual editor deletes. — PRIOR: **S264b — clasp push (Mike go-call): engine.38 Phase A — full-population life coverage — now LIVE (engine v3.4).** 8 Apps Script files: `phase05-citizens/generateCitizensEvents.js` (A1 dial-weighted participation replacing LIMIT=25 + the hidden 0.18 chance clamp; Tier-1/2-any-mode UNION Tier-3/4-ENGINE gate; A2 anti-inert floor; A4 T8 attendance radius), `phase04-events/buildCityEvents.js` (A4 supply 1-2→3-4), `phase01-config/godWorldEngine2.js` (B1 Phase-11 `maintainLifeHistoryLog_` wiring, both entry points), `utilities/archiveLifeHistory.js` (B1 — MOVED from `maintenance/` so it deploys; cycle-path log trim, row-gated 12k, retain 12cyc), `phase06-analysis/applyPatternDetection.js` + `applyShockMonitor.js` (B2/B3 population-relative thresholds + regime-shift guard), `phase08-v3-chicago/v3NeighborhoodWriter.js` (S2 pulse-fold volume normalization), `utilities/compressLifeHistory.js` (comment only). Idempotent re-push confirmed "Script is already up to date." Tests green pre-push: coverage 71%, archiver 8/8, pattern+shock 9/9, pulse 6/6, engine32 19/19, neighborhood 20/20. **Adversarial 4-lens panel cleared determinism + found the 3 blockers (B1/B2/B3) — all fixed before this push.** **C99 SMOKE (first living-city cycle):** coverage ~60-80% (coverageReport.js); B3 regime-shift guard fires ONCE by design (52→600 jump) and self-clears C100; patternFlag not pinned-strain; archiver dormant until >12k rows. **2 MANUAL EDITOR DELETES OWED:** `generateNamedCitizensEvents.gs` (A3 orphan, zero callers) + `applyCycleWeightForLatestCycle.gs` (long-owed S238) — clasp doesn't auto-delete; dead code, harmless if left. Clean attribution window — deployed onto the C98-verified stack, no pending smoke-test jumped. **S5 verified no edition-canon conflict** (sift + famous-spotlight don't read ambient LifeHistory). engine.38 commits: `42b7f9bd` (Phase A) + `e86f200a` (blockers) + `e0f181a0` (S2/S5), all pushed to origin/main. — PRIOR: **S264 — full-tree clasp push (Mike go-call): citizen-loop dual-tag write-back substrate + affect tags now LIVE (INERT).** Pending delta since the S261 push = 3 commits, all inert/cosmetic: `ec8c1ea` affect tags (DIAL_MAP, no cycle emitter), `ecbdca42` col-O milestone string → in-world `C{cycle}` (cosmetic, C99-effective, measure-twiced), `98bd6091` dual-tag (`nudgesForReflection_` + `applyReflectionDualTag_` in the two pushed ES5 utilities; `classifyDualReflection_`/gate are Node, not pushed). **NO smoke-test required — the deploy is inert** (no cycle caller, no phase reads `Reflection_Intake`; re-grep verified). The C98 engine stack (.31/.32/.33/.35) was already live from the S261 whole-tree push and **C98 has since run** — so this push did not jump any deploy window. Idempotent re-push confirmed "Script is already up to date." Only future-visible effect: col-O milestone strings stamp `C99` (not wall-clock) starting next cycle. **Write-back stays gated** until research-build's Phase-1 re-audit (then wire a Phase-9 sibling `safePhaseCall_`). No engine-version bump — inert addition, no behavior change. — PRIOR: **S261 — clasp push (Mike go-call): engine.35 seed-deck pre-cycle stack LIVE.** Two Apps Script files: `phase10-persistence/saveV3Seeds.js` (deck schema widened to 20 cols — PacketRef=S, CoveringJournalistPOPID=T) + `phase07-evening-media/applyStorySeeds.js` (Phase 1 storyline-followup gate: non-Oakland-locale + age-cap-12). Pre-step: live `Story_Seed_Deck` grid resized 18→20 cols + S/T headers written via service account (verified, 1750 data rows preserved). Idempotent re-push confirmed "Script is already up to date." `rosterLookup.js` module.exports guard verified (`typeof module !== 'undefined'` — safe in Apps Script). **C98 smoke (first run executes the whole stacked deploy):** (1) deck `storyline-followup` count halves + zero Chicago seeds (Phase 1 gate); (2) saveV3Seeds writes 20-col rows aligned to S/T headers; (3) engine.33 pulse-fold/microclimate texture + Neighborhood_Map 17→21; (4) engine.31/.32 dial folds; (5) generationalEventsEngine no CIV retirement fires. Then post-cycle: `/engine-review` → `routePatternSeeds.js --apply --cycle 98` (Node) → ES-1 col-O cycle stamp. *(Hygiene: `utilities/neighborhoodPulseMap.test.js` still rides the push — `.claspignore` lacks `*.test.js`; benign, add rule in a future pass.)* — PRIOR: **S257b — clasp push (Mike go-call): `generationalEventsEngine` retirement fix LIVE** — office-holder CIV guard (`civFlag.charAt(0)!=='y'`) + age-floor 58→68 (C97 mayor-retirement defect, commit `9b769ce`). Rode on top of the same-session engine.33 stack (attribution caveat: both first execute C98). Smoke-test pending C98: no CIV retirement fires. Live decontam done (SL O34 + LifeHistory_Log row 4100); published E97 canon was never touched (sift/dispatch fenced, G-S1). — PRIOR: **S257 — clasp push (Mike go-call post-C97): engine.33 citizen↔neighborhood texture loop + neighborhood-roster alignment + district anchors + demographics-roster fix now LIVE.** Gate cleared empirically: C97 engine review (`output/engine_review_c97.md`) confirms C97 is the first clean-measurement cycle post-C96 reactivation — the C96 dead-cityDynamics "strain" crash-string CLEARED as predicted, 2 genuine milestones (OARI live-dispatch, Transit construction-planning), 10 organic sentiment rises against a clean prior; C97 anomalies all benign crime-outlier `cover-as-story`. So the .31/.32 stack verified clean and engine.33 landed on a verified base (deploy-attribution discipline — no stacking on unverified). Pre-push: `testNeighborhoodLoop.js` 20/20 green on tree; `.claspignore` scope-checked (scripts/lib/.claude/output excluded → no stray files). Idempotent re-push confirmed "Script is already up to date." Apps Script files live: phase02 `loadNeighborhoodState.js`, phase04 faith-pulse + micro-event microclimate, phase05 state/weather/faith/conduct, phase08 `v3NeighborhoodWriter.js` (NEIGHBORHOOD_DISTRICT_MAP + 4 new hoods), `utilities/neighborhoodPulseMap.js`, `utilities/ensureNeighborhoodDemographics.js`. **C98 smoke-test:** Neighborhood_Map 17→21 rows + demographics rows for new hoods + pulse-fold/microclimate texture locality. *(Hygiene note: `utilities/neighborhoodPulseMap.test.js` also pushed — `.claspignore` has no `*.test.js` rule; benign [test file, not on cycle path], add the ignore rule in a future pass.)* — PRIOR: **S256 — full-tree clasp push (Mike-approved go-live): engine.31 dial engine + engine.32 life-event generation now LIVE.** Idempotent re-push confirmed "Script is already up to date"; key files verified in scope (`utilities/citizenMemory.js` + `citizenDialMap.js` + `compressLifeHistory.js` v2.0 fold-on-trim, `phase05-citizens/runConductEngine.js`, `phase04-events/buildCityEvents.js`, T8 pair `phase09-digest/finalizeCycleState.js` + `phase01-config/loadPreviousEvening.js`). Rode the same-session live ledger seed (DialState AV + O cleared to archive — see S256 block above). **C96 verifies:** T8 `PREV_EVENING_JSON` PropertiesService round-trip + live dial folds (closes engine.32); then full card `--apply` rebuild LAST per Mike. — PRIOR: **S247 — full-tree clasp push (Mike-authorized full-scope, "Script is already up to date" idempotent confirm).** Delivered the S247 engine stack + the still-pending S244 simYear fix. **Material behavior changes now LIVE — C96 smoke-tests TWO dormant-subsystem reactivations at once:** (1) `phase02-world-state/applyCityDynamics.js` — the var-hoisting crash that killed cityDynamics EVERY cycle since S136 is fixed; cityDynamics + neighborhood momentum + downstream sentiment (cycle-weight/v3NeighborhoodWriter/illness-drift) come back online. (2) `phase01-config/advanceSimulationCalendar.js` — the S244 simYear ordinal-vs-calendar-year fix (citizen life events: weddings/births/promotions). ENGINE_REPAIR Row 23 claimed "deployed S244" but git had NO S244 clasp commit + this block said "Last deploy S238" — the S247 push DEFINITIVELY makes it live, resolving the ambiguity. Also live: `utilities/ensureNeighborhoodDemographics.js` (clobber fix — writer no longer blanks unmanaged cols every cycle) + `phase05-citizens/educationCareerEngine.js` (canon comment only, no behavior) + `lib/canonNeighborhoods.js` (new shared canon-neighborhood set; inert in Apps Script). **C96 SMOKE-TEST — PASSED S252 (both gates, first actual run; was deferred/owed S248→S251).** (a) `node scripts/verifyCityDynamicsReactivation.js` → REACTIVATED (Issues column clean, Riley_Digest W–AB populated: CityTraffic=0.97 RetailLoad=1.56 TourismLoad=0.9 NightlifeLoad=1.6 PublicSpaceLoad=1.39 CitySentiment=0.59); (b) SL `LifeHistory` (col 14/O) carries `[Wedding]`×1 + `[Promotion]`×1 stamped `2026-06-01 23:35` (C96 run), `[Birth]`×0 — simYear life events UNFROZEN (frozen→firing confirmed). **WATCH next cycle:** absolute event volume is low (1 wedding + 1 promotion + 0 births / 904 rows) — confirm the rate looks right at C97; no pre-fix baseline to compare against. Deploy gate now CLEAR for engine.31 (the post-C96 hold is released — the S247 two-reactivation deploy is verified, no longer rides un-smoke-tested). Original revert candidates if regression surfaces later: `ee88439` (cityDynamics) / `162afda` (simYear). **PENDING manual editor cleanup (carried from S238, still owed):** delete `phase09-digest/applyCycleWeightForLatestCycle.js` from the live Apps Script editor (clasp doesn't auto-delete; dead code only). — PRIOR: S238 — 1 clasp push delivering accumulated S237 + S238 stack to live engine. 5 files changed + 1 file deleted since S236 deploy: `phase01-config/godWorldEngine2.js` (S237 audit-miss 3 godWorldEngine2 + Riley_Digest cohort: Phase 8 sites refactored to call applyCycleWeight_ signal-only, fixing latent cycle-order bug that corrupted previous cycle's CycleWeight/Reason; 2 dead-fn deletes resolving collisions), `phase07-evening-media/culturalLedger.js` v2.4 → v2.5 (S237 B6: writeIntents migration + ctx.summary.culturalRegistry same-cycle dedup pattern), `phase03-population/finalizeWorldPopulation.js` (2-line S237 cohort touch), `phase07-evening-media/parseMediaRoomMarkdown.js` (S237 collision-2 dead-fn delete: `determineNoteType_` zero-caller stub removed), `utilities/bylineEngine.js` + `utilities/priorityEngine.js` (S238 collision rename: `_runSelfTests_` x2 → engine-specific names, latent Apps-Script-namespace warning resolved). **DELETED** `phase09-digest/applyCycleWeightForLatestCycle.js` (S237 wrapper retirement — its function now lives signal-only in `phase09-digest/applyCycleWeight.js`; **clasp does NOT auto-delete from Apps Script editor — Mike needs to manually delete this file from the live editor**; leaving it deployed = dead code only, not a behavior bug). Idempotent re-push confirmed "Script is already up to date." Smoke-test pending next C95 cycle run (operator-gated): verifies S236 + S237 + S238 deploy stack commits cleanly through Phase 10 batchUpdate (B5 mechanical cohort + B6 culturalLedger + Riley_Digest writeDigest_ Phase-10 canonical write path). Prior deploys: S236 — 1 clasp push closing Phase 42 §B5 mechanical scope (3 migrated files); S229 — 2 clasp pushes. S232/S233/S234/S235 work was Node-side only.
