...
- **Status:** [ ] not started — depends on E1+E2 from `[[archive/plans/2026-04-30-dispatch-gap-followups]]` (the parser must extract entities first, then this writer can tag them properly)
...
- 2026-04-30 (S189, research-build) — Initial draft. Mirrors `[[archive/plans/2026-04-27-world-data-unified-ingest-rebuild]]` pattern from S182-S183 (now-complete sibling). Tag scheme covers 14 types across body / wiki / canon-save layers. customId derivation function specified. Wipe primitive scripted as standalone tool. Validation fixture grounded in S188 KONO dispatch (the smallest end-to-end artifact in the corpus). Engine-sheet phase order locked: inventory → ingestEdition retrofit → ingestEditionWiki migration → wipe primitive → migration → MCP filter. Plan motivated by S185-S186 Perkins scrub friction (mags doc `WL8kvoxQgmcvxSPW3Ph47n`) — current chunked storage has no targeted-replacement primitive. After this rebuild, equivalent scrub = `wipeBayTribuneByCustomId <slug-prefix>` + re-ingest.
