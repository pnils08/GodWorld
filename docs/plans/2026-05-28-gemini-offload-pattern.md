...
**Status framing (S248): FUTURE CONCEPT, not near-term build.** Mike S248: *"more a future concept."* The triage is decided + filed; Task 1 (the consultable doc) stays open but unhurried. The one gating decision — authenticating Gemini CLI on the headless server — is Mike's call and carries a billing tail (see §Cost gate below). Nothing here is scheduled; this plan documents the boundary so that *when* Gemini gets wired in, the routing is already settled.
...
## Cost gate (S248 — load-bearing before any CLI auth)

**Hard billing cap required from day one. Do not trust the free tier.** Precedent: AutoDream ran Gemini 2.5 Pro on the stated free tier and quietly billed **$100** — disabled S228 ([[../../MEMORY]] §Session Memories → S141 AutoDream entry; `project_s141-autodream-gemini-switch`). Gemini-CLI-on-the-headless-server (Cloud Shell "free" / native auth) has the same failure shape: free until a quota tips it into billed. Any wiring step must land with an explicit cap + alert, not a trust assumption. This is the single reason CLI auth is Mike's decision, not an in-scope task.
...
- 2026-05-31 (S248) — **Reframed FUTURE CONCEPT + 2 new Mike PDFs reviewed + cost gate added.** Mike shared two Gemini self-assessment PDFs (Drive `1ELaBxnEYEsEKVIZi4gpErB_iFtWWtoJ4` "headless server / Chromebook" + `1C2DQvQb5wdmfGvJQyZCb8UBVQ1g6gLvC` "use both / division of labor"). **Convergence (confirms the plan):** both PDFs independently land on Gemini's native Google-Workspace edge — live Sheets/Drive/Apps Script access without manual export/paste, 1M-token context for whole-Apps-Script-file dumps, built-in Google-API-syntax grounding. That's exactly offload paths 1+2. No change needed; the plan already had this. **Divergence (REJECT — inverts the finding):** PDF 2's "division of labor" matrix sells Gemini as **System Architect / Planner** ("feed it your whole simulation, dense rules, architecture at once"). This is the precise layer the No-Go zone keeps at Claude. The S241 Baylight×HCAI cross-canon mash is the structural counter-evidence: a 1M context window holds the tokens but has no `INSTITUTIONS.md` / canon tier system / reviewer lanes to keep two project files from bleeding into one sentence. **Context size ≠ canon discipline.** Take Gemini's Sheets/Apps-Script half; reject its architect half. The plan's No-Go zone already encodes this — the PDF framing doesn't override it. **Cost gate added** (§Cost gate) — AutoDream $100 scar makes a hard billing cap a precondition of CLI auth. **Honest token-math note (S248):** the three offload paths are real but modest savings — Apps Script + formulas + tier-4 Docs are not the dominant token burn. Dominant burn is interactive-session retry loops (e.g. the S248 render-glitch storm), boot loads, and large file reads. Offload is worth doing for the canon-hygiene boundary it draws, but it is NOT the main token lever; the boot-burn companion work + session-discipline are. Don't oversell offload as the token fix.
