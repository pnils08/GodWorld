...
5. **Supermemory sweep — everything cleared, per-article.** ALL staged articles (curated or not) → bay-tribune container, one document per article, tagged journalist + cycle + INTAKE metadata. Not edition-combined. Idempotent via the engine.91 customId scheme. The weekday runner writes the exact Rhea pass/hash into every new staged sidecar. Before any consumer sees the set, the reader requires `status=staged`, an Article path inside `staged/`, and an exact Rhea-pass SHA-256 match from that sidecar or an adjacent legacy verdict; a manually dropped-in or post-review-mutated file fails closed.
...
- 2026-08-05 (later) — Cron INSTALLED (Sat 16:00 UTC, Mike go) with cycle auto-detect via lib/getCurrentCycle; ingestPublishedEntities INTAKE parser + merge shipped (dry-proven); c102 seat-proof artifacts removed so the graduation trend holds real weeks only. First live Saturday = Aug 8.
