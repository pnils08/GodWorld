...
- Never write directly to sheets — use write-intents (`ctx.writeIntents`). Only `phase10-persistence/` files execute sheet writes.
  **Documented exceptions (direct sheet writes outside Phase 10):**
  ...
  - Phase 5 citizen generators: `generateChicagoCitizensv1.js` (Chicago_Citizens — `createChicagoCitizensSheet_` L173 lazy-create stays direct as schema-setup carve-out per Phase 42 §1.1 S229; cycle-path SEED/MAINTAIN/CHURN sites at L78/L102/L136 pending mechanical-w/-refactor batch per §3.5 — post-write reload at L142-157 requires in-memory tracking refactor before queue-intent migration), `generateGenericCitizens.js` (Generic_Citizens — **DISABLED S205, retained for reversibility**: Path B no-grow-legacy; SL is single source for citizens; both `safePhaseCall_(ctx, 'Phase5-GenericCitizens', …)` call sites commented out at `phase01-config/godWorldEngine2.js:192` + `:1501`), `processAdvancementIntake.js` (Advancement_Intake tabs + Generic_Citizens + Simulation_Ledger + LifeHistory_Log — citizen promotion pipeline; GC paths inert post-S205, retire pending C94+C95 monitor), `generateMediaModeEvents.js` (LifeHistory_Log + Simulation_Ledger)
