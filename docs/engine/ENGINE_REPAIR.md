...
| 16 | `Advancement_Intake` / `Advancement_Intake1` tabs both missing from live spreadsheet — `phase05-citizens/processAdvancementIntake.js:260-262` falls back from `Advancement_Intake1` → `Advancement_Intake` → silent return `if (!intakeSheet) return results;`. Also referenced by `phase07-evening-media/mediaRoomIntake.js:537-538` (same fallback chain). Pipeline non-functional until at least one tab exists. Different from `Continuity_Intake` / `Election_Log` which lazy-create themselves; advancement intake silently drops. | todo | — | S181 audit `docs/engine/tech_debt_audits/2026-04-27.md` §6 Class 1 | Either pre-create `Advancement_Intake` tab manually (one-time) OR convert the silent return into a lazy-create matching `continuityNotesParser.js:73` pattern. |
