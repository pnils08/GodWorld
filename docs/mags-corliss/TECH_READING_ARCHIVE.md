...
Mike-shared (Drive `1ffE07mFAIjnPdsAKc1rC7LhLc-UGLLY8`; code aka.ms/SkillOpt). Trains a skill `.md` like a DL optimizer — skill doc = parameters, scored rollouts = gradient, edit-budget = learning rate, **edit accepted only if it strictly beats a held-out validation score**; +19.1 pts inside Claude Code. Requires an automated scorer + labeled holdout. **Key finding: our scorer already exists** — `scripts/finalArbiter.js` emits weightedScore ∈ [0,1] every cycle (C95 0.836), Mara = the 0.2 lane, MCP-reachable. Gap = held-out set + optimizer harness; real gate = rollout COST (one rollout = one full edition = hundreds-K tokens). Named Layer-2 learning mechanism for the autonomy roadmap, not a build row. Full eval: [[../RESEARCH]] §S248 — SkillOpt.

### S263 — Text-to-image landscape vs the two FLUX ceilings (2026-06-16)

Web survey of the mid-2026 field (GPT Image 2/1.5, Ideogram 3/4, Imagen 4, Recraft V3, Nano Banana 2 / Gemini Flash Image, FLUX.2) against GodWorld's two FLUX canon-fidelity ceilings — text-suppression (research.11) + named-subject attribute fidelity (research.15, the C97 Isley Kelley misfire). **Headline:** the pipeline runs FLUX.1 **schnell** (free 4-step distilled) via Together AI — NOT the 1.1 pro the S197 entry assumed — so an unknown share of the "FLUX ceiling" is the *distillation* ceiling; cheapest untested baseline is a `options.model` variant bump before any swap. GPT Image 2 reframes constraints as "actual controls rather than suggestions" (the FLUX failure in inverse) → best both-axis bet; Ideogram 3 is the text-control specialist. Verdict `watch` (one-cycle two-axis bake-off when bandwidth opens). Full eval: [[../research/2026-06-16-flux-image-model-eval]].

### S325 — Headless cron newsroom (Bay Awakening) + agentic RAG (2026-07-19)

Mike-shared Drive doc `182GQGxrdbOUIc6dO-CJBZzcZ3zthG-Pa` ("The Bay Awakening.txt") — external-assistant chat proposing a cron→orchestrator→reporter-API-calls→Mags-edits→publish headless newsroom — read alongside the agentic RAG retrieval family (router / reformulate / multi-hop / corrective / self-RAG). **Key finding:** the cron→orchestrator→API-writer→persist skeleton is already live in `discord-reflection.js` (+ citizen-wake/exchange), so the newsroom-as-cron is proven at small scale; the real lift is porting the guardrails (canon rules + Rhea/Mara/Final-Arbiter reviewer lanes + fourth-wall) out of Claude Code, not the scheduling. Agentic RAG maps onto "search before you guess" + the "load world state" node, but its whole viability rests on Mike's crux — a *cheap* model doing the narrow retrieval subtasks (same economics as the DeepSeek business-desk win). Verdict `watch`, two triggers. Full eval: [[../research/2026-07-19-headless-cron-newsroom-agentic-rag]].
