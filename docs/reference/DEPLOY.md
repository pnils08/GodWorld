...
**Current bench pointer:** SANDBOX 0908 web app **@40 = repo `328bf87c` (PROD @90 + engine.225 + engine.226), no overlay** — isolated build from `git archive 328bf87c`, sandbox `.clasp.json` written last and grep-verified, version 40, read back @40. **RESYNCED FROM LIVE C107 twice 2026-09-15 (S462, `syncSandboxFromLive.js --apply`, 82 tabs, read-back OK)** — @39 C108–C109 = engine.225's proof, @40 C108–C109 = engine.226's (`output/engine-sheet/bench-readback-22{5,6}-c108-c109.md`); sheet at C109, bench-only. The `/exec` fire response comes back as Google's "Page Not Found" interstitial after the ~150–230 s run (node fetch and curl -L alike) — the run completes; read the ring and World_Config, not the response. **PROD push of 225+226 classifier-blocked under auto mode this session — Mike's go.**
