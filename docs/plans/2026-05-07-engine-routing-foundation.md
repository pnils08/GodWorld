...
- **Status:** [x] done — S206. `.claude/skills/sift/SKILL.md` Step 2 amended with §"Step 2 priority consumption (T4.1)". Two transformations documented: (1) **Floor tagging** — proposals get `[FLOOR]` when any matched seed has `consequenceFloor === TRUE`. Floor band rendered first, Mike can re-order within but can't suppress below non-floor. (2) **Priority ranking** — proposals sort by `max(priorityScore)` across matched seeds, in three bands (floored / non-floored / engine-silent). Engine-silent fallback (no matched seed) preserves sift's own front-page-score ranking and renders `[engine: silent]` per T5.2. Lookup pseudocode locked: per-proposal sourceSignal text-match → matched seed set → max priority, any-floor, mode byline, parsed JSON for components. Floor trigger conditions cited (HIGH severity + crisis -1; HIGH severity + top-3 domain + arc ≥2 cycles). Pairs cleanly with T5.2 rendering.
...
- **Status:** [x] done — S206. `.claude/skills/sift/SKILL.md` Step 3 amended with §"Step 3 byline pre-fill behavior (T4.2)". 4-band confidence threshold table locked (high / medium / low / silent), with **future** column for post-T6.2 cutover behavior + **current** column for shadow-phase behavior. Shadow phase: ALL bands fall through to Mags assignment regardless of confidence; engine candidate stays invisible in proposal table. T6.2 cutover gate documented: `high`-band agree-rate ≥85% across 3 T3.8 shadow logs required; below threshold, threshold rule stays disabled and tuning task fires. Editorial-integrity rationale captured: pre-filling before agree-rate validation could compound shadow-phase bias into editorial behavior.
...
- **Status:** [x] done — S206. `.claude/skills/write-supplemental/SKILL.md` §1b amended with §"Engine B byline pool (T4.3)". Documents: (1) Engine B's `bylineCandidate` from a supplemental's source seed is valid even when the byline isn't on the edition's 9-reporter front-page table — full 25-byline `rosterLookup.js` pool in scope. (2) `formatFitScore_('<journalist>', 'supplemental')` ranks long-view + deep-dive bylines highest (Hal/Simon/Sharon/Mags = 4-fit), aligning with bench-development goal. (3) Lookup: `Story_Seed_Deck.BylineCandidate` (col P) for seeds with engine-recommended bylines; fall-through to design-first-match-second for free-running supplementals.
...
- **Verify:** skill diff.
- **Status:** [x] done — S206. `.claude/skills/dispatch/SKILL.md` Reporter Selection Guide amended with §"Engine B scene-fit signal (T4.4)". **Plan-literal reconciliation:** spec said `bylineRationale.components.formatAxis` but shipped `bylineEngine.js` uses `format` as the component name; doc reflects the shipped naming. Canonical dispatch pool (format-fit 4) captured: DJ Hartley + Maria Keen + Mason Ortega + Kai Marston. Tier-2 (format-fit 3): Talia Finch / Sharon Okafor / Tanya Cruz. **Override behavior:** Mike's reporter pick is editorial; engine format-fit ≥3 is validation gate. Reporter outside the canonical pool surfaces a `[engine: format-fit N — outside canonical dispatch pool]` note (auditable transparency, not a block). No source-seed dispatches skip the format-fit check.
...
- **Verify:** skill diff.
- **Status:** [x] done — S206. `.claude/skills/interview/SKILL.md` Step 1 amended with §"Engine archetype-to-journalist match (T4.5)". Documents the call to existing `matchCitizenToJournalist_(archetype, neighborhood, domain)` at `utilities/rosterLookup.js:907`. Skill action: lookup citizen archetype + neighborhood from canonical home (Simulation_Ledger / Generic_Citizens / Cultural_Ledger / Faith_Organizations / Chicago_Citizens), call function, surface `interviewerCandidate` JSON in briefing alongside manual beat-fit recommendation. Mode 2 (Paulson): function still runs (Anchor archetype), confirmation rather than direction. Empty archetype → fall through to manual selection. Engine signal is best-effort transparency, not a pre-fill or block.
...
- 2026-05-08 — Phase 4 closed (S206, research-build). T4.1-T4.5 shipped — five skill-doc edits wiring consumer-side reads of Engine A/B output. **T4.1:** sift Step 2 priority consumption — `[FLOOR]` tagging + 3-band priority ranking (floored / non-floored / engine-silent). **T4.2:** sift Step 3 byline pre-fill — confidence threshold rule documented; shadow-phase behavior locked to "all bands fall through" until T6.2 cutover gate (high-band agree-rate ≥85% across 3 cycles). **T4.3:** write-supplemental — Engine B's full 25-byline pool in scope; edition's 9-reporter table doesn't constrain supplemental work. **T4.4:** dispatch scene-fit override — canonical pool (DJ/Maria/Mason/Kai) + Tier-2 (Talia/Sharon/Tanya) locked from `FORMAT_FIT.dispatch`; `formatAxis` plan literal reconciled to `format` (shipped naming); off-pool reporter surfaces transparency note, not block. **T4.5:** interview archetype-match — wires existing `matchCitizenToJournalist_` from rosterLookup.js:907 into briefing as `interviewerCandidate` field. All five skills now reference engine output for transparency without auto-pre-filling editorial decisions during shadow phase. Phase 4 closes editorial-side wiring; only Phase 6 (cutover) remains, gated on 3+ cycles of T3.8 shadow-log data.
