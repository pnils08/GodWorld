...
| pipeline.56 | Lore writer — all 4 tasks shipped, Vinnie test passed, first grounded run ingested to both canon stores | done-pending-archive | antigravity (spec+hardening+ship: research-build) | [[../plans/2026-08-15-lore-writer]] — `8c1b4107`/`71b7cdd6`/`be3e579c`. Repo `scripts/loreWriter.js` supersedes agy's CLI-scratch draft (execFileSync + real path clamp + retry logic vs. scratch's execSync/no-clamp/hardcoded model) |
| pipeline.61 | Weekly lore-writer cron (Fri 15:00) — generate-only by design, per lore-ingest's "grading is never delegated to agy or any scripted process." `scripts/loreTargetSelect.js` round-robins Tier-1/2 citizens with no passed lore entry (cursor-persisted); `scripts/cron-lore-run.js` builds the seam-compliant prompt, runs `loreWriter.js`, writes `output/lore-cron-status.json` flagging the new quarantine file for `/lore-ingest` at the next research-build session. First target: Vinnie Keane (POP-00001) | ready | research-build | [[../plans/2026-08-15-lore-writer]] — crontab installed, tests `scripts/loreTargetSelect.test.js` pass, `--dry-run` verified end-to-end |
| pipeline.59 | Lore canon-ingest, extends pipeline.56 into verify+ingest | done-pending-archive | engine-sheet (built S383, `3c5be5d5` + `aa00ab5b`) | [[../plans/2026-08-17-lore-canon-ingest-pipeline]] — Tasks 1-4 shipped; `/lore-ingest` skill is the gate. First run failed all 3 quarantine fixtures on ledger contradictions. One spinoff remains for media: c103 edition masthead carries a malformed `Y2C103`. Canon-source-policy spinoff CLOSED 2026-08-27 (research-build): live notebook had grown from 52 policy-classified to 70 sources (5 stale, then-18-now-23 unclassified); regenerated the inventory, classified all 23 (6 published incl. C102/C103/C104 editions, 17 excluded — 3 unverified POP-tagged narrative pieces with no backing artifact held out pending review, 6 Simulation_Narrative tab-dumps same class as world_summary_c100, Jax's baylight-toxic-soil/varek-column pieces held out on missing-artifact/unresolved-correction grounds), removed the 5 stale IDs, synced 26 pre-existing decision titles NotebookLM had silently re-titled, validator now PASSES clean (70/70 classified, 0 unknown/duplicate). |
...
| engine.125 | desk_signal lane labels built from raw NamesUsed — unresolved names ship to desks with no POPID. Build labels from resolved names. | ready | engine-sheet | `scripts/buildWorldSummary.js` resolveFeedNames_ |
| engine.126 | Civilian career logic overwrote 3 sports-layer citizens; guard missed ClockMode. Fixed, rows restored, exposure 17→0. | done-pending-archive | engine-sheet | `2942b56b` |
| engine.127 | parseEdition returned zero sections on the c103-forward edition format, blinding the edition review lane. | done-pending-archive | engine-sheet | `9c98be4e`, `e0154b55` |
| engine.128 | ctx.rng shared one attractor cycle across seeds (float-precision overflow, cycleModes.js) — dup/rotated BondIds. CLOSED: mulberry32 fix + collision-safe remint + restore (586/586 verified). | done-pending-archive | engine-sheet | [[../plans/2026-08-21-ctx-rng-attractor-collapse]] |
| engine.131 | Sports coupling restore — S302 shipped its prescribed 9 atmosphere guards AND an unprescribed blanket sentinel (`applySportsSeason.js:77`) pinning `S.sportsSeason='off-season'` on every feed-sourced cycle. ~92 dial-class branch sites across 44 files inert; `Neighborhood_Map.SportsSeason` persists `off-season` for all 22 neighborhoods while the A's sit 126-35. Restore the true per-team phase (deepest wins); guards stay armed. T1–T4+T6 shipped `2935a4a7`; T5 reversed `d984f9d2` (stadium not built — INIT-006 at construction-planning since C83, engine geography already matches the tracker). DEPLOYED 2026-08-27 (Mike-go), pull-back byte-verified. **PROVEN ON BENCH 0827, 2026-08-27.** C106 fired with a seeded feed row: `SportsSeason` = `late-season` x22 (sentinel gone), 0 `Engine_Errors`, and 5 sports-attributed `Ripple_Ledger` rows carrying real cause->effect (`A's W3 [player-energy]`, `Oaks L2 [player-frustration]`) plus neighborhood magnitude blobs. Control run C105 (no feed row) correctly stayed `off-season` — acceptance holds ONLY on a cycle that HAS a feed row; no row = no game = `off-season` by design. Bonus: engine.122's carry-forward mirror executed for the first time ever and wrote 3 rows. Payload carried a 5-day backlog: engine.126/128/129 were committed-but-never-deployed and rode the same push, so next-cycle attribution spans four rows. T7 specced, not built. | in-progress | engine-sheet | [[../plans/2026-08-27-sports-coupling-restore]] |
...
| civic.24 | Sunday nine-seat table — Tasks 1-8 all landed, nine council seat agents live. Task 9 dry Sunday blocked only on a new cycle firing (C103 closed) | in-progress | grok / research-build | [[../plans/2026-08-16-city-hall-nine-seat-table]] |
...
| engine.120 | WealthLevel dual-writer drift — stale v14.2 writer deleted S381 `ee92e5bf`; engine's per-cycle write self-repairs on first clean cycle. Proof in pointer | done-pending-archive | engine-sheet | [[../plans/2026-08-18-wealthlevel-dual-writer-drift]] |
...
| civic.26 | cron-civic-gate metric-decimal false-positive on in-world doc ids ("Sheet S-7.3") blocked the clean C103 apply — in-world-reference allowance landed, both id positions covered, 17/17 lint tests | done-pending-archive | engine-sheet | scripts/lintCivicPackets.js |
| civic.27 | Agenda/hearing no-phase wall was fatal, not repairable — killed the Sunday chain at mayor-open. Repair pass added, proven live on the c103 packet | done-pending-archive | engine-sheet | scripts/cron-civic-agenda-wall.test.js |
| engine.122 | Carry-forward triple redundancy (Mike-direct, C104 incident): props + Carry_Forward_Store tab + disk export, loud-fail on missing memory. Acceptance = C105 populates the tab | done-pending-archive | engine-sheet | [[SHEETS_MANIFEST]] §Carry_Forward_Store |
| engine.121 | Inheritance recording rewritten to real Family_Relationships schema; InheritanceCycle/From/Note cols live. Acceptance = next death logs "stamped N family row(s)" | done-pending-archive | engine-sheet | phase05-citizens/generationalWealthEngine.js |
...
| engine.129 | Canon→bond intake — `mintCanonBonds.js`, 11 bonds minted C104, verified. Open: corpus backfill | done-pending-archive | engine-sheet | [[../plans/2026-08-21-published-canon-bond-mint]] |
| engine.130 | Wake→bond pickup drop LOCATED: 37/42 misses were rows written before col I existed (landed 2026-08-09 c5f2073e), 3 were bonds created after the wake. Live drops = 3 writers never given the engine.101 shape: citizenVoice.js PRESS (A-H only), personaProvider.js INTERVIEW/DISCORD drain (A-H only), citizen-exchange.js INTERVIEW/DEBATE (structural '' with no name-match). All three now write A-K with the shared matcher | done-pending-archive | engine-sheet | scripts/bondTargetMatch.js + scripts/citizenVoice.js + lib/personaProvider.js + scripts/citizen-exchange.js |
