...
- **`Casino_Ledger`**: Gated 4b wager tab (review build 2026-08-31, grok). Missing tab = no-op in `processCasinoLedger_`. Append-only citizen slips + one `HOUSE` float row. ARMED on bench and live 2026-09-01 (S410, builder sign-off); the engine never inserts it. Sports settlement reads the franchise's `WeekRecord` (col U, engine.202 — first game of the week; `none` carries) when the Cycle has one, else the first `Oakland_Sports_Feed` `game-result` whose `Streak` is `W#`/`L#`.
