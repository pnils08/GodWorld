...
- **Status:** [x] **DONE S256 (research-build).** D-1.1 `docs/mara-vance/INITIATIVE_TRACKER_CONTRACT.md` shipped (§1 28-col schema live-read from sheet / §2 20-phase ImplementationPhase vocab + intensities from engine PHASE_INTENSITY / §3 per-Type lifecycle arcs vote·visioning·program / §4 add-an-initiative procedure / §5 drift rule + variant map), registered in index.md, VOTER_LOGIC supersede note added. D-1.2 city-hall SKILL read-pointer at the trackerUpdates step. D-1.3 back-links: civic.md + all 5 project-agent RULES. **Measure-twice catch (pattern: feedback_measure-twice-cascading-effects):** all 5 project-agent RULES carried NON-canonical `ImplementationPhase` example literals that would zero on emit — `active-construction` (baylight, transposed), `pre-design` (health), `pre-deployment` (oari), `disbursement-pending` (stab), `pre-vote-visioning` (transit). Corrected each to the initiative's live canonical phase in the same pass (this is the exact writer-surface drift civic.14 exists to stop; the back-link would have been incoherent next to a template that violates it). Phase 3 still owns the full emit-enforcement step ("what to do when your phase isn't in the vocab" behavior change); Phase 1 fixed the literal landmines + routed every surface to the authority.
