...
- **`Business_Ledger`**: Tracking businesses, employee counts, and revenue. Writers, all by intent: `applyBusinessDynamics_` (engine.96, Phase5-BusinessDynamics — one range intent over Annual_Revenue..Growth_Rate every Cycle), `applyChaosDecay_` (Annual_Revenue cells), `flushBusinessFold_` (chaos cars), `runCareerEngine_` (Employee_Count write-back), `updateHeritage_` (heritage business row append). `Key_Personnel` is the ownership link (no engine reader yet — engine.96 Task 10). Row removal only by `archiveClosedBusinesses_` (§9).
