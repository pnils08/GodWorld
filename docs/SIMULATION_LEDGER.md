...
| AV | 48 | DialState | JSON 8-dial machine state (engine.31, S256); additive `maneuver: {p, g, a, c}` field since engine.157 (S416) — the cycle's posture memory, written by `runManeuverEngine_` on change, carried through the Phase 9 fold like `chaosExposure` | Phase-5 dial engines | 9 Phase-5 engines via `getCitizenDialBands_` | **CAUSAL** — every phase-5 engine reads `getCitizenDialBands_` for frequency multipliers 0.5-1.5x: `runEducationEngine.js:409`, `runNeighborhoodEngine.js:438`, `runRelationshipEngine.js:477`, `runCareerEngine.js:838`, `runHouseholdEngine.js:505`, `runConductEngine.js:176`, `runYouthEngine.js:195`, `generationalEventsEngine.js:442`, `generateGameModeMicroEvents.js:515` |
