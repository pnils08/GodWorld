...
- **One clock per stage, not per lifetime.** Funded: unchanged for 5 cycles (`civicStageStallCycles`). Standing and Delivering: 12 cycles untended (`civicStageUntendedStallCycles`, ruled 2026-09-21), long enough that a neglected service visibly weakens before it fails. A stage past its clock sets the row to `stalled` (**built 2026-09-21, Task 4 step 3**); the existing `failed` motion then costs its owners −2 a cycle. No clock on Proposed or on a legacy row; every other staged row is accountable for neglect, whether or not its domain can deliver yet. A stalled Delivering row falls to Standing and must re-prove delivery. A staged row never reads `silence`: its only drain is the stall. At Funded, work never resets the clock, only the stage change does; at Standing and Delivering, tending is exactly what resets it. A `work` move revives a stalled row once per stall, with no `advanced` credit. Clock step: **not built**.
