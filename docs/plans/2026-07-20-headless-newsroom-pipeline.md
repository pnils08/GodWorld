...
**STATUS (S347, 2026-08-01): BUILT AND LIVE — all six tasks.** 2.5.1 `f1f0a459`; 2.5.2/2.5.3/2.5.4 `07383541`; 2.5.6 + rest-cycles `ac3dd137`; canon-search enforcement + wire pulse + self-score footer + 2.5.5 tool-loop in the S347 close commit. End-to-end proof: Dana Reeve business c102 staged (Rhea PASS, 0 flags); Elliot Graye culture run exercised the full loop (4 validated canon facts incl. 3 deep threads, 2 self-directed tool calls traced, footer present, gate correctly flagged a leaky draft). All five 2.5.5 tools are live: canon_search / citizen_lookup / sheet_read (read-only) + memory_recall / memory_note — the memory pair reads/writes the reporter's OWN citizen page (`lib/citizenPage`, tag `cp-<POPID>` under `citizen-pages`; reporters are citizens, their page IS their container — the S347 "no containers exist" note was wrong, corrected same session). Cap 6 calls, every call traced. 2.5.2's assignment-choice open question stays open (decide at build when a desk's seed pool runs deep).
...
**Canon boundary + severity ranking (Mike-direct S344):** only the Saturday edition run injects to canon. The crons' staged output saved to their files is *their past work* — real to them, never canon. And the contamination classes are NOT equal: **changing a citizen is the worst offense** (ledger facts bent — age, role, neighborhood, or a real-world person imported); **engine verbiage/data numbers are a lesser one** — Mike has loosened here because in-world the engine and its data ARE Civis Systems' proprietary platform (Elias Varek's firm), so a reporter citing "Civis Systems data" is plausible sim texture. Still gated, but weighted below citizen-bending. Gate severity re-weighting is future tuning, not a now-change.
...
- **Files:** `scripts/cron-desk-run.js` (buildLaneState + per-wake prompts); templates as `docs/media/wake_templates/` (wake1-assignment / wake2-reporting / wake3-article)
- **Uniformity rule (Mike-direct S344):** scripts and searches work only if what each wake completes is UNIFORM. Scratch pads may be free-form; **canon searches, fictional interviews, and articles must be templated** — every reporter, every desk, same artifact shape. Templates written once as docs, loaded by the wake prompts.
- **One growing template per story** — the ONE document all three wakes append to, in order:
  - **WAKE 1 — assignment + canon research.** §1 Assignment header (assigned angle or assignment-choice + desk approach prompt — pre-filled at rota build). §2 Canon search: **minimum 3 canon facts**, each with a source ref — a storyline, a local business, or data compiled across **minimum 2 editions** — current AND deep threads attached to the angle. §3 Citizens: the angle's affected citizens presented HERE (seed citizens + ledger profiles — they surface during the canon search, not later). *Tool reality: the raw-API model cannot search; the SCRIPT gathers the material (edition archive grep, ledger profiles, storyline/tracker rows, world summary + engine audit slices) and the model selects + cites its 3+ facts from it. A deterministic check enforces the 3-fact minimum and that every ref exists.*
  - **WAKE 2 — citizen work.** §4 Interviews: the §3 citizens only — interview via `citizenVoice`, canon-search them, or use their likeness. **Citizen creation restricted** (unnamed street color stays free per the color doctrine; named people come from §3 or the ledger).
