...
- [ ] **Catalog shapes Task 4 step 1 has to handle (engine-sheet, 2026-09-20).** `transit-project` is `type: visioning`, and the visioning arc has no vote step while `Funded` clears on a vote passing — either a seat-proposed transit row mints as `vote`, or step 1 gives visioning rows their own entry into Funded. `sports-district` carries a two-column metric (`RetailVitality` + `NightlifeProfile`); the engine mirror must read an array.
