...
**Current bench pointer:** SANDBOX 0908 web app **@42 = repo `8ea1624b` (PROD @90 + engine.225 + 226 + 228b), no overlay** — isolated build from `git archive 8ea1624b`, sandbox `.clasp.json` written last and grep-verified, read back @42. **RESYNCED FROM LIVE C107 four times 2026-09-15 (S462)**: @39 C108–C109 = engine.225's proof, @40 = engine.226's, @41 C108 exposed the zero observation, @42 C108–C109 = engine.228b's (`output/engine-sheet/bench-readback-22{5,6,8}-c108-c109.md`); sheet at C109, bench-only. The `/exec` fire response comes back as Google's "Page Not Found" interstitial after the ~150–230 s run (node fetch and curl -L alike) — the run completes; read the ring and World_Config, not the response. **PROD push of `8ea1624b` (five engine files vs the live pull-back) classifier-blocked under auto mode — Mike's go.**
