...
**Letters.** A citizen who writes one cycle rests at least two before coming back. The desk defends this even when a brief names a returning writer too soon — trust the desk over the brief.
...
C100 spine + slate decisions live as machine ground-truth in `output/sift_proposals_c100.json` + the production log — not duplicated here as prose. Reusable craft lessons roll up into the notes above at post-publish; the per-cycle read does not.

C100 (the hundredth cycle) split the audit exactly along the one doctrine already encoded above ("Civic earns the page only when a citizen is voiced through it"): the back half — Richmond's Keane/Dillon farewell at the autograph table (a child's drawing in a jacket pocket), Velez making Perez's crew-lead promotion the proof the pipeline works, the three letters — is the **strongest citizen-centered run in the audit record**, A-range on its own terms. The civic section reverted: Q1/Q2/Q3 ran as a status board (a working group convened, a spike on Piedmont, five initiatives tallied) with **no citizen on the page**, and Q3 closed on "the machine is working" — the literal inversion of the doctrine (Entry 202 lesson, surfacing in print on the landmark cycle). That third of the edition dropped the grade to B+ despite an A-range back half. No net-new craft rule — the rule predicted it; the civic desk didn't apply it. Three canon-reconciliation items owed forward (not prose-edited backward per ADR-0007): "signal" (prohibited term) in Q2 → in-world substitute; Stab Fund "~30 cleared / ~280 remaining" doesn't reconcile against the 280-total fund (30 cleared ⇒ ~250 remaining, not ~280); OARI D7 8:04 must be pinned as *current* with 6.0-min *target-not-yet-achieved* so future editions don't cite the lock as done. New canon clean: Elio Perez (POP-00201) promoted to crew lead, flagged in NEW CANON, consistent with E98 framing.
