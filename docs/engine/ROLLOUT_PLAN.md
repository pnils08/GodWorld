...
| engine.175 | Undocked cast drawn in the engine + show markets keyed to the pilot — fire N draws cycle N+1's 3 pilots + alternates (`ctx.rng`, undockedDraw eligibility) into `S.undockedNextCast` + a draw tab (NEW TAB — builder nod); `credits_sign`/`mishap` resolve on the pilot's aggregated aired rows for the cycle, `night_winner` unchanged, `casinoUpcoming_` retired. Fixes the never-opened show market (off-by-one). Rulings confirmed builder-direct 2026-09-08 (S438). **CUT 2026-09-08 (S438b): `undockedDrawCast_` + pilot-keyed resolve/placement in casinoLedgerEngine.js, call at wealth Step 2.44; casino suite 24→51 green, dialGates 30/30.** **BENCH-PROVEN 2026-09-08 (SANDBOX 0831 @81, C119–C121): draw 6 rows/fire (3 fires, no dup, no redraw), 9 show slips placed at C120 keyed `c121:<POPID>` against the drawn cast, C121 settled 9/9 to the pre-fire prediction (6 win / 2 loss / 1 void on windowed credits) off 4 seeded feed rows, house float 250000→246970, 0 Engine_Errors.** PROD deploy rides the dials wave (HEAD's casino file calls engine.176's `pressureBar_` unguarded — not cherry-pickable onto @74); **live `Undocked_Draw` tab CREATED 2026-09-08 19:08 (headers only) after kimi's mint proof; the switch flips at the first PROD fire after the dials-wave deploy** | in-progress | engine-sheet (engine draw + casino re-key + tab create) / kimi (minting, briefs, orchestrator draw-tab read, Nia-slice standings wire — builder go S438; contract = plan (a′)) | [[../research/2026-09-08-kimi-casino-undocked-trueup]] §Review + [[../plans/2026-08-07-spacemolt-game-show]] §Post-ship open items (a)(c)(e) |
