...
| engine.125 | desk_signal sports labels now use resolved names only; unresolved stay in notes. Code landed, next `buildWorldSummary` rebuild picks it up | done-pending-archive | grok | `scripts/buildWorldSummary.js` resolveFeedNames_ |
...
| engine.132 | Initiative → illness wire: a delivering health initiative relieves hood illness; sick convergence 25%/cycle (floor 3), not flat ±3. Built + tested, NOT deployed — bench first. | in-progress | engine-sheet | scripts/illnessInitiativeRelief.test.js |
| engine.133 | `engine-wiring` subagent — Haiku, read-only, returns a fixed WIRING CARD (callers by phase, S fields with writers/readers, write path, tabs, open rows, git log) from the deterministic maps so the lead holds the card, not the hunt. Acceptance: `applySportsSeason_` (function) + `Initiative_Tracker` (tab), every spot-checked claim true against source, map-staleness flag fired correctly on engine.132. Cost ~95k tokens/40 calls per card at one-grep-per-turn — tool restriction did not move it, per-turn resend does; batching rule added (target ~10 calls). Hot-loads without restart. | done-pending-archive | engine-sheet | `.claude/agents/engine-wiring/SKILL.md` + [[../plans/2026-08-22-engine-agent-fleet]] §9 |
...
| civic.24 | Sunday nine-seat table — Tasks 1-9 dry C104 landed (nine hearings + gavel, applied=false). Gate blocked on D3 metric-decimal. Crontab still `--apply`. Task 10 sheet tab still gated | in-progress | grok / research-build | [[../plans/2026-08-16-city-hall-nine-seat-table]] |
...
| engine.130 | Wake→bond pickup drop CLOSED — 3 writers were never given the engine.101 A-K shape; 37/42 replay misses predate col I. All now write A-K via the shared matcher. | done-pending-archive | engine-sheet | scripts/bondTargetMatch.js + scripts/citizenVoice.js + lib/personaProvider.js + scripts/citizen-exchange.js |
