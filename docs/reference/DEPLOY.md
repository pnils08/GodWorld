...
0. **Prod has NO `CYCLE_TRIGGER_TOKEN` script property** (probed 2026-08-29 15:01 — the trigger answered `CYCLE_TRIGGER_TOKEN script property not set`, no cycle ran, live untouched at C104). Live fires are therefore Mike's: either he sets that property on the live script once (same value as the sandboxes, from the shared env file) and this checklist runs unattended from here, or he runs `runWorldCycle()` in the editor at steps 1, 4 and 5. Prod staging dir is built and verified (bench-proven tree + prod `.clasp.json`, sandbox ID absent, 0 test files) — rebuild it fresh on the day, don't trust a stale scratchpad.
