...
async function appendReflection_(popId, text, opts) {
  ...
  const customId = (o.cycle != null)
    ? `${tag}-c${o.cycle}-${o.daypart || 'wake'}${o.key ? '-' + o.key : ''}` // o.key: extra docs same wake (e.g. tension lines) must not collide with the reflection doc
...
// recentPage_(popId, limit) -> { results: [{content, createdAt, customId, metadata}] } | { error }
  ...
  try {
    ...
    for (const m of mems) {
      try {
        ...
        // metadata rides along (affect/event tags, seams Task 2) — resonanceRecall's affectWeight input
        if (content.trim()) results.push({ content, createdAt: m.createdAt, customId: m.customId, metadata: (gj && gj.metadata) || m.metadata || null });
