...
- **Status:** [x] DONE — S180, engine-sheet. CLI flags landed (`--type` with same allowed list as T6; `--cycle` required when type ≠ edition; cycle-from-text/filename fallback only for edition). Per-memory `metadata.type` (which previously held wiki-record-type values like `citizen-appearance`) renamed to `recordType` — no consumers found via grep, safe rename. New per-memory metadata: `type: <artifactType>` + `cycle: <N>` for retrieval filtering structure consistency with T6. **Container search filtering mechanism:** each memory's content gets a `[TYPE: <type> | C<cycle>] ` prefix — full-text Supermemory search filters by this prefix even though the existing `addMemory` v4/memories endpoint discards the metadata object (pre-existing latent shape; unchanged here per "don't change the Supermemory ingest API call shape" discipline). Dry-run prints `[METADATA] {...}` block + a sample memory showing prefix-applied content + metadata so verifier can confirm both layers. **Verifier output** (ran against `editions/cycle_pulse_edition_92.txt` as fixture stand-in since C92 interview .txt awaits T2 emit, with `--type interview --cycle 92 --dry-run`): metadata block printed `artifactType: "interview"`, `cycle: 92`; sample memory content prefixed `[TYPE: interview | C92] Patricia Nolan ...`; per-memory metadata `{"recordType":"citizen-appearance","type":"interview","cycle":92,...}`. Edge cases: non-edition without `--cycle` → exits with explicit error; edition default → back-compat preserved (text/filename regex unchanged); bad `--type` → rejected with allowed-values list. Real `--apply` end-to-end smoke test deferred to T10 (media validation) since the C92 interview .txt fixture doesn't yet exist and an `--apply` against an existing edition would pollute bay-tribune canon with mistagged records.
...
- **Status:** [x] DONE — S180, engine-sheet. All four scripts plumbed with `--type` + `--cycle` flags following T6/T7 pattern (ALLOWED_TYPES with 5 values; bad `--type` rejected with allowed-list; `--cycle` required when non-edition; edition default back-compat preserved; dry-run `[METADATA] {…}` block). **Per-type photo budget** enforced in `generate-edition-photos.js` via post-`assignPhotos` cap (5–8 edition; 1–3 interview/supplemental/dispatch/interview-transcript). **Output paths** type-aware: edition keeps `e<XX>` slug, non-edition gets `<type>_c<XX>[_<slug>]` (photos dir, manifest, HTML, PDF). **Masthead** in `generate-edition-pdf.js` adjusts label by type ("The Cycle Pulse — Interview, Cycle 92" instead of forced "Edition"). **Drive routing** judgment call: non-edition types share the existing `supplement` folder ID (the "non-edition subfolder" the plan defaulted to); type-prefixed PDF filename disambiguates. New DESTINATIONS aliases (`supplemental`, `interview`, `dispatch`, `interview-transcript`) added so positional invocation works AND `--type` auto-resolves destination when no positional is given. mapSubfolders re-syncs aliases after manifest discovery. **photoQA** reads type/cycle from manifest with flag override; emits to `qa_report.json` metadata. **Verifier output** (ran against `editions/cycle_pulse_edition_92.txt` as fixture stand-in since C92 interview .txt awaits T2 emit): all four scripts print correct METADATA blocks under `--type interview --cycle 92 --dry-run`; edge cases pass — missing `--cycle` on non-edition exits with explicit error; bad `--type` rejected with allowed-list; edition default preserves existing slug/path conventions (e92.html, bay_tribune_e92.pdf, edition Drive folder). DJ Hartley agent files NOT modified. Real `--apply` smoke + non-edition .txt fixture creation deferred to T10 media validation.
