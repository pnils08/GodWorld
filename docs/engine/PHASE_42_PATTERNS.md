...
|-------|-------|------------------------|------------------------|
| **(a) ship-now mechanical** | 2 | runCivicElections L394 + L432 *(low-cadence — fires C45 of even years; next C97)* | **GATED on C94 smoke-test of live S204+S205 deploys.** When unblocked, bundle with B3 carve-out for L213-219 in same file → one commit covers all 3 runCivicElectionsv1.js sites. |
| **(a) mechanical-w/-refactor** | 3 | generateChicagoCitizens L78 + L102 + L136 | Separate commit; in-memory tracking refactor + post-write reload removal. **GATED on C94 smoke-test.** |
| **(b) B3 schema-setup carve-out** | 3 | runCivicElections L213-219; generateChicagoCitizens L173; bondEngine L1417 (fallback dead) | B3 batch; engine.md exception entries OR `queueEnsureTabIntent_` on the carve-out files. **GATED on C94 smoke-test.** Bundle the runCivicElections carve-out with the (a) ship-now sites in same file. |
