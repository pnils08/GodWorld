...
- 2026-08-03 — Builder policy recorded (Mike, two SEPARATE dynamics — do not merge): **(A) Citizens.** The voice models ARE the ledger citizens, with cron lives. A citizen saying "my friend Jane told me" is their real social world — essence, not citation; never meant to be gated (blanket gating was a training-convenience byproduct, not the concept). The hard line is **mutating a tracked citizen** — invented words in a real POPID's mouth, a renamed project director ("Roberta"/Bobby class). If media ever mints a citizen, it must never ingest straight into the ledger — birth into Generic_Citizens (nothing is free); and a genuinely missing citizen-type (his hypothetical: a needed bus driver) is the **last-case, strict-necessity** scenario for that ever happening. **(B) Businesses.** 90 tracked against 386k population; new businesses become tracked by coming **from stories honestly** — that is how a business becomes tracked, and it is a business dynamic, not a citizen one (there is no generic-birth pool for businesses). The real-world-entity gate (canon substitutes) lives here — it exists against real-world business/institution influx, not sim-type. Same mutation line applies: don't change a tracked business either. Application: supplied-line policy (P1 of the deep-dispatch guardrail proposals).
