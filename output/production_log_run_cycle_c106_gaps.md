...
- **G-EC67 [judgment] [INFO]** — engine.90 Citizen Archive first live fire: flag 1, Phase 11 archived 54 (44 traded-away, 10 deceased), 0 skipped, ledger 968→922 (+8 minted), LifeHistory retain 12 cycles archived 106 rows, audit on-both 0 / restored 0 / flagged 0. Commit 11 acceptance MET. `buildCitizenCards.js --apply --from-archive` ran S432 late: 54 cards written, 0 errors. **Status:** CLOSED.

### Autonomy — what this run still needed a person for (builder ask, S432: "eventually it needs to run without me")

- **G-EC68 [judgment] [HIGH] — Step 3 is the only hand-fired step.** The engine runs in Apps Script on the builder's click; every other step is a script. The bench already fires by web-app GET with a trigger token (`CYCLE_TRIGGER_TOKEN`, shared env). Same GET on live from a cron (after pre-flight exit 0 + pre-mortem SAFE) removes the person from the fire. Gate design: cron fires only when both scans passed within the hour and `Engine_Errors` is empty from the prior cycle. **Status:** OPEN — the go-live gate is the builder's (GPU/hardware memory says the threshold is hardware, not software; the fire itself is software).

- **G-EC69 [judgment] [HIGH] — Steps 4–6 are a Claude session, not a job.** 4 (auditor), 5, 5.5–5.85, 6-mechanical are all deterministic scripts run in a fixed order with file gates; only the engine-review framing and the judgment entries need a model. A `scripts/runCyclePost.js` orchestrator (order = this skill's step list, each gate = the documented file/field check, stop-and-post on first failure) makes the whole post-fire chain a cron. The two prose steps go to a cheap model (the auditor JSON already carries the framing fields) or stay with a session. **Status:** OPEN.

- **G-EC70 [judgment] [MED] — world-data ingest only existed on edition publish.** Cycles without an edition (C103–C106) never reached `world-data`; `search_world` was answering from C102. Ingested C105+C106 tonight; the curl is now in run-cycle Step 5. Fold it into `buildWorldSummary.js --ingest` so it rides the script, not a skill paragraph. **Status:** half-closed (skill wired; script flag OPEN).

- **G-EC71 [judgment] [MED] — a cron that dies is silent.** The Sunday 15:00 status check-in crashed every week since S431 and nobody knew until rb read the log. Cron exit codes aren't watched. One line to the existing civic Discord webhook on non-zero exit (or a wrapper `scripts/cronGuard.sh <job>` around every crontab row) turns silent death into a ping. **Status:** OPEN.

- **G-EC72 [judgment] [MED] — one muted seat halts the Sunday chain.** Hearing HALTs on any failed voice (D1 today), so one model flake costs the whole city-hall run and a re-entry. With existingVoice reuse the cost fell to one re-roll, but unattended the chain should record the muted seat as ABSENT (voteDispositions already has the disposition) and continue; a seat muted twice in a row pages. rb's call (civic.26 design). **Status:** OPEN — research-build.

- **G-EC73 [judgment] [MED] — every model call needs a provider chain.** DeepSeek 429'd the civic chain and the texture builder the same evening. Texture now has a chain; `draftContentRows.js` (5.6), `cron-civic-run.js` datawake, and any other single-model script should share one `lib/modelChain.js` (families in order, distinct-family rule) instead of each carrying its own list. **Status:** OPEN.

- **G-EC74 [judgment] [LOW] — no cross-cycle invariant detector.** The sim-year drift (G-EC59) was visible from one row: `simYear − BirthYear` ≠ stamped Age on a citizen born this cycle. The mechanical audit has no class for "field A must equal f(field B)"; one detector over (Age, BirthYear, simYear), (HouseholdId ↔ Household_Ledger), (popIdHighWater ≥ max POPID) would have flagged it at C105. **Status:** OPEN.

- **G-EC75 [judgment] [LOW] — pre-flight's sports-feed name warnings need an unattended policy.** Unminted sports names are canon by design (row 215 tonight). Unattended, the check should distinguish "unresolved by design" (a `NamesUsed` entry with no POPID and no ledger match) from "misspelled/malformed" (rescued spelling, missing comma) and only block on the second. **Status:** OPEN.

- **G-EC76 [judgment] [LOW] — archive-side follow-ups aren't in the chain.** `buildCitizenCards.js --apply --from-archive` and `dumpLedger.js` ran by hand after the archive flip; the run-cycle skill has 5.56 for the ledger dump but nothing for cards. Add a 5.57 `buildCitizenCards.js --apply` (post-publish already runs it, but a cycle without an edition never refreshes cards). **Status:** OPEN.
