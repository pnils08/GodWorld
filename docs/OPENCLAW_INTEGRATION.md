...
## Cycle Context Pack Export System

Machine-friendly export system that generates prompt-ready artifacts for OpenClaw consumption.
...
| File | Purpose | Size |
|------|---------|------|
| `cycle-XX-summary.json` | Full `ctx.summary` snapshot | Large |
| `cycle-XX-context.json` | Prompt-ready distilled pack | Small |
| `manifest.json` | Latest cycle + checksums | Tiny |
...
```json
{
  ...
  "civic": {
    "votes": [{ "name": "West Oakland Fund", "outcome": "PASSED", "voteCount": "5-2" }],
    "grants": [{ "name": "Transit Safety Grant", "outcome": "AWARDED" }],
    "initiatives": [{ "id": "INI-012", "name": "Housing Trust", "outcome": "PENDING" }]
  },

  "safety": {
    "crimeCityWide": {
      "avgPropertyCrime": 45.2,
      "avgViolentCrime": 12.1,
      "totalIncidents": 67
    }
  },
  ...
  "citizens": {
    ...
    "activeCitizens": ["POP-00042", "POP-00043", "..."]
  },

  "riskFlags": ["high-tension", "negative-sentiment"],
  "continuityHints": [],
...
```json
{
  ...
  "contextPackVersion": "1.0",
  "cycles": {
    "78": {
      "cycleId": 78,
      "exportedAt": "2026-02-03T12:00:00Z",
      "summary": { "file": "cycle-78-summary.json", "checksum": "sha256:abc123..." },
      "context": { "file": "cycle-78-context.json", "checksum": "sha256:def456..." }
    }
  }
}
```
...
Add this call at the **end of your cycle run** (after `ctx.summary` is finalized):

```javascript
// In godWorldEngine2.js, after Phase 6 / summary finalization:
if (typeof exportCycleArtifacts_ === "function") {
  exportCycleArtifacts_(ctx, { includePretty: true });
}
```

**Placement:** After all phases complete and `ctx.summary` contains:
- `cityDynamics` (sentiment, migration)
- `crimeMetrics` (cityWide aggregates)
- `votesThisCycle` / `grantsThisCycle` (civic outcomes)
- `worldEvents` (event list)
- `cycleActiveCitizens` (POPIDs)

### Risk Flags (Deterministic)

The context pack derives risk flags without LLM:
...
OpenClaw reads `manifest.json` to detect new cycles:

```javascript
// In OpenClaw skill
const manifest = await fs.readJSON('exports/manifest.json');
const latestCycle = manifest.latestCycle;
const contextFile = manifest.cycles[latestCycle].context.file;
const contextPack = await fs.readJSON(`exports/${contextFile}`);

// Use context pack for prompt building
const prompt = buildPrompt(contextPack);
```

### Design Goals

- **No sheet schema changes** — only adds new export files
