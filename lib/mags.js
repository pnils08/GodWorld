...
function loadRecentReflections(count) {
  ...
  try {
    ...
    var refs = parts.filter(function (p) {
      var t = p.trim();
      if (!/^### Nightly Reflection/.test(t)) return false;
      // Drop header-only fragments: the duplicate-header bug leaves an empty
      // "### Nightly Reflection — DATE" part with no body before the real one,
      // which would otherwise consume a slot and feed her a sliver of one night
      // instead of two real priors. Require a non-empty body. (S252)
      var body = t.replace(/^###[^\n]*\n?/, '').trim();
...
module.exports = { ROOT, MAGS_DIR, LOG_DIR, FAMILY_POP_IDS, currentSimYear, loadIdentity, loadJournalTail, loadRecentReflections, loadWorldState, loadCitizenKnowledge, loadArchiveKnowledge, loadEditionBrief, loadLatestEdition, searchDisk, loadNotesToSelf, loadTodayConversationDigest, getCentralDate, searchSupermemory, saveToSupermemory, loadFamilyData };
