...
function buildLaneState(desk, cycle, lane, byline, quotes, persona, angleRead, assignment, wallBlock) {
  ...
  if (quotes && quotes.length) {
    ...
    for (const q of quotes) {
      L.push('- ' + q.name + ': "' + String(q.quote).replace(/\s+/g, ' ').trim() + '"'); // the quote carries the temperament already (builder ruling 2026-09-08: the writer hears a person, not a dial)
