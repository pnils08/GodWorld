 ...
 *
 * NAMES/BIZ take the id-bearing 3-field form OR the 2-field form (name |
 * role) with popid/bizId parsed as null. The 2-field form is what the
 * wake-3 MODEL emits — POPIDs are deliberately withheld from the writer
 * state (prose-leak class, first 2.5.2 live run; THE WALL forbids printing
 * them), so the GATE resolves name→id deterministically from its own
 * records (quotes[].pop, story.popids, ledger snapshot) and writes the
 * enriched object into the `.staged.json` sidecar. An id in the article
 * text is accepted but never required.
 *
  ...
  block.lines.forEach(function (entry) {
    ...
    if (kind === 'NAMES') {
      if (fields.length !== 2 && fields.length !== 3) {
        err('bad-field-count', 'NAMES needs `Name | POPID | role` or `Name | role`, got ' + fields.length + ' field(s)', n);
        return;
      }
      var name = fields[0];
      var popid = fields.length === 3 ? fields[1] : null;
      var role = fields[fields.length - 1];
      if (!name) { err('empty-field', 'NAMES with empty name', n); return; }
      if (popid !== null && !POPID_RE.test(popid)) {
      ...
      if (fields.length !== 2 && fields.length !== 3) {
        err('bad-field-count', 'BIZ needs `Name | BIZ-ID | role` or `Name | role`, got ' + fields.length + ' field(s)', n);
        return;
      }
      var bizName = fields[0];
      var bizIdRaw = fields.length === 3 ? fields[1] : null;
      var bizRole = fields[fields.length - 1];
      if (!bizName) { err('empty-field', 'BIZ with empty name', n); return; }
      var bizId = null;
      if (bizIdRaw === null || bizIdRaw === '-') {
        bizId = null; // no id emitted / named org with no BIZ row — gate resolves or decides clearance
