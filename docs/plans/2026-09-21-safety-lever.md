...
**Identity discrepancy resolved by the current plan:** The request calls OARI INIT-007. The fresh six-row Sheet read identifies **Oakland Alternative Response Initiative as INIT-002**, PolicyDomain safety, Type vote, Status passed, Outcome PASSED, signed, ImplementationPhase dispatch-live, targeting West Oakland, Fruitvale and East Oakland. **INIT-007 is Oakland Youth Apprenticeship Pipeline**, workforce, operational. Task 4 engine ruling 3, landed in 5694a383 while this draft was being written, confirms INIT-002 as the safety row. This draft uses that verified identity; no Sheet correction or activation is executed. The implementation must require an ID/name/domain match and stop on disagreement. Task 4.4 mentions operational INIT-007 and OARI separately; it does not explicitly assign OARI that ID.
...
StageBaseline must use Task 4 engine ruling 4's descriptor: `{v, origin: 'conversion'|'vote', cycle, tab, columns, scope, keys, cityMiddle}`. For OARI, preserve a valid conversion-time baseline and its actual committed source Cycle, fixed sorted hood set, each ViolentLevel and the observed city median. Do not relabel a cut-time C108 reading as an old C82 baseline. An absent/no-lever placeholder is unavailable; engine-sheet must explicitly arm its first valid descriptor before the first safety treatment, using the shared conversion contract. Subsequent activation, service, stalls and revival never replace a valid baseline.

Task 4 engine ruling 5 supersedes this draft's initial raw-mean suggestion: compare each fixed hood's ViolentLevel/city-median ratio, average those ratios over the fixed hood set, and require a decrease from the baseline ratio by the builder's civicDeliverMargin for civicDeliverHoldCycles consecutive committed observations. The builder subsequently ruled **margin 0.20 / hold 3** (4b502a52); use the current World_Config values, not a zero-margin or one-Cycle substitute. A missing/nonpositive city median, missing hood or incomplete hold window is unavailable. No population weights or cohort shrinkage are introduced. Recommend also requiring positive own-source receipts during the qualifying window, matched to the persisted observations; exact receipt coverage is part of safety design review. This proves measured benefit plus service contribution, not sole causation of every observed change; matched-control bench evidence establishes that the mechanism itself can move the gate input. An unrelated decline with no own-source receipt cannot clear Delivering. Counter current-Cycle visibility and the fixed historical baseline remain separate.

**Reachability conflict exposed by the new ruling:** In the local C108 case, OARI's initial mean ratio is `(33.31 + 30.31 + 26.90) / 3 / 26.90 = 1.121685`. Holding the city median and other mechanisms fixed, the proposed above-median-only lever can move that mean no lower than 1.0: a maximum direct improvement of **0.121685**, below the ruled 0.20 margin. A larger rate or per-Cycle cap only reaches that limit faster; it does not remove it. Other city dynamics could make the comparator clear, but that is not proof this channel alone can reach the gate. This draft therefore has an explicit implementation/activation prerequisite: builder/engine-sheet must reconcile the service's endpoint with the safety delivery threshold after a matched-control measurement. Possible design choices are a separately reviewed safety margin or a different bounded service endpoint; neither is selected here, and the global ruling remains unchanged. Do not flip safety playable on the strength of this draft or count unrelated median movement as the missing causal proof.
...
- **Steps:** Match OARI against the confirmed INIT-002/name/domain tuple before selecting a live row. Resolve the documented margin-versus-service-endpoint conflict; add reviewed enable/rate/cap keys through the existing config seed/load pattern. Pin synthetic six-row identity and the one-time baseline arming contract, including absent/placeholder/valid baseline behavior.
...
- **Status:** Not started — service policy and gate reachability need review; identity is confirmed in Task 4 ruling 3.
