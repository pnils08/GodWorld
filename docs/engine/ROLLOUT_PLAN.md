...
| engine.184 | DemographicMarker is scored per hood, not once for the city: `v3NeighborhoodWriter` reads `S.neighborhoodMigration[hood].drift` inside the hood loop; `applyMigrationDrift` per-hood thresholds become ratios of the cycle's own cross-hood median (engine.38 B2 pattern) and the city term drops from `drift/8` to `drift/25` — CUT S441, projected 5 distinct labels across 22 hoods (was 1) | in-progress | engine-sheet | [[../plans/2026-09-09-health-slice-freshness]] §criterion 4 |
