...
**S315 drift addition:** Neighborhood_Map 28 → 23 cols (trajectory schema — gentrification block repurposed, 5 empty real-world-study cols deleted, headers renamed; authoritative layout in `schemas/SCHEMA_HEADERS.md` regen S315). Writer split: v3NeighborhoodWriter now writes texture cols A–O positionally + District by header lookup (S315 clobber fix); MigrationFlow owned by Phase 6; trajectory block owned by phase05 neighborhoodTrajectoryEngine.

**S316 drift addition (engine.55):** no schema change; writer-behavior change. migrationTrackingEngine v1.2 now writes Simulation_Ledger Neighborhood + AN–AP (MigrationReason/MigrationDestination/MigratedCycle) via ctx.ledger on intra-city moves, and re-hoods/re-prices Household_Ledger rows (Neighborhood, MonthlyRent, LastUpdated) directly on household moves. Detail: [[../plans/2026-07-12-engine55-intra-city-relocation]].

**S316 drift addition (engine.56):** Household_Ledger 14 → 15 cols — `HouseholdSavings` (sum of member NetWorth, per-cycle) self-ensures at the header on first post-deploy cycle (schema-setup carve-out in householdFormationEngine v1.3 `updateHouseholdIncomes_`). Same engine now runs `reconcileHouseholds_` every cycle: Members/Head/Neighborhood/Type/Status repaired against SL HouseholdId truth, missing SL households adopted as new rows, spouse-split households merged; HouseholdIncome becomes live member-income sum (the flat 50000/85000/95000 formation estimates were a double-parse bug — real sums had never executed). Dissolution now clears members' SL HouseholdId. generationalWealthEngine inheritance resolves household-first (spouse/household survivors 50%, outside children 50%; either group alone takes all). Regen SCHEMA_HEADERS after the first sandbox cycle adds the column.

**S282 drift additions (seams plan):** Simulation_Ledger 49 → 50 cols — `MemoryRegisters` at AX (B1 bias + B3 unlived registers, single DialState-pattern JSON cell; sole writer is the Phase-9 compressor fold `utilities/compressLifeHistory.js` — `.biases` drain LIVE-in-code S283 seams Task 6 (`foldBiasIntents_` drains `S.biasIntents` tallied by the `generateCitizensEvents.js` bias-lite pool at emit time), `.unlived` capture LIVE-in-code S283 Task 8 (`foldAgedOutEntries_` branch-tag rider, cap 3); both clasp-deploy pending). Same S283 commit: `parseHistoryLine_` gained the in-world stamp branch (`Y2C48/C100/C? — [Tag]`) — these previously parsed as Untagged, degrading fold tag-routing for all post-S264 events. Determinism caveat for the coming `Event_Content_Ledger` tab (seams Design A): replay fidelity = seed × ledger content, same property as Neighborhood_Map. Also fixed S282: `Reflection_Intake` col H header was blank while the wake writer + compressor drain used it positionally for the affect tag — header `Affect` written, no reader change (drain is positional, `RI_COL_AFFECT=8`).

**S327 drift addition (engine.70):** no schema change; writer-behavior changes. Three NEW `Ripple_Ledger` writers (all via `recordRipple_` intents): `applyWeatherModel.js` PART 13 salience pass (causeType `weather-event`, effectTypes storm/flood/heat-wave), `updateTransitMetrics.js` salience pass (causeType `transit-event`, effectTypes service-disruption/gridlock), `generationalEventsEngine.js` heat-health block (citizen-scope `heat-wave-health`). generationalEventsEngine heat block also writes Simulation_Ledger Status/StatusStartCycle/HealthCause via ctx.ledger (chaos step-8 class) + LifeHistory_Log intents; chaosCarsEngine pre-flush block dents Business_Ledger Annual_Revenue for storm/flood via the existing chaosBusinessFold. PropertiesService `PREV_CYCLE_STATE_JSON` payload +~220 bytes (weatherFrontTracking carry + transitDisrupted flag, finalizeCycleState v1.9). Detail: [[../plans/2026-07-20-weather-transit-citizen-coupling]].

**S327 drift addition (engine.71):** no schema change; writer-behavior changes. `generateCrisisBuckets.js` v3.0 becomes an `Event_Arc_Ledger` writer again (32-col audit rows at crisis onset/peak/resolved, via intents — first new rows since the C70-81 fossil block) + a fourth `Ripple_Ledger` writer (causeType `crisis-event`). PropertiesService `PREV_CYCLE_STATE_JSON` +~1KB worst case (crisisArcs cap 8 + hospitalEvents cap 12, finalizeCycleState v1.9). Detail: [[../plans/2026-07-20-crisis-detection-rebuild]].

**S289 drift addition (seams Task 10):** NEW TAB `Event_Content_Ledger` (9 cols A-I: Kind, PoolKey, Slot, Text, Weight, Conditions, Tags, Grain, Active) — operator-authored event content: `line` rows (pool lines, may carry `$SLOT` tokens) + `fragment` rows (slot fillers). Sole reader: `phase02-world-state/loadEventContentLedger.js` (`Phase2-ContentLedger`, both entry points) → `S.contentLedger`; fail-closed row validation (unknown source tag / DSL field / enum ⇒ skipped + counted). No engine writers — content is authored by hand in the sheet. Missing/empty tab = no-op (hardcoded pools remain the fallback). Replay caveat as noted above (seed × ledger content). Plan: `docs/plans/2026-07-01-persistence-seams-content-ledger.md` Design A + §Pool injection.
