...
function loadArchiveKnowledge() {
  ...
  try {
    // Phase 19 desk-based structure: output/drive-files/{desk}/{reporter}/
    var deskNames = ['sports', 'civic', 'culture', 'business', 'chicago', 'letters', 'editorial', 'data', 'reference'];
    var totalFiles = 0;

    lines.push('\n**Tribune Desk Article Counts:**');
    deskNames.forEach(function(desk) {
      var deskPath = path.join(archiveDir, desk);
      if (!fs.existsSync(deskPath)) return;
      var count = 0;
      try {
        // Count .txt files recursively (desk has reporter subfolders)
        var entries = fs.readdirSync(deskPath);
        entries.forEach(function(entry) {
          var entryPath = path.join(deskPath, entry);
          try {
            if (fs.statSync(entryPath).isDirectory()) {
              count += fs.readdirSync(entryPath).filter(function(f) { return f.endsWith('.txt'); }).length;
      ...
      if (count > 0) {
        lines.push('- ' + desk + ': ' + count + ' articles');
        totalFiles += count;
      }
    });

    if (totalFiles > 0) {
      lines.push('\n**Total Archive:** ' + totalFiles + ' files across ' + deskNames.length + ' desks');
    }
