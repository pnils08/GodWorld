...
{
  const all = dm.getAllNeighborhoods();
  // 22 = exactly the live Neighborhood_Map rows. If this trips, either the ledger
  // gained/lost a hood or someone re-added a district entry with no live row —
  // the drift class civic.18 closed. Reconcile with scripts/auditHoodDrift.js.
  assert('22 hoods total (= the live Neighborhood_Map rows)', all.length === 22);
  ...
  // Montclair is not a tracked hood (builder 2026-09-05): no map row, no district.
  assert('Montclair has no district', dm.getDistrictForNeighborhood('Montclair') === null);
  assert('D6 is Piedmont Ave only', dm.DISTRICT_NEIGHBORHOODS.D6.length === 1 && dm.DISTRICT_NEIGHBORHOODS.D6[0] === 'Piedmont Ave');
}
