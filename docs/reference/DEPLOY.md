...
**Bench:** `SANDBOX 0908` — **S449: resynced from live C106 again (05:10), now @22 (a discarded local variant of engine.201b) at C122; BENCH-ONLY, NEVER REPLAY: C107–C121 and everything they wrote.** (stood up 2026-09-08 S438b, Mike-made copy of live at C106 — cycleCount 106, lastRun 9/6, 81 tabs, 922 SL rows, `Carry_Forward_Store` 6 rows → NOT a cold start, `CARRY_FORWARD_COLD_START_OK` not set). Spreadsheet `1FFpUs98L0wrEcfAaQy6-Ir2xYiCq9baCc0SzqzAMrtM` / Apps Script `1XcrDvYB89zaEQcXCK3oe4ir8e9fyTTj_uFAxZbEfzj2QzjsDdHC4CNvL` / web-app deployment `AKfycby-f9gv5sEOj6wURRFk-7H7oDNqUPShLt7KNWbhcYnQJ4e5F85EWWsaDP4Vu3ouFhWs` **RE-SYNCED FROM LIVE AT C106 2026-09-12 ~11:20 (S447, builder-run `syncSandboxFromLive.js --apply`, 82 tabs, 50,343 rows, `Undocked_Draw` created on the bench, 11 oversized cells sanitized, read-back OK on the 5 biggest). The 27-cycle Summer drift and ALL C120-C133 bench-only rows are GONE with this sync — including the S447 C132/C133 dummy feed rows and their 30 casino slips. Bench cycle now **C107**.** **C107 FIRED CLEAN ON LIVE-EQUIVALENT STATE — ok:true, 238s, 0 Engine_Errors — and this is the full dress rehearsal of the builder's own next live fire: bench code (@14) == PROD (@76) == repo HEAD, bench sheet == live sheet.** Every prediction made before the fire held: (1) cycleCount 106 -> 107; (2) **all 12 open C106 casino slips SETTLED — 6 `settled-win`, 6 `settled-loss`, every one stamped `CycleSettled` 107** — engine.207b confirmed empirically, settlement was NEVER blocked and needed no feed-contract change; (3) `HouseFloatAfter` 250,000 -> **247,028**, exactly the 2,972 win-side payout computed from the ledger beforehand; (4) 10 new C107 sports slips priced **win 1.57 / loss 2.20** off the A's own 3-1 record. Note (4) also confirms the pre-deploy claim that engine.207a is INERT on C107's live rows — C107's last feed row is itself an A's `game-result`, so old and new pricing agree there; the fix earns its keep on any cycle whose last row is the other franchise. **This fire also smoked the 21-commit PROD @75 wave on live-equivalent data with zero errors, so the builder's live C107 is de-risked.** Was @14 = HEAD `6b4a8701` (S447 2026-09-12: engine.207a casino team-correct pricing `fe057fab` + engine.203 record-clobber / one SeasonType parser `6b4a8701`. **PROVEN BY A CONTROLLED A/B ON THE BENCH — identical dummy feed rows both cycles, only the code differs:** C132 on @13 (control) priced the A's market **win 2.75 / loss 1.37** — heavy underdog, because the last feed row of the cycle was the Oaks at 0-4 and the old pricing took the last row regardless of team. C133 on @14 (treatment) priced the same market **win 1.19 / loss 3.99** — correctly favoured for a 127-35 club. Both fires ok:true, 121s / 127s, **0 Engine_Errors**, cycleCount 133. **BENCH-ONLY, NEVER REPLAY: the six `Oakland_Sports_Feed` rows at C132 and C133 (Notes stamped `S447 BENCH-ONLY ... NEVER REPLAY`) and everything C132-C133 wrote, including 30 casino slips.** **NEW DEPLOY GOTCHA, S447 — `clasp deploy -i <id> -d "..."` CREATED version 14 but left the deployment pinned at @13**; `clasp versions` showed 14 existed while `clasp deployments` still read @13. The bare form is not reliable for repointing. **Use `clasp deploy -i <id> -V <n> -d "..."` and read `clasp deployments` back before firing** — this is a second, distinct trap from S325 (push-does-not-deploy) and S443 (fired the old pinned version for two cycles).) Was @13 = HEAD `8db9ee1b`** (S443 2026-09-10: engine.188 — the sentiment TOP end. Deployed as a VERSION (`clasp deploy -i <id>`), not a bare push: the bench web app was pinned at @12, so the first two fires (C120, C121) ran the OLD code and the climbing series they produced was not the fix failing. **PRE-FIX CONTROL, @12: C122 city sentiment 0.98, all 22 hoods between 0.99 and 1.00 — a citywide spread of 0.01, 21 of 22 pinned at the ceiling. Per-hood sentiment carried no information at all.** **@13 PROVEN C123-C131, four cycles, ok:true, 0 Engine_Errors:** city 0.98 -> 0.57 -> 0.43 -> 0.43 -> 1.00, settling at 0.43 on the two no-holiday cycles — the offline model in `scripts/sentimentRestingLevel.test.js` predicted 0.42 for a Spring resting cycle. Hood spread 0.43-0.66 at rest (was 0.01), 0 cells at the ceiling on every non-holiday cycle. C126 is OaklandPride and reads 1.00 with hoods 0.59-1.00, 10 at ceiling — the biggest festival of the year saturating is a peak, not a floor-to-ceiling flat. engine.186 unaffected: Event_Arc_Ledger 12 resolved, Downtown closed at C120. **Summer watch RAN C127-C131 (cy23-cy27), 5 cycles ok:true 0 errors — and it FIRED: PrideMonth 0.76, Juneteenth 0.86 (11/22 hoods at ceiling), FathersDay 0.54, SummerSolstice 0.47, Independence 1.00 with 20 of 22 hoods at exactly 1.00. Between festivals the city is healthy and differentiated; on the biggest holidays it still flattens. Filed engine.196 — 188 still ships (it fixed the ORDINARY cycle, which is 46 of 52), the holiday peak is a separate cut.** shockFlag still `shock-flag` 8 of 8 (engine.187 open), so sentiment momentum runs at the 0.40 in-shock factor. **BENCH-ONLY since the C119 mark — NEVER REPLAY: C120-C131 and everything they wrote.** C120-C122 ran the OLD code (@12) before the version pin was spotted and are drift, not proof; C123-C131 are the engine.188 proof run. The bench sits at cycleOfYear 27 (July) against live at C106 / cycleOfYear 2 — a 25-cycle calendar gap, so any hood, arc or sentiment figure read off this bench is Summer state and does not describe the live city.) Was **@11 = HEAD `6db160c6` (S441 2026-09-10 ~02:10: + engine.186 carry fix (consecutiveGood survives compactCrisisArcs_); script version 12; over @10 = `ce9dbc75` engine.185 fix-up (relative crime/shock gates, hood sentiment floor off; script version 11). C114–C119 FIRED CLEAN — six cycles, ok:true, 0 Engine_Errors, dials acceptance cycles 8–13 (past the 12 the plan asked for). **engine.186 PROVEN at C119: five crisis arcs wrote `resolved` in one cycle — Rockridge (onset C108), Ivy Hill (C109), Baylight District (C109), San Antonio (C111), Brooklyn (C113) — "eased after 3 cycles back within city range". The ledger held 6 resolved rows across its whole history (C69+); it holds 11. Brooklyn: onset C113 tension 5 -> resolved C119 tension 0.11, predicted before the fire.** engine.185: sentiment has range now, both directions — city series C110-C119: 0.12 0.31 0.09 0.02 0.52 0.60 0.55 0.45 0.59 0.80. **Open:** the resting level has moved up and is climbing (hood mean 0.893 at C119, max pinned at 1.00) — the +10.26 upward vocabulary was tuned against a −0.48/cycle tax that no longer exists; the top end now needs the same look the bottom got. Not touched this session.)** Was **@9 = HEAD `738256d0` (S441 2026-09-10 ~01:40: + engine.185 adversity repricing + engine.186 symmetric arc resolution; script version 10. C112 + C113 FIRED CLEAN — ok:true, 214s/195s, 0 Engine_Errors; dials acceptance cycles 6-7. **Over-shot:** hood sentiment mean 0.333 -> 0.153 -> 0.094, 17 of 22 under the 0.15 crisis bar, four pinned at 0.00. Cause found: `shocks >= 3` and `crime >= 3` gate on raw activity counts (`worldEvents.length` runs 8-13 every cycle) so were true 19 of 19 cycles — the first pass made a permanent tax 2.5x heavier; plus a `Math.max(0)` floor on hood sentiment at v3NeighborhoodWriter:433. Both fixed at HEAD for @10. Plumbing proof only: `CRISIS-113-BROOKLYN` onset on the sentiment channel (first time in 37 cycles) — but on a 0.00 that the tax+floor produced, NOT a real event.)** Was **@8 = HEAD `83707f93` (S441 2026-09-10 ~01:05: + engine.184 — per-hood `DemographicMarker` + median-relative `MigrationFlow`; script version 9. **C111 FIRED CLEAN — ok:true, 149s, 0 Engine_Errors, cycleCount 111; dials acceptance cycle 5.** Proof, both halves: `MigrationFlow` was pinned at the ceiling (`5` x19, `4` x3) -> real distribution `-4`x1 `-3`x1 `-2`x3 `-1`x5 `0`x6 `+1`x1 `+2`x4 `+5`x1; `DemographicMarker` went from ONE base label for all 22 hoods -> 6 distinct (Mild outflow 7, Civic pressure zone 5, Mild inflow 5, Stable 3, Inflow surge 1, Outflow pressure 1). Reads true to hood character: East Oakland outflow pressure, Baylight District -4 (construction), Grand Lake the one inflow surge.)** Was @7 = HEAD `e358293c` (S441 2026-09-10 ~00:15: + the kimi health/drift wave (drift-clobber fix `e0f69e0d`, hospital-capacity key + packet illnessRate `fc3b1d2f`, cause-intake retirement `1240a80a`) + engine.96 T12 owner-door reel-2 `68dc5a66` + two engine-sheet review fixes (`62838898` orphaned test, `e358293c` world-summary capacity); script version 8; pull-back verified 169 files, `phase11-media-intake/healthCauseIntake.js` absent from the remote. **C110 FIRED CLEAN ~00:20 — ok:true, 143s, 0 Engine_Errors, cycleCount 110; dials acceptance cycle 4.** Drift-clobber proof: `S.demographicDrift` survives Phase 8 as an object, so the Phase-9 digest reads real migration/economy — PROVEN. Hood labels went `Stable` x18 + `Civic pressure zone` x4 -> `Inflow surge` x17 + `First Friday arts walk zone` x1 + `Civic pressure zone` x4 — unpinned from always-'Stable' but now pinned to always-'Inflow surge': `v3NeighborhoodWriter.js:249-262` thresholds at +/-5/20/35 against `demographicDrift.migration`, which is the net COUNT (1306, clamp +/-5000), not the +/-20-scale `S.migrationDrift` (21) the thresholds were written for. Pre-existing mismatch the fix uncovered; **decide scale/field before this wave goes to PROD** — plan §criterion 4. No signal this fire for the hospital-capacity change (Hospital_Ledger 0 open rows -> load 0 under both 40 and 100).)** Was @6 = HEAD `6ee31754` (S440 2026-09-09 ~00:55: + engine.183 transit causes on top of @5; C108 FIRED CLEAN ~01:05 — ok:true 200s, 0 Engine_Errors, cycleCount 108, `Transit_Metrics` col I `Factors` self-armed and on 18/18 rows, V3 event hoods + initiative slice on the rows, no game day (feed has no C108 row); dials acceptance cycle 2; fired from the terminal via a node fetch that reads the token from env — the shell-literal GET the classifier blocked is not needed).** Was @5 = HEAD `1368b1ff` (S440 2026-09-09 00:02: + intake CitizenBio carry, seat-named Key_Personnel tag, principals/managing partners draw — `c0731fdd`; never fired — @6 superseded it). Was @4 = HEAD `09a20fea` (S440 2026-09-08 ~22:20: engine.96 Task 12 owner door `3de5d632` + queue-aware pool `09a20fea`, on top of engine.175 + dials 176–182; pushed via the temp-dir route, sandbox id grep-verified both ways; NOT YET FIRED — the fire GET was blocked by the harness classifier for the token read, awaiting the builder's trigger or a permission rule). Two unbenched changes ride one bench by engine-sheet's call: separable outputs (the door's own log lines, `Owner, …` roles, `(owner)` cells), the engine.175-on-dials precedent.** Was @2 = HEAD `1ca6c944` (engine.175 + dials 176–182, pushed 2026-09-08 18:35). **3b done by the builder 18:34; bumped to @2; probe → JSON `bad token` (authorized). FIRST FIRE C107 18:36: `ok:true`, 206s, 0 Engine_Errors; 10 `dial*` World_Config keys self-armed (176: 4 keys — `dialStumbleUnemployedCycles` absent, matches the advisor fix-up gating on Income 0; 178: 6); every DialState row carries `mood` (929/929); negative-pole tags on LifeHistory after one cycle: Friction 51 / Setback 7 / Strain 8; casino 7 sports slips, no show market (no draw tab by design). Dials acceptance = 12 cycles per plan §Acceptance; C107 is cycle 1.** No pending sheet-side migrations (dials keys self-arm at Phase 1 via `ensureEngine176Config_` / `ensureEngine178Config_`; `Undocked_Draw` deliberately NOT created on this bench even though LIVE now has it (created 19:08 after kimi's mint proof) — the dials acceptance run stays a dials measurement; engine.175 is proven on 0831).**

**Previous bench:** `SANDBOX 0831` (stood up 2026-08-31, RETIRED 2026-09-08 into the table below). **Was @81 C121 (S438b 2026-09-08): engine.175 proven C119–C121 on top of the unbenched dials code (176–182 rode along, 0 errors across C120–C121); bench carries an `Undocked_Draw` tab (18 rows) + 4 bench-only `Undocked_Feed` rows for TargetCycle 121 — bench-only, never replay; the dials wave's fresh re-sync wipes them and the live tab is created separately when kimi's apparatus is ready.** **RE-SYNCED FROM LIVE AT C106 2026-09-07 ~14:35 (S436, `syncSandboxFromLive.js --apply`, 81/81 tabs, read-back OK on the 5 biggest). Code @68 = HEAD `b1f364d7`: @67 (engine.164) fired C107 clean — 8 mints POP-01092–01099, newborn BirthYear 2042; @68 (engine.165 + 166) fired C108 clean — 22 hoods bounded, the two reset Lake Merritt incomes re-floored at the hood rate. Bench cycle now **C108**. Bench-only since this sync — never replay: C107–C108 rows and memory, the two Income resets on POP-00260/POP-00268 (live got the same values as a deliberate restore, see [[DEPLOY_HISTORY]] §PROD @66).** Prior trail: RE-SYNCED FROM LIVE AT C105 2026-09-06 ~02:50 (S428, builder-run `syncSandboxFromLive.js --apply`, 80/80 tabs, 47,400 rows, read-back OK on the 5 biggest tabs; bench-only `Citizen_Archive` kept but its 58 rows cleared; World_Config re-seeded by `ensureCitizenArchive.js --seed=1083` = exactly what live looks like after the ensure: `popIdHighWater` 1083 / `citizenArchiveEnabled` 0 / `citizenArchiveTabLive` 1). Code **@66** = HEAD (S431 engine.163 `00d76804`: C110 fired — 3 seeded unrouted stem rows (Citizen_Media_Usage 653–655, C109 stems) routed, Advancement_Intake1 J notes in-world; C111 fired — their 3 LifeHistory_Log Advancement rows clean, 0 Engine_Errors both; bench cycle now **C111**; bench-only: those 3 usage rows + C110–C111 memory). Before that @65 (S430: @64 C6/C12 fired C108 clean; @65 C8 fired C109 clean, one organic death archived with its ArchiveNote — bench cycle **C109**, mark 1112, archive 55, SL 942). Builder fired C106 from the Apps Script editor (flag 0: whole stack @46–@63, mover no-op) then C107 after the flag flips to 1 (expect 48 traded + 5 deceased moved). Prior sync trail: 2026-09-05 ~21:40 (S428, 79/79 tabs, 47,399 rows, read-back OK on the 5 biggest tabs) — the builder deleted the three script-property blobs first, so the bench is a clean C105 with NO bench-only rows; first fire is a groundhog cold-start off `Carry_Forward_Store` (no override needed). **engine.90 on the bench (S428 ~02:05–02:30): code @61 (Commits 1–4, `847527d2`) → @62 (+ Commit 5 mover, `5f0ddd39`) → @63 (+ Commit 9 restore planner + close-out fixes, no fire — no GAS caller) → @64 (S430, C6/C12) → @65 (S430, C8); World_Config `popIdHighWater` (now 1137) / `citizenArchiveEnabled` **1** / `citizenArchiveTabLive` 1; `Citizen_Archive` 62 cols, 58 rows (56 moved at C109 + 2 deaths at C111); Simulation_Ledger 965; POP-00026 restored (`ReturnedCycle 109`); bench cycle **C111**. Proof trail: plan §Commit 10. Live: LANDED 2026-09-06 ~03:29 (S430) — ensure ran (`popIdHighWater` 1083, flag 0, `citizenArchiveTabLive` 1, empty 62-col tab) and the code is PROD @61; mover stays off until Commits 7/8 + the builder's flip.** Code at **@60** (S428, engine.119: @58 = Wave A `3b8b51b4` requireTab_ + hospital-ledger retry; @59 = Wave B `saveCarryForwardBlob_` ring + ghost guard; @60 = + `carryForward` diag-emit). **Proofs: C106 on @58 clean — ok:true 155s, 131 phases, cycleCount 106, 0 Engine_Errors, all three Carry_Forward_Store keys @106 through the retry path, one Hospital_Ledger admit via `appendRowWithRetry_`, Cycle_Weather 106.** C107 on @60 + the ghost re-fire (cycleCount rolled back 107→106 on the bench, expect `carryForward` recoveries in the fire JSON) are the Wave B proof — see the plan changelog. Bench-only since the S428 sync: the C106+ memory and rows; never replay.** Prior trail: re-synced from live at C105 2026-09-05 ~18:15 (S427, 79 tabs, read-back OK; carried the new live `Neighborhood_Map.Scenes` col AI); **C106 + C107 fired clean on @56 (P3 proof — 0 errors, 12 wave rows, 68 exact label-pool + 254 venue lines strict, ten-hood hits in 7 of 10; Scenes readers calendar-gated and not exercised — unit-proven + no-throw; trail in [[DEPLOY_HISTORY]]), C108 + C109 clean on @57 (door guard; the three late hoods draw — ten of ten), cycle C109; that sync's bench-only rows (24 wave rows POP-01085+, feeder rows and ticks, C106–C109 memory) are gone with the S428 re-sync.** Earlier trail: re-synced from live at C105 2026-09-05 ~12:40 (S423, 79 tabs, read-back OK) — builder reset the script properties; C106 + C107 fired clean on @51 (engine.148 P1 proof: 12 wave rows, 0 errors); C108 on @52 (P2 proof: 22 Crime_Metrics rows refreshed, 0 errors); C109 @53 / C110 @54 / C111 @55 (P1c proof: room women 61→69 while the wave ran; the 8 rows minted on C111 all stayed Active), cycle **C111**. Bench-only since this sync: 36 wave rows POP-01085+, ~24 feeder rows and their EmergenceCount ticks — never replay. Neighborhood_Map AF–AH were seeded here first and replayed to live the same hour — NOT bench-only.** Prior trail (before this sync): cycle **C114** — re-synced from live at C105 (S419); C106–C108 fired on the synced state (S419), C109–C110 (S423, engine.134 T4–8 + civic.18 4c/4d proof), C111–C112 (S423, engine.131 T7 + engine.134 T2–3 proof), C113 (fired on @48 by a deploy-ID typo, no change under test), C114 (S423, engine.99 #9 ChildAreas proof; the column was seeded here first and replayed to live the same hour, so it is NOT bench-only). **Bench-only data since the sync — never replay:** Intake rows 4–9 (two engine.109 test households, Bell + Okafor) and the citizens/households they minted (POP-01085–01088, POP-01090–01092, HH-0107-I001, HH-0108-I001); Intake columns `Relation` (J) and `Sex` (K) — live self-arms `Relation` at its next fire; `Chaos_Cars` row `EventId=synt1s423` (S423, synthetic Tier-1 `ConsequenceFloorFired=TRUE` hit at C110 for the engine.11 T5.3 reader proof — never replay); `Oakland_Sports_Feed` row C111 / Oaks / mid-season (S423, the T7 opening trigger — never replay) and everything it moved (INIT-006 `operational`, Baylight profile lift, the C112 Santana turnover). Spreadsheet `18BOJmzlO7EoaEhvUsUaqLIZvgrTC1yltUYk_Gz3I3W8` / Apps Script `1_3PDs7CSUsYvjjcXWtYGKjpPd7ekiIsUKKLSwlMtj3ioQKV0ew8TAfPh` / web-app deployment `AKfycbxbfP2Wu6haP4HXUGLk9T4wlW4ixWPsnfZJ51G5wVFTbkhESbT4teJG2xvRowP3BygIkw`. Script Properties `SIM_SSID` + `CYCLE_TRIGGER_TOKEN` set, authorized (protocol step 3b done). **`CARRY_FORWARD_COLD_START_OK` — do not set**: the bench's `Carry_Forward_Store` sheet already carries real C105 carry-forward, so this is not a cold start (see [[DEPLOY_HISTORY]] for the read-back that proved it). Full per-wave trail: [[DEPLOY_HISTORY]] §Current sandbox.

**Live:** PROD **@78** (2026-09-13 ~06:40, S449 — UNAPPROVED by the builder, not yet fired; builder verdict: broke the sim) = repo `61274362` engine tree, pull-back 168 js byte-identical, 0 tests — engine.201b (bond neglect from contact, feuds rest, hood line carries the hood's sign, pole residue unpinned), bench SANDBOX 0908 @20 C107–C119 + @21 C120–C121 clean on a live-C106 resync; smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @78. Earlier: PROD **@77** (2026-09-13 ~04:25, S449) = repo `830e05ac` engine tree, pull-back 168 js byte-identical, 0 tests — engine.201 dials both ways from causes, bench SANDBOX 0908 @18 C107–C118 + @19 C119–C120 clean on a live-C106 resync; smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @77. @76 / @75 recorded in SESSION_CONTEXT history and git (`c2487169`, S445). Earlier: PROD **@74** (2026-09-07 ~23:20, S436) = repo HEAD `63e2084a` engine tree, byte-identical (pull-back 0 differing, 169 files, 0 tests). **@74 = engine.171 (2 files: `hoodRentFromIncome_(ctx, medianIncome, hoodShare)` takes the hood's own `Neighborhood_Map.RentShare` when it holds one, the loader carries `rentShare`, the trajectory engine renders MedianRent with it; live sheet write the same hour: Neighborhood_Map grid 35 → 36 cols, col AJ `RentShare` header + 22 cells — D4 villages 0.24, Eastlake / East Oakland 0.26, Chinatown / San Antonio 0.28, else 0.30 — read back 22/22; bench SANDBOX 0831 @78 C118 clean, MedianRent rendered at the hood share 22/22; [[DEPLOY_HISTORY]] §PROD @74; script version 68).** @73 was: **@73 = engine.167 (1 file: `ROLE_SECTOR_HINTS_` re-ordered — sports → null, newsroom, civic, port, tech, then the rest; bare director / planner / counselor / data gone; bench SANDBOX 0831 @77 C117 clean; [[DEPLOY_HISTORY]] §PROD @73; script version 67).** @72 was: **@72 = engine.174 (1 file: `generateGenericCitizens.js` — the feeder refills per short hood: `feederHoodShortfall_` = open seats (floor 12 − tracked) minus the Active pool rows waiting, one queue serves the sex floor and the hood floor together, short hoods placed first; bench SANDBOX 0831 @76 C116 clean — Glenview 0→4 waiting, East Oakland 1→3, 8 minted into short hoods; live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @72; script version 66).** @71 was: **@71 = engine.173 (1 file: `migrationTrackingEngine.js` `MISFIT_INCOME_RATIO` 2.5 → 1.5 — the move-up lane opens when a unit's income reaches 1.5× its hood median; bench SANDBOX 0831 @74 C114 clean — 22 units moved (the backlog: ~54 units had been waiting behind the 2.5× gate), 9 of the first 15 to Piedmont Ave, the empty hoods +8 in total; live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @71; script version 65).** @70 was: **@70 = engine.172 (4 files: `jobReferencePay_` prices a position in the catalog's [min, max] by years / credential / drive / tier / employer growth, `payProfileFromRow_` reads them off the row, every row-bearing caller passes the profile; bench SANDBOX 0831 @73 C113 clean — 25/25 untracked rows at their predicted personal figure; live smoke = the builder's C107: ~102 of 238 untracked rows lift to their own figure, 37 by >1.4×; [[DEPLOY_HISTORY]] §PROD @70; script version 64).** @69 was: **@69 = engine.169 + engine.170 (2 files: `applyTrackedEmployerFloor_` applies the company average only to jobs in the business's sector — an out-of-sector job floors at min(its band, employer × stage), never past either signal; the rehire matcher re-roles a field-change hire into the employer's field with the current-field tag and the job's band; bench SANDBOX 0831 @72 C111 + C112 clean — 169 touched 0 rows (everyone already at the company average; forward-only), C112's one hire same-field and unchanged; live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @69; script version 63).** @68 was: **@68 = engine.168 (1 file: `utilities/citizenDerivation.js` ECONOMIC_PARAMETERS synced from `data/economic_parameters.json` — 108 working jobs added, 306 roles; bench SANDBOX 0831 @70 C110 clean — the reset warehouse worker floored at 57,400 = the new band, the janitor stayed at his C104 93,271 (above the new band, raise-only); live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @68; script version 62).** @67 was: **@67 = engine.166 v2 + the calendar-year snapshot brackets (8 files: `jobReferencePay_` prices from the economic catalog — the builder ruled 2026-09-07 that a neighborhood has nothing to do with income — hood-reference pay and `loadHoodBusinessPay_` deleted, nine callers re-pointed, `applyUntrackedJobReference_`; `buildLedgerFreqSnapshot_` takes `options.simYear` from `simYearOf_`; bench SANDBOX 0831 @69 C109 clean — POP-00260 / POP-00268 re-floored at 117,800 / 101,400 = the catalog's Port & Labor mid / Small Business senior bands; live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @67; script version 61).** @66 was: **@66 = engine.165 + engine.166 (2 files: `v3NeighborhoodWriter.js` saves Sentiment clamped to [-1, 1]; `generationalWealthEngine.js` `hoodReferencePay_` prices the role's field first, tags only for unplaceable roles, janitor/custodian/clerk sector hint; bench SANDBOX 0831 @68 C108 clean — 22 hoods bounded, the two restored Lake Merritt incomes re-floored at 104,500 / 137,800 = the six-business median × stage; live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @66; script version 60).** @65 was: **@65 = engine.164 (14 pushed files: every sim-year literal `2041` → `simYearOf_(ctx[, cycle])`; `citizenExitDefects_`/`citizenArchiveNote_`/`citizenArchiveRow_`, `scoreLedgerCitizenForOffice_`, `checkEmergencePromotions_`, `seedNeighborhoodDemographicsFromLedger_` take the year from their ctx-bearing caller; `citizenContextBuilder` module SIM_YEAR gone; bench SANDBOX 0831 @67 C107 clean on a same-day live re-sync — 8 mints, newborn BirthYear 2042, 0 Engine_Errors; live smoke = the builder's C107; [[DEPLOY_HISTORY]] §PROD @65). Note: `clasp deploy` on the PROD deployment cut script version 59 — PROD @59–@64 were push-only, so the Apps Script version counter sits six behind this trail's @N.** @64 was: **@64 = engine.163 (1 file: `describeUsageContext_` in mediaRoomIntake.js — the writer-wake packet stem stops at the Citizen_Media_Usage ops key and reaches Advancement_Intake1 / Intake notes as in-world text; bench @66 C110/C111 clean; live smoke = the builder's C106, 64 unrouted stem rows waiting; [[DEPLOY_HISTORY]] §PROD @64).** @63 was: **@63 = engine.90 Commit 8 (2 files: `citizenExitDefects_` → `ArchiveNote` stamped at exit, migration engine skips Traded; live: 6 Traded `MigrationIntent` cells cleared + read back; bench @65 C109 — an organic death left with its note; [[DEPLOY_HISTORY]] §PROD @63). @62 = engine.90 Commit 6 + Commit 12 GAS half (3 files: `citizenArchiveLatestByPop_`, heritage resolves archived members — never living, never in TotalNetWorth, generation chain kept — inheritance readers + healthCauseIntake read the same view; bench @64 C108 clean; [[DEPLOY_HISTORY]] §PROD @62). @61 = engine.90 Citizen Archive code (11 files: `popIdHighWater` allocator on every cycle-path minter, `resolveCitizen_`, `Phase11-CitizenArchive` mover gated on `citizenArchiveEnabled` — live 0 — and the restore planner; live sheet write the same hour: World_Config +3 rows, empty 62-col `Citizen_Archive` tab, 81 tabs; [[DEPLOY_HISTORY]] §PROD @61). @60 = engine.119 Phase-11 remainder (3 files: the last 8 lazy creates → `requireTab_`, replay-safe ghost guard; [[DEPLOY_HISTORY]] §PROD @60). @59 = engine.119 (28 files: `requireTab_` ×25 sites — the cycle never creates a tab; `persistHospitalLedger_` + carry-forward mirror under retry; cycle-stamped carry-forward blobs in a 3-slot `Carry_Forward_Store` ring with a self-ghost guard; `carryForward` diag in the fire JSON; bench @58 C106 + @60 C107 + simulated-crash re-fire all clean — trail in [[DEPLOY_HISTORY]] §PROD @59). Live sheet write this session: `Business_Archive` tab created with `BIZ_ARCHIVE_HEADERS` (80 tabs now). Fifteen deploys @46–@60 are unsmoked on live.** @58 = engine.148 P3 door guard (`processAdvancementIntake.js` folds an authored hood to the map, throws on an off-map name naming the row; bench @57 C108 clean) over @57. @57 = engine.148 Phase 3 (texture pools by `EmployerCharacter` label, `Scenes` column — seeded live FIRST, 22/22; bench @56 C106/C107 clean) over @56. @56 = engine.148 P1c (World_Config `gcPoolFloorFemale` 120 / `gcPoolFloorMale` 40 self-arm on the next fire; wave draws the under-represented sex first; one-cycle seasoning; bench @53–@55 C109–C111 clean) over @55. @55 = engine.148 Phase 2 (WeatherZone / Adjacent / AttentionWeight columns AF–AH seeded on live FIRST — the loader throws on a blank; crisis weight earned; gender table + dead transit map deleted; bench @52 C108 clean) over @54. @54 = engine.148 Phase 1 (migration wave + floor dials on World_Config, feeder weights from the rank order, surfacing floor draw; bench @51 C106/C107 clean) over @53. @53 = engine.99 Finding #9 child areas (`canonNeighborhoodLoader.js`, `commuteFlowEngine.js`, `checkForPromotions.js`, `economicRippleEngine.js`, `v3NeighborhoodWriter.js`) over @52; @52 = engine.131 T7 lit + engine.134 Tasks 2–3 over @51; @51 = engine.134 Tasks 4–8 + civic.18 4c/4d over @50; @50 = engine.109 the household door; @49 engine.141; @48 engine.142; @47 civic.32; @46 engine.162 + civic.30/31. **No HELD files — repo HEAD == live.** **Eighteen deploys @46–@63 are unsmoked on live** — all bench-proven on live-synced C105 state; the builder's next fire (C106) smokes them together. Live sheet writes this session, applied + read back: `Business_Ledger` BIZ-00101 Sector → Services; `Neighborhood_Map` ChildAreas column (header + 10 authored cells). **S427 live write (2026-09-05 ~18:10, replayed to the bench by the re-sync): `Neighborhood_Map` grid 34 → 35 cols, col AI `Scenes` header + 22 authored cells from `scripts/fixtures/hood-scenes.json`, read back 22/22 — inert until the P3 code deploys.** T7 is live; Baylight opens when an Oaks `mid-season` feed row lands after C104. Proofs: `docs/plans/2026-08-02-neighborhood-truth-source-migration.md` §Changelog 2026-09-05, `docs/plans/2026-08-27-sports-coupling-restore.md` §T7 shipped, `docs/plans/2026-08-30-hood-identity-remainder-plan.md` §Changelog (S423). Full deploy ledger back to @1: [[DEPLOY_HISTORY]] §PROD deploy log.
