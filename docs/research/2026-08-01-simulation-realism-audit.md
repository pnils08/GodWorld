...
- 2026-08-03 — Builder policy steer recorded (Mike, on named off-ledger people in citizen testimony): **first-name relationship color is essence, not citation — and new citizens are born from stories into the generic pool, never minted into the ledger by media.** "My friend Jane told me" ≠ "Jane Doe said 'xxx'"; blanket name-gating was a training-convenience byproduct, not the core concept — the canon-name gate blocks *real-world* entities, not sim-native neighbors. Citizens' social worlds are real. The citizen-gap is a legitimate LLM use case (900 tracked vs 386k population; a needed bus driver is a citizen-type gap). But nothing-is-earned applies to creation too: off-ledger named people enter tracked status only story-origin → Generic_Citizens birth → earned promotion. Applications: supplied-line policy (P1 of the deep-dispatch guardrail proposals), a mentioned-person candidate intake design (engine.51/engine.90 adjacent, unbuilt).
