...
- **Civic echo** (Mike, S280) — a hood-scale world event should reach the civic layer: the district rep speaks on the storm, the outage, the closure wave. Cross-layer seam (world events → civic-office agenda), media/civic terminal pickup — not a generator change.
- **Household-aware housing texture** (Mike, S280) — Household_Ledger carries `HousingType`/rent per household; a join would unlock renter-vs-owner texture (pipes vs roof). Sheet read + member join + coverage fallback → same design pass.
- **Upstream event-name vocab** (S280) — the crisis engines' subtype pools ("Power Fluctuations" etc., 3–4 per category/condition) are as thin as the citizen pools were; natural resident for the content ledger.
