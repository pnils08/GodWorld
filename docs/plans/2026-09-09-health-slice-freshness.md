...
4. (Engine, post-land) Phase-9 compressed digest reports real `migration`/`economy` values — **PROVEN on bench @7 / C110**: `S.demographicDrift` survives Phase 8 as an object, so `applyCompressionDigestSummary.js:155-157` reads real values instead of `0`/`'stable'` off a string. `saveV3NeighborhoodMap_` emitting non-'Stable' labels — **PARTIAL, and it exposed a pre-existing scale mismatch (see below)**: `Neighborhood_Map.DemographicMarker` went `Stable` x18 + `Civic pressure zone` x4 → `Inflow surge` x17 + `First Friday arts walk zone` x1 + `Civic pressure zone` x4. The label is unpinned from always-'Stable', but it is now pinned to always-'Inflow surge'.
   ...
   **Scale mismatch — decide before this wave goes to PROD (engine-sheet, 2026-09-10).** `v3NeighborhoodWriter.js:249-262` reads `S.demographicDrift.migration` and thresholds it at ±5 / ±20 / ±35. That field is the city's **net migration count** (1306 at C110, clamped ±5000 by `applyDemographicDrift.js:95-96`) — it clears +20 every cycle, so every hood reads 'Inflow surge' forever. The correctly-scaled field is `S.migrationDrift` (21 at C110) — already the writer's own documented fallback at `:250`, and almost certainly what the ±5/20/35 thresholds were written against. The clobber fix did not cause this; it uncovered it (while `demographicDrift` was a string, `driftNum` was forced to 0 and the thresholds never ran). It matters because the label is consumed downstream: `scripts/buildCivicVoicePackets.js:381` feeds `DemographicMarker` into civic voice packets. **Not fixed here** — what the marker should measure is a design call, not a one-liner. Recommendation: repoint `driftRaw` to `S.migrationDrift`, or rescale the thresholds to the count.
