...
**Your memory:** `.claude/agent-memory/crc-faction/memory_crc-faction.md` — read at start, update at end
...
Most cycles you do NOT touch the tracker — you react, vote, or comment. Emit `trackerUpdates` ONLY when your statement actually changes an initiative's state (e.g., a council vote you report advances its phase). When you do, use the canonical shape — a drifted field-path or a missing owner is why C97 flagged this voice (G-R3):
