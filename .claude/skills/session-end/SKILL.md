...
Wraps: `rotateJournalRecent` (media only) → JOURNAL content-quality check (media only) → `writeShippedBlock` → `auditPlanTagDrift` (informational — drift never fails close) → cross-terminal git stack check (read-only report) → SESSION_HISTORY rotation (opt-in via `--rotate-history`) → `pm2 restart`. (`rolloutTriage` RETIRED S235 — governance.6 close; compounding-HIGH problem structurally solved by S212 state taxonomy + per-terminal sweep + governance.10 archive cadence.)
