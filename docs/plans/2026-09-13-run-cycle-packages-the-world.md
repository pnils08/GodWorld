...
- `crisisSaturation` (`:433-449`): `crime events + major civic + major health` from `eventCategories`; carried value 0 at C105, C106, C107 — the count is 0 every Cycle on record, so the ≥ 0.6 / ≥ 0.8 gates downstream cannot fire either. Why the categoriser sees no crime while the ring carries a CRIME_SPIKE ripple is the first thing to measure.
