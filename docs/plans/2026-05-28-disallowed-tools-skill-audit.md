...
- **The editing rollout is AUTONOMY-GATED, not near-term picker work.** Trigger = the transition to unattended/scheduled skill execution (deployed-agent autonomy). Applying it sooner adds friction + the untested-Q4/Q5 risk for marginal current benefit.
- **Small current-era upside worth noting** (does not change the gating): a reviewer/auditor skill that structurally cannot Write/Edit can't repeat the documented "skill mutated something it shouldn't" failure class (S229 edited-the-approved-print, S230 destructive-recovery-proposals). Minor defensive value now; primary value is autonomy.
