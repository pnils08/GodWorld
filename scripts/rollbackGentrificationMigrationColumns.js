/**
 * ============================================================================
 * ROLLBACK: GENTRIFICATION & MIGRATION TRACKING (Week 4)
 * ============================================================================
 *
 * Removes columns and sheets added by addGentrificationMigrationColumns.js migration.
 *
 * WARNING: This will delete all gentrification and migration tracking data.
 * Use only if you need to completely roll back the Week 4 enhancement.
 *
 * Columns removed:
 * - Neighborhood_Map: GentrificationPhase, DisplacementPressure, GentrificationStartCycle,
 *   MedianIncome, MedianIncomeChange5yr, MedianRent, MedianRentChange5yr,
 *   DemographicShiftIndex, WhitePopulationPct, WhitePopulationChange5yr,
 *   HighEducationPct (11 columns)
 * - Simulation_Ledger: DisplacementRisk, MigrationIntent, MigrationReason,
 *   MigrationDestination, MigratedCycle, ReturnedCycle (6 columns)
 *
 * Sheets deleted:
 * - Migration_Events
 *
 * Total: 17 columns removed, 1 sheet deleted
 *
 * Usage:
 *   node scripts/rollbackGentrificationMigrationColumns.js
 *   node scripts/rollbackGentrificationMigrationColumns.js --dry-run
 *
 * ============================================================================
 */
...
async function main() {
  ...
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════════════╗');
  console.log('║   ROLLBACK: GENTRIFICATION & MIGRATION TRACKING (Week 4)          ║');
  ...
  if (dryRun) {
    ...
    console.log('⚠️  WARNING: This will DELETE all gentrification and migration data!');
  ...
  console.log(`\nColumns removed from Neighborhood_Map: ${results.columns['Neighborhood_Map'].removed}`);
  ...
  if (dryRun) {
    console.log(`\n🔍 DRY RUN: Would remove ${totalRemoved} columns and delete ${sheetsDeleted} sheet(s)`);
    console.log('\nRun without --dry-run to apply rollback');
  } else {
    console.log(`\n✅ Rollback complete!`);
    console.log('\nNext steps:');
    console.log('1. Remove gentrificationEngine.js from Apps Script');
    console.log('2. Remove migrationTrackingEngine.js from Apps Script');
    console.log('3. Remove processGentrification_() and processMigrationTracking_() calls from Phase 05');
