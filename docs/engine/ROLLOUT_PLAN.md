...
| pipeline.59 | Lore canon-ingest, extends pipeline.56 into verify+ingest | done-pending-archive | engine-sheet (built S383, `3c5be5d5` + `aa00ab5b`) | [[../plans/2026-08-17-lore-canon-ingest-pipeline]] — Tasks 1-4 shipped; `/lore-ingest` skill is the gate. First run failed all 3 quarantine fixtures on ledger contradictions. Two spinoffs for other lanes: c103 edition masthead carries a malformed `Y2C103` (media), and the canon-source policy is 18 sources behind the live notebook (research-build). |
