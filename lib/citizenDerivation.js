...
// engine.148 P2: the per-neighborhood gender table is gone (parity with
// utilities/citizenDerivation.js). Every hood draws at BASE_FEMALE_PCT.

// The sim's calendar year — the ENGINE formula (advanceSimulationCalendar.js
// simYearFromCycle_): cycles 1–52 = 2040, 53–104 = 2041, 105–156 = 2042. Every
// Node reader computes age as currentSimYear() − BirthYear (builder ruling
// 2026-09-07: the sim is in 2042; the 2041 anchor is retired).
function simYearFromCycle(cycle) { const c = Number(cycle); return 2040 + (Math.ceil((c > 0 ? c : 1) / 52) - 1); }
function currentCycle() {
  const fs = require('fs'), path = require('path');
  try { const m = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'output', 'simulation_ledger_snapshot.meta.json'), 'utf8')); if (Number(m.cycle) > 0) return Number(m.cycle); } catch (e) { /* fall through */ }
  try {
    const files = fs.readdirSync(path.join(__dirname, '..', 'output')).filter(f => /^world_summary_c\d+\.md$/.test(f));
    if (files.length) return Math.max(...files.map(f => parseInt(f.match(/c(\d+)/)[1], 10)));
  } catch (e) { /* fall through */ }
  return 1;
}
function currentSimYear() { return simYearFromCycle(currentCycle()); }
...
function buildLedgerFreqSnapshot(headers, data, options) {
  ...
  const simYear = Number(options.simYear);
  if (!(simYear > 0)) throw new Error('buildLedgerFreqSnapshot: options.simYear required');
  ...
  for (let r = startRow; r < data.length; r++) {
    ...
    if (edu && birthYear) {
      const bracket = ageBracket(simYear - birthYear);
...
function deriveGender(seed, neighborhood) {
  const p = BASE_FEMALE_PCT; // engine.148 P2 — neighborhood no longer tilts the draw
...
function lookupNeighborhood(provided, seed, ledgerFreq) {
  ...
  // Final fallback — core-12 uniform, from the canon cache (engine.99 Cohort 2;
  // reconciled against Neighborhood_Map CoreSimRank by scripts/auditHoodDrift.js).
  const { CANON_12 } = require('./canonNeighborhoods');
  return CANON_12[hashSeed(seed + '|nbhd-fallback') % CANON_12.length];
}
