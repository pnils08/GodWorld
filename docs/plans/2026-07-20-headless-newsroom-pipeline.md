...
**Jax's "chaos" is a feature, and doubles as sim QA.** His job is connecting the **gaps** in the sim — e.g. the C102 Baylight "Dirt Carnival": the engine let a public carnival happen on a construction site whose remediation bond never cleared the soil test. That is a real consistency hole the engine produced; Jax exploiting it as a toxic-soil allegation is him surfacing a genuine sim gap, not fabricating. A firebrand's clearly-framed **allegation from real signal is canon-valid** (subjective-hallucination-is-canon); the gate keeps him honest at the canon boundary (engine verbiage, contradicted facts, invented *attributed* facts) without neutering the chaos — see the Phase 2.1 gate-calibration task.
...
### Phase 2.1 — Rhea firebrand/opinion-aware calibration  *(research-build reviewer lane; Mike-direct S332)*
The gate must judge an **opinion/firebrand** piece differently from a news report. Reference case: the C102 "Baylight Dirt Carnival" Rhea run (`output/cron-compare/baylight_dirt_carnival.rhea.json`, `pass:false`, 4 flags, Sonnet $0.96/147s). Rhea was **right** to flag: the "A's win 10 straight" stale-C100 stat (contradicts live 107-31/W3), the "active internal state" engine-verbiage leak, "5 recoveries" vs canon's 4, and the *asserted* Keisha guilty-knowledge ("she knows the soil is toxic" — a named official's internal state stated as fact; the fix is to **press her for a real quote**, she was an awake interview subject). Rhea was **mis-calibrated** to flag the toxic-soil *premise* as "fabricated fact" — it is Jax's clearly-framed **allegation from real signal** (unexecuted bond + carnival-on-site + illness rate), which is canon-valid opinion. Tune: an opinion-lane gate flags engine verbiage + canon contradictions + invented **attributed** facts/quotes, but lets clearly-marked subjective inference stand — else every firebrand piece fails the canon gate and the canon-approved chaos voice can never publish. **Open verify:** Rhea *passed* "Jack London retail down / crime spiking," but the raw audit read as Jack London **up (+0.16)** with the real ripple-vs-audit decline at **KONO** — reconcile which read is right before trusting either. Build: `rhea-morgan` agent RULES + the `cron-rhea-gate.js` prompt gain an opinion-lane branch.
...
- 2026-07-23 — Voice doctrine + Phase 2.1 gate-calibration (S332). Street-level crons proven (Jax=Grok chaos/sim-gap agent). Rhea ran the Dirt Carnival (pass:false, 4 flags); needs opinion-lane branch. Cadence ~5 short daily → 1 compiled.
