...
- **[[plans/2026-05-03-vote-trigger-mechanism]]** — S197 Wave 4 of C93 triage. Closes G-R11 (vote-that-didn't-trigger) + structural complement to G-R6/R7/R10 canon-fabrication cluster. Two-mechanism design: /city-hall-prep vote-this-cycle detection routes vote-ready initiatives to all 9 council voices (via 3 faction-bloc agents); new /council-vote skill runs between Layer 2 and Layer 3, computes deterministic 9-member tally, advances phase. Project agents read tally as canon — never invent it. 5 phases (design / city-hall-prep extension / new skill / RULES update / C94 validation). Phase 1 open questions: absent/abstain/not_voting semantics, bloc vs member-override conflict, tie-breaking, sequencing, model-vs-script split. *(plan, civic, architecture, draft)*
- **[[plans/2026-05-03-run-cycle-gap-log-surface]]** — S197 Wave 4 meta-gap. /run-cycle is the only active skill without a gap-log surface; engine bugs go undocumented except in commits/SESSION_CONTEXT. Carve out gap-log files as explicit exception to engine-sheet's "no new MDs" rule (S156). 3 phases (design + rule update / build template + optional engineCycleAudit script / C94 validation). Phase 1 open questions: rule wording, engine-cycle gap class taxonomy, manual/script/hybrid authorship, triage destination. *(plan, engine, infrastructure, architecture, draft)*
- **[[plans/2026-05-03-rollout-triage-cadence]]** — S197 Wave 4 closes G-W16 meta-pattern (HIGHs sit on shelf and compound — S193 G-R6/R7/R10 → S195 G-W12 recurrence). Cycles-on-shelf metadata + STALE-2C threshold + auto-surfaced stale-list at /session-end. PARKED override for deliberate deferrals. 4 phases (design / backfill existing entries / build rolloutTriage.js + wire to session-end / validation). Phase 1 open questions: metadata format, threshold, in-place vs section-move, PARKED semantics, scan integration point. *(plan, architecture, infrastructure, draft)*
