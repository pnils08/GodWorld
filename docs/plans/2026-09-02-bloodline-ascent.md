...
| 8 | Tier 4 → 3 → 2 → 1 | UsageCount ladder ≥3 / ≥6 / ≥9 — fires **only inside a citation-processing pass**, on `newUsage` | `processAdvancementIntake.js:428-437` | WIRED, EVENT-ONLY — **13 ENGINE Tier-4 rows sit at UsageCount 3–15 and never climbed** (POP-00529/00531 at 9, 00530 at 14, 00532 at 15, 00590 at 12; no "Updated to Tier" marker). The bar is not a state rule |
...
The ascent exists on paper and is closed at both ends. The bottom door (hop 3) has never opened: 269 Tier-5s, nobody at the bar. The top door (hop 11) needs $1M across three tracked family members registered in a 101-row family tab, which no non-founding family has. In between, the one wired climb (hop 8) is an event, not a state, so 13 citizens sit above the Tier-3 bar without a promotion. Dials never touch Tier, heritage never touches Tier, the wake never touches anything but coverage. The inverse has money loss only.

### Build order (proposed for the builder's approval — nothing started)

1. **engine.150 — the ladder as a state.** Every cycle, a row's Tier follows its UsageCount bar (and decays as today); one pass, fixes the 13 stranded rows, makes hop 8 a rule. Smallest cut, own commit.
