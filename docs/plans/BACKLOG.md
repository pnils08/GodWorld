...
**41.4 Skill frontmatter standard (connects to Phase 39/40 skill-audit) — STANDARD LOCKED + 3 EXEMPLARS APPLIED S156.** Standard codified in [[SCHEMA]] §11 (required: `name`, `description`, `version`, `updated`, `tags`, `effort`; optional: `allowed-tools`, `argument-hint`, `disable-model-invocation`, `related_skills`, `sources`; deliberately skipped: hermes `license` + nested `metadata.hermes.*`). Exemplars applied: `boot` (small infrastructure), `write-edition` (big media), `city-hall` (big civic). **Remaining sweep: 38 skills backfill** — mechanical once standard is locked, no decisions required. Pairs with S115 skill-audit remainder. Next session picks up the sweep.
