...
- **D2 / D4 — BENCH-PROVEN S399 (2026-08-30 04:13; SANDBOX 0827 @15 = @14 + the six D2/D4 files at `d93a5532`, bench synced from live C104 first). C105: HTTP 200 / 140s / 0 Engine_Errors; 121/131 target rows at the predicted number, the other 10 all `Career-FieldChange` hires the same cycle (a real event priced them, then the D3 floor) — 0 unexplained Income moves. C106: 0 errors; re-running `applyUntrackedHoodReference_` on the post-C106 bench ledger → 220 checked / 0 raised. WP employmentRate 0.9315 → 0.9349 → 0.9379 toward the 0.96 dial. No bench-side sheet writes this wave — nothing to replay; the raise happens in-engine on live's first fire after deploy. LIVE DEPLOY GATED on the one-change-in-flight rule: prod @9 (E1/D1/D3/D5) has not fired its first live cycle yet — deploy these six files as prod @10 only after C105 lands clean on @9. Follow-up seen, not touched: `POP-00114` Income is a non-numeric cell (live and bench).** The concept, builder-direct 03:54: a citizen's money is what their life in their neighborhood pays — their own hood's canon-re-based businesses, in their sector, at their stage, with their own variance; never `Economic_Parameters` (real-world Oakland leaking through the ledger); nobody is ever lowered; no mass edit that ignores the life.** Mechanism: `hoodReferencePay_(ctx, hood, roleText, skillTags, stage, seed)` in `generationalWealthEngine.js` = median `Business_Ledger.Avg_Salary` of the hood's businesses in the citizen's category (SkillTags → role text via `roleSectorCategory_` → whole hood) × stage 0.75/1.0/1.3 (the D3 scale) × seeded ±8% per citizen, $100-rounded; null where the hood has no reference. `sectorCategory_` lifted to file scope in `runCareerEngine.js` (pure; `strict` mode). **D4** = `applyUntrackedHoodReference_` Step 1.6 after the D3 floor: SELF_EMPLOYED / UNTRACKED, raise-only, ENGINE Tier 3–4 active, blank employer = unemployed excluded, idempotent. **D2** = the five draw sites now price by the hood first (legacy draw only as fallback): `calculateCitizenIncomes_` fallback, `processAdvancementRows_` (role redraw + promoted row — Income now set at promotion, Debt/NetWorth from it), `godWorldEngine2` intake redraw, `bondEngine` both spouse mints (was flat $48k), `householdFormationEngine` off-camera members (was flat $48k). Proof `scripts/hoodIncome.test.js` 49/49; careerStage 74/74, envelope 18/18 untouched. **Live measure (C104 snapshot, real function, read-only):** 235 checked / **130 raised / 0 lowered / second pass 0**; raise p50 $22.9k; new pay p10/p50/p90 $45k/$69k/$116k; Downtown attorneys $29–34k → $125–131k; Lake Merritt senior scientists → ~$268k. List: `output/hood-biz-ref-c104.json`. Rejected on the way (builder): a hood-factor floor off `Economic_Parameters` (everyone at the identical minimum), and any lowering.
...
- **E2 — CODE DONE S399 (2026-08-30 05:25), bench proof next.** Derivation (the "obvious decision", so it is on record): the sim's one standing per-business success signal is `Business_Ledger.Growth_Rate`, and it *already* sizes each business's hiring windows — E2 extends the same causation to raises and layoffs. `applyEmployerSuccess_(ctx, cycle, roll, logRows, S, gapFactor)` (file scope, `runCareerEngine.js`, runs before the hiring matcher): per tracked business per cycle, p(promotion) = g/100/52 × staff × gapFactor (g > 0), p(layoff) = |g|/100/52 × staff ÷ gapFactor (g < 0); one draw, at most one event each per business; expected ≈ 1–2 Income-moving events per cycle across the ledger. Beneficiary = longest since `LastPromotionCycle` → lowest Income → POPID (Income +6–12%, `LastPromotionCycle` = cycle, `stampPromotion_` narrative + `Promotion` log — its one caller since E1); victim = lowest [CareerState] level → lowest Income → POPID (Income −12–20%, employer cleared, `Career-Layoff` log, businessDelta). gapFactor = clamp(1 + 8 × (attractor − employmentRate), 0.5, 1.5) from `S.demographicDrift` (attractor now exported there) — also multiplies hiring `perCycle`. **Deleted:** `maybeTransition_` (the ≤8%/≤7%/≤5%/≤5% per-citizen rolls), `careerMoveText_`, `resolveNewBizId_`, the S302 live BIZ pools + frozen fallback (one Business_Ledger read fewer per cycle), the season/chaos/weather pressure helpers, the sector-shift/lateral handlers. Reconciliation firings rate-limited to 2 per business per cycle. `detectCareerMobility_` stagnation hook: tracked employees only, 0.5%/cycle (was 5% of everyone; `LastPromotionCycle` now only moves on a real promotion). Closes the D4 forcing-loop note: a SELF_EMPLOYED row can no longer be "laid off" below its floor. `CareerMobility`'s only reader (`mobilityF`) died with the rolls — column orphaned again (gap-log observation). Proof `scripts/employerSuccess.test.js` 29/29. `docs/SIMULATION_LEDGER.md` LastPromotionCycle writer trued.
- **E2 — original spec:** promotions / raises / layoffs / hiring caused by employer success (`runCareerEngine_` `maybeTransition_` re-based to ≈1–2 Income-moving transitions per cycle, pressure-steered by dial-vs-attractor; `stampPromotion_` becomes its narrative; `detectCareerMobility_`'s CAREER_STAGNATION hook (5%/cycle of rows ≥40 cycles since promotion) needs re-basing too, since LastPromotionCycle now only moves on a real promotion).
...
- **F — CODE DONE S399 (2026-08-30 05:30), bench proof next.** Wiring cards (engine-wiring agent): the real mover is `processRelocations_` (`migrationTrackingEngine.js:577`, households sort by rent/income fit, never read the band); `runNeighborhoodEngine_` only places blank-hood citizens; `householdFormationEngine` forms households in place (no move) — so F folds into the two movers only. `hoodAdmits_(hoodState, wealthLevel)` in `loadNeighborhoodState.js` (owner of the B1 fields): WealthLevel inside [WealthMin, WealthMax]; no band → admits; WL 0/blank = unpriced → not gated; residents never evicted. `processRelocations_` skips any destination that does not admit the unit's best-off member; `pickDemographicNeighborhood_` filters the placement pool by the band (full pool stands if nothing admits, so nobody is left unplaced). Proof `scripts/hoodAdmission.test.js` 15/15.
- Original: `migrationTrackingEngine` / `householdFormationEngine` / intake placement read the B1 income tier so moves land where a citizen's net worth belongs. Fold, not add.
...
## Changelog
- 2026-08-30 (S399, 05:30) — E2 + F coded (employer success = Growth_Rate causation; random transitions deleted; hood admission bands on both movers); 29/29 + 15/15; bench proof next. D2/D4 pushed to prod script (deployment bump pending Mike). Detail §Phase E/F.
- 2026-08-30 (S399, 04:13) — D2/D4 BENCH-PROVEN (0827 @15, C105–C106: 0 errors, 130 raised then 0, hires explain the rest). Live deploy gated until prod @9 fires clean. Detail §Phase D.
- 2026-08-30 (S399, 04:20) — D2/D4 coded on the builder's concept (pay = own hood's businesses, own sector, own stage, own variance; raise-only; no `Economic_Parameters`); 49/49; live measure 130 raised / 0 lowered / pass-2 zero. Bench proof next. Detail §Phase D.
- 2026-08-30 (S399, 03:50) — Builder-direct: the hood-factor floor draft rejected (identical minimums, no life in the number; and nobody is lowered). Reworked to the hood-business reference above.
- 2026-08-30 (S398, 03:45) — Builder-direct: GAME/CIVIC/MEDIA rows are outside E1/D3/D5 (`isEngineClockRow_` gate); 18–21-year-olds keep their stored stage (47 working under-22s on live would have been demoted to student). Tests 74/74; bench @14.
- 2026-08-30 (S398, 03:30) — **Rule change, builder-direct: retirement is an event, never an age.** The ≥65 → retired derivation (inherited from the old engine) is removed; 45+ is senior; existing 'retired' stamps are left alone (no reverse mass edit); D5 zeroes Income only for deceased or `Status=Retired` (Tier 1–2 exempt). Live pre-fire prediction: 40 zeroed (16 deceased + Status=Retired Tier 3–4) / 214 floor raises / 367 re-stamps; the five retired A's (ENGINE clock, no sports marker) untouched. Ages checked: Mags 55, Santana 55, Ashford 57, Vega 63; 98 citizens 65+ (63 Active) would have been mass-retired. Also noted: the engine's simYear (2040 + ⌊cycle/52⌋ = 2042 at C105) runs one year ahead of the 2041 anchor convention — consistent inside the engine, flagged for the age docs.
