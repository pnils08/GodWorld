...
### engine.180 — CUT LOCALLY 2026-09-08 (S438, builder go)
Measured first: the wake pack already rendered posture + goal through the engine.147 seam (`lib/wakePerception.js` `renderStanding`, wired into wake/exchange/voice), so Tasks 1–2 were narrower than filed. What landed:
- **`lib/citizenDials.js`**: `stance(json)`, `thrown(lifeHistory, lookback=2)` (pressure tags, setbacks, the named good and bad events as words off `TAG_WORDS`; ambient tints omitted; four at most; never a digit), `postureChangedCycle(lh)`, `absCycleOf(line)`.
- **The standing line** gains `What the last stretch threw at you: …` and, on a hold with a goal, `What you're playing for, when the moment comes: …` (hold still claims no posture; the life line stays silent per engine.157 — the wake-perception test trued to that). The pool carries `postureChangedCycle`.
- **The posture slot** (`scripts/citizen-wake.js` `selectCitizen`): a citizen whose `[Maneuver-*]` line is stamped on the current cycle and who has not woken since is picked before the weighted draw, least-recently-woken first, `slot=posture` in the wake log. Deviation: draws from the shaped pool (the deviation gate stays in `buildPool`), not "regardless of deviation" as filed — a posture change needs ambition ≥ 60, which is a shaped citizen anyway.
- **The push**: `pushFromResolves_` (drain) reads a `Resolves` text — an action that reaches (+1: look for / apply / open / move / save / start / ask for / take / build / go for / sign up) or pulls in (−1: stop / wait / hold off / sit tight / rest / pull back / let it go / step back / slow down / stay put) — and queues `DialState.maneuver.push = {d, u: cycle+1}`, one per citizen per drain. `maneuverApplyPush_` (Phase5-Maneuver) applies it AFTER the cause recompute: one notch on the retreat/hold/climb ladder, never over a cause-held retreat (debt / standing), expired pushes ignored, consumed on every row whether or not it moved (`counts.pushed` / `pushBlocked`). A pushed climb writes `[Maneuver-Climb] playing to climb — said so out loud, and meant it`.
- **Desk render** — REVERTED same night (builder ruling 2026-09-08): the quote already carries the citizen's temperament because the voice that produced it was prompted with the dials; labeling the quote with the dials shows the writer the machinery, not the person. Task 5's render half is withdrawn; `.disp` stays an unread field on the quote object. **`buildWorldState.js`**: the orphaned `dispositions` fold removed (stands).
- Proof: `scripts/cronStance.test.js` 32/32 (stance/thrown/postureChangedCycle, the standing sentences with no digits, the resolve classifier, the drain queuing the push, the phase applying / blocking / consuming it and returning to causes the next cycle); `lib/citizenDials.test.js` 17, `lib/wakePerception.test.js` 27, `maneuverEngine` 47, the dial suites.
- Acceptance: the next three scheduled wakes after deploy (`logs/citizen-wake.log` shows `slot=posture` when a citizen's posture moved that cycle; prompts carry the thrown line with no digits); the C+1 maneuver log shows `pushed` / `pushBlocked`.
