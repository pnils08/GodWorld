...
| # | Item | State | Terminal | Pointer |
|---|------|-------|----------|---------|
| engine.164 | Sim-year literal 2041 swept to `simYearOf_` at 17 engine sites (`5337fbff`); bench C107 clean; PROD @65 2026-09-07; live smoke = the builder's C107. Scripts-side 2041 anchor NOT moved (builder-gated — SESSION_CONTEXT NEXT) | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @65; C106 gap log G-EC59 |
| engine.165 | Sawtooth = the New Year (+0.4 city term at C105 reaching every hood); Neighborhood_Map Sentiment now clamped to [-1, 1] on save (`a1b4e9d5`); bench C108 clean; PROD @66. Sift: C105 up / C106 down = holiday ending, not decay | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @66; C106 gap log G-EC58 |
| engine.167 | `ROLE_SECTOR_HINTS_` order is load-bearing since engine.166: `director` (arts) beats `community` (faith) so a Community Director prices as an artist (POP-00787, 85,600); `Maintenance tech` reads tech. Re-order / tighten, bench one cycle | ready | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @66 (cohort table) |
| engine.166 | Writer = `applyUntrackedHoodReference_` via a stale `2041-Specific` tag → tech pay. `hoodReferencePay_` now role-field first (`b1f364d7`); bench C108 104,500 / 137,800; PROD @66; live cells restored to C104. 5 rows rise, none drop | done-pending-archive | engine-sheet | [[../reference/DEPLOY_HISTORY]] §PROD @66; C106 gap log G-EC60 |
