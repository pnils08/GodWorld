...
1. **The current phase's defining deliverable has materially landed.** Your phase is `post-deadline` (per `Initiative_Tracker`); it advances when its real milestone is done or substantially done — the program formally launches (or enters formal review) and the Chief Montez MOU is signed — not on a process tick. That completed deliverable is the evidence.
2. **Observed effects are positive or cascade-directed.** Your packet's neighborhood / population / prior-cascade context shows the implementation producing its intended result (crews hired, dispatch live), OR the Mayor/faction cascade directed the move. If the milestone stalled or effects are absent, **hold the phase** (below) — never advance on schedule alone.
