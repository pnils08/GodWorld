...
/**
 * Fetch exact ranges with formulas left visible.
 *
 * Formula cells are returned as their leading "=" expressions, while
 * ordinary cells retain their underlying scalar values. Sports mutations use
 * this immediately before batchUpdate so they never replace a formula with a
 * static value.
 *
 * @param {Array<string>} ranges - Exact A1 ranges, normally one cell each
 * @returns {Promise<Array<{range: string, value: string|number|boolean}>>}
 */
async function getFormulaVisibleRanges(ranges) {
  const requested = Array.isArray(ranges)
  ...
  return requested.map((range, index) => {
    ...
    return {
      range,
      value: Array.isArray(values) && Array.isArray(values[0]) &&
        values[0][0] != null
        ? values[0][0]
        : '',
    };
...
/**
 * BIZ-ID allocator — persisted high-water mark (engine.96, business-lifecycle
 * plan Task 7 step 1). Active-sheet max-id scans break the moment rows move to
 * Business_Archive; the World_Config bizIdHighWater mark is monotonic over
 * every ID that was ever active. Contract for all Node-side minters:
 *   next = Math.max(highWater || 0, activeSheetMax) + 1
 *   ...mint the batch...
 *   await setBizIdHighWater(lastNumberUsed)
 * Never scan the archive. A lost write-back heals on the next run via the
 * active-scan max (until the first archive move ships).
 * @returns {{value: number|null, row: number|null}} null value = key absent
 */
async function getBizIdHighWater() {
  const data = await getSheetData('World_Config');
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0] || '').trim() === 'bizIdHighWater') {
      const v = Number(data[i][1]);
      return { value: isNaN(v) ? null : v, row: i + 1 };
    }
  }
...
/**
 * Persist the BIZ-ID high-water mark after a mint batch. Monotonic — never
 * lowers an existing mark. Absent key self-seeds the row, logged loud
 * (one-time migration per ADR-0015 fail-loud intent, not a silent default).
 * @param {number} lastUsed - highest BIZ number the batch minted
 */
async function setBizIdHighWater(lastUsed) {
  const n = Number(lastUsed);
  if (isNaN(n) || n < 1) throw new Error('setBizIdHighWater: invalid lastUsed ' + lastUsed);
  const cur = await getBizIdHighWater();
  if (cur.row) {
    if (cur.value !== null && cur.value >= n) return; // monotonic guard
    await updateRange('World_Config!B' + cur.row, [[n]]);
  } else {
    console.warn('[bizId] World_Config bizIdHighWater row missing — self-seeding at ' + n + ' (one-time migration)');
    await appendRows('World_Config', [['bizIdHighWater', n]]);
  }
}

/**
 * POPID allocator — persisted high-water mark (engine.90 Citizen Archive).
 * Same contract as bizIdHighWater: an active-sheet max scan alone re-issues a
 * POPID the moment its row moves to Citizen_Archive; the World_Config
 * popIdHighWater mark is monotonic over every POPID that was ever active.
 *   next = nextPopIdNumber(highWater, activeSheetMax)   // max(hw||0, active)+1
 *   ...mint the batch...
 *   await setPopIdHighWater(lastNumberUsed)
 * Never scan Citizen_Archive. The pure core is shared with the clasp half
 * (utilities/popIdAllocator.js) so the two artifacts cannot drift.
 * @returns {{value: number|null, row: number|null}} null value = key absent
 */
const popIdAlloc = require('../utilities/popIdAllocator');
async function getPopIdHighWater() {
  const data = await getSheetData('World_Config');
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0] || '').trim() === popIdAlloc.POP_ID_HIGH_WATER_KEY) {
      const v = Number(data[i][1]);
      return { value: isNaN(v) ? null : v, row: i + 1 };
    }
  }
...
/** Pure: next POPID number from the mark and the active-sheet max. */
function nextPopIdNumber(highWater, activeMax) {
  return popIdAlloc.popIdNext_(highWater, activeMax);
}

/**
 * Persist the POPID high-water mark after a mint batch. Monotonic — never
 * lowers an existing mark. Unlike setBizIdHighWater this does NOT self-seed:
 * the row is created at rollout (bench, then live with the builder's go);
 * an absent row is a loud warning and no write (engine.90 plan §Commit 1).
 * @param {number} lastUsed - highest POP number the batch minted
 * @returns {boolean} true when the sheet was written
 */
async function setPopIdHighWater(lastUsed) {
  const n = Number(lastUsed);
  if (isNaN(n) || n < 1) throw new Error('setPopIdHighWater: invalid lastUsed ' + lastUsed);
  const cur = await getPopIdHighWater();
  if (!cur.row) {
    console.warn('[popId] World_Config popIdHighWater row missing — mark NOT persisted (allocation stayed safe via the active scan). Seed the row.');
    return false;
  }
  if (cur.value !== null && cur.value >= n) return false; // monotonic guard
  await updateRange('World_Config!B' + cur.row, [[n]]);
  return true;
}
...
/**
 * citizenFullName(row) — 'First Last' for a Simulation_Ledger row object.
 *
 * G-PF22 (S409): the live sheet has NO `Name` column — the header is
 * `POPID, First, MaidenName, Last, …`. Only the local snapshot
 * (`output/simulation_ledger_snapshot.jsonl`, via `scripts/dumpLedger.js`)
 * carries a synthesized `Name`. A sheet query written as
 * `headers.indexOf('Name')` gets -1 and reads `undefined` for every row,
 * reporting "not in ledger" for citizens who are present (S405 hit: Carmen
 * Mesa, Pablo Almanzar). These helpers own the First+Last join so no caller
 * hand-rolls it again. Standing rule: resolve by NAME, never by a supplied
 * POPID — this is the path that obeys it.
 */
function citizenFullName(row) {
  if (!row) return '';
  return [row.First, row.Last].map(v => String(v || '').trim()).filter(Boolean).join(' ');
}
...
/**
 * matchCitizensByName(name, rows) — pure. Exact full-name match (case- and
 * whitespace-insensitive) wins; if none, substring match on the full name.
 * Returns the matching row objects (possibly several — homonyms are real;
 * the caller disambiguates, never picks index 0 blindly).
 */
function matchCitizensByName(name, rows) {
  const q = normalizeName_(name);
  if (!q || !Array.isArray(rows)) return [];
  const exact = rows.filter(r => normalizeName_(citizenFullName(r)) === q);
  if (exact.length) return exact;
  return rows.filter(r => normalizeName_(citizenFullName(r)).includes(q));
}

/**
 * findCitizensByName(name) — live read of Simulation_Ledger, then match.
 */
async function findCitizensByName(name) {
  const rows = await getSheetAsObjects('Simulation_Ledger');
  return matchCitizensByName(name, rows);
}
