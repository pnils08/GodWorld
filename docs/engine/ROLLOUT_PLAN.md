...
- **BUILD: Prep skills — extract prep work from write-edition and city-hall (media terminal) — HIGHEST PRIORITY S141.** Mike's insight 2026-04-10: the reason write-edition drifts is that prep work (world summary build, desk packet assembly, voice questions, civic sift) is sandwiched inside the same pipeline as the orchestration work. Mags can't hold a 9-step process when the first 3-4 steps are heavy compilation tasks — she drifts, burns tokens, and breaks rhythm before the editorial work even starts. **The fix:** pull prep into its own skills so by the time write-edition or city-hall opens, all the inputs already exist on disk. Mags' job inside those skills becomes pure orchestration: read the prep, launch reporters, compile, publish. **Skills to build:**
  ...
  2. **`/build-world-summary`** — Mechanical. Reads Riley_Digest, sports feed, civic production log. Produces `output/world_summary_c{XX}.md`. No editorial judgment. Just data routing. Can be scripted or semi-automated.
  3. **`/build-desk-packets`** — Reads world summary + sift output. Generates per-desk briefings. Writes to `output/desks/{desk}/current/`. Mechanical.
  4. **`/build-voice-questions`** — Generates civic voice agent questions from world state. Runs before city-hall. Mechanical.
  **Result:** write-edition becomes ~4 orchestration steps. City-hall becomes ~3. Both stay inside Mags' reliable attention span. The hard creative work gets its own container with room to breathe. Source: S141 Mike architectural insight. **BUILD FIRST NEXT SESSION.**
...
- **BUILD: Capability verification axis for editorial review (from O'Reilly Mezzalira essay)** — Companion to the dimensional orthogonality + process/outcome items above. The MIA three-lane framework (reasoning, sourcing, result validity) and the Microsoft UV process/outcome split are both *behavioral* verification — they check whether reporters did what they were told. What's missing is **capability verification**: did the edition do what an edition *should* do at this moment in the city? A reporter can be behaviorally perfect (every citizen grounded, every quote sourced, every date consistent) and still produce an edition that fails as an edition because nobody checked "is this the right lead?", "does the front page represent the highest-severity engine signal?", "is coverage balanced across neighborhoods?", "did we cover the Temescal health crisis or lead with sports again?" This is the **`assert True` failure class** — surface-level compliance without judgment. Implementation: add a capability-verification reviewer (or extend cycle-review) with rubric items that evaluate *editorial capability* independent of execution correctness. Sources: `output/drive-files/Googlepaper.pdf` (O'Reilly Radar reaction essay by Luca Mezzalira to Neal Ford + Sam Newman fireside chat on agentic AI architecture, Apr 2026) — especially §"Behavior versus capabilities" which names the distinction. MEDIUM. Added S142.
- **AUDIT: Agent briefing context bloat (from O'Reilly Mezzalira essay)** — Mezzalira (and Sam Newman) flag that as context files grow for agents, output quality *degrades*, not improves — "you're not guiding the agent, you're accumulating scar tissue from previous disasters." GodWorld's agent briefings assembled by `buildDeskFolders.js` have grown over months (lens guidance, exemplars, grades history, craft notes, editorial briefings, canon pointers). Concrete action: (1) measure current briefing file sizes per desk across last 5 editions, (2) chart correlation between briefing size and Mara's edition grade / Rhea's scores, (3) identify components that are never referenced in agent output (semantic similarity check — if a briefing section never shows up in the resulting article, it wasn't load-bearing), (4) trim. Pairs with Phase 26.2.3 (briefing evolution) which has the same instinct but frames it as self-improvement rather than audit. Reference document: `output/drive-files/Googlepaper.pdf`. MEDIUM. Added S142.
