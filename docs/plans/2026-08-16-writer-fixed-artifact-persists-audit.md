...
- 2026-09-06 (engine-sheet) — engine.114 writer half SHIPPED (`71af6b6c`): the 4 canon-ingestion writers exit 1 on errors > 0; `auditWriterExitCodes.js --gate` clean, `ingestEdition.test.js` 19/19 incl. new exit-1 case.
- 2026-09-06 (research-build) — engine.114 chain half SHIPPED (`df4c4fe5`): `main()` isolates `stepPublish`, logs+Discord-pings on failure, still runs coverage/sweep/sheets/signals, exits 1 at the end. CLOSED — acceptance rides the next unattended Saturday run, 2026-09-12.
