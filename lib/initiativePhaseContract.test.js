...
(function () {
  ...
  ok(JSON.stringify(unplayable) === JSON.stringify(['economic', 'safety', 'sports', 'workforce']), 'catalog: safety has no lever; economic, workforce and sports have one too weak to open their gate (matched-control pair, builder ruling b); housing playable again under engine.255 (budgeted disbursement, Task 7 pair 2026-09-22)');
  ...
  if (typeof slice.loadInterventionMenu === 'function') {
    ...
    ok(menu.available === true && menu.playable.length === keys.length - 4 && !/safety-program|economic-program|workforce-program|sports-district/.test(menu.text), 'catalog: the seat pack menu lists the playable keys only');
  }
})();
...
(function () {
  ...
  Object.keys(cat).filter(function (d) { return cat[d].playable; }).forEach(function (d) {
    ...
    var expectCols = m.column[0] === C.HOUSING_FLAGGED_COLUMN ? ['MonthlyRent', 'HouseholdIncome', 'HouseholdSavings', 'HousingType', 'Status'] : m.column;
...
console.log('=== housingFlaggedCohort (engine.255 Task 6) ===');
(function () {
  var HH = function (id, hood, type, rent, inc, sav, status) { return { HouseholdId: id, Neighborhood: hood, HousingType: type, MonthlyRent: rent, HouseholdIncome: inc, HouseholdSavings: sav == null ? 0 : sav, Status: status || 'active' }; };
  var rows = [];
  for (var i = 0; i < 6; i++) rows.push(HH('W' + i, 'West Oakland', 'rented', 3000, 60000, 0));   // burden .6, savings 0 → flagged ×6
  for (var j = 0; j < 5; j++) rows.push(HH('L' + j, 'Laurel', 'rented', 2000, 40000, 0));          // flagged ×5 (+ C1 folds in → 6) → member
  for (var t = 0; t < 3; t++) rows.push(HH('T' + t, 'Temescal', 'rented', 2000, 40000, 0));        // flagged ×3 → thin (3 < 5)
  rows.push(HH('U1', 'West Oakland', 'rented', 3000, 60000, 500000));                                // buffered → not flagged
  rows.push(HH('U2', 'West Oakland', 'rented', 1000, 60000, 0));                                     // burden .2 → not flagged
  rows.push(HH('O1', 'West Oakland', 'owned', 4000, 60000, 0));
  rows.push(HH('D1', 'West Oakland', 'rented', 3000, 30000, 0, 'dissolved'));
  rows.push(HH('Z1', 'West Oakland', 'rented', '', 30000, 0));
  rows.push(HH('Z2', 'West Oakland', 'rented', 1200, 0, 0));
  rows.push(HH('Z3', '', 'rented', 1200, 30000, 0));
  rows.push(HH('C1', 'Laurel Heights', 'rented', 1300, 12000, 0));                                   // folds to Laurel → Laurel 6
  var fold = function (h) { return h === 'Laurel Heights' ? 'Laurel' : h; };
  var c = C.housingFlaggedCohort({ rows: rows, cycle: 110, minFlagged: 5, resolveHood: fold });
  ok(c.available === true && c.tab === 'Household_Ledger' && c.cycle === 110, 'baseline cohort: available, tab, cycle');
  ok(c.rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 1 && c.members['West Oakland'].join() === 'W0,W1,W2,W3,W4,W5' && c.counts['West Oakland'].flagged === 6, 'West Oakland: six flagged households frozen, reads 1');
  ok(c.rows['Laurel'][C.HOUSING_FLAGGED_COLUMN] === 1 && c.members['Laurel'].length === 6 && c.members['Laurel'].indexOf('C1') >= 0, 'Laurel: five plus one folded child-hood row → member');
  ok(!c.rows['Temescal'] && c.thin.indexOf('Temescal') >= 0 && c.counts['Temescal'].flagged === 3 && !c.members['Temescal'], 'a hood under minFlagged is thin: named, no members, out of the city');
  ok(c.skipped.notRented === 1 && c.skipped.notActive === 1 && c.skipped.invalidRent === 1 && c.skipped.invalidIncome === 1 && c.skipped.noHood === 1, 'every exclusion counted');
  ok(C.housingFlaggedCohort({ rows: rows.slice(11), cycle: 110, minFlagged: 5 }).reason === 'no-hood-clears-min-flagged' && C.housingFlaggedCohort({ rows: rows.slice(6), cycle: 110, minFlagged: 5, resolveHood: fold }).reason === 'city-under-two-hoods' && C.housingFlaggedCohort({ rows: rows, cycle: 'x', minFlagged: 5 }).reason === 'bad-cycle' && C.housingFlaggedCohort({ rows: rows, cycle: 110, minFlagged: 0 }).reason === 'bad-min-flagged' && C.housingFlaggedCohort({ cycle: 110, minFlagged: 5 }).reason === 'no-rows', 'refusals by name');
  ok(C.HOUSING_FLAGGED_COLUMN === C.stageCatalogByDomain().housing.stage3Metric.column[0] && /[*/]/.test(C.HOUSING_FLAGGED_COLUMN), 'the cohort column is the catalog identifier and stays a derived name the pack helper refuses');
  // baseline + members
  var b = C.stageBaselineFrom({ metric: { tab: 'Household_Ledger', column: [C.HOUSING_FLAGGED_COLUMN], direction: 'down', scope: 'hood' }, origin: 'vote', captureCycle: 111, cohort: c, hoods: ['West Oakland'] });
  ok(b.ok === true && b.descriptor.cityN === 2 && b.descriptor.keys['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 1, 'stageBaselineFrom accepts the cohort as-is (city = the two member hoods)');
  C.baselineWithMembers(b.descriptor, c);
  ok(b.descriptor.members['West Oakland'].join() === 'W0,W1,W2,W3,W4,W5' && b.descriptor.members['Laurel'].length === 6 && !b.descriptor.members['Temescal'], 'members ride the descriptor for the city hoods only');
  ok(C.baselineWithMembers({ city: ['X'] }, { tab: 'Neighborhood_Demographics', members: { X: ['a'] } }).members === undefined, 'a fixed-canon tab attaches nothing');
  // observation: two relieved, one dissolved, one moved out, two still flagged → (2 + 2) / 6
  var later = rows.map(function (r) { return Object.assign({}, r); });
  later[0].HouseholdSavings = 40000; later[1].HouseholdSavings = 40000;     // W0, W1 relieved (buffer)
  later[2].Status = 'dissolved';                                            // W2 lost
  later[3].Neighborhood = 'Rockridge';                                      // W3 moved out → lost
  var obs = { available: true, reason: null, tab: 'Household_Ledger', cycle: 112, rows: { 'West Oakland': {}, 'Laurel': {}, 'Newcomer': {} }, households: later, resolveHood: fold };
  obs.rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] = 1; obs.rows['Laurel'][C.HOUSING_FLAGGED_COLUMN] = 1; obs.rows['Newcomer'][C.HOUSING_FLAGGED_COLUMN] = 1;
  var judged = C.cohortForBaseline(obs, b.descriptor);
  ok(judged.rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 0.6667 && judged.counts['West Oakland'].relieved === 2 && judged.counts['West Oakland'].lost === 2 && judged.counts['West Oakland'].flagged === 2 && judged.rows['Newcomer'] === undefined, 'judged over the frozen members: 2 relieved, 2 lost (dissolved + moved), 2 flagged → 0.6667; newcomer hood dropped');
  var edge = C.deliveryEdge(b.descriptor, judged);
  ok(edge.available === true && Math.abs(edge.minEdge - 0.2) < 1e-4, 'edge: city median of {0.6667, 1} = 0.8333, West Oakland ratio 0.8 → edge 0.2 (baseline ratio 1)');
  // control arm: dissolution is not relief
  var control = rows.map(function (r) { return Object.assign({}, r); }); control[0].Status = 'dissolved'; control[1].Status = 'dissolved';
  var jc = C.cohortForBaseline({ available: true, tab: 'Household_Ledger', cycle: 112, rows: { 'West Oakland': { 'flagged/cohort': 1 }, 'Laurel': { 'flagged/cohort': 1 } }, households: control, resolveHood: fold }, b.descriptor);
  ok(jc.rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 1 && C.deliveryEdge(b.descriptor, jc).minEdge === 0, 'control: two households dissolved → share still 1, edge 0 — a loss never reads as delivery');
  // all members gone
  var gone = C.cohortForBaseline({ available: true, tab: 'Household_Ledger', cycle: 112, rows: { 'West Oakland': { 'flagged/cohort': 1 }, 'Laurel': { 'flagged/cohort': 1 } }, households: [], resolveHood: fold }, b.descriptor);
  ok(gone.rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 1 && gone.counts['West Oakland'].lost === 6, 'every member gone from the tab → all lost, share 1');
  // no members on the baseline (pre-Task-6 descriptor) → city narrowing only, rows untouched
  var legacy = { v: 1, city: ['Laurel', 'West Oakland'], keys: {} };
  var narrowedOnly = C.cohortForBaseline(obs, legacy);
  ok(narrowedOnly.rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 1 && narrowedOnly.rows['Newcomer'] === undefined, 'a baseline without members is only narrowed to its city');
  ok(C.cohortForBaseline(c, b.descriptor).rows['West Oakland'][C.HOUSING_FLAGGED_COLUMN] === 1 && C.cohortForBaseline(null, b.descriptor) === null, 'a cohort without households passes through; no cohort → passthrough');
  var hoodTab = { available: true, tab: 'Neighborhood_Demographics', cycle: 112, rows: { 'West Oakland': { Sick: 1 }, 'Extra': { Sick: 2 } } };
  ok(C.cohortForBaseline(hoodTab, b.descriptor) === hoodTab, 'a fixed-canon hood tab is never narrowed');
  // engine mirror parity
  ...
  var E2 = new Function('Logger', src2 + '\nreturn { civicHousingFlaggedCohort_: civicHousingFlaggedCohort_, CIVIC_HOUSING_FLAGGED_COLUMN_: CIVIC_HOUSING_FLAGGED_COLUMN_, civicCohortForBaseline_: civicCohortForBaseline_, civicDeliveryEdge_: civicDeliveryEdge_, civicBaselineWithMembers_: civicBaselineWithMembers_ };')({ log: function () {} });
  ok(E2.CIVIC_HOUSING_FLAGGED_COLUMN_ === C.HOUSING_FLAGGED_COLUMN, 'engine mirrors the column identifier');
  var fixtures = [
    { rows: rows, cycle: 110, minFlagged: 5, resolveHood: fold },
    { rows: rows, cycle: 110, minFlagged: 1 },
    { rows: later, cycle: 112, minFlagged: 1, resolveHood: fold, members: b.descriptor.members },
    { rows: control, cycle: 112, minFlagged: 1, members: b.descriptor.members },
    { rows: [], cycle: 112, minFlagged: 1, members: b.descriptor.members },
    { rows: rows.slice(6), cycle: 110, minFlagged: 5, resolveHood: fold }, { rows: rows.slice(11), cycle: 110, minFlagged: 5 }, { rows: [], cycle: 110, minFlagged: 5 },
    { rows: rows, cycle: 'x', minFlagged: 5 }, { rows: rows, cycle: 110, minFlagged: 0 }, { cycle: 110, minFlagged: 5 },
  ];
  var mismatch = 0;
  for (var fi = 0; fi < fixtures.length; fi++) { if (JSON.stringify(C.housingFlaggedCohort(fixtures[fi])) !== JSON.stringify(E2.civicHousingFlaggedCohort_(fixtures[fi]))) mismatch++; }
  ok(mismatch === 0, 'parity: lib housingFlaggedCohort === engine civicHousingFlaggedCohort_ over ' + fixtures.length + ' fixtures');
  ok(JSON.stringify(E2.civicCohortForBaseline_(obs, b.descriptor)) === JSON.stringify(judged) && JSON.stringify(E2.civicDeliveryEdge_(b.descriptor, E2.civicCohortForBaseline_(obs, b.descriptor))) === JSON.stringify(edge), 'parity: engine civicCohortForBaseline_ + civicDeliveryEdge_ match lib on the judged cohort');
  var d2 = JSON.parse(JSON.stringify(b.descriptor)); delete d2.members;
  ok(JSON.stringify(E2.civicBaselineWithMembers_(d2, c).members) === JSON.stringify(b.descriptor.members), 'parity: engine civicBaselineWithMembers_');
  // three-way: the flag rule is the household engine's own
  var srcH = fs2.readFileSync(path2.join(__dirname, '..', 'phase05-citizens', 'householdFormationEngine.js'), 'utf8');
  var EH = new Function('Logger', srcH + '\nreturn { RENT_BURDEN_WARNING: RENT_BURDEN_WARNING, SAVINGS_BUFFER_MONTHS: SAVINGS_BUFFER_MONTHS, detectHouseholdStress_: detectHouseholdStress_ };')({ log: function () {} });
  ok(EH.RENT_BURDEN_WARNING === C.HOUSING_FLAG_WARNING && EH.SAVINGS_BUFFER_MONTHS === C.HOUSING_FLAG_BUFFER_MONTHS && /rent \* 12 \/ inc >= 0\.40\) && \(sav < rent \* 12\)/.test(src2), 'three-way: engine constants 0.40 / 12 == lib == the literals in the cohort body');
  var stressed = EH.detectHouseholdStress_(null, rows.filter(function (r) { return r.Status === 'active' && r.HousingType === 'rented'; }).map(function (r) { return { householdId: r.HouseholdId, monthlyRent: Number(r.MonthlyRent) || 0, householdIncome: Number(r.HouseholdIncome) || 0, householdSavings: Number(r.HouseholdSavings) || 0 }; })).map(function (x) { return x.household.householdId; }).sort();
  var cohortFlagged = [].concat(c.members['West Oakland'], c.members['Laurel'], ['T0', 'T1', 'T2']).sort();
  ok(JSON.stringify(stressed.filter(function (id) { return id !== 'Z3'; })) === JSON.stringify(cohortFlagged), 'three-way: detectHouseholdStress_ flags the same households the cohort does (Z3 has no hood: the detector does not read hoods, the cohort cannot place it — the only difference, and it is the freezer\'s noHood count)');
  ok(/civicDeliveryEdge_\(baseline, civicCohortForBaseline_\(civicStageCohortFor_\(ctx, entry\.stage3Metric\.tab\), baseline\)\)/.test(src2), 'judge: the Delivering step narrows/judges the cohort against the baseline before the edge');
  ok(/if \(tab === 'Household_Ledger'\) \{[\s\S]*?civicHousingCohortMinFlagged[\s\S]*?civicHousingFlaggedCohort_\(\{[\s\S]*?co\.households = hRows; co\.resolveHood = fold;/.test(src2), 'freezer: Household_Ledger branch builds the flagged cohort with the min-flagged dial and carries the rows for re-judging');
  ok(/if \(tab === 'Household_Ledger'\) \{[\s\S]*?nwBy\[String\(ctx\.ledger\.rows\[lr\]\[lP\]\)\.trim\(\)\][\s\S]*?if \(seen && sum > cellSav\) rec\.HouseholdSavings = sum;[\s\S]*?civicHousingFlaggedCohort_\(\{/.test(src2), 'freezer: member NetWorth off ctx.ledger overlays a stale HouseholdSavings cell before the cohort judges (fresh-row trap)');
  ok(/civicBaselineWithMembers_\(built\.descriptor, civicStageCohortFor_\(ctx, entry\.stage3Metric\.tab\)\);/.test(src2), 'stamp: members attached to the descriptor before it is written');
})();
