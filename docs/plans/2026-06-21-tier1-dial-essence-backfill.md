...
- **▶ RESUME HERE (next session): project directors + media reporters.** Same method (`lookup_citizen` for POPID — NOT fuzzy `verify`; read persona file / role; honor every stated dial; harness to ESSENCE MATCH). **5 project directors:** OARI Tran-Muñoz / Stab Fund Marcus Webb / Health Center Chen-Ramirez / Transit Soria-Dominguez / Baylight Keisha Ramos (`.claude/agents/civic-project-*/IDENTITY.md`). **Media reporters:** Hal Richmond, P Slayer, Elliot Graye POP-00012, Maria Keen, Jordan Velez, the desk personas, DJ Hartley, Jax Caldera (lookup each). Then engine-sheet live write for the full set. **Note:** integrity-forward figures (Dane done, plus any DA/oversight types) want engine.39 (pure-integrity tag) for ideal vhigh-integrity — bundle a re-upgrade pass after engine.39 deploys.
