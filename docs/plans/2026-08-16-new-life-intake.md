...
- 2026-09-04 (S419, engine-sheet) — **Tasks 3–6 built, LIVE.** The household door: `processIntake_` hands `queueHouseholdIntake_` the Intake rows before its own loop; rows sharing a `Family` key with a `Relation` (self-armed column) form a group; a group of one stays S320's; any member already on the ledger, two heads, no head, an honorific, or a hood with no canon rent routes the whole group to review; a complete group is queued to `Advancement_Intake1` head-first (Tier 4, ENGINE, `MatchType`/`MatchName` = the head, `HouseholdKey`, inferred `Gender`, children `student`). `processAdvancementRows_` mints them exactly as an ascent, resolves `MatchName` against the head it minted earlier in the pass, blanks the profile's marital/children dice for household rows (no phantom kid slots for the drip), wires links through `wireFamilyMatch_` (now carrying its provenance — no household line claims a lottery), and `formIntakeHouseholds_` sets one `HH-CCCC-Innn` on every member, marries head + spouse, counts the children onto both, appends the `Household_Ledger` row (hood rent rule) and the `Family_Relationships` row, writes the head's `[Household]` line and a `HOUSEHOLD_ARRIVED` hook. Task 6: `processIntakeRows_` deleted. Suite `scripts/householdIntake.test.js` 35/35; neighbours unchanged. **Bench-proven C107 (Bell, four) + C108 (Okafor, three, `Sex` column) on SANDBOX 0831 re-synced from live C105 — every predicted structure landed (POPIDs, one HouseholdId, symmetric SpouseIds, both parents on each child, one household row, one register row, 131 phases, 0 errors). Two follow-ups the bench caught and the same session fixed: the populator placed the children with an employer (gated at 18 — `c90b1bde`… `2f4f8808`), and a name outside the inference list left the spouse's gender to the dice (optional Intake `Sex` column, read when present). The live Intake grid is exactly 9 columns — the `Relation` self-arm now grows the grid first. LIVE PROD @50.** Task 7 is the builder's: author the families on the Intake tab. Grouping assumption: the door keys on a non-blank `Family` cell. The only automated Intake writer is the media room (`appendLeanIntakeRow_`, three call sites) and none passes `family`, so automated rows leave it blank and fall through to the S320 lone-name path — `Family` is builder-authored only. If an automated writer ever fills it, tighten the key to rows that also carry `Relation`.
