 ...
 * output/beats/prev/ when it exists), Hospital_Ledger and Health_Cause_Queue
 * (the named residents), Story_Hook_Deck hooks. A pro athlete on the hospital
 * record is a sports story, not a health one (same rule as the civic desk).
...
const SEAT = {
  ...
  version: 'HEALTH-SLICE-1', nameRe: /lila\s*mezran/i,
  tabs: ['Neighborhood_Demographics', 'Hospital_Ledger', 'Health_Cause_Queue', 'Story_Hook_Deck'],
  approach: 'Health approach: this slice is the illness count by neighborhood and every resident named on the hospital and cause records this cycle. Clinical calm, human cost, no diagnosis beyond what the record says. When the named rows are few, write the neighborhood, not a ward.',
...
function build(cycle, { root, beats, profiles }) {
  ...
  for (const r of (beats.Health_Cause_Queue || []).filter(ok)) {
