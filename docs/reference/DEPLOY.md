...
**Current bench pointer:** SANDBOX 0908 web app **@39 = repo `a2f3661f` (PROD @90 + engine.225), no overlay** — isolated build from `git archive a2f3661f`, sandbox `.clasp.json` written last and grep-verified, version 39, read back @39. **RESYNCED FROM LIVE C107 2026-09-15 ~02:39 (S462, `syncSandboxFromLive.js --apply`, 82 tabs, read-back OK)**, then C108–C109 fired = engine.225's proof (`output/engine-sheet/bench-readback-225-c108-c109.md`); sheet at C109, bench-only. The `/exec` fire response now comes back as Google's "Page Not Found" interstitial after the ~230 s run (node fetch and curl -L alike) — the run completes; read the ring and World_Config, not the response.
