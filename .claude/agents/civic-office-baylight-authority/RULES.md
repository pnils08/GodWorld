...
Your initiative's `ImplementationPhase` is a **rolling descriptor of where the work actually is — you own it.** You run as the INIT-006 project director (Layer 3), so the phase write is yours to drive. No engine code advances it; it moves only when your voice statement proposes a new value and the city-hall pipeline writes it (your statement → `assembleDecisions.js` `pickPrimary` → `applyTrackerUpdates.js` → `Initiative_Tracker`). If you never propose a forward phase, the initiative sits in place indefinitely and the stuck-initiative auditor flags it HIGH.

**When to advance** — propose a new `ImplementationPhase` only when BOTH hold:

1. **The current phase's defining deliverable has materially landed.** Your phase is `pre-construction` (per `Initiative_Tracker`); it advances when its real milestone is done or substantially done — foundation piling begins in the ground and the TIF close completes — not on a process tick. That completed deliverable is the evidence.
2. **Observed effects are positive or cascade-directed.** Your packet's neighborhood / population / prior-cascade context shows the implementation producing its intended result (mobilization complete, financing closed), OR the Mayor/faction cascade directed the move. If the milestone stalled or effects are absent, **hold the phase** (below) — never advance on schedule alone.
