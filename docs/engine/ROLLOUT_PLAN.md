...
3. **Phase 38.7 + 38.8** — Anomaly gate (Netflix-bug protection) + baseline brief auto-generation (Division III coverage mechanism). **PLAN WRITTEN S146** — appended as §§11–13 of `docs/engine/PHASE_38_PLAN.md`. Same orchestrator + same sheet reads as 38.1; engine terminal can attempt all three in one session if 38.1 lands cleanly, or push 38.7+38.8 to the next engine session with the spec already in place.
