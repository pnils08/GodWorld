...
- **Status:** [x] shipped S424 (research-build). `.claude/skills/sift/SKILL.md` v2.7: new Step 3 §3e reads `logs/citizen-tension-state.json`, emits one `TENSION-REGISTER` candidate per OPEN entry (`{POPID} | "{q}" | opened c{cy}`, `source: "tension-register"`); scope note carries the subjective→canon wall (door-knock only, never a publishable claim, named citizens still clear `lookup_citizen`). Bucket table + candidate-shape enum updated. Verified against the live register (`logs/citizen-tension-state.json`, 2026-09-05): non-empty, multiple OPEN entries across POP-00018/00789/00799/etc. — text-level acceptance (candidates-section-present-iff-non-empty) verified by inspection of the added rule text, not yet by a live `/sift` invocation this session. **Reach caveat (corrected 2026-09-05, Mike-direct — "there is no sift step of human involvement any longer"):** `/sift` is FROZEN, not merely manual — `docs/research/2026-07-11-desk-slice-fork.md` (pipeline.44, S313) retired it in favor of the desk-slice fork: "Old path frozen, not retired. Stays runnable; no new investment." There is no hand-run path in practice; the live daily edition runs unattended end-to-end via `cron-desk-run.js`. The SKILL.md edit above closes the plan-text gap Task 8 named but has zero live effect on its own — it sits in dead code.

**Live wire (S424, same session, Mike-direct — "of course wire it").** `scripts/cron-desk-run.js` gets the real fix: not at the angle-selection layer (10 separate `loadXSlice.js` files, and the citizen-quote layer runs the strict LEP/2 typed-fact lattice — `livedExperiencePacketV2.js` — which is architecturally the wrong place for freeform subjective text; ADR-0017 built that lattice specifically to keep unverified prose out of quotes). Instead, new `openTensionFor(pop, cycle)` + one call site inside `buildLaneState()` (the WRITE-stage state builder, not the quote-validation path): for every citizen already in `quotes` (already vetted, already interviewed this cycle — no new citizen-surfacing path), if they carry an OPEN tension ≤12 cycles old, one bounded texture line is appended under their quote: "{name} has also been privately sitting with an open question this cycle: '{q}' — texture only, never something they said publicly; do not quote it or attribute it as a statement." Read-only (never writes `TENSION_FILE`), fail-open (try/catch, missing/malformed file → no block), bounded (only citizens already selected). Verified: `node -c` clean; `openTensionFor` unit-tested standalone against the live register (real hits for POP-00001/00018/00789, correct null on missing POP/null pop/missing file/malformed JSON/stale >12-cycle entry, correct hit on a non-stale entry). Not live-fired this session (Saturday — no desk stage runs until Monday `06:15`; per no-manual-pipeline-demos doctrine, Monday's scheduled `--stage=write` run is the acceptance test, not a hand-driven fire today). Confirmed real overlap exists: `logs/citizen-tension-state.json` POPIDs (POP-00001, POP-00018, POP-00789, POP-00799, etc.) already appear in live C105 packet/slice files across sports, business, and civic desks.
