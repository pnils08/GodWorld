...
3. **Run the Rhea gate** — the `rhea-morgan` agent, same as an edition. Fail conditions: an invented family member, career, or household; a citizen rendered as someone the ledger contradicts; a cycle reference that puts an event where the ledger does not.
   - **Ledger numbers in lore are facts to verify, never a fail on their own** (builder ruling 2026-09-06, S429 — reverses the S420 "fail on sight" rule). Citizens are open-book: lore's job is to start the ledger's facts into canon and tell the story on top of them. Income, net worth, years in a career, household/lineage ids, a wealth band — check each against the ledger; a match passes, a mismatch fails. The `feedback_canon-is-color-not-data-echo` rule is a NEWS rule (an article restating an engine number is a data echo); it does not govern lore. What still fails: a number the ledger contradicts, an invented person/career/household, malformed cycle math.
   - **Engine/system words and IDs are NOT contamination (builder ruling 2026-09-20, S426 — retracts the "engine vocabulary" fail condition this line used to carry).** A POPID, household ID, lineage ID, or a real mechanic (the Heritage_Ledger founding a lineage, a wealth tier) described in ordinary civic-flavor prose — "the municipal registry certified the Corliss name under lineage LIN-00001" — is texture around a real, ledger-verified event, not fabricated machinery. Don't gate on "is this institution named in `docs/canon/INSTITUTIONS.md`" — that doc tracks recurring named institutions, not every generic office a piece can invent in passing (the same license a real newsroom takes citing "case number 2024-CV-1102"). A past AUTOSAVE grade wrongly stripped three such sentences from a Mags Corliss piece under this reasoning; all three were reverted (`4a453252`).
