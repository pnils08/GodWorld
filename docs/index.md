...
- **[[plans/2026-05-05-disk-inventory-and-dead-file-detection]]** — S202 standing job for `/root` filesystem inventory + dead-file detection. Phase 1 inventory walker (read-only Node, JSON manifest of path/size/ext/mtime/sha1/git-tracked), Phase 2 basename ref-scan + claude-batch classification (mechanical buckets handle ≥80%, batch API handles ambiguous tail at 50% cost), Phase 3 triage report + `/disk-audit` skill wrapper. Read-only end-to-end; archival is human-gated like `/md-audit`. Triggered by S202 droplet hygiene pass (88%→81% via ad-hoc audit; standing tool missing). 5 open Phase 1 questions to resolve before tooling starts. *(plan, architecture, infrastructure, draft)*
- **[[plans/2026-05-05-writer-header-alignment-detector]]** — S202 V2 detector slot for `engineCycleAudit.js`. Catches the schema-drift class S201 surfaced (Story_Seed_Deck/Story_Hook_Deck writer column-index expectations diverged from live header layout for ~10 cycles, silently dropping engine routing data into stale calendar header cols; 469 stale cells scrubbed S201 cleanup). Detector compares engine-writer field-name + col-position expectations against `schemas/SCHEMA_HEADERS.md` regen across the ~38 documented direct-write exceptions. 4 type tags (out-of-range / field-name mismatch / position drift / schema-shrunk-writer); HIGH for Types 1+3+4, MED for Type 2. Self-test fixture: synthesize pre-S201 schema state, assert detector would have caught the original drift. 5 open Phase 1 questions for grill (taxonomy class slot, writer-set scope, self-test path, field-name extraction depth, cadence). Status: DRAFT. *(plan, engine, infrastructure, audit, draft)*
