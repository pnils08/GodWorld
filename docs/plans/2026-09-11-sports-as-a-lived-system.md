...
**Status: LIVE (engine-sheet, 2026-09-18). PROD @98 + `Oakland_Sports_Feed!U1` = `WeekRecord`; bench-proven SANDBOX 0908 C115 (DEPLOY_HISTORY §PROD @98). Cuts 1–3 done; the dead-column deletions wait on Mike's go; cuts 4–6 fold into Tasks 3–4.**
...
**`WeekRecord` is APPENDED as column U** (engine-sheet call, 2026-09-18). **This conflicts with the §0 second-block ruling "No new column — repurpose a dead one"; engine-sheet missed that it was a ruling, not a mechanism note, and has told Mike.** The ruling's end state (no net growth, a dead column's slot becomes the week) is reached by deleting `VideoGameDate` / `VideoGame` — pending Mike's go, since it deletes 139 historical cells either way (a rename would have filed them under `WeekRecord`). Measured: `VideoGame` holds 81 historical cells (C30–C83, "MLB The Show 25") and `VideoGameDate` 58; a rename would file them under a `WeekRecord` header for every all-cycle reader (projection, desk packets, beats dump), and the engine only reads current-cycle rows so nothing would flag it. Appending keeps every existing physical position, so no positional consumer moves. Deleting `VideoGame` / `VideoGameDate` (and later `HomeNeighborhood`, after Task 3 moves transit onto the zone set) is a separate schema deletion on Mike's go; each step is one `FEED_LAYOUTS` entry in `scripts/sportsFeedContract.js`. The initial cut recognizes only the explicit new header; it never reinterprets historical `VideoGame` text as results. Existing Sheet headers stay authoritative until migration.
