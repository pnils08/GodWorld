...
**Current bench pointer:** SANDBOX 0908 web app **@47 = repo `ea3fda33` (PROD @92 + engine.231), clean `git archive` stage — the DIAG229 flood overlay is gone** — sheet at C109 (S463 engine.231 proof fire, 166 s, 0 errors). Was @46 = `47063de6` + DIAG229 bench-only synthetic flood at C108 (never shipped); sheet at C108 after the sixth typed live-C107 resync of 2026-09-15 (S462). Earlier today: @39 225, @40 226, @42 228b proofs; @43–@45 died at the Apps Script 6-minute cap on afternoon latency (engine.230 fixed the per-row Advancement writes; `Diag_Fire` tab on the bench holds those runs' phase progress). The `/exec` fire response comes back as Google's "Page Not Found" interstitial after long runs — read the ring and World_Config, not the response; a run that dies at the cap leaves World_Config.cycleCount one behind every other tab (engine.136 stall signature) → resync.
