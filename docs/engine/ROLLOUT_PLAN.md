...
| pipeline.62 | Rhea is the only article gate — strip pre-Rhea word tests (`skipRhea`, `phase-lede`, `"I've heard"`). Empty W2 stays. | ready | engine-sheet | [[../plans/2026-08-29-rhea-is-the-gate]] |
...
| engine.134 | Hood identity — the engine has no single source for "what the neighborhoods are": 25 files declare their own hood list/table (~30 constants), 70 name a hood literally, hand-maintained lists bit twice (KONO, East Oakland). Node has `lib/districtMap.js` off `Neighborhood_Map`; Apps Script has nothing. Build: one engine-side loader, lists become lookups. | ready | engine-sheet | [[../plans/2026-08-29-city-health-system]] §Blast radius (measure); plan TBD |
| engine.135 | The other side of engine.133: employment city → hood envelope (unemployment is still `adults × (1−rate)` ±3 copied per hood, `updateNeighborhoodDemographics.js:220`) weighted by hood economics + `Business_Ledger` depth (grok's 72-row six-per-hood fill in the inbox, adopt after Mike's name sign-off); upstream of kimi's casino-ledger stake caps. Wiring cards → test → bench → live, as engine.133. | in-progress | engine-sheet | [[../plans/2026-08-29-employment-system-cascade]] (plan, builder direction 2026-08-29: dial + hood envelope from canon profile + BL fill + wealth-scale realign + quiet the rewriters + placement); grok fill in inbox — A/B1/B2 on SANDBOX 0827 @7 (S397), dial definition RESOLVED S398 (adults 23–64 employed share, attractor 0.96 — builder delegated); B3 East Oakland ND row seeded on bench (S398); OAKLAND_SCHOOLS youth throw fixed in canon voice (S398); Phase C 72-row BL fill on bench 72/72 (S398, §Pay scale set); E1 CareerStage-by-age + D1 WL 10–12 + D5 retired/deceased Income 0 proven C133; D6 storefront re-base (32 rows) + D3 employer floor on bench @11 (S398); NEXT: D2/D4 hood income draws, E2 employer-success transitions, F placement, then the live wave (replay B1 → ND East Oakland → BL fill → BL re-base) |
