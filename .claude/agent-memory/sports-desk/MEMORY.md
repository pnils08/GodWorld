# Sports Desk — Editorial Memory

Last Updated: 2026-03-21 | Last edition written: C88

## A's Roster Notes (C87 confirmed — ALL verified against briefing/packet)
- **Mark Aitken**: 1B (NOT 3B — recurring error). 3x Gold Glove, .273 AVG, 195 HR, Playoff MVP 2037. Community liaison.
- **Darrin Davis**: LF (left field). 247 HR, 269 SB, 2x WS MVP. ACL rehab complete C85-86.
- **Ernesto Quintero**: DH. NO Gold Gloves (DH doesn't field — never claim Gold Glove for DH). NOTE: C88 briefing lists "Ernesto Quintero" — previous MEMORY had "Hector." Trust briefing. MEMORY now corrected to ERNESTO.
- **Benji Dillon**: SP/LHP (LEFT-HANDED — always note). 5x Cy Young, 3,219 K, 200 W. FINAL season — rotation slot 5 (C87).
- **Isley Kelley**: SS. 3x MVP, 447 HR, .284 AVG, 2,398 hits. FINAL season.
- **Vinnie Keane**: DH. 436 HR, 6x WS champion, 2031 MVP. FINAL season.
- **Martin Richards**: 3B. Contract year.
- **Travis Coles**: SP (Starting Pitcher, NOT 2B — recurring error risk). 2-year, $46.18M deal. Rotation slot 3.
- **Danny Horn**: CF.
- **Arturo Ramos**: SP (NOT Ramirez — recurring error). Opening Day starter, Ace C87. Slot 1.
- **Henry Rivas**: SP (NOT Marcus — recurring error). Rotation slot 2.
- **John Ellis**: SP (NOT Jarrett — recurring error). Rotation slot 4.
- **Cy Newell**: RHP.
- **Deacon Seymour**: Manager (first year managing).

## C87 Rotation (confirmed — never change without briefing update)
1. Arturo Ramos (Ace/Opening Day)
2. Henry Rivas
3. Travis Coles ($46.18M/2yr)
4. John Ellis
5. Benji Dillon (farewell/final season anchor — LHP)

## A's Records
- C86: Preseason 10-3.
- C87: 0-0 (regular season not yet started).
- C88: 1-0 — Opening Day WIN vs Angels 8-1. Ramos 7IP/3H/10SO. Richards 3H/2HR/2RBI. Tavares 1HR. Horn 1HR.

## C86 Prospects (preseason stats — for reference)
- Sidney Tumolo: 2B, .261 AVG, 2 HR, 6 H
- Bryan Franco: SS, .400 AVG, 2 HR, 5 RBI
- Buford Park: 3B, .389 AVG, 7 H
- Clark Saryan: CL, 0.00 ERA, 6.1 IP, 7 SO

## Warriors Notes
- Warriors 39-28 per C87 seed text only. NEVER print this as verified record.
- No Warriors articles without confirmed feed data.
- If Warriors mentioned: frame as "pushing toward forty wins" or similar color — never print raw stat record.

## Fan Characters / Citizens Quoted

### ACTIVE — Do Not Retire
- **DeShawn Hartley**, 59, Jack London, Photographer/Bay Tribune (POP-00015) — used C87 P Slayer piece (Jack London bar, Paulson two-city). RETURNING character — continue thread naturally.
- **Paul Skenes**, 37, Jack London, Retired Former A's (POP-00125) — used C87 P Slayer piece (bar regular, presence/Baylight question). RETURNING character.

### Retired from Recent Rotation
- **Dante Nelson**, 41, West Oakland, Security Guard — used C86 Keane Academy piece. ERRATA WARNING: overexposed E84. Do NOT reuse without checking cross-desk usage first.
- **Raymond Polk**, contractor — used E84-E85 Aitken civic crossover. Check before reuse.

## Corrections Applied (C87 fresh rewrite)
- E82: Aitken 3B → 1B. Davis position wrong (was listed as 2B). Gold Glove at DH nonsensical.
- E83: Coles stat transposition (207 vs 219.1 innings). Hal spells numbers — extra transposition risk.
- E85: Arturo Ramirez should be Arturo Ramos. ALWAYS verify: it's RAMOS.
- E87 rejected: Eduardo Ramos → Arturo Ramos. Marcus Rivas → Henry Rivas. Jarrett Ellis → John Ellis. Paulson listed as Owner/GM → he is GM only (not owner). P Slayer wrote policy analysis → corrected to fan column about the team and dynasty.
- Travis Coles: was incorrectly listed as 2B in C86 Names Index. He is SP (Starting Pitcher). ALWAYS verify.

## Coverage Notes
- P Slayer: first person, emotional, "we" voice. NEVER analyst voice. NEVER civic analysis (NO policy, NO Baylight specifics, NO stadium mechanics). Must include friction pivot (counter-argument + dismantling). Writes about the TEAM, the GAMES, the DYNASTY.
- Anthony: third person, insider, long sentences. NEVER dashboard language. NEVER first person. Spells out contract figures.
- Hal: first person reflective, literary, spells out numbers (two thousand thirty-one, not 2031). NEVER cynical. NEVER breaking news. Dillon is LEFT-HANDED — always verify.
- Hal's Metrics Key: always append for era-normalization and stat-heavy essays.

## Active Storylines (C88)
- Dynasty farewell: Keane/Dillon/Kelley final season. Regular season underway — A's 1-0 after Opening Day win.
- Busch trade: Peter Busch + Baez (SP minors) + Davila (RP minors) to Guardians for Liam Wade (2B/SS, Double-A). Busch frustrated. Minor league pitching depth now thin.
- Eric Tavares: New 2B acquisition. HR on Opening Day. Development arc in progress.
- Allen Lopez: Now listed as canonical SP in C88 briefing — was not in previous MEMORY. Verify rotation slot next edition.
- Paulson two-city tension: courtside Chase Center in Bulls red. Baylight arena question unresolved. DO NOT RESOLVE. Carry forward.
- NBA expansion: Oakland confirmed in final group of candidates. Local tech mogul rumored in ownership consortium. Story unresolved — do not resolve. Hal covered cautiously C88.
- Bulls: 58-24, #1 seed, playoffs per prompt. NEVER print raw record in A's coverage without confirmed feed data.

## Name Verification Checklist (run before every edition)
Before writing, verify these names are correct (errata-prone):
1. Arturo RAMOS (not Ramirez)
2. Henry RIVAS (not Marcus)
3. John ELLIS (not Jarrett)
4. Mark AITKEN at 1B (not 3B)
5. Travis COLES as SP (not 2B)
6. Benji DILLON is LEFT-HANDED
7. Ernesto QUINTERO is DH (no Gold Gloves) — C88 briefing confirmed ERNESTO not Hector
8. Mike PAULSON is GM of Oakland A's and Chicago Bulls — NEVER "owner." General Manager of both franchises.
