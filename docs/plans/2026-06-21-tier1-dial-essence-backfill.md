...
- [ ] Civic + media voiced extension — **Mike confirmed (S265): all civic and media personas ARE on the Simulation_Ledger** (my earlier "most lack rows" was wrong — inferred from IDENTITY files not declaring POPIDs; Rivers/Ashford/Montez/Okoro etc. all resolve). Author from civic-office / reporter persona files; use `lookup_citizen` for accurate POPIDs (the fuzzy `verify` mis-hits — e.g. "Hal Richmond"→POP-00001). ~13 civic (Mayor/Deputy/DA/Chief/factions/5 projects) + media reporters.
...
- **▶ RESUME HERE (next session): media reporters only.** Same method (MCP `lookup_citizen` for POPID — `verify` mis-hits; read voice file `docs/media/voices/<name>.md` or coverage; honor every stated dial; harness to ESSENCE MATCH). Roster: Hal Richmond, P Slayer, Elliot Graye POP-00012, Maria Keen, Jordan Velez, Carmen Delaine, Luis Navarro, the desk reporter personas, DJ Hartley, Jax Caldera (lookup each). Then **engine-sheet live write for the FULL set** (34 done + media). **Note:** integrity-forward figures want engine.39 (pure-integrity tag) for ideal vhigh-integrity — Dane authored at the 'high' ceiling; bundle a re-upgrade pass after engine.39 deploys.
