...
| engine.102 | City/hood cascade integrity — T1–T9 all done (T9 /sift half landed S487, v2.8) | done-pending-archive | kimi / research-build | [[../plans/2026-08-08-engine-102-cascade-consistency]] + [[../research/2026-08-07-city-neighborhood-cascade-team-review]] |
...
| engine.255 | Housing lever redesigned as a budgeted disbursement (builder 2026-09-22, all ten calls as recommended): COMPLETE, live PROD @124 `bb15f9c9` — Budget parsed and spent, tranche onto flagged households, flagged-at-vote cohort Delivering gate (bench pair Delivering at C113), housing playable, seat proposals carry a budget (kimi Task 9), engine.251 code removed. Smoke rides C109 (Sun 2026-09-27) | done-pending-archive | engine-sheet | [[../plans/2026-09-22-initiative-budget-disbursement]]; [[../reference/DEPLOY_HISTORY]] §PROD @120–@124 |
| engine.257 | Builder call 8 — the household stress flag is near-silent because the 12-month savings buffer counts whole net worth (gap log G-EC72: 5 real flagged renters citywide; Beverly Hayes NetWorth $939,915): rule a liquid-savings share (or another number) so the flag fires for a world-sized share and a housing row can reach Delivering on live. Sim call → builder; wealth engine is the owner | ready | engine-sheet (sim call — builder included) | gap log `output/production_log_run_cycle_c108_gaps.md` G-EC72; [[../plans/2026-09-22-initiative-budget-disbursement]] §Data reality, call 8 |
| engine.258 | Owner move — a home is a rung, not a cage (builder ruling 2026-09-23: G-EC70 anchored all 123 owned households and purchase only happens in the current hood, so a low-hood buyer could never leave). Owned ENGINE-clock households take the misfit (move-up) lane: sell at the hood's market (`sellHouseholdHome_`), buy at the destination through the three purchase gates or, if they can't yet, rent there and bank the equity (builder: "sell and rent up"). Authored (non-ENGINE) homes stay anchored; owners never take the pressure lane; a sale after heritage Step 7 decrements HomesOwned on the sheet. Built + tests (migrationRelocation 74/74, sweep 261/261); 258b canon anchor for every non-ENGINE unit + honest move phrase; bench C110–C111 clean; LIVE PROD @125 (`17daa559`); 258d (builder 2026-09-24: a house is a status rung) — a seeded home sells for the down share, not full price, LIVE PROD @127 (`02a50b75`); smoke Sun 2026-09-27 | done-pending-archive | engine-sheet | live read 2026-09-23: 27 of 123 owned units ≥1.5× hood median, ~17 ENGINE-only |
| engine.259 | The fund moves (builder 2026-09-24: "the fund should move or the story will always be it's stuck"; tick drain per Cycle for untracked usage + the few tracked who need it, ~1:448). engine.255's drain + grant writer recovered from `c58a1adc^` and taken off housing: keyed on phase `disbursement-active` (never domain — INIT-001 is `economic`), grants onto stressed households owned or rented (`detectHouseholdStress_`'s rule), dials `civicDisburseTranche` 400k / `civicGrantCapMonths` 12 / `civicGrantHeadroomMonths` 1 / `civicGrantCooldownCycles` 26; crash-retry now spends only the tranche left. Business recipients: none in West Oakland shrinking (0/14) and no cash column — later cut. Which other phases spend is the builder's call. Bench C115 @106: 28M → 27.66M (tend 0.85), 1 grant $36,262 (Habiba Ward), 0 errors; LIVE PROD @128 (`578c2237`). Live INIT-001 seeded 23.8M / LastDisburseCycle 90 (2026-09-24, read back); first live drain C109 | in-progress | engine-sheet | `scripts/fundDisbursement.test.js` |
| engine.256 | Leases are one hood-median stamp per row and never move (gap log G-EC71): draw each lease around its hood level at formation/move (unit size × spread), drift toward the hood level on renewal, prices inherit the lease; dials spread / cadence / drift share — no price table, the hood level stays the S414 income rule. Design after engine.255 Tasks 1–4 land; sim calls → builder | ready | engine-sheet (sim calls — builder included) | gap log `output/production_log_run_cycle_c108_gaps.md` G-EC71; [[../plans/2026-09-22-initiative-budget-disbursement]] §Data reality |
...
| civic.38 | Stage machine + budgets + fund drain live (PROD @128); housing program removed 2026-09-23. Next, in order: schedule civic.39 Sunday so pending moves fold (17 pending at C108); no-menu categories; per-initiative impact model (health center → business that hires); build duration on the tracker. Where we sit: plan §Where we sit 2026-09-24. Executing: [[../plans/2026-09-24-initiatives-in-the-world]] | in-progress | research-build / engine-sheet | [[../plans/2026-09-19-civic-wake-game-loop]] + [[../research/2026-09-19-kimi-initiative-stage-voting-and-wake-incentives]] + [[../research/2026-09-20-codex-civic38-game-loop-review]] (Codex review — accepted + reconciled into the plan 2026-09-20; Task 7 withdrawn, Task 2 rebuilt, engine.250 filed) |
...
| infrastructure.11 | claude-mem observer: refused calls were dropped unretried (200-body error filed unrecoverable). Bundle patched 09-21, retries 2→5, stays free (builder: never paid). Close after a day near-zero `Observer failed`; a plugin update reverts it — re-apply per plan §infrastructure.11 | in-progress | engine-sheet | [[../plans/2026-08-20-consolidate-model-calls-on-openrouter]]; `~/.claude-mem/logs/claude-mem-2026-09-20.log` |
