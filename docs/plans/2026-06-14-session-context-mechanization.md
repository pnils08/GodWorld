...
**Goal:** Make every mechanical field in `SESSION_CONTEXT.md` script-derived from authoritative on-disk sources so the boot display and header never drift from hand-edits — leaving exactly one hand-authored slot (the STATUS handoff note).
