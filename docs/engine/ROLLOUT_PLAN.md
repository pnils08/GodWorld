...
| engine.162 | ENGINE-clock gate is inverted — builder rule 2026-09-04: money + education are universal, every clock but GAME carries a profession floor, MEDIA/CIVIC/GAME cannot change jobs. `isEngineClockRow_` (educationCareerEngine.js:416) excludes MEDIA/CIVIC from 3 income functions + `deriveMinorEducationStage_`; `settleAdulthood_` (:1143) assigns jobs to them, guarding GAME only. `updateCareerProgression_` (:526) undecided. 97 MEDIA/CIVIC rows, 1 under $60k. Reverses employment-cascade plan:263 (S398). Reading unconfirmed by builder | needs-info | engine-sheet | [[../plans/2026-08-29-employment-system-cascade]] §263 |
...
| civic.30 | Health-beat mismatch (Mike spot-check, S417) — root cause was an empty pool, not the matcher: the civic desk_signal lane carries zero health data. Health record now parsed from the world summary into tiered `health-*` entries; other seats' kinds gated out. Same leak may exist for Angela/Trevor — research-build's seats to assess | done-pending-archive | engine-sheet | `scripts/buildCivicDomainSlice.js` + `.test.js` |
