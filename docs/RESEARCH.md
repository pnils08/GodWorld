...
## S248 — SkillOpt (Microsoft: text-space optimizer for agent skills) (2026-05-31)

**Source:** "SkillOpt" (Microsoft + SJTU/Tongji/Fudan, May 2026). Code `https://aka.ms/SkillOpt`. Drive `1ffE07mFAIjnPdsAKc1rC7LhLc-UGLLY8` (Mike-shared S248). Full extraction via subagent (114K-token paper, read out-of-context per token discipline).

**What it is:** trains a skill `.md` like a deep-learning optimizer trains weights — frozen target model, **the skill document is the parameters**, scored rollouts are the gradient, an **"edit budget" `Lt` is the learning rate** (bounds how far one skill version moves from the last), a separate frontier "optimizer model" turns trajectories into bounded add/delete/replace edits, and — the load-bearing piece — **an edit is accepted only if it strictly improves a held-out validation score** (ties rejected). Plus a rejected-edit buffer (don't retry failed edits) + epoch-wise slow/meta update (momentum). Deployed output is one `best_skill.md` (~300–2,000 tokens), **zero added model calls at runtime** (optimizer runs offline only). Beat every competitor (human / one-shot-LLM / Trace2Skill / TextGrad / GEPA / EvoSkill) on all 52 (model×benchmark×harness) cells; **+19.1 avg points inside Claude Code specifically** on GPT-5.5.
...
**Cost — the actual gate (Mike asked "is 1000 rollouts a lot?"):** 1000 is trivial for SkillOpt's benchmarks (one rollout = answer a trivia question / fill a spreadsheet, ~2–10K tokens). **For us one rollout = an entire edition** (sift + 9–12 desk agents writing 800–5,000-word articles + full reviewer chain) = multi-hundred-K tokens, the single most expensive operation in the project. So 1000 rollouts ≈ hundreds of millions of tokens per skill trained — the largest bill the project would ever run. **The unit cost is the whole story; the rollout COUNT is the gate, not the optimizer.** Practical version for us would never be 1000 — it's "can we get signal from 20–50 rollouts + a small validation set," accepting weaker optimization for a survivable bill. Same cost-gate class as the citizen-agent layer (GPU/cost).

**Verdict: WATCH → named build path (NOT a build row yet).** Upgraded from "gated on a scorer we lack" to "scorer in hand; gap is held-out set + optimizer harness + the rollout-cost design." This is the **Layer 2 learning mechanism** for the autonomy roadmap (research.12) — newsroom-autonomous doesn't just need to *run* unattended, it needs to *improve*; SkillOpt-over-finalArbiter is how. Adopt-trigger: when the autonomy roadmap reaches Layer 2 execution AND a rollout-cost ceiling is set. Cross-linked from [[plans/2026-05-31-autonomy-roadmap]]. Pattern note: first-pass fit-read was wrong (missed the existing scorer); corrected on grep-verify — the scorer was sitting in `scripts/finalArbiter.js` the whole time.

**⚠ MOLDING DELTAS — read before building (S248). SkillOpt does NOT drop in as published; four adaptations, two are tuning, two are unsolved design we own. A future session that builds straight to the paper's spec will hit #2 and #4 as walls.**
1. **Rollout count 1000 → ~20–50 (TUNING).** Cost-driven, not signal-driven. Their rollout = a trivia question (~2–10K tokens); ours = one full edition (hundreds-K). Run far fewer, accept noisier optimization, lean harder on the gate. A dial, not a redesign.
2. **Held-out validation set — WE BUILD IT, not in the paper (DESIGN, unsolved).** Their benchmarks ship labeled train/selection/test splits. We have none, and can't trivially make one — every cycle is unique (different engine state + events). Open question: replay PAST cycles with their known Final Arbiter scores as the holdout? Synthesize fixtures? This is the load-bearing unknown — the gate cannot run without it. Decide this first.
3. **Scorer is subjective-STABLE, not deterministic — changes the gate's meaning (TUNING, but mandatory).** Their `r∈[0,1]` is exact-match (same input → same score). Ours folds LLM-judgment lanes (Mara, cycle-review) → the same edition scored twice drifts a few points. SkillOpt's "strictly improves" gate assumes a stable score; ours has noise. Fix: require improvement bigger than the noise band, OR score 2–3× and average. Otherwise you "accept" edits that are just scorer jitter.
4. **Optimize ONE skill at a time, not the bag (DESIGN/scoping).** SkillOpt optimizes a single `best_skill.md`. Our edition quality emerges from ~12 desk agents + sift + reviewer chain interacting — optimizing all at once explodes the rollout cost. Pick one high-leverage skill (`/write-edition` or `/sift`), freeze everything else as fixed harness, optimize it alone. Matches the paper's actual unit anyway.
**Summary: #1 + #3 are dials; #2 (held-out set construction) + #4 (one-skill scoping) are genuine design we invent — the paper gives the loop shape + proof-it-works-in-Claude-Code, not these two answers.**

---

## S272 — Autonomous-Agent Deep-Dispatch: write-edition Retool, Concept Proven (2026-06-25)

**Source:** Mike-directed design session (S272), off the S272 storytelling-depth seed (`mags` Supermemory `4pvyA3PVyG8h6LEADbZjoN`). Live test run + external review (GPT, Drive `14vvTqPEMVzhGROCsuvoa-iDyrQjUyabf`) + deep piece artifact (Drive `1hL1CeoKXa_M-GaAtaCpbuIUIresOQYh_`, local `output/desk-test/sports_c100_deep_hal.md`).

**Scope:** A retooling of the *write-edition* stage only — NOT a pipeline rebuild. Pipeline structure is set and stays (engine-review → build-world-summary → city-hall → interviews → the verification floor). The open question is the process *inside* write-edition: how desks produce stories.

**The shift:** parallel-desk template-fill → **one-desk-deep + bounded source-search subagents + a charge-brief.** The charge (project + beat + cycle + canon map) gives the agent latitude over **angle AND subject selection** — present the citizen pool, let the agent pick who carries the story — replacing a packet that prescribes both. The supplemental already IS this unit (search-and-write, MCP source access); the morph re-sequences it into the edition slot, one desk at a time.

**Test (C100, sports, deliberately minimal structure):** charge + 3 source-search Explore agents (truesource/season, prior-canon, cycle-events) + one Hal Richmond writer (Sonnet, matched to baseline tier to isolate *shape* not model). Output "The Threshold Season" found the connective thesis — the franchise's best young season happening *as* its anchors leave, "whose dynasty is this now" — that the shipped C100 edition split across two reporters (Raines = young core, Richmond = farewell) and never fused. One desk holding the whole cycle found the seam two parallel desks each walked past.

**Result:** external reviewer ranked it **Tier 1 / A, above both shipped E100 sports pieces**, classified *permanent canon* ("captures what Cycle 100 felt like"). It read better partly because it acknowledged decline (Dillon's 4.08 farewell line) instead of worshipping — era-driven, not event-driven. **Concept proven with almost no scaffolding** — the strong signal, since the real pipeline carries far more structure, and C100's chaos-cars (engine.11, first chaos cycle) + deeper desk packets give the next run more depth to pull.

**First true autonomous-agent test:** the S131/C89 supplementals (existence proof, RESEARCH.md:1421) were Claude.ai-in-chat with MCP — human-driven, not an agent. S272 is the first *dispatched autonomous agent with latitude*. Track the autonomous agent from here, against next cycle's richer substrate.

**Audit reframe (load-bearing).** My real-world-leak flag on the Rickey Henderson comp was WRONG — the A's are a real entity ([[reference_sports-layer-real-nba-canon]]); real-player comparison is correct sports journalism. Over-strict audit ("use ONLY packet data, invent nothing") is itself a primary cause of pipeline flatness — it forbids the reach that produces depth. The floor must catch *modern-Oakland civic invention*, not journalistic range.

**Engine-output-is-canon / bug-is-event (Mike, standing canon principle).** The builder does not audit what the data *means* to those living it — the agents live in the world; Mike does not, and cannot author what an event means to them. If the engine fires wrong crime numbers and they surface as C100 output, that is not just a bug — it is an *event*: the spike happened, and every generated event for that neighborhood's citizens is shaped by it. Engine repair for future cycles is engine-sheet's job; un-happening the in-world event is not on the table. The edition is also how the builder *reads* what happened in the world — "I see a crime spike, I have to understand why; that's the game."

**Documented failure of the current pipeline (the proof case) — TRACED to the real stage.** The engine flagged two high-severity crime outliers at C100, both triaged `cover-as-story` (`output/engine_anomalies_c100.json`): **Coliseum** violent-crime +4.9σ (the spike, near the Baylight Celebration footprint) and **Temescal** −5.0σ (an anomalous dip). These **reached the editor intact**: `engine_anomalies` → `/engine-review` → `engine_review_c100.md` → `/sift`, and **sift caught both** (production_log_c100 Step 1) — Coliseum assigned to Chief Montez as a civic-safety beat, Temescal as a "quiet-streets" civic beat. The sentiment-uniformity anomaly (G-EC33, all 21 hoods compressed +0.77–1.06) was handled the opposite way — **deliberately suppressed**: "G-EC33 lockstep sentiment kept out of prose," "sentiment trending upward" → "holding steady," scrubbed as an engine artifact. Net at publication (claude-mem 33970): **no crime-anomaly coverage surfaced** — the Coliseum spike dissolved into the Celebration "crowd-safety framework" (softer civic-voice framing) rather than a crime-spike story, and the sentiment anomaly was deleted. **So the miss is NOT upstream burial** (sift demotes the world summary to orientation-only and caught the signal via engine-review) — **it is downstream dilution + a suppression reflex**: even a caught `cover-as-story` signal gets reframed/softened across sift → civic-voice → edition, and a known anomaly gets scrubbed as an "artifact" rather than covered as an event. This is "filtered to a bone" (Mike) surviving the catch, and the bug-is-event inversion: an autonomous desk reading raw data under a cover-what-the-people-lived charge would not soften a violent-crime spike into crowd-safety or delete a sentiment anomaly. (Two earlier drafts of this entry were wrong — first "desks walked past available data," then "the world summary buried it." Tracing the pipeline showed the signal was caught at sift and lost downstream. Verify-before-write, and trace-before-conclude.)

**The loop it strengthens — dual-axis growth:** a citizen grows **externally** (engine events that happen to them) + **internally** (an accumulating file of its own work + query ability). Free subject+angle pick → the subject's file thickens → next cycle's pick is richer. Latitude generates world-texture beyond the data — the telos ("give the citizens a life"). Same principle as [[project_subjective-hallucination-is-canon]], applied to the whole newsroom; the wall stays at subjective→canon-publication.

**Deltas to design (carried, not yet built):** (1) dispatch inversion — kill the monolithic edition, run desks one-at-a-time deep; (2) charge-brief with free subject+angle pick; (3) per-desk/per-citizen corpus — storage flip, half-scaffolded in `output/desks/{desk}/` but article-store dead since c90; (4) byline/desk ingest tags so an agent can search "what I've said" (today's bay-tribune ingest tags cycle/type/citizen, NOT byline/desk); (5) audit-floor recalibration (catch civic invention, not journalistic reach); (6) **raw-data access + anti-suppression** — route the desk to raw signal (`engine_anomalies` cover-as-story, full neighborhoodState) directly rather than a multiply-summarized packet, AND replace the suppress-the-artifact reflex with the bug-is-event mandate (C100 shows even a sift-caught anomaly gets reframed/scrubbed downstream — the failure is dilution + suppression, not upstream burial). Storage + ingest = engine-sheet lane. Recon detail: claude-mem 34124; C100 trace: claude-mem 34177 + production_log_c100 Step 1.

**Civic floor test result (S272, C100, Carmen Delaine — the floor test).** PASSED the hard part. Given all civic material evenhandedly + latitude, Carmen independently chose the story the production pipeline buried — the Coliseum +4.95σ crime spike on the Celebration's doorstep + the sentiment-uniformity ("aggregate measurement that doesn't disaggregate," zero system language) — as a real accountability piece, not the template's three program-milestone recaps. External review ranked it the **best civic article from GodWorld, #2 overall** behind Threshold Season, and independently flagged the emergent **lane differentiation** (Carmen = governance gaps; "a voice Anthony or Richmond couldn't convincingly write") as the paper maturing — corroborating latitude→distinct-voice→moat ([[user_mags-bleed-proprietary-element]]). **Floor risk, verified + precise:** one real accuracy error — OARI described as a D1/D3/D5 pilot with D2 "still conditional," when INIT-002 is citywide / 9-district dispatch-live since ~C97 — traced to a *stale source-search subagent return* the writer didn't reconcile against the contradicting "D7 vehicle live" signal in her own brief. NOT reach-invention; a sourcing/verification miss = exactly Rhea's job. (Her "Stab Fund passed in C78" read as suspect but verified CORRECT — INIT-001 vote cycle 78, 6-3 — verify-before-conclude.) **Calibration:** keep the latitude (it is the depth AND the voice); the floor recalibrates to verify *specific civic facts* (vote math, program scope, dates) without policing angle/reach; **source-search freshness is a named new risk** — a stale subagent return poisons the writer. **Two craft refinements for the charge:** (a) invite ONE lived-experience anchor (a resident/officer/business owner, a sentence of lived experience) — the deep piece skews all-analysis, "relentless"; (b) ingest/corpus (delta 4) should canonize the *story-level meaning* (blind-spot identified, governance mismatch, D6-to-watch), NOT the supporting metrics — what is canon-worthy is the finding, not the stats.

**Next:** both proofs in, both externally A-graded (sports Tier-1; civic best-civic-ever / #2 overall). Graduates to **ADR** (kill the monolithic edition, invert dispatch to one-desk-deep + bounded source-search, raw-data charge with free subject+angle pick, recalibrate the floor to verify-specifics-not-strangle-reach, source-search freshness) + a **phased build plan** handed to engine-sheet for the storage / ingest-tags / summary-merge deltas. Test artifacts: `output/desk-test/sports_c100_deep_hal.md`, `output/desk-test/civic_c100_deep_carmen.md`.
