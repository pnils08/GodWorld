...
// Proves: the pool (institutions, size, blank books, no room, a filled seat
// all excluded), the two reels (profit-weighted business, a fitting citizen
// from its hood; whiff when the hood has no one), the queue row shape, the
// mint through the populator (RoleType from the business, EmployerBizId set),
// the wiring (Key_Personnel by Phase-10 cell intent, life line, log, hook, GC
// row Emerged), a seat taken in the meantime, a name collision, the cap
// plumbing at the entry, and the four self-armed keys.
...
const E = new Function(...Object.keys(sandbox), src + '\nreturn { checkBusinessOwnerPromotions_, buildOwnerDoorPool_, ownerDoorConfig_, processAdvancementRows_, wireBusinessOwners_, OWNER_QUEUE_COLS_, OWNER_DOOR_REQUIRED_KEYS };')(...Object.values(sandbox));
...
console.log('\n2. the two reels — the business by profit, the citizen from its hood:');
{
  const w = world(); // slot 1: fires (0.1), roll 0.5 of 184M → Coastline, candidate 0 of West Oakland 35+ → Ray; slot 2: fires, roll 0.9 → Coliseum → nobody there
  const res = E.checkBusinessOwnerPromotions_(w.ctx, CYCLE, 2);
  const q = w.sheets.Advancement_Intake1;
  check('one queued, one whiff, three eligible', res.queued === 1 && res.whiffs === 1 && res.eligible === 3, JSON.stringify(res));
  ...
  check('Ray Okada, not the 32-year-old, not the Emerged row, not the blank', r[qc(w, 'First')] === 'Ray' && r[qc(w, 'Last')] === 'Okada');
  check('the business writes the role; Tier 4 ENGINE; the pool\'s trade rides in the notes', r[qc(w, 'RoleType')] === 'Owner, Coastline Construction' && r[qc(w, 'Tier')] === 4 && r[qc(w, 'ClockMode')] === 'ENGINE' && /the pool listed them as Mechanic/.test(r[qc(w, 'Notes')]) && /the business is the reason/.test(r[qc(w, 'Notes')]));
  check('birth year, hood, employer = owner-of = the business, sex carried', r[qc(w, 'BirthYear')] === 1985 && r[qc(w, 'Neighborhood')] === 'West Oakland' && r[qc(w, 'EmployerBizId')] === 'BIZ-00056' && r[qc(w, 'OwnerOfBizId')] === 'BIZ-00056' && r[qc(w, 'Gender')] === 'male');
  check('the whiff is logged by name and hood', logs.some(m => /Coliseum District Development drew, but no one in Coliseum fits/.test(m)));
  ...
  const w4 = world({ seq: [0.1, 0.999, 0.0] }); // roll at the top of the range → the last in the pool (Marigold) → Tomas
  E.checkBusinessOwnerPromotions_(w4.ctx, CYCLE, 1);
  const r4 = w4.sheets.Advancement_Intake1.appended[0];
  check('a low roll lands on the shop tail too — Marigold Cafe draws Tomas Reyes from Fruitvale', r4 && r4[qc(w4, 'First')] === 'Tomas' && r4[qc(w4, 'OwnerOfBizId')] === 'BIZ-00068' && r4[qc(w4, 'Neighborhood')] === 'Fruitvale');
}
...
console.log('\n3. the mint — through the populator, wired to the ledger:');
{
  ...
  const nu = w.ctx.ledger.rows.slice(before), ray = nu[0];
  ...
  check('POPID continues the ledger; role from the business; employed there (221 stated, 0 Active tracked → room)', ray[col('POPID')] === 'POP-00903' && ray[col('RoleType')] === 'Owner, Coastline Construction' && ray[col('EmployerBizId')] === 'BIZ-00056' && ray[col('Neighborhood')] === 'West Oakland' && ray[col('Gender')] === 'male');
  check('catalog pay at C — the books take over at C+1 through applyOwnerDraw_', ray[col('Income')] === 61000);
  check('Key_Personnel by ONE Phase-10 cell intent: Coastline\'s row, the Key_Personnel column, POPID Name (owner), economy/90', cells.length === 1 && cells[0].tab === 'Business_Ledger' && cells[0].row === 2 && cells[0].col === 9 && cells[0].value === 'POP-00903 Ray Okada (owner)' && cells[0].domain === 'economy' && cells[0].priority === 90, JSON.stringify(cells));
  check('the sheet itself was not written', w.sheets.Business_Ledger.setCells.length === 0 && w.sheets.Business_Ledger.rows[1][8] === '');
  check('a [Business] life line, the log row, the hook', /\[Business\] Took over Coastline Construction in West Oakland — the business is the reason/.test(ray[col('LifeHistory')]) && w.sheets.LifeHistory_Log.appended.some(r => r[3] === 'Business-Owner' && /Became the owner of Coastline Construction/.test(r[4])) && w.ctx.summary.storyHooks.some(h => h.hookType === 'BUSINESS_OWNER_ARRIVED' && /Ray Okada is the new owner of Coastline Construction/.test(h.text)));
  const gcRay = w.sheets.Generic_Citizens.rows.find(r => r[0] === 'Ray');
  check('the GC row is marked Emerged by the birth name (Status only — markAsEmergedInGeneric_ sets nothing else)', gcRay[9] === 'Emerged', JSON.stringify(gcRay));
}
...
console.log('\n4. a seat taken in the meantime, and a name collision:');
{
  ...
  const before = w.ctx.ledger.rows.length;
  const res = E.processAdvancementRows_(w.ctx, 'C' + CYCLE, CYCLE);
  check('minted, not wired, no intent, logged', w.ctx.ledger.rows.length === before + 1 && res.ownersWired === 0 && cells.length === 0 && logs.some(m => /BIZ-00056 already names its people — Ray Okada/.test(m)));
  const w2 = world({ gc: [['Avery','Santana',56,1986,'West Oakland','Painter',0,'','','Active','female','']] });
  E.checkBusinessOwnerPromotions_(w2.ctx, CYCLE, 1);
