...
### Rulings, fifth block (Mike to research-build, 2026-09-19 — record: [[../research/2026-09-18-sports-intensity-and-game-day-economy]] §6, `e1e0cdde`)

- **Q1 — a week without games does not move the city.** The buzz still happens, but the crons generate it (media, citizen wakes), not the engine's dials — the `sportsAtmosphereEnabled` invent-prose / move-dial seam. RULED.
- **Q2 — split volume from result.** Crowd, traffic and transit follow game volume at the venue, unsigned. Retail, nightlife and sentiment follow the result, signed. RULED.
- **Q3 — judge each team against its own expectation.** The A's against the dynasty norm; the Oaks against an expansion team's inaugural season. RULED.
- **Q4 — each playoff round casts a bigger net; by the championship the whole city buys in.** Reach grows round by round, not in one playoff step. RULED.
### Rulings, sixth block (Mike-direct to engine-sheet, 2026-09-19)

- **The casino settles on the first game of each franchise's WeekRecord** (one WeekRecord per franchise per Cycle). RULED — matches the engine.202 build.
- **The last A's entry and the last Oaks entry of the Cycle set the sim's lens of each team's season state.** RULED — this is today's `deriveSeasonByTeamFromFeed_` behaviour, and it supersedes item 3c(b) (the WeekRecord row owning the phase). The builder controls the phase by what the last row says, as at C108 (`championship` → `playoffs`).
- **An EventTrigger is a moment in the week** (`draft-night`, not `draft`). His recurring non-hook words were SeasonType (`playoffs`, `pre-season`) and EventType (`breaking-news`, `awards`, `draft`, `community outreach`) values in the wrong column. RULED — six moment words added as hooks (draft-night, trade-deadline, all-star, series-clinch, walk-off, no-hitter); the entry kinds added to EventType.

- *Consequences recorded by research-build, not separately ruled:* an announcement row with no games (C108's "World Series set") is cron buzz, and the week's phase for engine magnitude is the round its games were played in; the Oaks losing preseason games costs little against an expansion expectation; volume saturates, so a long September cannot outweigh a World Series.
...
**Status: LIVE (engine-sheet, 2026-09-18). PROD @98 + the `WeekRecord` column (appended at U 2026-09-18; S after the 2026-09-19 dead-column deletion); bench-proven SANDBOX 0908 C115 (DEPLOY_HISTORY §PROD @98). Cuts 1–3 done; the dead-column deletions wait on Mike's go; cuts 4–6 fold into Tasks 3–4.**
...
**`WeekRecord` is APPENDED as column U** (engine-sheet call, 2026-09-18). **This conflicted with the §0 second-block ruling "No new column — repurpose a dead one"; engine-sheet missed that it was a ruling, not a mechanism note, and told Mike.** RESOLVED 2026-09-19 on Mike's go: `VideoGameDate` / `VideoGame` deleted in one atomic request (228 rows kept, 19 columns, 0 cell mismatches vs the pre-delete copy, dropdowns moved with their columns); 139 historical cells archived in `output/engine-sheet/oakland_sports_feed_dead_columns_2026-09-19.json`. Net: the tab is one column smaller and the week sits where the dead columns were. Measured: `VideoGame` holds 81 historical cells (C30–C83, "MLB The Show 25") and `VideoGameDate` 58; a rename would file them under a `WeekRecord` header for every all-cycle reader (projection, desk packets, beats dump), and the engine only reads current-cycle rows so nothing would flag it. Appending keeps every existing physical position, so no positional consumer moves. Deleting `VideoGame` / `VideoGameDate` (and later `HomeNeighborhood`, after Task 3 moves transit onto the zone set) is a separate schema deletion on Mike's go; each step is one `FEED_LAYOUTS` entry in `scripts/sportsFeedContract.js`. The initial cut recognizes only the explicit new header; it never reinterprets historical `VideoGame` text as results. Existing Sheet headers stay authoritative until migration.
...
1. **Header migration must follow compatible code — Node side DONE (engine-sheet, 2026-09-18).** `FEED_LAYOUTS` in `sportsFeedContract.js` names the accepted layouts (legacy 20; 20 + `WeekRecord`); `resolveFeedLayout` replaces both exact-20 checks (writer `normalizedFeedSnapshot`, dashboard `feedRowsFromSnapshot`). `projectNewRow(draft, layout)` projects by header name. The writer recovers the preview's layout from the signed `feedHeaderHash` precondition (commit already requires the preconditions to match the live sheet, so a migration between preview and commit is `sports_source_changed`), writes `A:<last>` at the layout's width, and compares preview/read-back at that width — the old compare truncated to 20 and would have ignored column U. `sportsWorkspaceProjection` carries `WeekRecord`. Also done: `setupFeedSheet_` / `clearSportsFeedValidation` by header name (`6319babf`), handoff export (cut 5, `6319babf`), bench proof C115, live header.
2. **Draft data loss — DONE (engine-sheet, 2026-09-18).** `validateDraft` covers every layout's fields and validates `WeekRecord` with the engine's own `sportsWeekForEntry_` (grammar + `game-result`/`season-state` pairing), normalizing case and spacing. A weekly draft against the legacy layout is refused at projection (`WeekRecord column … does not have it yet`), never dropped. A second weekly summary for the same franchise and Cycle is refused at preview and at commit (`sports_week_record_duplicate`) — the engine keeps the first and rejects the rest.
3. **Vocabulary parity — DONE (engine-sheet, 2026-09-18).** One source: the lists in `utilities/setupSportsFeedValidation.js` (sheet dropdowns) are exported to Node and are what `sportsFeedContract.js` `SAFE_ENUMS` enforces. `EventTrigger` = exactly the `TRIGGER_HOOKS` keys (`storyHook.js`): `trade-deadline` / `all-star` / `draft` dropped (no hook — giving them one is a sim call, Mike's), `injury` / `injury-return` / `debut` added. P–T now get dropdowns from lists equal to the words each `parse*_` reads (FanSentiment keeps `neutral`/`moderate`, read as neutral by fall-through). `sportsFeedContract.test.js` pins both directions; allowInvalid stays on, so historical free-text triggers (`breaking-news` ×16 etc.) still reach the media handoff. The setup menu item must be re-run on live to apply the new dropdowns.
3c. **Feed-contract needs raised by the fifth block — mechanism implementing Q1/Q3/Q4 (engine-sheet's to build, no new ruling), except the Oaks' expansion prior, which is a sim number for Mike:** (a) round-level SeasonType values (e.g. wild-card / division-series / league-championship-series; first-round / conference-semifinals / conference-finals) canonicalizing to `playoffs` for today's readers but carrying a round depth for Q4; (b) ~~the `WeekRecord` row owns the week's phase~~ SUPERSEDED by the sixth-block ruling — the last entry per team sets the season lens; (c) a per-franchise expectation baseline for Q3 (A's: own running record; Oaks: expansion prior, default .300, Mike-overridable); plus EventType values Mike authors that the dropdown lacks — live counts 2026-09-19: injury 12, trade-recap 5, team-update 5, re-signing 1 (also rumor 5, draft 2); injuries and trades are the S446-ruled negative-drift channel, so listing them publishes a ruling, and nothing collides with the dashboard's `ACTION_MATRIX` (its `injury` action files under EventType `roster-move`). Each lands through the single vocabulary source in `setupSportsFeedValidation.js` + the parity test.
3d. **Column audit (engine-sheet, 2026-09-19, Mike-asked):** every dashboard option exists on the sheet (dashboard ⊆ sheet; `front-office` / `fan-civic` are sheet-only EventTypes). Fixed: `HomeNeighborhood` now = the 22 Neighborhood_Map hoods (sheet dropdown reads the map at setup; Node reads `lib/canonNeighborhoods`) — Montclair / Old Oakland (child areas transit never matches) out, Baylight District + 7 in; EventType lists the six types Mike authors; every column note rewritten from the code (`docs/OAKLAND_SPORTS_FEED.md` table). **RESOLVED 2026-09-19 (sixth block + PROD @100):** triggers are moments; six moment hooks added; moment hooks no longer lose the domain dedupe to the phase hook (bench C116 showed the drop, C117 the fix). Was open for Mike: (i) which of his trigger words get story hooks — 4 of 18 authored C100–C108 made one, and a non-hook word also blocks the inferred hook (C108: `playoffs` suppresses `championship`); (ii) the `player-frustration` / `player-energy` triggers PlayerMood raises reach no hook. **Latent mechanism (engine-sheet's):** a blank row carries the team's last EventTrigger and HomeNeighborhood forward (`processFeedSheet_` later-rows-win state) — not observed C100–C108 because triggers are authored every Cycle; FranchiseStability / CommunityInvestment / most of FanSentiment and EconomicFootprint compute terms (`retail`, `nightlife`, `communityEngagement`) that nothing reads — only `traffic` reaches the crowd map (Tasks 3–4).
3b. **Open from cuts 1–3 (engine-sheet, 2026-09-18):** (a) the dashboard React intake form (`dashboard/src/components/SportsIntakeWorkspace.jsx`) has a fixed field list with no `WeekRecord` input — the API accepts and validates it, sheet entry works, dashboard entry needs the field + a `vite build`; (b) the `HomeNeighborhood` vocabulary (now single-sourced in `setupSportsFeedValidation.js` `FEED_NEIGHBORHOODS`) lists 16 hoods — no Baylight District, the stadium zone — pre-existing, fold into Task 3.
...
**Design research (research-build, 2026-09-18) for Tasks 3, 4 and 10:** [[../research/2026-09-18-sports-intensity-and-game-day-economy]] — consumer census, candidate intensity formulas measured on C101–C108, city-wide vs zone split, per-franchise needs, and the sim questions for Mike (§6). Its §1.6 finds the phase-only boosts still ungated in `updateWorldPopulation_` (Task 4 owns them).
...
**Status: DONE (2026-09-19).** Weekly input via engine.202 `WeekRecord` (bench C115: weekly settlement + payouts); live settlement verified at C107 — 12 C106 A's wagers settled, 6/6 at posted 1.83, payouts = stake × odds, casino life lines on winners (research-build found it, engine-sheet re-read the live ledger). The paragraph below is the original scope.
