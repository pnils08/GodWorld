...
| `CLAUDE_MEM_PROVIDER` | `openrouter` | AI provider for summarization. Verified 2026-09-20 from the worker log: every observer call runs on OpenRouter at `costUSD=0`. `CLAUDE_MEM_OPENROUTER_MODEL=nvidia/nemotron-3-super-120b-a12b:free` (swapped 2026-09-20 17:19 off `nemotron-3.5-lightning:free`, which hung ~40% of calls at any payload size; fallback if it degrades: `qwen/qwen3.8-27b:free`; measurements in ROLLOUT `infrastructure.11`). `CLAUDE_MEM_OPENROUTER_API_KEY` is **set** in `~/.claude-mem/settings.json`; `CLAUDE_MEM_GEMINI_API_KEY` is empty; `CLAUDE_MEM_CLAUDE_AUTH_METHOD=cli`. Worker env `CLAUDE_MEM_LLM_TIMEOUT_MS=90000` is defined in `~/.bashrc` and `~/.claude/settings.json`; a changed worker env var needs `npx claude-mem stop` then `start` (`restart` re-spawns with the old env). To re-test models: time an observer-shaped call per candidate before swapping. v13's local mode does **not** expose the server-beta multi-provider routing ([[archive/plans/2026-05-28-claude-mem-v13-upgrade-evaluation]] §Multi-provider verdict: DEFER). |
