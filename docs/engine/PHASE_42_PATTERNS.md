...
|------|-------|----------|-------|
| mediaRoomIntake.js | 32 | P5 (intake tabs) + P3 (cell updates) + P4 (4 internal CS helpers) | B7 — **per-file analysis S257:** cycle-path `processMediaIntake_(ctx)` (Phase 11 `safePhaseCall_` L467/L1762) AND operator `processMediaIntakeV2()` (ctx=null) both route the **same** `processAllIntakeSheets_` → `processArticleIntake_`/`processStorylineIntake_`/`processCitizenUsageIntake_` writers (direct-write via `ss`). **SHARED-PATH HAZARD:** migrating writers to `queueCellIntent_` breaks the operator path (null ctx, no Phase 10 commit). Resolution = split-helper (cycle→intents, operator→direct) or ctx-branch — NOT a mechanical P4 swap. Also a **retire-candidate**: confirm intake tabs are non-empty at Phase 11 cycle time (operator clears them) before building. Deploy clasp-gated → post-C98. |
| finalizeWorldPopulation.js | 27 | P2 (range commits) + P3 (cell updates) | B5; uses `getLastColumn()` reads — confirm not write-position |
| processAdvancementIntake.js | 14 | P5 (intake tab + ledger appends) + P3 (cleanup-clear cells) | B7 — **per-file analysis S257:** cycle-path `processAdvancementIntake_(ctx)` (Phase 5 `safePhaseCall_` L291/L1611) AND operator `runAdvancementIntakeManual()` (hand-built ctx) both route the same `processMediaUsage_`/`processAdvancementRows_` ctx-helpers. SL writes already via `ctx.ledger` (§5.6 done). **SAME SHARED-PATH HAZARD as mediaRoomIntake:** manual path commits the **ledger inline** (L138-141) but has NO writeIntents array + NO Phase 10 commit → any *non-ledger* direct write migrated to `queueX Intent_` is lost on the manual path. Resolution = split/branch, not mechanical. `clearContent` cleanup stays direct. Deploy clasp-gated → post-C98. |
| ~~continuityNotesParser.js~~ | 10 | — | **B7 RESOLVED → OPERATOR-FIRED CARVE-OUT (reconciled S268; classified S237 in engine.md, this table lagged).** All 3 entry points (`parseContinuityNotes()` L29 / `setupContinuityParser()` L310 / `parseContinuityText(textBlock, arcName)` L341) take no ctx, ZERO `safePhaseCall_` cycle-path callers (re-verified caller-graph S268 — the only external ref is `parseMediaRoomMarkdown.js`'s own internal `parseContinuityNotes_(ss,…)`, a different fn). Migration would queue intents with no Phase 10 commit. Same class as parseMediaRoomMarkdown/healthCauseIntake. Retains its value as the canonical P5 batch-append *reference shape* (lines 305/329). Pattern: feedback_measure-twice-cascading-effects |
| ~~parseMediaRoomMarkdown.js~~ | 9 | — | **B7 RESOLVED S257 → OPERATOR-FIRED CARVE-OUT, no migration.** All entry points (`parseMediaRoomMarkdown`/`parseAndProcessMediaRoom`/`diagnosePasteSheet`) self-open the spreadsheet, use `SpreadsheetApp.getUi().alert`, take no ctx, ZERO cycle-path callers (no `safePhaseCall_`). Same class as continuityNotesParser/healthCauseIntake — migration would queue intents with no Phase 10 commit. Confirmed via caller-graph + claude-mem #29167. engine.md exceptions list updated. Pattern: feedback_measure-twice-cascading-effects |
...
|------|------|-------|---------|-------|------------------|
| `phase05-citizens/runCivicElectionsv1.js` | L213-219 `electionLog.appendRow([14 headers])` lazy-create | 5 | once/lifetime | **(b) B3 carve-out → DONE S229** | Carve-out shipped per Phase 2.1 decision A — inline schema-setup comment block + narrowed `.claude/rules/engine.md` line 44 entry. No `queueEnsureTabIntent_` migration; direct ops stay for the lazy-create branch only. |
| | L394 `electionLog.appendRow(row)` per-election | 5 | C45 even-yr only | **(a) mechanical → DONE S229** | P1 shipped → `queueAppendIntent_(ctx, 'Election_Log', [...], 'log election result — ' + seat.officeId + ' ' + winner, 'civic')`. File v1.1 → v1.2. Smoke-test pre-firing (next C97; we're at C94). Pending clasp push. |
| | L432 `officeLedger.getRange(1,1,…).setValues(officeData)` full-sheet rewrite | 5 | C45 even-yr only | **(a) mechanical → DONE S229** | P2 shipped → `queueRangeIntent_(ctx, 'Civic_Office_Ledger', 1, 1, officeData, 'rewrite office ledger after C' + cycle + ' elections', 'civic')`. Same commit as L394 + L213-219 carve-out per §3.5 routing recommendation. Pending clasp push. |
| `phase05-citizens/generateGenericCitizens.js` | L662 `getRange(getLastRow()+1, …).setValues(rowsToWrite)` | 5 | **DEAD in cycle path** | **(d) verify-only — phase disabled S205** | No migration. Both `safePhaseCall_(ctx, 'Phase5-GenericCitizens', …)` sites at `godWorldEngine2.js:197+1511` commented out S205 (Path B no-grow-legacy). L662 unreachable. **If phase is ever re-enabled** (future SL-Tier-5 generator), reclassify back to **(a) mechanical** before flipping the call sites; migration target is `queueBatchAppendIntent_(ctx, 'Generic_Citizens', rowsToWrite, …)`. |
| `phase05-citizens/generateChicagoCitizensv1.js` | L78 SEED `sheet.appendRow(row)` in loop | 8 | **DEAD in cycle path** | **(d) verify-only — phase disabled S229** | No migration. `safePhaseCall_(ctx, 'Phase8-ChicagoCitizens', …)` commented out at `godWorldEngine2.js` both entry points S229 (Path B no-grow-legacy, mirror of S205 generateGenericCitizens disable). Chicago canonically dead in sim; pool frozen at ~124 rows. **If Phase8-ChicagoCitizens is ever re-enabled**, reclassify back to **(a) mechanical-w/-refactor** before flipping the call sites; migration target is the original P1 → batch-collect → `queueBatchAppendIntent_` shape + in-memory tracking refactor for the post-write reload at L142-157. |
| | L102 MAINTAIN `sheet.appendRow(row)` in loop | 8 | **DEAD in cycle path** | **(d) verify-only — phase disabled S229** | No migration. Same gate as L78. |
| | L136 CHURN `sheet.getRange(row, 10).setValue('inactive')` | 8 | **DEAD in cycle path** | **(d) verify-only — phase disabled S229** | No migration. Same gate as L78. |
| | L173 `createChicagoCitizensSheet_` `appendRow([10 headers])` | 8 | once/lifetime | **(b) B3 carve-out → DONE S229** | Inline schema-setup carve-out comment block added per Phase 2.1 decision A; `.claude/rules/engine.md` line 46 narrowed to specify the carve-out + flag the remaining L78/L102/L136 mechanical-w/-refactor pending. |
| `phase05-citizens/bondEngine.js` | L1417 `sheet.appendRow(headers)` in `else` fallback | 10 | DEAD (ensureSheet_ exists) | **(b) B3 → fail-loud throw S229** | Fail-loud rewrite (ADR-0006 Contract B family). `else` fallback removed; replaced with explicit `throw new Error(...)` when `typeof ensureSheet_ !== 'function'`. Removes direct write from engine.md exception list entirely. Reasoning: `ensureSheet_` has 12 callers across the engine; if it ever failed to load that's a systemic infrastructure failure, not a per-call defensive concern — silently lazy-creating via the fallback would hide the deeper utilities-not-loaded bug. |
...
### S236 B5 audit-miss 2 + 3 (2 files / 12 sites)

S236 B5/2 (advanceSimulationCalendar) shipped clean. Pre-migration grep on remaining B5 files surfaced two more carve-out / non-mechanical shapes:

**`phase11-media-intake/healthCauseIntake.js` — operator-fired carve-out, NOT B5 mechanical.** Entry point `runProcessHealthCauseIntake()` at L527 creates stub ctx (`{ ss, summary: {}, config: {} }`) and calls `processHealthCauseIntake_(ctx)` directly with NO Phase 10 commit path. SpreadsheetApp UI alerts surface the result to the operator. Migrating the 6 writes (L92 appendRow / L96 setFontWeight / L173 batch setValues / L309 schema setValue / L348 + L409 full-range setValues) to `queueCellIntent_`/`queueRangeIntent_` would break the operator workflow — intents queue but never commit since there's no cycle pipeline. Same shape as `generateMonthlyDriftReport` (S236 §3.5 first audit-miss) but actively used via spreadsheet UI menu. **Classification: (d) verify-only — operator-fired carve-out.** No migration. **Note vs B7 plan:** B7 batch is "Tier-1 intake heavies" (parseMediaRoomMarkdown, continuityNotesParser, processAdvancementIntake, mediaRoomIntake) per Phase 4 batch table; healthCauseIntake is parallel-shape (operator-fired intake) but smaller, was placed in B5 by S184 inventory. Reclassification to carve-out is the cleaner answer than reshuffling between batches.

**`phase01-config/godWorldEngine2.js` — multi-domain, NOT mechanical sweep.** 6 plan-listed sites, live grep shows 5 active writes (L92 + L99 Engine_Errors error log via `errorLog_` helper / L937 unidentified appendRow / L1297 unidentified appendRow / L1323 unidentified setValue) + 1 comment line (L1062 — historic note, not a site). The Engine_Errors writes are **error-path carve-outs** — fire when the engine itself catches an exception. Queueing intents through Phase 10 is the wrong primitive: if Phase 10 is the thing failing, the error log write must NOT depend on that same pipeline (`feedback_silent-failure` family). Other 3 sites (L937 / L1297 / L1323) need per-site READ + caller-graph + domain classification before migration target can be decided. **Classification: deferred — split into per-site decisions in a focused commit.** No mechanical sweep; this file is engine-core scaffolding, treat like the §5.6 redesign cohort-A approach (decided per-site with explicit carve-out vs migration).

| File | Site | Phase | Cadence | Class | Pattern → target |
|------|------|-------|---------|-------|------------------|
| `phase11-media-intake/healthCauseIntake.js` | L92 / L96 / L173 / L309 / L348 / L409 | 11 | operator-fired | **(d) verify-only — operator carve-out** | No migration. Stub ctx in `runProcessHealthCauseIntake` has no Phase 10 commit; direct writes appropriate. **engine.md narrowed inline S236 with operator-fired carve-out marker.** |
| `phase01-config/godWorldEngine2.js` | L92 + L99 Engine_Errors (errorLog_) | 1 | error-path | **(b) B3-class — error-path carve-out** | No mechanical migration. Error log writes must NOT route through Phase 10 — failure-of-Phase-10 is precisely when the error log fires. Schema-setup at L91 (insertSheet + appendRow headers) is standard §1.1 carve-out. |
| | L937 / L1297 / L1323 | 1 | unclear (cycle vs operator) | **deferred — per-site classification needed** | Pre-migration READ + caller-graph + domain identification required. May be Simulation_Ledger / Intake / Riley_Digest / World_Population per engine.md cluster description. Treat as focused commit (cohort-A-style decision per site) not mechanical batch sweep. |

**engine.md exception list update S236:** Phase 11 media intake entry for `healthCauseIntake.js` narrowed inline with operator-fired carve-out marker. Phase 1 engine core entry for `godWorldEngine2.js` retained — still a live exception cluster pending per-site classification. **B5 effective scope drops 5 files → 3 files** mechanical: applyEditionCoverageEffects (DONE S236 B5/1) + advanceSimulationCalendar (DONE S236 B5/2) + finalizeWorldPopulation (17 sites, P3-class, still ahead — biggest single-file in B5, plan-recommended own commit). Other 3 files reclassified as carve-out / deferred classification.

**Cumulative B5 lesson (S236):** plan-inventory shipment shape was "6 files / ~50 sites mechanical sweep" — live audit shows "3 files / ~24 sites mechanical + 3 files reclassified as carve-out or per-site". Mechanical-sweep framing was too coarse; B5 is half-mechanical, half-substrate. Future Phase 42 batch planning should pre-flight call-graph + caller-shape audits BEFORE locking site counts.

**Routing summary (S205 triage):**

| Class | Sites | Files (with site refs) | Recommended sequencing |
|-------|-------|------------------------|------------------------|
| **(a) ship-now mechanical** | 2 | ~~runCivicElections L394 + L432~~ **DONE S229** | C94 gate satisfied (E94 published; live S204+S205 deploys ran cleanly through C94 cycle). Bundle landed per §3.5 recommendation — single commit covers L213-219 carve-out + L394 + L432. Pending clasp push (cross-boundary; deferred for explicit go). |
| **(a) mechanical-w/-refactor** | 3 | ~~generateChicagoCitizens L78 + L102 + L136~~ **DEAD S229** (phase disabled) | Mike call S229: Chicago canonically dead in sim — Path B no-grow-legacy disable applied at `godWorldEngine2.js` Phase8-ChicagoCitizens + Phase10-Chicago (both entry points). Mechanical-w/-refactor batch unnecessary; all 3 sites unreachable in cycle path. Reclassified to (d) verify-only-phase-disabled. **If Chicago ever revives**, reclassify back + ship the in-memory tracking refactor before flipping call sites. |
| **(b) B3 schema-setup carve-out** | 3 | ~~runCivicElections L213-219~~ **DONE S229** (bundled w/ mechanical); ~~generateChicagoCitizens L173~~ **DONE S229** (carve-out doc); ~~bondEngine L1417~~ **DONE S229** (fail-loud throw, removed from exceptions). All 3 B3 carve-outs CLOSED end-to-end. |
| **(d) verify-only** | 6 | bondEngine L1469 (Phase-10 location → benign); worldEventsLedger (deleted S199); generateGenericCitizens L662 *(phase disabled S205)*; generateChicagoCitizens L78 + L102 + L136 *(phase disabled S229 — Path B no-grow-legacy)* | No code change in any of the 6; documented here for completeness. |
...
6. **SIMULATION_LEDGER FULL-RANGE COMMIT BLOCKER (surfaced S184 B0 canary, expanded by S184 B1 audit).** 11 engines target `Simulation_Ledger` range `(2, 1, full-table)` — full read-mutate-write per cycle. **Currently works in production via a fragile invariant** (engines run sequentially; each direct write materializes before the next reads). **Under intent semantics this BREAKS or is already broken** — multiple engines queue identical-range intents at the same address; Phase 10 last-write-wins clobbers earlier engines' mutations.
   ...
   **⚠️ S200 correction (engine-sheet):** the categorical orphan-clear above was wrong about 4 of 6 engines. `educationCareerEngine`, `generationalWealthEngine`, `householdFormationEngine`, `migrationTrackingEngine` are all wired into Phase 5 via `process*_` entry functions (godWorldEngine2.js:230-233 + 1535-1538). The audit grep used file names but Apps Script flat-namespace dispatches by function name. See **A7** below for the full correction + S200 cohort-C migration. `gentrificationEngine` and `processIntakeV3` were correctly cleared (different reasoning, accidentally right).
   ...
   **A7. Cohort-C scope expansion LOCKED (S200, engine-sheet)** — the §5.6.6 audit's categorical-clear at line 489 ("Cleared categorically: orphans …") was wrong about 4 of the 6 engines listed. The audit grep'd file names but cycle entry points use generic `process*_` exposed names that don't match file names. `phase01-config/godWorldEngine2.js:230-233 + 1535-1538` calls `processHouseholdFormation_`, `processGenerationalWealth_`, `processEducationCareer_`, `processGentrification_`, `processMigrationTracking_` directly — 5 of those 6 engines were live in the cycle path the whole time.

   *Cohort-C SL writers (full-range read-mutate-write — 4):*

   | # | Phase | File | Entry function | SL touchpoints |
   |---|-------|------|----------------|----------------|
   | 19 | Phase5-HouseholdFormation | `phase05-citizens/householdFormationEngine.js` | `processHouseholdFormation_` | 2 reads + 1 setValue (HouseholdId at line 374) |
   | 20 | Phase5-GenerationalWealth | `phase05-citizens/generationalWealthEngine.js` | `processGenerationalWealth_` | 4 read+write pairs (Income/Wealth/Inheritance) + SL read inside `updateHouseholdWealth_` aggregator |
   | 21 | Phase5-EducationCareer | `phase05-citizens/educationCareerEngine.js` | `processEducationCareer_` | 3 read+write pairs (Education/CareerStage/Mobility) |
   | 22 | Phase5-MigrationTracking | `phase05-citizens/migrationTrackingEngine.js` | `processMigrationTracking_` | 2 read+write pairs (DisplacementRisk/MigrationIntent) + 2 read-only consumers |

   *S185 audit was right about (correctly cleared):*
   - `gentrificationEngine.js` — Neighborhood_Map only, never touches SL.
   - `processIntakeV3` — orphan confirmed; `phase05-citizens/processIntakeV3.js` defines `processIntakeV3_`, but the wired entry is `processIntake_` (different function, lives in `godWorldEngine2.js:894`).

   **Live impact since §5.6 went live S188 (~7 days, 1 cycle so far at C93):** Phase 10's `commitSimulationLedger_` overwrote SL with `ctx.ledger.rows` every cycle, silently clobbering all 4 cohort-C engines' direct writes. Concrete losses:
   - Wealth: Income recalc for 271/836 citizens (32% — those hitting unseeded fallback path), WealthLevel for all live citizens, every inheritance distribution.
   - Education: EducationLevel derivation, career-stage advancement (entry→mid, mid→senior), CareerMobility flags.
   - HouseholdFormation: SL.HouseholdId on newly formed households (HH-Ledger row landed; citizen never linked back).
   - MigrationTracking: DisplacementRisk + MigrationIntent for every assessed citizen.

   **Migration shipped S200 (4 commits + 1 cleanup + this spec amendment):**

   | # | Engine | Commit | Version |
   |---|--------|--------|---------|
   | 19 | householdFormationEngine | `a829c7f` | v1.2 |
   | 20 | generationalWealthEngine | `ed25ea8` | v2.1 |
   | 21 | educationCareerEngine | `7f95521` | v2.1 |
   | 22 | migrationTrackingEngine | `93cd3a4` | v1.1 |
   | — | godWorldEngine2.js (Session-30 rationale ×2) | `c589b0a` | — |

   **Side fix included:** `educationCareerEngine.detectCareerMobility_` pre-fix wrote SL only when `events > 0`, so most cycles' Mobility values never persisted (loop mutated unconditionally, but the gating write meant the mutations didn't land). v2.1 flips dirty unconditionally so Mobility always commits.

   **Audit lesson:** future cycle-path orphan checks must grep BOTH `<file-name>` AND `process<thing>_` (the typical Apps Script entry-point convention) before clearing. The flat-namespace dispatch from `godWorldEngine2.js` doesn't reference module/file names anywhere; only function names matter.
   ...
   **Cohort-C status:** DONE S200 (engine-sheet). 4-commit batch `a829c7f..93cd3a4` migrated the 4 mis-cleared engines per A7. Engine-sheet now totals 22 SL writers + 5 readers all routing through `ctx.ledger`. Side fix: `educationCareerEngine.detectCareerMobility_` Mobility persistence bug closed in same batch. Smoke-test pending C94 (operator-gated).
...
- 2026-05-09 — S210 §3.5 stale-classification fix (engine-sheet). `generateGenericCitizens.js` L662 reclassified from `(a) ship-now mechanical (HIGH PRIO — every-cycle getLastRow+1 hazard)` to `(d) verify-only — phase disabled S205`. Phase5-GenericCitizens disabled S205 same-session as triage authoring; both `safePhaseCall_(ctx, 'Phase5-GenericCitizens', …)` sites at `godWorldEngine2.js:197+1511` commented out (`d89693f`). L662 verified unreachable in cycle path: only references are legacy-snapshot copies + a doc comment in `scripts/ingestFemaleCitizensBalance.js`. Routing summary updated: ship-now mechanical 3 → 2 sites (now runCivicElections L394 + L432 only); verify-only 2 → 3 sites. **Recommended-sequencing column updated to note C94 smoke-test gate** on next mechanical / mechanical-w/-refactor / B3 carve-out batches — all live S204+S205 changes deployed but unverified (operator-gated). Bundle suggestion added: ship-now (L394+L432) + B3 carve-out (L213-219) all live in `phase05-citizens/runCivicElectionsv1.js` — one commit covering all 3 sites is cheaper than splitting across batches once C94 clears. Doc-only commit, no code touch. **If Phase5-GenericCitizens is ever re-enabled** (future SL-Tier-5 generator), reclassify L662 back to (a) before flipping the call sites.
- 2026-05-23 — S229 Chicago Path B disable (engine-sheet, engine.2 §3.5 batch 3 — final). Mike call: Chicago is canonically dead in the sim, no editorial role going forward. Path B no-grow-legacy applied — mirror of S205 generateGenericCitizens disable. All 4 `safePhaseCall_` Chicago sites in `phase01-config/godWorldEngine2.js` commented out (Phase8-ChicagoCitizens + Phase10-Chicago × 2 entry points). File `phase05-citizens/generateChicagoCitizensv1.js` retained for reversibility — no deletion. Pool frozen at ~124 rows in Chicago_Citizens sheet; Chicago_Feed sheet frozen at current row count (no more per-cycle appends). `chicagoSatellite_` in `phase08-v3-chicago/chicagoSatellite.js` has NO wired caller in godWorldEngine2.js (verified pre-disable) — disconnected helper, harmless. `saveV3Chicago_` defaults `ctx.summary.chicagoCitizens || []` and `ctx.summary.chicagoFeed || []` so it handled the missing-context case gracefully pre-disable; now it never fires. §3.5 routing reclassification: (a) mechanical-w/-refactor 3 sites (generateChicagoCitizens L78/L102/L136) → DEAD S229 (phase disabled); (d) verify-only 3 → 6 sites. **engine.2 §3.5 deferred-list cluster CLOSED end-to-end: all 11 sites resolved.** Remaining engine.2 work (B5/B6/B7 from original Phase 42 plan) is separate from §3.5 — engine.2 ROLLOUT row stays in-progress for those. **Pending clasp push** — cross-boundary, deferred for explicit go. `.claude/rules/engine.md` line 46 generateChicagoCitizens entry rewritten to "DISABLED S229, retained for reversibility" parallel to S205 generateGenericCitizens. Engine cost saved per cycle: Chicago pool maintenance (~2-5 appendRow ops avg + occasional setValue churn) + Chicago_Feed batch append (22-col row) — small but compounding over cycles. **Editorial implication for media terminal: chicago-desk agent + Bulls coverage consumers now read frozen Chicago_Citizens (124 rows) + frozen Chicago_Feed; if those become stale relative to canonical Chicago state, that's editorial-side handling, not an engine concern.** Pattern: `feedback_measure-twice-cascading-effects` (verified consumer graph BEFORE disable — chicagoCitizens consumers default to `|| []`; chicagoSatellite_ unwired; cycleExportAutomation reads sheet directly so it doesn't care about phase status; buildDeskPackets + buildMaraReference are Node-side and read sheet directly).
- 2026-05-23 — S229 B3 carve-out finish (engine-sheet, engine.2 §3.5 batch 2). Closes remaining 2 B3 carve-outs from §3.5 after the runCivicElectionsv1.js bundle (S229 batch 1, see entry below). **generateChicagoCitizensv1.js L173** `createChicagoCitizensSheet_` lazy-create — doc-only carve-out per Phase 2.1 decision A. Inline carve-out comment block added on the helper function; `.claude/rules/engine.md` line 46 narrowed from `(Chicago_Citizens)` to specify `createChicagoCitizensSheet_` L173 lazy-create stays direct as schema-setup carve-out + flag remaining L78/L102/L136 mechanical-w/-refactor pending. **bondEngine.js L1417** `else` fallback — fail-loud rewrite (ADR-0006 Contract B family). Removed dead `else` branch (`sheet.getSheetByName + insertSheet + appendRow + setFrozenRows`) per S205 triage sub-decision; replaced with explicit `throw new Error('saveV3BondsToLedger_: ensureSheet_ helper missing — utilities/utilityFunctions.js failed to load. 12 engine call sites depend on this helper; surface the load failure rather than silently shimming.')` guard before the `ensureSheet_` call. Removes the direct write from `.claude/rules/engine.md` exception list entirely — bondEngine now has only the documented L1469 Phase-10-location carve-out. Reasoning: `ensureSheet_` (utilities/utilityFunctions.js:64) has 12 callers across the engine; if it ever failed to load, that's a systemic infrastructure failure, not a per-call defensive concern. The old fallback would silently lazy-create the sheet with the same headers + setFrozenRows but in a totally different code path — hiding the deeper utilities-not-loaded bug + duplicating schema-setup logic in two places (the file already calls `ensureSheet_` in the happy path). Routing-summary table in §3.5 updated: (b) B3 carve-out 2 → 0 open. **All 3 B3 carve-out sites CLOSED end-to-end.** Remaining engine.2 §3.5 open: (a) mechanical-w/-refactor 3 sites — generateChicagoCitizensv1.js L78/L102/L136 cycle-path append+setValue migration, gated on the in-memory tracking refactor that replaces the post-write reload at L142-157. **Pending clasp push** — cross-boundary, deferred for explicit go per TERMINAL.md §Mode. Pattern citations: `feedback_measure-twice-cascading-effects` (verified `ensureSheet_` caller graph at 12 sites before deciding throw vs carve-out; verified the dead-fallback path was actually unreachable in production) + `feedback_senior-engineer-default` (B3 sub-decision was per-site judgment call, shipped throw inline rather than re-queueing).
- 2026-05-23 — S229 runCivicElectionsv1.js bundle shipped (engine-sheet, engine.2 §3.5 routing). C94 smoke-test gate satisfied (E94 published cleanly, live S204+S205 deploys ran through full C94 cycle without regression). Per §3.5 recommendation, bundled all 3 sites in `phase05-citizens/runCivicElectionsv1.js` into single commit: (a) L394 `electionLog.appendRow(row)` → `queueAppendIntent_(ctx, 'Election_Log', [...], 'log election result — ' + seat.officeId + ' ' + winner, 'civic')` (P1); (a) L432 `officeLedger.getRange(1,1,…).setValues(officeData)` → `queueRangeIntent_(ctx, 'Civic_Office_Ledger', 1, 1, officeData, 'rewrite office ledger after C' + cycle + ' elections', 'civic')` (P2); (b) L213-219 `insertSheet('Election_Log')` + headers `appendRow` + `setFrozenRows(1)` stays direct as schema-setup carve-out per Phase 2.1 decision A — inline carve-out comment block added + `.claude/rules/engine.md` line 44 entry narrowed to specify Election_Log schema-setup-only carve-out with `S229` pointer to cycle-path migration. File version bump v1.1 → v1.2 with header changelog block documenting all 3 migrations + cadence note. Low cadence: cycle 45 of even God-world years; next firing C97 (3-cycle smoke-test buffer post-clasp-push). **Pending clasp push** — cross-boundary, deferred for explicit go per TERMINAL.md §Mode. Routing-summary table in §3.5 updated: (a) ship-now mechanical 2 → 0 open; (b) B3 carve-out 3 → 2 remaining (generateChicagoCitizens L173 + bondEngine L1417). Bundle approach validated for future B3 batches — combining carve-out doc-update with adjacent mechanical migration in same file ships cleaner than splitting across batches. **Pattern:** `feedback_measure-twice-cascading-effects` (per-file READ + caller-graph + API contract verification BEFORE edit; confirmed L394+L432 site state hadn't drifted from S205 triage; confirmed `queueAppendIntent_`/`queueRangeIntent_` signatures match expected ctx/tab/payload/reason/domain shape).
