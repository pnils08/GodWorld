...
**Documented failure of the current pipeline (the proof case) — and it runs deeper than dispatch.** The engine flagged TWO high-severity crime outliers at C100, both triaged `cover-as-story` (`output/engine_anomalies_c100.json`): **Coliseum** violent-crime +4.9σ (the spike, ViolentCrimeIndex 59) and **Temescal** −5.0σ (an anomalous drop, 37). Sentiment across all 21 mapped neighborhoods compressed into a uniform high band (+0.77 to +1.06) — the sentiment-uniformity pattern under engine.38 watch (G-EC33). The edition covered none of it (claude-mem 33970). **But the deeper miss is upstream: `output/world_summary_c100.md` — the artifact desks read for context — buries all of it.** Coliseum is not in the 21-neighborhood snapshot table at all; the sentiment compression is reframed as "Very positive sentiment / +0.80"; and the summary's own Engine-Review line reads **"0 high severity"** because it ingests `engine_audit` (24 patterns, 0 high), NOT `engine_anomalies` (where the 2 highs live). The `cover-as-story` signal existed but siloed in a sibling file the world summary never merges. So the desks couldn't cover what their context artifact deleted. **The fix is two-layered: route `engine_anomalies` (cover-as-story triage) into the charge/canon-map AND invert the dispatch** — a desk charged to "find the story that matters to the people here" still needs the anomaly in its source set, because the people lived it even though the summary erased it. (Verify-before-write correction: an earlier draft of this entry implied the data was desk-available and merely un-covered; opening the world summary showed it was buried upstream — the failure is bigger than dispatch shape.)
...
**Deltas to design (carried, not yet built):** (1) dispatch inversion — kill the monolithic edition, run desks one-at-a-time deep; (2) charge-brief with free subject+angle pick; (3) per-desk/per-citizen corpus — storage flip, half-scaffolded in `output/desks/{desk}/` but article-store dead since c90; (4) byline/desk ingest tags so an agent can search "what I've said" (today's bay-tribune ingest tags cycle/type/citizen, NOT byline/desk); (5) audit-floor recalibration (catch civic invention, not journalistic reach); (6) **world-summary reconciliation** — surface `engine_anomalies` `cover-as-story` items in the desk-facing summary (today the world summary ingests `engine_audit` only and shows "0 high" while high-severity anomalies silo in `engine_anomalies`, invisible to desks). Storage + ingest + summary-merge = engine-sheet lane. Recon detail: claude-mem 34124.
