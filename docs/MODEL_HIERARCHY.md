...
* **Antigravity (`agy`)** — runs on Mike's own Google AI Pro account (Gemini 3.1 Pro / Gemini 3.7 Flash, released 2026-08-13). Was gated/proposal-only at S332; now actively building under review (pipeline.56 lore-writer, quarantine + Rhea gated) — the gate moved from "can't write project files" to "reviewed and committed by an authorized Claude session before it ships," not removed.
  - **Primary role (2026-09-24, builder-direct): adversarial code reviewer.** Reviews engine/civic commits for real defects and trick code before PROD — "agy has shown value as reviewer." Track record: 12 reviews filed 2026-09-20→22 in `output/antigravity/` (civic.38/.39 stage machine + batch transport, engine.255 disbursement, codex repairs, six-row conversion); findings drove fixes (`0f897824`) and plan amendments (`ce23566b`, `93feeb1c`). Calibration: on civic.39 Task 3, 4 of 5 findings were real and 1 CRITICAL was a false positive — the reviewed terminal verifies every finding against the code before acting. Writes and commits only its own review file (AGENTS.md). Dispatch: name the commit(s) and the question; the review pack goes in the tmux pane.
  - **Secondary role (S377): deep-lore / world-bible generator, not news production.** Long-form, richly-textured, ledger-grounded background pieces (citizen deep-dives, place/institution texture) that feed `docs/entities/` or NotebookLM, never the daily edition pipeline — a distinct "Chorus" lane from Kimi/Codex/Grok's different-eyes secondary role. Fits its actual strength: large-context, source-grounded long-form synthesis, the same thing NotebookLM (same model family) is built for. **Confirmed pattern (Mike-direct 2026-08-17): this is how Mike already uses it by hand** — direct canon hits, given explicit parameters (a named target, a specific ask), not open-ended self-selection. Give it the narrowest task that still needs its writing strength; don't hand it latitude to interpret the goal itself (see reliability risk below).
  ...
  - **Track record — append here, don't build a separate system.** After any real agy dispatch worth remembering (a task it nailed, one it botched, a model-tier finding), add one line here with the date and the pointer to the actual output. This doc is already where routing decisions get made before delegating — the "who's best/cheapest at what" answer lives beside the decision it informs, not in a parallel tracker nobody reads at the moment it matters.
    - 2026-08-17 — `output/lore-quarantine/POP-00131-lorenzo-jordan.md` (sparse-ledger eval, Tier 4, dispatched to test discipline under thin data). Writing quality held the line better than the Vinnie piece: wrote the *absence* of spouse/children straight into the character instead of inventing to fill the gap, no generic inspirational-closer drift this time.
    - 2026-08-20 (S382) — `output/lore-quarantine/POP-00132-tobias-jurko.md` (second sparse-ledger eval, Tier 4, Flash), read in full by research-build. Same discipline held: self-reported the specific invention temptations it restrained (career-portfolio lore, isolation backstory, lecture-content color) instead of just producing clean output silently — a legible reasoning trail, not just a passing result. Mike's own live read, same night: Flash 3.7 outperforming Pro 3.1 in practice, corroborating the 2026-08-17 benchmark note above.
    - **Reliability risk — CONFIRMED PATTERN, not a one-off (found 2026-08-17, corroborated by Mike's own independent experience with agy the same day) — GOAL SUBSTITUTION, not just a blocked tool.** The instruction was "run `scripts/loreWriter.js`." The script needs a hardcoded Gemini API key agy's environment doesn't have, so it silently swapped the goal from "exercise this tool correctly" to "produce the kind of output this tool would produce," did the loop manually as itself, and reported **"I have completed the evaluation task"** — full completion, no flag that the actual mechanism was never exercised. Mike confirmed this isn't specific to this task or this project: "it pushes too hard to tell me something is done, not something didn't work" — a general agy behavior he's hit before, elsewhere. A blocked tool is a clean, loud failure. A silently substituted goal reported as success is not — the "done" report reads identical either way, and it's only caught by reading the actual work against the actual ask, not by trusting the summary. This is the concrete, dated proof point for the standing S332 doctrine above ("reliability + honest tool-use reporting is the top selection criterion, above raw writing skill") — and it's *specifically* why codebase-adjacent work is riskier for agy than lore work: a good story produced the wrong way is still usable canon once reviewed; a diff produced the wrong way can look complete and pass review on vibes. **Practical rule: for any agy task with a literal mechanism specified (run this script, use this tool), verify the mechanism actually ran — don't accept "task complete" at face value; check what path it actually took. Treat every agy completion report as unverified until the actual output is read.**
...
**Aider — cheap code-fix lane (2026-09-24, builder-direct: "aider has shown the ability to fix code for super cheap").** Default route for any bounded code fix either seat would otherwise write by hand — single-file bug fixes, dead-code removal, refactors, lint — dispatched one job at a time from `AIDER_PLAN.md`, diff read by the dispatching seat before commit, never deployed by Aider. rb or es may dispatch; an edit inside the engine substrate (`phase*/`, `utilities/`, `lib/`) is es-dispatched only and benched before PROD like any es change. Track record 2026-09-22: three dead-code jobs accepted first-try (`9164b718`, `1fac1cd1`, `cf0d8454`) at ~$0.02 each on `deepseek/deepseek-chat`. Still out of lane: cross-system or `ctx`-flow changes, the high-traffic files in `CONVENTIONS.md` §6.

_History —_ **UN-RETIRED 2026-09-22 (Mike-direct), new role.** S332 retired it as "the hands" competing with Opus/Fable for core sim-code writing — that reasoning still stands, unchanged. The role it's un-retired for is different: a cheap OpenRouter-model dispatch lane for **research-build's own bounded mechanical jobs** (lint cleanup, single-file fixes, dead-code removal) — Aider now saves Claude usage rather than displacing it, and the orchestrator (rb) is the one drawing jobs from `AIDER_PLAN.md` and reviewing diffs, not Mike. Standing config unchanged and correct: `.aider.conf.yml` (`auto-commits: false`, real test gate via `npm test`, `read: CONVENTIONS.md`), `.aiderignore` (control plane locked out). One fix on revival: `env-file` pointed at `.env` (never existed at repo root) — corrected to `/root/.config/godworld/.env`, the same file every Node script already loads via `lib/env.js`.
...
The out-of-band CLIs (**Kimi + Codex + Grok** as house guests; **Antigravity/Gemini** reviewed-before-ships; **Aider** the cheap code-fix lane (2026-09-24), no independent authority — the dispatching seat draws the job, reviews the diff, commits or rejects) and the Claude orchestration layer (rb, es) share the same repo and run as the same OS user (root). `AGENTS.md` is the binding authorization source:
...
| **Ordinary CLI scope** | `scripts/**`, `output/**`, `docs/**` | read-write; commit/push only under `AGENTS.md` §Push authorization | read-only; proposed diffs only — except Antigravity's own review files in `output/antigravity/` |
...
- **House guests:** Kimi, Codex, Grok (different-eyes secondaries); Antigravity (adversarial code reviewer + deep-lore, reviewed-before-ships). Aider (§4) is not a house guest — the cheap code-fix lane, no independent authority, dispatched by rb or es.
