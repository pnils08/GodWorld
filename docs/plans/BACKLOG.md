...
sources:
  - "[[engine/archive/ROLLOUT_PLAN]] §Open Phases — source for everything below"
pointers:
  - "[[engine/archive/ROLLOUT_PLAN]] — parent"
...
See also [[engine/archive/PHASE_24_PLAN]] for the existing plan doc.
...
- 2026-04-16 — Initial consolidation (S152). Content extracted verbatim from [[engine/archive/ROLLOUT_PLAN]] §Open Phases + research patterns. When any phase here becomes active work, extract to its own [[plans/TEMPLATE]]-shaped plan file and replace the section here with a pointer.
