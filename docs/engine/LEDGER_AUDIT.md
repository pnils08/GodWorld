...
**Headline count (S386, 2026-08-21):** **964 rows live**, read-back verified — max POPID POP-01079. Two athlete mints this session (Almanzar POP-01078, Brooks POP-01079). Also this session, engine.126: three GAME-mode rows were found overwritten by civilian career logic (Dybantsa POP-01024 role+pay, Sarr POP-01025 / Carr POP-01026 salaries zeroed), restored from the git-tracked snapshot, and `SPORTS_OVERRIDE` backfilled across all 17 previously-unguarded active GAME rows — sports-layer exposure is now 0 of 53. **This is a headline correction, NOT a full truing pass** — the S322 structural sections below are unre-verified and the full refresh is still the open citizen-completeness work.

**Last Refresh:** Session 322 (2026-07-17) — **truing pass, live + 0716c bench, all read-back verified:** YearsInCareer age-fossils re-derived via `lib/citizenDerivation.deriveYearsInCareer` capped age-18 (131 live / 141 bench, pre-S318 accrual era); DebtLevel blank-fills derivation-mirrored (130/135); CareerStage blank-fills per engine enum — student <22, retired ≥65, else entry-level <5y / mid-career ≥5y, senior NEVER assigned (promotions earned) (108/110); minor SchoolQuality stamps (38/7). Root-cause fix same pass: **Neighborhood_Demographics was missing 4 canon hoods** (Lake Merritt / Uptown / KONO / Baylight District — S256 added them to code rosters but rows never landed; engine only maintains existing rows, so kids there could never receive SchoolQuality). Rows appended both sheets with `addEducationCareerColumns.js` canonical edu values + ledger-seeded counts. Dial-less rows remain: 5 bench (compressLifeHistory fills next fire), 19 live (structural — live never fires). Detector+apply scripts session-scratchpad only (no repo .js added). *Prior: Session 312 (2026-07-11) — **schema 50→52 cols**: AY `StatusStartCycle` + AZ `HealthCause` added prod+sandbox (engine.52 — activated the health state machine's dormant duration wiring; both read/written by header name in `generationalEventsEngine.js`). Live rows 914 (POP-01028 max). Relationship_Bonds bond-key repair same session (6 prod name-keyed rows → POPIDs). *Prior: Session 250 (2026-06-01) — **headline counts refreshed real-time via `auditSimulationLedger.js`:** 903 rows (+45 vs S234 = S243 engine.5 Phase 1 youth seed POP-00974..01018, all T4), Status lowercase-'active' normalized. Structural rows (non-canon neighborhoods / age sanity / RoleType sentinel) carry from the S234 baseline below — not re-verified S250. *Prior: S234 — S229 canon.3 T9 backfill (POP-00958..00973 + 6 squatter realignments) + pending-status + lowercase-active drift.*
**Priority:** CRITICAL — All downstream ledgers depend on this data being correct
**Rule:** Age = SimYear − BirthYear, with SimYear read live from Simulation_Calendar (stores relative year; anchor 2040, so Y2 = 2042 as of S322). Never hardcode the year — it advances.
...
> **Document timeline.** S68 baseline (this doc) → S94 corruption recovery (`LEDGER_REPAIR.md`) → S99 schema bump + civic-officials cleanup (`LEDGER_REPAIR.md` post-recovery section) → S181 verification + drift snapshot → S199 refresh (S184 +150 female-balance ingest + S185 trim + post-S184 drift) → S234 refresh — captures S232 canon.3 T9 backfill + new pending-Status drift + lowercase 'active' regression + +2 non-canon-neighborhood drift → **S250 headline refresh — 903 rows (+45 S243 youth seed), lowercase-'active' normalized; structural rows carry from S234** → **S312 schema bump 50→52 cols (AY StatusStartCycle / AZ HealthCause, engine.52) + 914 rows**. Read top-down for current state; the historical S199 / S181 / S72 / S69 / S68 sections below are archive.
