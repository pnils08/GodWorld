...
Grade 1 citizen attachment (commit 45723366; grades defined in
[[../research/2026-07-06-city-citizen-seam-audit]]): `contractSeedPickCitizens_` fills behind
exact targets from the seed's neighborhood (citywide → cross-hood draw), ctx.rng-deterministic.
Dry-run against real C116 data: 9/9 rows filled. Live C117 run: **8/8 rows carry 4 POPIDs +
event lines** through the full in-cycle path (Phase 7 build → Phase 10 persist). Column test =
open the sheet: passed. Grade 2 (exposure-weighted draw) deferred until Grade 1 rows appear in
an edition.
