...
updated: 2026-05-28
type: plan
tags: [governance, infrastructure, memory, claude-mem, complete]
sources:
...
- 2026-05-28 — **Task 4 DONE (S242, post-restart).** Restart happened; Mike flagged session-end "junk." Root cause: 13.3.0 cache shipped with `zod` undeclared-but-imported (`worker-service.cjs` imports `zod/v3`; only tree-sitter native modules were built, zod absent) → all four memory hooks threw `Cannot find module 'zod/v3'` and captured nothing. **The "SAFE-TO-UPGRADE" changelog read held for the DB migrations but missed an incomplete dependency install — exactly the "changelog read ≠ test result" caveat the S241 gate-reframe called out.** Fix: `npm install zod@^4.3.6 --no-save` into `/root/.claude/plugins/cache/thedotmack/claude-mem/13.3.0/` (zod 4 ships `zod/v3` back-compat subpath). Smoke gate then passed: Stop hook exit 0 with a real payload + `/mem-search` returns pre-upgrade S241 observations. `docs/CLAUDE-MEM.md` rewritten (header version, post-upgrade-gotcha §, 2.2 GB disk, expanded skill table, provider-drift config note). **Carry-forward gotcha:** the zod fix lives in the version-pinned cache dir — a future claude-mem auto-update may reintroduce the gap; same one-line fix. Pattern: feedback_measure-twice-cascading-effects (smoke test was the real gate, not the changelog).
