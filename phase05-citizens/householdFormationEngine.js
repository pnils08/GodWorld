...
function processHouseholdFormation_(ctx) {
  ...
  try {
    ...
    // engine.259: a standing fund with money left pays grants onto this Cycle's
    // stressed households in its hoods BEFORE stress reads their savings. Then
    ...
    results.fundDisbursement = applyFundDisbursement_(ctx, cycle);
...
// ============================================================================
// engine.259 — the fund grant writer (builder 2026-09-24: "the fund should move";
// engine.255's writer, recovered from c58a1adc^ and taken off housing).
//
// For each program on S.initiativeDisbursement (Phase 2, one per Standing/
// Delivering fund in disbursement-active with money left): the stressed active
// households in its folded hoods, owned or rented — exactly detectHouseholdStress_'s
// rule, burden ≥ RENT_BURDEN_WARNING and savings under the 12-month buffer of their
// own monthly payment — not granted by ANY fund inside
...
// paid again this Cycle; a program whose tracker LastDisburseCycle == cycle is not
// debited again, and the debit is absolute (remaining − tranche), so a retry after
// a crash re-queues the same value.
// Direct own-tab column-vector write, same class as updateHouseholdIncomes_
// (SHEETS_MANIFEST §9). Fail loud, never fatal.
// ============================================================================
var FUND_GRANT_COLUMNS_ = ['LastGrantCycle', 'LastGrantInitiativeID', 'LastGrantAmount'];

function ensureFundGrantColumns_(sheet, header) {
  var missing = [];
  for (var i = 0; i < FUND_GRANT_COLUMNS_.length; i++) {
    if (header.indexOf(FUND_GRANT_COLUMNS_[i]) === -1) missing.push(FUND_GRANT_COLUMNS_[i]);
  }
  ...
  Logger.log('householdFormationEngine: engine.259 appended grant receipt columns to Household_Ledger — ' + missing.join(', '));
  return true;
}
...
function planFundDisbursement_(header, rows, program, cycle, hoodOf, savingsOf) {
  var idx = function (n) { return header.indexOf(n); };
  var iId = idx('HouseholdId'), iHead = idx('HeadOfHousehold'), iHood = idx('Neighborhood'),
  ...
  var out = { eligible: 0, grants: [], paid: 0, trancheLeft: Number(program.tranche) || 0,
              skipped: { inactive: 0, offHood: 0, invalid: 0, unflagged: 0, cooldown: 0, alreadyThisCycle: 0 } };
  if (iId < 0 || iHood < 0 || iRent < 0 || iInc < 0 || iSav < 0) { out.reason = 'columns-missing'; return out; }
  ...
  // engine.259: this program's receipts already stamped this Cycle (a crashed
  // earlier attempt of THIS fire) were paid out of this Cycle's tranche — a retry
  // spends only what is left, never the tranche twice.
  var iLI = idx('LastGrantInitiativeID'), iLA = idx('LastGrantAmount');
  if (iLG >= 0 && iLI >= 0 && iLA >= 0) {
    for (var q = 0; q < rows.length; q++) {
      if (Number(rows[q][iLG]) === Number(cycle) && String(rows[q][iLI] == null ? '' : rows[q][iLI]).trim() === String(program.initiativeId)) {
        var prev = Number(rows[q][iLA]);
        if (isFinite(prev) && prev > 0) out.trancheLeft = Math.max(0, Math.round((out.trancheLeft - prev) * 100) / 100);
      }
    }
  }
  var cands = [];
...
function applyFundDisbursement_(ctx, cycle) {
  var out = { available: false, reason: null, programs: 0, grants: 0, paid: 0, debited: 0, armed: false, error: null, detail: [] };
  try {
    return applyFundDisbursementBody_(ctx, cycle, out);
  } catch (e) {
    out.error = String(e && e.message ? e.message : e);
    out.reason = 'error';
    Logger.log('householdFormationEngine: engine.259 fund disbursement ERROR — ' + out.error);
    if (typeof logEngineError_ === 'function') {
      try { logEngineError_(ctx, 'Phase5-FundDisbursement', e); } catch (e2) { /* the log itself must not throw */ }
    }
    return out;
  }
}

function applyFundDisbursementBody_(ctx, cycle, out) {
  var S = ctx.summary || {};
  ...
  var programs = (slice.programs || []).filter(function (p) { return !!p; });
  ...
  for (var pi = 0; pi < programs.length; pi++) {
    ...
    var plan = planFundDisbursement_(header, body, program, cycle, hoodOf, savingsOf);
    ...
    if (plan.grants.length && ensureFundGrantColumns_(sheet, header)) {
      out.armed = true;
      header = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      data = sheet.getDataRange().getValues(); body = data.slice(1);
      plan = planFundDisbursement_(header, body, program, cycle, hoodOf, savingsOf);
    }
    ...
    for (var g = 0; g < plan.grants.length; g++) {
      ...
      if (typeof queueAppendIntent_ === 'function' && gr.head) {
        queueAppendIntent_(ctx, 'LifeHistory_Log', [ctx.now, gr.head, '', 'Relief',
          'received a $' + Math.round(gr.amount) + ' grant from ' + program.name + ' (' + program.initiativeId + ')' + (gr.clearsBuffer ? ' — a year of breathing room in the bank' : ' — a partial, money is still tight'), '', cycle],
          'engine.259 fund grant', 'civic', 5);
      }
    }
    ...
    if (tracker && typeof queueCellIntent_ === 'function' && program.sheetRow > 1) {
      if (tRemain >= 0) queueCellIntent_(ctx, 'Initiative_Tracker', program.sheetRow, tRemain + 1, newRemaining, 'engine.259 tranche C' + cycle + ' ' + program.initiativeId, 'civic', 5);
      if (tLast >= 0) queueCellIntent_(ctx, 'Initiative_Tracker', program.sheetRow, tLast + 1, Number(cycle), 'engine.259 disburse receipt C' + cycle + ' ' + program.initiativeId, 'civic', 5);
  ...
  Logger.log('householdFormationEngine: engine.259 fund disbursement — ' + out.programs + ' program(s), ' + out.grants + ' grant(s) $' + out.paid + ', debited $' + out.debited + (out.detail.length ? ' | ' + out.detail.join(' | ') : ''));
  return out;
}
