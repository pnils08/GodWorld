...
**GATE unchanged:** no clasp deploy until the affect re-audit clears (N-consecutive-green, or certified on the COMBINED/AFFECT sign criteria — S269 read: 6 post-fix wakes, 5/6 correct sign, not yet a clean streak) + a **deliberate** push (`utilities/tier1EssenceEvents.js` rides any clasp push per boot state).
