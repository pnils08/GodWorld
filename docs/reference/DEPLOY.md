...
- **Bench 2026-08-30: synced from live C104, at @18 / C114 (engine.135 wave — per-deployment detail in the plan §Phase D/E/F).** One bench-only sheet write since the sync — NEVER replay to live: `World_Config!B8 cycleCount` 109 → 110 (engine.136 diff-restore).
- **PROD @10 (2026-08-30) = repo HEAD `f7811139`.** Prod `clasp deploy` fails under the auto-mode classifier — run bumps outside auto mode.
