...
**Cadence anchor — the 7-day static cycle + fame-on-selection (Mike-direct S332, go-live spine).** Cycles advance ~**every 7 real days**, so the world state is **static for a week**. This is the axis the whole go-live cadence turns on:
- **The newsroom runs a WEEK on ONE frozen cycle**, not a fresh cycle per day. M–F the four desks wake against the *same* `desk_signal_c{N}` and produce a **bounded pool of diverse submissions** — different journalists, different storylines/angles on that one cycle — submitted to Mags + Mike. You never pay to re-generate the frozen world; you pay for a controlled *spread*. Cost control = the spread + a weekly submission cap + **no same-byline/same-storyline repeat** across the 5 days (dedup on what's already submitted, not just what's already a cycle).
- **Saturday = selection, near-free.** Mags reads the week's submission pool and picks the best for the edition (curation over already-written drafts, not fresh generation).
- **Selection is the reward + the fame ledger — via the EXISTING edition ingest (Mike-direct S332, not a new build).** Being **chosen to run in the edition** is the journalist's goal. Fame/usage is tracked by the **edition-rating ingest that already runs on edition articles** (`/post-publish` → `ingestEdition*.js`) — when a piece is selected and the edition ingests, the rating write-back already happens. So there is **nothing new to build for fame**; the M–F pipeline just has to feed the pool, and the existing Saturday edition-ingest closes the loop. This closes the byline loop: layer-1 rotation surfaces who *writes* (`byline_shadow_log` = candidacy); edition-selection + the existing ingest record who *lands* (= fame).
- **Related citizen-side change (distinct from the newsroom):** citizen wakes drop **5 → 2 (morning + night)** — a frozen 7-day world doesn't need 5 dayparts; citizens cycle on their own internal lives across the week, and 2 wakes cut redundant spend. This is a `scripts/citizen-wake.js` daypart change, separate from the journalist cadence.
- **"voice-packets"** — named as the next build packet (successor to the desk_signal partition, for the citizen/voice side). Not yet designed; flagged so it isn't lost.

This **refines "those four desks wake daily" above**: they wake M–F, but on a *static* cycle to build the week's submission pool — not to write a fresh cycle each day. The submission budget + selection reward are what keep the spend bounded.
