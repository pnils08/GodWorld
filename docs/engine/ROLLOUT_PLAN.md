...
| engine.175 | Undocked cast drawn in the engine + show markets keyed to the pilot — fire N draws cycle N+1's 3 pilots + alternates (`ctx.rng`, undockedDraw eligibility) into `S.undockedNextCast` + a draw tab (NEW TAB — builder nod); `credits_sign`/`mishap` resolve on the pilot's aggregated aired rows for the cycle, `night_winner` unchanged, `casinoUpcoming_` retired. Fixes the never-opened show market (off-by-one). GATED: rb account minting + brief generation (plan open item (a)) before PROD, and builder confirmation of the 2026-09-08 reseed + fame rulings (relayed via kimi) | needs-info | engine-sheet (engine) / research-build (minting, briefs, orchestrator draw-tab read, Nia-slice standings wire) | [[../research/2026-09-08-kimi-casino-undocked-trueup]] §Review + [[../plans/2026-08-07-spacemolt-game-show]] §Post-ship open items (a)(c)(e) |
