...
5. **Phase 38.2 + 38.3 + 38.4** — Mitigator check + remedy recommendation + Tribune framing brief. **PLAN WRITTEN S146** as §§14–17 of `docs/engine/PHASE_38_PLAN.md`. Three enricher modules added to the same orchestrator: `checkMitigators.js` annotates each pattern with `mitigatorState` (4-value gap classifier + initiative-effect verification), `recommendRemedy.js` generates `remedyPath.worldSide` with tech-side fallback only when writeback chains are structurally broken, `generateTribuneFraming.js` produces per-desk `storyHandles` + pre-filled `threeLayerCoverage` + `capabilityHooks` for Phase 39.1. Temescal end-to-end is the validating acceptance: INIT-005 should thread `stuck-initiative` → `mitigator-stuck` → `advance-initiative` → suggested-front-page-with-hooks. Two follow-ups noted: rewrite `/engine-review` and `/sift` skills to consume the new fields. **READY TO BUILD `(engine-sheet terminal)`** — same orchestrator pattern as 38.1/38.7/38.8, declarative registries (`mitigatorRegistry.json`, `remedyTemplates.json`) versioned independently from code. Combined runtime budget still 60s.
