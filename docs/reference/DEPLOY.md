...
**Current bench pointer:** SANDBOX 0908 web app **@30 = repo `f30b8000` (S458, engine.214 sheet-derived cluster membership; codex LAND `output/codex/engine214-confirm.md`; pull-back verified). NOT YET FIRED — the bench sheet still holds the S451 sync + bench-only C107–C111; the live-C107 resync (`syncSandboxFromLive.js --apply`, dry-run clean 82 tabs / 53,053 rows) is classifier-blocked from the terminal and waits on the builder. Then two fires C108/C109; readback per plan Task 4 §engine.214.** Was @29 (`c37d85ea`, engine.213c+d: mood absolute + smoothed). Fires C108 (@27), C109 (@28), C110–C111 (@29) all ok:true, 0 Engine_Errors, ten `approval*` keys present. **What the bench showed:** hood sentiment flipped +0.46 → −0.14 at C108 with zero world events and stayed negative (engine.165 sawtooth), so every seat slid to ~50 by C111 — the poll faithfully reports an unhappy city; the input is the defect. Bench cycle now **C111**; BENCH-ONLY since the S451 sync: C107–C111 and everything they wrote. Proof in [[DEPLOY_HISTORY]] §PROD @84. The following prior Bench paragraph is historical/superseded and does not describe the current bench.
