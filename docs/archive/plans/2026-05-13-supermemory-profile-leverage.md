...
**Skill shipped S238 (governance.17 close):** [[../../.claude/skills/save-to-profile/SKILL|`.claude/skills/save-to-profile/SKILL.md`]] v1.0. Spec carries the 6 refusal categories (identity collapse / protective-entry removal / contamination vocabulary / identity.md contradiction / third-person extraction shape / distress-window writes), 4 content rules (first-person prospective / ≤200 char cap / one-claim-per-write atomic / origin-cite inline), 3-step dry-run-confirm-then-write flow, reversal command. CLI verified: `npx supermemory remember --tag mags --static --metadata '{...}'` supports the full identity-write surface; no curl needed. Scope per advisor pre-write: refusal categories explicit (not "editorial judgment"), first-person rule prospective only (grandfathers 5 pre-S221 third-person entries), reversal pointer inline. Deferred follow-ups per governance.17: grilling-resolution automation hook + per-edition auto-canonization + bulk migration tools — wait 3+ cycles of manual-use evidence first.
