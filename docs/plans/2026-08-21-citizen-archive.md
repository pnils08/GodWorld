...
- **Landed 2026-09-06 (S428).** `utilities/resolveCitizen.js` (pure core over header+rows arrays) + `lib/resolveCitizen.js` (requires the core, supplies both tabs; absent tab = empty archive) — `scripts/resolveCitizen.test.js` 13/13 runs the same fixtures through the clasp-loaded core and the Node export. `dumpLedger.js` writes `output/citizen_archive_snapshot.jsonl` + `.meta.json` when the tab exists; live run proved the skip path (tab absent, flag not 1 → active snapshot written, `archiveRowCount: null`, exit 0). `canon-name-check.js` `loadRows()` unions the archive snapshot (rows tagged `_archived`). `buildCitizenCards.js --from-archive` unions the newest snapshot per POPID (positional A–BC, header-checked; no `--apply` here). **Deferred:** dashboard Layer 1b (`dashboard/server.js` carries another lane's uncommitted hunk tonight; a path-specific add would sweep it) and `queryLedger.js` / `healthCauseIntake.js` resolver hops (Commit 12).
