...
The draft recommends a concrete policy; the request authorizes drafting only. Builder/engine-sheet acceptance is needed for the above-median-only benefit, 10%/1-point dials, service whitelist, receipt coverage and runtime capability handling. The relative-to-city comparator and immutable baseline follow Task 4 engine rulings; civicDeliverMargin and civicDeliverHoldCycles remain builder decisions. OARI's ID is resolved by ruling 3 as INIT-002. These are explicit implementation prerequisites, not permissions implied by filing this draft. Safety petition signature units and support bands remain outside this lever design.
