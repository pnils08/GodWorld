...
// Initiatives in the World Job 2 (builder rulings 2026-09-25) — no menu. A seat
// proposes in its own words and files the row under a CATEGORY it picks: the
// part of life the initiative touches. The category is written straight to
// PolicyDomain; nothing is refused for its content (SIM_DOCTRINE §13). Housing
// is not a category — housing effects are a result of an initiative, never a
// program. A category whose Delivering gate is not playable below still mints,
// votes, funds and stands; it stops at `no-delivering-gate` and the pack says so.
// REACH is how far it goes: hood (named hoods), district (every hood in the
// seat's district), all (every canonical hood — citywide seats only). Reach is
// expanded to explicit hood names before a row mints, because an empty
// AffectedNeighborhoods fails the engine baseline (`no-target-hoods`) and would
// leave a gate that can never be read (§15).
const PROPOSAL_CATEGORIES = {
  health:      'clinics, care',
  transit:     'stations, lines',
  education:   'schools, youth learning',
  economic:    'business, local economy',
  workforce:   'jobs, training',
  safety:      'public safety, response',
  sports:      'stadiums, recreation',
  environment: 'parks, air, water',
};
const PROPOSAL_REACHES = ['hood', 'district', 'all'];

// The delivering-gate catalog. Since Job 2 a seat no longer names a key from
// here — this table only says, per PolicyDomain, which metric a Standing row
// must move to reach Delivering (stageCatalogByDomain → the engine mirror).
// Historically it was the closed proposal menu (civic.38 Task 4 step 0). One catalog, shared by every script gate (cron-civic-run,
...
//                  so a non-playable domain never reaches Delivering until its
//                  engine lever lands. It is still proposable (Job 2).
...
// Initiatives in the World Job 3 — the build clock (engine mirror INITIATIVE_BUILD_COLUMNS_).
const BUILD_COLUMNS = ['OpensCycle'];

// engine.255 Task 9 (builder call 6, 2026-09-22): a seat-proposed row carries a
// budget inside its category's band, or it stands as `blocked: no-budget`. One
...
// afford to lose one. Job 2 (2026-09-25): one band per PROPOSAL_CATEGORIES key
// (parity-tested); the housing band left with the housing category.
const BUDGET_BANDS = {
  health:      { min: 5000000,  max: 100000000 },
  transit:     { min: 20000000, max: 500000000 },
  education:   { min: 1000000,  max: 50000000 },
  economic:    { min: 2000000,  max: 50000000 },
  workforce:   { min: 1000000,  max: 30000000 },
  safety:      { min: 2000000,  max: 30000000 },
  sports:      { min: 10000000, max: 500000000 },
  environment: { min: 1000000,  max: 30000000 },
};
...
//
// input  { stage, phase, policyDomain, lastWorkCycle, lastStageChangeCycle, metricMoved, opensCycle }
