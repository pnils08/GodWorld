...
1. **Live smoke of PROD @50** at the builder's next fire — five deployments (@46–@50) ride it, all bench-proven on live-synced C105 (C106–C108). Expect 13 D3 raises to the dollar, two engine.96 owner's-draw resets, Intake `Relation` self-arm, two media-room honorific rows to review. Then `node utilities/exportSchemaHeaders.js`.
2. **engine.109 Task 7** is the builder's (families on the Intake tab). Row stays in-progress until a live household lands.
3. **engine.134 hood identity** — Tasks 1/4–8 in flight S423; Tasks 2–3 wait on the engine.131 T7 ruling (same two functions in a HELD file).
...
| engine.134 | Hood identity — Tasks 4–8 SHIPPED S423 (crime on the 22-hood set + INSTITUTIONS characters; evening food from Business_Ledger; famous on S.sportsZones; two orphan writes gone). Tasks 2–3 HELD: `economicRippleEngine.js` is an engine.131 T7 HELD file and T7's hunks hit the same two functions — ships with T7's ruling. Open: bench @46 → PROD, then live Crime_Metrics replay (5 appends) + Montclair ghost row hand-delete | in-progress | engine-sheet | [[../plans/2026-08-30-hood-identity-remainder-plan]] §Changelog |
...
| civic.18 | District map reconciliation — 4c/4d DONE S423 (approval engine reads `Neighborhood_Map.District` via `getDistrictHoods_`; literal gone). Open: Task 1 Montclair row (builder's seed), 4e Node reconcile of the District column (on touch) | in-progress | engine-sheet | [[../plans/2026-08-15-district-map-reconciliation]] §7 |
