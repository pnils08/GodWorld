...
- **[[plans/2026-04-28-female-citizen-balance]]** — S184 follow-up consuming the intake-derivation library. Goal: 150 new female SL rows to bring live gender share 33% → 40.7%. Direct-write batch script `scripts/ingestFemaleCitizensBalance.js` mirroring `backfillLifecycleDefaults.js` pattern. Phase 1 (curated 150-name pool `data/citizen_female_first_names.json`, 7 origins) + Phase 2 (Oakland age CDF + canon-12 neighborhood CDF with female-density skew) DONE research-build S184. Phases 3-5 (script + 5 dry-run gates + apply-and-verify) handed to engine-sheet. Acceptance: post-apply F share ≥40%, capability reviewer assertion #9 has more headroom on next sift. MEDIUM. *(plan, engine, citizens, architecture, draft)*
