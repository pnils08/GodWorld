...
- **Files/Steps:** (each sub-item measure-twice'd per engine.md before the cut)
  ...
  - **3c Crime event/headline generators (trace K2):** wire `generateCrimeEvents_`/`getCrimeStorySignals_` into the cycle path with ledger attribution, or retire them explicitly — decision on live data volume (don't let dead pools sit ambiguous). **RETIRED S311 (`6d5388ef`):** zero callers since landing; both are pre-S296-contract patterns (citizen-less template pools, pre-shaped headlines). Mechanical purpose already served by T3b spike carry + ledger crime rows + contract seeds. 195 lines deleted, suite green, STUB_MAP regenerated.
  ...
  - **3e Initiative-implementation hollow engine (found S294 during 3a caller-graph):** `applyInitiativeImplementationEffects_` is wired (Phase2-InitiativeEffects, godWorldEngine2.js:222/1566) but ALL outputs are dead — sentiment scalar wrote dead `S.sentiment`, `S.initiativeNeighborhoodEffects` + `S.initiativeImplementationTriggers` have zero readers. Measure-twice in-task: fold-vs-retire decision (may overlap civicInitiativeEngine's Phase-5 ripples). **FOLDED S311:** live check settled it — all 6 tracker initiatives carry voice-agent-set ImplementationPhase (dispatch-live, disbursement-active, construction-active…), so the input side is alive and this is the only city-hall→engine implementation feedback; retiring would kill a maintained causal loop. Built: per-initiative Ripple_Ledger rows at compute site (`causeType: initiative-implementation`, phase/domain/hoods causeDetail) + sentimentBoost folds into `finalCity.sentiment` in applyCityDynamics (same seam as T3a sports/v3.2 edition, clamp ±0.15 upstream); dead `S.sentiment` write + unread triggers publish removed; v1.0→v1.1. No overlap with civicInitiativeEngine Phase-5 ripples — those are passage-event ripples, this is ongoing implementation-state grain. **Open follow-up (per-hood fold):** `S.initiativeNeighborhoodEffects` kept published but per-hood consumption remains unwired — same seam gap as per-hood `editionNeighborhoodEffects` (only ['city'] is consumed); design the shared per-hood fold once, apply to both. Sandbox verify pending; deploy held behind C102 smoke.
...
> **2026-07-05 (S296, Mike-direct): Tasks 4–6 DEAD as JSON designs.** Mike rejected this plan's
> consumption layer as not his design: sheets are the world, no JSON carries logic, and entities
> attach at generation time — no post-hoc joins. The corrected contract lives in
...
>
> **T4/T5 PURPOSES DELIVERED in contract form (S296, same day):** in-cycle
> `buildContractSeeds_` (Phase 7) + `saveV3Seeds_` v4.1 write desk-addressed contract rows to
> Story_Seed_Deck — Desk column routes per desk (T4's purpose, in-sheet, no JSON files);
> sign-aware promotion Class major/texture with ≥3-cluster merge (T5's purpose, inside the
> builder). EXACT citizens only — engine-named POPIDs, no neighborhood-bystander fallback
> (Mike-direct). Live-verified sandbox C115: 10 rows, 7 major / 3 texture, deck self-migrated,
> 2,505 v3 rows parked. Commits b8359fc9 → this one. T6's purpose (WHY from persisted causes)
> is inherent in the contract rows' Why column; the Node re-point died with the JSON layer.
