...
**Last verified:** 2026-09-02, S412 (engine-sheet /session-end audit — **functions added:** `simYearFromCycle_`, `simYearOf_` (`advanceSimulationCalendar.js`, engine.148 — the one year formula; 25 per-site derivations in 12 files now call `simYearOf_`); `graduationTradeTrack_` + `GRADUATION_TRADE_FIELDS_` (`generationalEventsEngine.js`, engine.149 — `graduationCredential_` takes a third `skillTags` arg); `intakeTierForExisting_`, `tierForEarnedUsage_`, `earnedCitationsByKey_`, `applyTierLadderState_`, `dimCulturalFame_`, `culturalFameOf_`, `ensureFamousOnCulturalLedger_` + `FAME_USAGE_BAR`, `MEDIA_PUBLISHED_WORTH` (`processAdvancementIntake.js`, engine.150 / .152 / .118 — the ladder is a state on earned counts, coverage runs both ways via `Citizen_Media_Usage.Sentiment`, fame is permanent via `Simulation_Ledger.Famous` and registers on the Cultural_Ledger; the citation event no longer decides Tier; decay reads `Promotion` / `Media|tier-climb` log rows as earned, skips famous rows, covers ENGINE + GAME only); `householdHousingById_`, `courtshipHousingBoost_` + `HOME_OWNED_COURTSHIP_BOOST` (`bondEngine.js`, engine.153); `SOLO_TIER_MAX` removed (`householdFormationEngine.js`, engine.153 — solo household on income alone); **S fields added:** `S.tierLadder`; **schema:** `Citizen_Media_Usage.Sentiment` (P), `Simulation_Ledger.Famous` (BC, column appended) on live + bench; **PROD @29** = @21 + engine.148 (@22) + engine.149 (@23) + engine.150 (@24–@25) + engine.152 (@26) + engine.118 (@27–@28) + engine.153 (@29); bench SANDBOX 0831 @22 == live @29 at C120. Prior: S411 line below.)
