...
| civic.26 | Civic Sunday chain halts at mayor-open, so nothing has applied since C104 — `mistralai/mistral-large` returns HTTP 429 (`upstream_provider_shared_pool`, model still listed and valid) and the caller retries twice ~0.4s apart with no backoff and no fallback seat (`scripts/civic-office-map.json:36`). `close_c104.json` stays `applied:false`; INIT-001/005/007 stuck at `NextActionCycle=C104` — the "3 past-due" pre-flight warning. Second, independent blocker on the same apply: `gate_c104.json` fails `engine-verbiage` (council_d3 quoted "0.42 sentiment"), so clearing the 429 alone will not let C104 apply. Scope: fallback seat + real backoff, unwrap the provider error so the log prints the 429 and its remedy, and clear the verbiage row. | ready | engine-sheet | [[../../output/production_log_run_cycle_c105_gaps]] §G-PF8, §G-PF10 |
