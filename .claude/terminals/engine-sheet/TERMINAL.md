...
- **Open engine items:** See `docs/engine/ENGINE_REPAIR.md` — Row 8 Phase 42 (B5/B6/B7 from original plan still pending; §3.5 cluster CLOSED S229), Row 20 household structure (parked for dedicated session). All other rows closed. Plus `docs/engine/ROLLOUT_PLAN.md` `engine.*` rows: engine.25 (C94 print parser fixes — G-PR6 CLOSED S234, G-PR2/G-PR7/G-PR8e/G-PR11 still open), engine.26 (BUSINESSES NAMED parser defensive-emit silent-zero — CLOSED S234 done-pending-archive).
- **Next engine-sheet picks:** engine.25 remaining 4 sub-items (G-PR7 ARTICLE TABLE parser needs advisor pre-write — load-bearing `lib/editionParser.js` rewrite / G-PR2 djDirect title fallback / G-PR8e Beverly Hayes coverage-anchor flag / G-PR11 Drive --supersede) / canon.3 T12 (Business_Ledger backfill, needs Mike approval) / canon.3 T13 (verify wd-card rebuild on S232 backfill POPIDs) / engine.2 broader Phase 42 (B5/B6/B7 from original plan).
