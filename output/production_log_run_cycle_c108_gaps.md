...
- **Status:** OPEN — sim call for the builder: does an owning household relocate at all, and if so does it sell (HousingCost → savings, back to rented at the destination lease) or carry two homes? engine.251's relief writer already refuses to stamp gross/relief on an owned mover (a3bd3343+); the mortgage rewrite itself is untouched.
