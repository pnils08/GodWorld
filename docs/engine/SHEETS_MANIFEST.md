...
| `phase05-citizens/householdFormationEngine.js` | own tracking tab (Household_Ledger formation / income / stress / dissolution); **engine.259 `applyFundDisbursement_`** (the fund moves, builder 2026-09-24 — engine.255's writer taken off housing) arms `LastGrantCycle` / `LastGrantInitiativeID` / `LastGrantAmount` on `Household_Ledger` the first time a grant is paid and writes them + `HouseholdSavings` (grant added, same-Cycle so stress and the dissolution roll see it; the durable money is the head's `NetWorth` on `ctx.ledger`) as column vectors before `detectHouseholdStress_`; the tracker debit (`BudgetRemaining`, `LastDisburseCycle`, phase `complete` at zero) is a Phase-10 cell intent, not a direct write | own-tab | Tier-5 engine |
| `phase05-citizens/generationalWealthEngine.js` | own tracking tab; READS `Business_Ledger` (engine.135 D3 `applyTrackedEmployerFloor_`, Avg_Salary by BIZ_ID); `Household_Ledger` HousingType / MonthlyRent / HousingCost on a purchase (`trackHomeOwnership_`, engine.65 T2 — the three `setValue` cells that flip rented → owned; engine.158 gates the roll, write shape unchanged); `decrementHeritageHomesLate_` (owner move 2026-09-23) — one `HomesOwned` cell per line on Heritage_Ledger when a sale lands after Step 7 this Cycle (the Phase-5 mover runs after the wealth engine) | own-tab | Tier-5 engine; Income floor writes go through `ctx.ledger` → Phase-10 persist; the purchase's NetWorth / LifeHistory mutations ride `ctx.ledger` too |
