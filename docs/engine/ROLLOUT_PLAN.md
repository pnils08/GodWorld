...
| engine.144 | Education → career, all four builder loops LIVE: credential as 2nd E2/E3 key (@16); field-first settlement (@17); youth-mode gate (@18); **youth texture — the youth engine is the minors' whole texture (@19, S411)**. Backlog closed: 5 → engine.149, 6 → engine.145, 7 → engine.148 — all LIVE | done-pending-archive | engine-sheet | [[../plans/2026-09-01-education-career-connection]] |
| engine.148 | One year formula (backlog 7) — `simYearFromCycle_`/`simYearOf_` in the calendar file; 25 per-site `2040+floor(cycle/52)` derivations in 12 files replaced (they disagreed with the calendar on cycle 52·n); casino `ageYear = 2040 + S.simYear` (≈4082, 18+ gate never held) fixed same commit. LIVE PROD @22 S412 (bench @13 C111 clean); detail in plan §Changelog | done-pending-archive | engine-sheet | [[../plans/2026-09-01-education-career-connection]] |
| engine.149 | Graduation follows the field (backlog 5) — builder ruling S412 trade-cert = associates: `credentialRank_` re-rate (3, level with associates) + `graduationCredential_` takes `trade-cert` at the hs-diploma step when the CURRENT field is Trades / Construction & Baylight / Port & Labor / Transit & Infrastructure. LIVE PROD @23 S412 (bench @14 C112 clean); detail in plan §Changelog | done-pending-archive | engine-sheet | [[../plans/2026-09-01-education-career-connection]] |
| engine.152 | Coverage runs both ways (bloodline cut 2) — `Citizen_Media_Usage.Sentiment` classified at Saturday publish (cheap model, `classifyCitationLight`); a negative citation counts −1 on UsageCount (floor 0) + a hard-light life line; earned count nets hard citations; a hard mention is no GC lottery ticket; bylines carry no sentiment. LIVE PROD @26 S412 (bench @19 C117 hard-light proof clean); first live sentiment at the 09-05 Saturday run. **Feed fix S412:** quoted citizens credited at the canon door (`ROLE_TO_USAGE quoted-source → quoted`, Node-side) — the interview wake never wrote usage rows, 4 citizen citations in 15 cycles | done-pending-archive | engine-sheet | [[../plans/2026-09-02-bloodline-ascent]] §Build order 2 |
| engine.153 | A single can own a home (bloodline cut 4) — solo household on income alone (the Tier ≤ 3 gate dropped; 85k floor + 10 %/cycle stay), so home purchase — household physics — opens to a Tier-4 earner; courtship step reads housing: owned ×2.0 > rented ×1.5 > none. LIVE PROD @29 S412 (bench @22 C120: 19 solo formed, 3 homes bought, 0 errors) | done-pending-archive | engine-sheet | [[../plans/2026-09-02-bloodline-ascent]] §Build order 4 |
| engine.150 | The ladder as a state (bloodline cut 1) — Tier follows the EARNED count (min(cell, citations on record)) every cycle, upward only; seeded cells held (gifted era); promotions write the `Updated to Tier N` marker decay reads (the old climb logged `Advanced from Tier`, so media rungs never demoted). LIVE PROD @24→@25 S412 (bench C113–C116 clean); + intake guard (never lowers); earned = usage-climbed only; MEDIA/CIVIC held from decay; detail in plan §Changelog | done-pending-archive | engine-sheet | [[../plans/2026-09-02-bloodline-ascent]] §Build order 1 |
| engine.147 | Bloodline ascent — a generic citizen's path Tier-5 → Tier 1 → heritage, and the inverse; a cron that maneuvers Tier + dial state. MEASURED S412 + builder chain direction captured (plan §The chain as directed vs as built); nine cuts in chain order (plan §Build order, revised), first = engine.150 ladder-as-state | ready | research-build / engine-sheet | [[../plans/2026-09-02-bloodline-ascent]] |
| engine.142 | Dead-file delete batch (G-PF32 S-D): 6 retired/never-wired files, 0 live refs, each tagged `delete candidate engine.142`; cost is ~15 truth-doc rows + STUB regen, same commit; clasp must drop remote files | ready | engine-sheet | [[../plans/2026-08-31-c105-chase-sessions]] §S-D DONE |
| engine.141 | Storyline_Tracker still has TWO live engine readers — `monitorStorylineHealth_` and (S408) `storyHookEngine_` via the v3 registry; caller-graph pass, repoint or retire both, dispose of `updateStorylineStatusv1.2.js` | ready | engine-sheet | [[../engine/SHEETS_MANIFEST]] §Storyline_Tracker |
...
| engine.118 | Fame is permanent (bloodline cut 3; re-spec builder 2026-09-02: bar = **UsageCount 25 on the cell**, not FameScore) — `Famous` column BC; crossing writes `C<cycle>`, floors Tier at 1 forever, freezes the row out of decay; a hard light on a famous name dims Cultural_Ledger FameScore instead of the count. Coded + unit-proven S412 (42/42); live dry-run: five assume fame at the first fire (Keane, Dillon, Paulson, Aitken, Kelley — all Tier-1 GAME today). LIVE PROD @27 S412 (bench @20 C118 clean); **cut 3b** (famous → Cultural_Ledger row registered / brightened to the bar) LIVE PROD @28 S412 (bench @21 C119 clean) | done-pending-archive | engine-sheet | [[../plans/2026-09-02-bloodline-ascent]] §Build order 3 |
