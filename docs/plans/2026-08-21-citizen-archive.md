...
- **Landed 2026-09-06 (S428, SANDBOX 0831 @61/@62, C108–C111).** Bench seeded from live C105 + engine.119 proofs (SL 991 rows, max POP-01106). Sequence and read-backs:
  ...
  - **After the restore — C110 + C111 (two cycles advanced in the window; one fire was this terminal's, the other's source is not identifiable from here — no GCP logs; both clean, 0 Engine_Errors):** POP-00026 still Active with `ReturnedCycle 109`, one archive snapshot (not re-archived); two natural deaths at C111 (POP-00431, POP-00901) archived at `ExitCycle 111`; archive 58 rows; SL 965; mark 1137. The steady-state mover works on organic exits, not only the backfill batch.
