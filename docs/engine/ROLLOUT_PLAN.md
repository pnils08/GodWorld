...
| engine.175 | Undocked cast drawn in the engine + show markets keyed to the pilot — fire N draws cycle N+1's 3 pilots + alternates (`ctx.rng`, undockedDraw eligibility) into `S.undockedNextCast` + a draw tab (NEW TAB — builder nod); `credits_sign`/`mishap` resolve on the pilot's aggregated aired rows for the cycle, `night_winner` unchanged, `casinoUpcoming_` retired. Fixes the never-opened show market (off-by-one). Rulings confirmed builder-direct 2026-09-08 (S438). Live tab creation gated on kimi's minting + briefs; bench first | in-progress | engine-sheet (engine draw + casino re-key + tab create) / kimi (minting, briefs, orchestrator draw-tab read, Nia-slice standings wire — builder go S438; contract = plan (a′)) | [[../research/2026-09-08-kimi-casino-undocked-trueup]] §Review + [[../plans/2026-08-07-spacemolt-game-show]] §Post-ship open items (a)(c)(e) |
| engine.176 | Dials: the negative pole from causes + ambient as tint (Cut B) — Friction/Strain/Stumble/Setback off rent burden, DebtLevel, unemployment, money loss; hood tint off persisted Neighborhood_Map; one pressure tag per citizen per cycle, adapts after 6; ambient ±1. CUT LOCALLY S438, unit 33/33 + suites green, unbenched — waits for a fresh live re-sync after engine.175 clears the bench. Lands WITH engine.177 | in-progress | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.176 |
| engine.177 | Dials: fold every cycle behind a sim-cycle watermark, mood persisted + decays (settleCycle_ gains its first caller), legacy lines fold once at trim, unlived capture at arrival, edge-damped hardening (no pin either direction). CUT LOCALLY S438, dial suites green, offline C106 catch-up replay clean (13,924 entries, pins 5→5), unbenched. Lands WITH engine.176, never alone | in-progress | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.177 |
| engine.178 | Dials close doors (Cut C) — openness gates the misfit move, integrity/composure gate the casino around posture, warmth+family consolidated to bands in bondEngine (face regex retired) with warmth as maintenance, owner composure/drive on closure/expansion (owner by POPID or name off Key_Personnel), integrity on the scandal ceiling (holder by name). CUT LOCALLY S438, dialGates 30/30 + touched suites green, unbenched — one bench cycle after 176+177's twelve | in-progress | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.178 |
| engine.179 | Contests resolved by character (Cut D) — contestRoll_ (two dials, weight 1, p=0.5+0.08×gap clamp 0.2–0.8, credential on the band scale) at job slot (last slot, same need band), heritage stake (wealth the floor), civic challenger (second draw), romantic triangle (loser's bond skips the tick); §10 lotteries fenced; S.contests on the Phase 9 log. CUT LOCALLY S438, contestRoll 31/31 + touched suites green, unbenched — one cycle after 178's | in-progress | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.179 |
| engine.180 | The cron reads the game (Cut E) — stance()/thrown() in lib/citizenDials.js, three fact lines in wake/exchange/voice, posture-changed wake slot, Resolves → one-notch posture push applied after Phase5-Maneuver and never over a cause-held retreat; desk render of the temperament REVERTED (builder: the quote already carries it); world_state.dispositions dropped. CUT LOCALLY S438, cronStance 32/32 + wake/maneuver suites green; proves on the next scheduled wakes after deploy | in-progress | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.180 |
| engine.181 | The citywide floor (Cut F) — WATCH: after 176+177 run three live cycles, if negative share > 35% or a hood's mean composure band hits −1, design the RimWorld floor | watch | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.181 |
| engine.182 | Ambition costs (builder go 2026-09-08: 'what does that trait do — make her sick, on drugs, in the hospital?') — overwork as a pressure cause (drive +2 with composure ≤ 0 for 3 cycles → Strain, never adapts while drive stays +2), burnout opens the health read + the conduct throttle (composure −2 as a second crimeReachable key), consequence-shaped POLES words; inside engine.176's system, no new dial. CUT LOCALLY S438, pressureTags 41/41, unbenched — rides the 176+177 bench | in-progress | engine-sheet | [[../plans/2026-09-08-dials-as-a-game-plan]] §engine.182 |
