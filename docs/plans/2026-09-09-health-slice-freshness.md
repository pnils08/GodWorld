...
4. (Engine, post-land) Phase-9 compressed digest reports real `migration`/`economy` values and `saveV3NeighborhoodMap_` emits non-'Stable' demographic labels when drift is non-zero — **PROVEN on bench @7 / C110 (engine-sheet, 2026-09-10)**: `Neighborhood_Map.DemographicMarker` went `Stable` ×18 + `Civic pressure zone` ×4 → `Inflow surge` ×17 + `First Friday arts walk zone` ×1 + `Civic pressure zone` ×4. Zero hoods read 'Stable'; `driftNum` picked up the real migration (1306) instead of the string-coerced 0.
