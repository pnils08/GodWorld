...
// ---- engine.152 (S412): the light a citation casts ----------------------------------------
// Coverage runs both ways (builder, 2026-09-02): a citizen named in a hard light loses a
// count where a kind one gains. The desk writer's article is the only place the judgment
// exists, so the classifier reads the sentences that name the citizen and returns one of
// three words. Same transport, same closed-vocab discipline as the reflection tagger.
const LIGHT_VOCAB = ['positive', 'neutral', 'negative'];
function buildLightPrompt_(name, snippet) {
  return 'A newspaper article names a citizen. Judge ONLY how the coverage reflects on that person — not the topic, not the mood of the piece.\n' +
    'Citizen: ' + name + '\n' +
    'Passage: """' + String(snippet || '').slice(0, 1200) + '"""\n\n' +
    'Answer with exactly one word: positive (they come off well — praised, achieving, trusted), neutral (named without judgment), negative (they come off badly — blamed, failing, accused, mocked).';
}
// Pure: first in-vocab word in the model's output, else null.
function extractLight_(raw) {
  const toks = String(raw || '').toLowerCase().match(/[a-z]+/g) || [];
  for (const t of toks) if (LIGHT_VOCAB.includes(t)) return t;
  return null;
}
async function classifyCitationLight(name, snippet) {
  if (!String(snippet || '').trim()) return { light: null, raw: '[no-snippet]' };
  let raw;
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      ...
      body: JSON.stringify({ model: MODEL, max_tokens: MAX_TOKENS, temperature: TEMP, messages: [{ role: 'user', content: buildLightPrompt_(name, snippet) }] }),
    });
    const j = await r.json();
    if (j.error) return { light: null, raw: '[err] ' + (j.error.message || JSON.stringify(j.error)) };
    raw = String(j.choices?.[0]?.message?.content || '').trim();
  } catch (e) {
    return { light: null, raw: '[err] ' + e.message };
  }
  return { light: extractLight_(raw), raw };
}
...
module.exports = {
  LIGHT_VOCAB, extractLight_, classifyCitationLight, buildLightPrompt_,
  MODEL,
  VOCAB,
  VALID,
  AFFECT_TAGS,
