...
| T8 | **Memory as a RAG brain, not a boot load — builder direction 2026-08-29 (the why, protected).** A core base of hard rules beats a long list of soft ones; the memory list grew past what any instance can manage. Two behaviours must be *enforced*, not asked for: (a) no new memory file without first checking whether a similar one exists (T4 states it; the enforcement is a hook — extend `memory-write-gate.sh` so a write under `memory/` requires a recent `brainSearch` marker for the fact); (b) no change-logs or notes stacked onto load-bearing files — a new instance needs the current rule, never the evolution of it (`boot-doc-guard` covers boot docs; the rule holds for memory files too). Once the directory is maintained (T3), the memories rotate into Supermemory — either `sl-godworld` or a new `sl-memories` container — so persistence is retrieval on demand through `brainSearch` (claude-mem + `sl-godworld` + `session-logs` + `sl-memories`), not front-loaded every session; established doctrines may join the same store. Design the rotation (what qualifies, cadence, dedup against what is already in the container) before building. | engine-sheet | open |
