...
**Current bench pointer:** SANDBOX 0908 web app @26 (`35a087a7`, engine.213 approval-reads-the-city). Bench fired C106→C107 ok:true 182s, 0 Engine_Errors, six `approval*` World_Config keys self-armed; approvals moved +1/0/-1 across the ten seats (Ashford 45→47, Santana 64→65 — the bench tracker predates the 18:36 live apply so no phase move credited there). Proof in [[DEPLOY_HISTORY]] §PROD @82. The following prior Bench paragraph is historical/superseded and does not describe the current bench.
...
**Live:** PROD **@82** (2026-09-13 ~19:12 Chicago, engine.213 approval reads the city) = repo commit `35a087a7`; `clasp push` + version 71 + `clasp deploy -i -V 71`, deployments read back @71; pull-back 168 files byte-identical, 0 test files. Bench @26 C107 clean. Live remains C106, unfired — C107 is the smoke for @75–@82. Full details: [[DEPLOY_HISTORY]] §PROD @82.
