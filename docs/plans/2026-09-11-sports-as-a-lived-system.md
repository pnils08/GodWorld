...
**`WeekRecord` is APPENDED as column U** (engine-sheet schema call, 2026-09-18 — supersedes "repurpose `VideoGame`"). Measured: `VideoGame` holds 81 historical cells (C30–C83, "MLB The Show 25") and `VideoGameDate` 58; a rename would file them under a `WeekRecord` header for every all-cycle reader (projection, desk packets, beats dump), and the engine only reads current-cycle rows so nothing would flag it. Appending keeps every existing physical position, so no positional consumer moves. Deleting `VideoGame` / `VideoGameDate` (and later `HomeNeighborhood`, after Task 3 moves transit onto the zone set) is a separate schema deletion on Mike's go; each step is one `FEED_LAYOUTS` entry in `scripts/sportsFeedContract.js`. The initial cut recognizes only the explicit new header; it never reinterprets historical `VideoGame` text as results. Existing Sheet headers stay authoritative until migration.
