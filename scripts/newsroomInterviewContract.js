...
function prepareInterviewPacket(packet) {
  ...
  out.output = {
    ...
    schema: {
      ...
      basis: 'lived-context|direct-reaction|null',
...
function validateInterviewOutput(value, input) {
  ...
  if (!Array.isArray(out.unverifiedLead)) errors.push('unverifiedLead must be an array');
...
function citizenEvidenceGuard() {
  return '\n\nINTERVIEW MODE: Return ONLY the JSON requested by packet.output. This is an interview happening now. Use the supplied story facts and the real simulated-life context above—your history, relationships, health, neighborhood, disposition, and private reflections—to answer as yourself. The quote field is your actual response and will be preserved exactly. Speak only for yourself. Your attributed memory, feeling, interpretation, or intention is speech, not independent proof of a public event. Do not invent a person, institution, event, official act, number, date, relationship, job, or place. If you have no grounded response, abstain. Never mention packets, data, records, prompts, or systems.';
}
