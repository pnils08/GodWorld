...
3. **Hospital capacity constant mismatch — DONE.** Builder ruled the sheet key wins. `phase10-persistence/buildCyclePacket.js`: the hardcoded `HOSPITAL_CAPACITY = 40` placeholder is deleted; census `load` now divides by `hospitalBaseCapacity` (100, World_Config) as surfaced by `applyDemographicDrift_` (W2b `hospitalConfig`), with a warn-logged 100 fallback. Talk-back behavior unchanged — it already bound at 100. Consequence: the census line the beat quotes reads 3% (honest) instead of 8% at the current census of 3.
4. **Stale twin city number — DONE with one deliberate exception.** `buildCyclePacket.js`'s packet `IllnessRate` line now publishes the fresh post-attractor `S.demographicDrift.illnessRate` (fallback to the old field). `utilities/cycleModes.js:328` was NOT repointed: it is `buildCycleChecksum_`, whose output is stored in `Cycle_Seeds` and compared verbatim by `compareReplayOutput_` on replay — changing the formula would false-MISMATCH every historical cycle replayed with new code. A checksum is not a published number; ruling scoped to the publication. Builder to confirm.
