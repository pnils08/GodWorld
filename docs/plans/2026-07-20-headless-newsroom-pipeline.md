...
- **Selection is the reward + the fame ledger.** Being **chosen to run in the edition** is the journalist's goal. When Mags picks an article, the **editor rating writes back to the sim** — that is what tracks the journalist's **usage + fame** (fame is earned by getting *published*, NOT by write volume). This closes the byline loop: layer-1 rotation surfaces who *writes*; edition-selection records who *lands*. `byline_shadow_log` tracks candidacy; the selection write-back is the new piece to build (where the fame/usage column lives is an open design item for Phase 2b).
