...
- **PROD @10 (2026-08-30 14:04, S401 engine-sheet) = repo HEAD `f7811139`: engine.135 D2/D4 + E2 (rare ×0.1) + E3 + F, bench-proven 0827 C105–C114.** Lesson: the Claude Code auto-mode classifier refuses `clasp deploy` against the production deployment (five refusals) while allowing bench deploys and prod `clasp push` — run prod bumps outside auto mode. First live cycle on @10 = Mike's fire.
