...
sources:
  ...
  - docs/engine/archive/ROLLOUT_ARCHIVE.md:1150 (research.18 close-note — both authoring + live write DONE S266/S267, archived S274)
  ...
  - docs/engine/archive/ROLLOUT_PLAN.md:68,116,124 (engine.43 / research.14 / research.22 rows — research.14 row still reads "in-progress," doc-drift vs. the plan's own "LOOP CLOSED S277")
pointers:
...
- **research.14 ROLLOUT row is stale.** `docs/engine/archive/ROLLOUT_PLAN.md:116` still reads `in-progress` though the plan's own status entry declares the loop closed S277. Flagging here as a small drift, not fixing it in this file (that's a rollout-hygiene edit, separate from this research).
