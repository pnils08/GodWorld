...
'use strict';
/**
 * engine.259 — the fund moves (builder 2026-09-24). engine.255's disbursement,
 * recovered and taken off housing: keyed on the `disbursement-active` phase, any
 * domain; grants onto stressed households owned or rented.
 *
...
const E = new Function(SRC + '\nreturn { applyInitiativeImplementationEffects_, buildDisbursementSlice_, getCivicDisburseDials_, planFundDisbursement_, applyFundDisbursement_, FUND_GRANT_COLUMNS_, CIVIC_STAGE_CATALOG_ };')();
...
const TR = (id, status, phase, stage, hoods, mayoral, lsc, remaining, domain, lastDisb) =>
  [id, 'Fund ' + id, status, domain || 'economic', hoods, phase, '$28M', mayoral, stage, lsc || 108, lsc || 108, 28000000, remaining == null ? 28000000 : remaining, lastDisb == null ? '' : lastDisb, 'notes'];
...
// ---- keyed on phase, never domain ----
{
  const ctx = ctxWith([TR('INIT-001', 'passed', 'disbursement-active', 'Standing', 'West Oakland', 'signed', 108, 28000000, 'economic'),
    TR('INIT-960', 'passed', 'disbursement-active', 'Standing', 'Laurel', 'signed', 108, 28000000, 'health')], [], {}, 110);
  E.applyInitiativeImplementationEffects_(ctx);
  ok(ctx.summary.initiativeDisbursement.programs.map(p => p.initiativeId).join() === 'INIT-001,INIT-960', 'any domain in disbursement-active spends — the live INIT-001 (economic) shape included; no housing program');
}
...
// ---- producer → slice ----
{
  ...
  ok(p.initiativeId === 'INIT-001' && p.domain === 'economic' && p.tranche === 400000 && p.remaining === 28000000 && p.tend === 1 && p.hoods.join() === 'West Oakland' && p.sheetRow === 2, 'tended standing signed program: tranche = dial, hoods folded, sheet row carried');
...
{
  const ctx = ctxWith([TR('INIT-911', 'passed', 'disbursement-active', 'Standing', 'Laurel', 'signed', 108, 1234.5)], [], {}, 110);
  E.applyInitiativeImplementationEffects_(ctx);
...
{
  const ctx = ctxWith([TR('INIT-912', 'passed', 'disbursement-active', 'Standing', 'Laurel', 'signed')], [], { civicDisburseTranche: '' }, 110);
  let threw = false; try { E.applyInitiativeImplementationEffects_(ctx); } catch (e) { threw = /civicDisburseTranche/.test(e.message); }
...
function pool() {
  return [
    ...
    HH('HH-904', 'West Oakland', 'owned', 3000, 88000, 0),               // owned warning 0.409 → eligible (not a housing program), last in order
...
{
  const hdr = HH_HEAD.concat(E.FUND_GRANT_COLUMNS_);
  ...
  const plan = E.planFundDisbursement_(hdr, rows, program, 110, h => h);
  ok(plan.eligible === 4 && plan.skipped.inactive === 1 && plan.skipped.offHood === 1 && plan.skipped.unflagged === 2 && plan.skipped.cooldown === 1 && plan.skipped.alreadyThisCycle === 1, 'eligibility matrix: an owned stressed row counts; dissolved, off-hood, buffered, low-burden, cooldown and same-Cycle rows excluded and counted');
...
}
{
  ...
  const plan = E.planFundDisbursement_(hdr, rows, { initiativeId: 'X', tranche: 400000, remaining: 1e6, hoods: ['West Oakland'], grantCapMonths: 6, grantHeadroomMonths: 1, cooldownCycles: 26 }, 110, h => h);
...
{ // fresh-row trap (bench C110/C111): cell savings 0, head holds 400k on the ledger → unflagged
  ...
  const plan = E.planFundDisbursement_(hdr, rows, { initiativeId: 'X', tranche: 400000, remaining: 1e6, hoods: ['West Oakland'], grantCapMonths: 12, grantHeadroomMonths: 1, cooldownCycles: 26 }, 110, h => h, savingsOf);
  ...
  const noLedger = E.planFundDisbursement_(hdr, rows, { initiativeId: 'X', tranche: 400000, remaining: 1e6, hoods: ['West Oakland'], grantCapMonths: 12, grantHeadroomMonths: 1, cooldownCycles: 26 }, 110, h => h, () => NaN);
...
// ---- apply: household vectors + ledger + intents + tracker debit ----
{
  ...
  const ctx = ctxWith([TR('INIT-001', 'passed', 'disbursement-active', 'Standing', 'West Oakland', 'signed')], pool(), { civicDisburseTranche: 50000 }, 110, ledger);
  E.applyInitiativeImplementationEffects_(ctx);
  const out = E.applyFundDisbursement_(ctx, 110);
  ...
  ok(v[0].slice(13).join(',') === E.FUND_GRANT_COLUMNS_.join(','), 'receipt columns armed after the live 13');
  ...
  ok(v[3][col('HouseholdSavings')] === 500000 && v[4][col('LastGrantCycle')] === '' && v[6][col('LastGrantCycle')] === '', 'buffered row, off-hood row, and the owned row past the tranche untouched');
  ...
  ok(appends.length === 3 && appends.every(a => a.tab === 'LifeHistory_Log' && a.row[3] === 'Relief' && /received a \$\d+ grant from Fund INIT-001 \(INIT-001\)/.test(a.row[4])) && /breathing room/.test(appends.find(a => a.row[1] === 'POP-99901').row[4]) && /partial/.test(appends.find(a => a.row[1] === 'POP-99902').row[4]), 'three LifeHistory lines, tag Relief, naming the fund; full vs partial worded');
  ...
  appends.length = 0; cells.length = 0;
  const out2 = E.applyFundDisbursement_(ctx, 110);
...
{ // crash recovery (2026-09-23, kimi): household vectors committed, Phase-10
  ...
  const ctx = ctxWith([TR('INIT-001', 'passed', 'disbursement-active', 'Standing', 'West Oakland', 'signed')], hhRows, { civicDisburseTranche: 50000 }, 110, ledger);
...
{ // two funds over the same hood in one fire: neither suppresses the
  ...
  const rows = [
    ...
    TR('INIT-950', 'passed', 'disbursement-active', 'Standing', 'West Oakland', 'signed', 108, 500000),
  ];
  const ctx = ctxWith(rows, pool(), { civicDisburseTranche: 50000 }, 110, []);
  E.applyInitiativeImplementationEffects_(ctx);
  const out = E.applyFundDisbursement_(ctx, 110);
  ...
  ok(out.grants === 4 && progs[0].grants === 3 && progs[1].grants === 1 && paidIds[1] === 39000, 'fund B skips fund A\'s receipt-stamped grantees and pays only the row A\'s tranche never reached (HH-904, 12 × 3000 + 3000)');
}
{ // tracker already stamped this Cycle → skip program
  appends.length = 0; cells.length = 0;
  const ctx = ctxWith([TR('INIT-001', 'passed', 'disbursement-active', 'Standing', 'West Oakland', 'signed', 108, 28000000, 'economic', 110)], pool(), {}, 110, []);
  E.applyInitiativeImplementationEffects_(ctx);
  const out = E.applyFundDisbursement_(ctx, 110);
...
// ---- seeds ----
{
  ...
  ok(['civicDisburseTranche', 'civicGrantCapMonths', 'civicGrantHeadroomMonths', 'civicGrantCooldownCycles'].every(k => keys.includes(k)) && arr.every(x => Array.isArray(x) && x.length === 6), 'four fund dials seeded as 6-tuples');
  ok(!keys.some(k => /housing/i.test(k) && /Disburse|Grant|Cohort/.test(k)), 'no housing-named disbursement key');
  const by = Object.fromEntries(arr.map(x => [x[0], x[1]]));
  ok(by.civicDisburseTranche === 400000 && by.civicGrantCapMonths === 12 && by.civicGrantHeadroomMonths === 1 && by.civicGrantCooldownCycles === 26, 'seed values as the builder ruled for engine.255 (400k / 12 / +1 / 26)');
}
// ---- catalog parity ----
{
  const C = require('../lib/initiativePhaseContract');
  ok(JSON.stringify(E.CIVIC_STAGE_CATALOG_) === JSON.stringify(C.stageCatalogByDomain()), 'engine stage catalog deep-equals the lib projection');
  ok(Object.keys(C.INTERVENTION_CATALOG).filter(k => C.INTERVENTION_CATALOG[k].disburses).length === 0, 'no domain flag decides the money — the phase does');
}

console.log('fundDisbursement.test.js: ' + n + ' assertions passed');
