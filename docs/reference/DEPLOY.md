...
**Bench:** `SANDBOX 0831` (stood up 2026-08-31) — copy of live post-C105. Code at **@47** (= live @51 tree, 166 files), cycle **C110** — re-synced from live at C105 (S419); C106–C108 fired on the synced state (S419), C109–C110 (S423, engine.134 T4–8 + civic.18 4c/4d proof). **Bench-only data since the sync — never replay:** Intake rows 4–9 (two engine.109 test households, Bell + Okafor) and the citizens/households they minted (POP-01085–01088, POP-01090–01092, HH-0107-I001, HH-0108-I001); Intake columns `Relation` (J) and `Sex` (K) — live self-arms `Relation` at its next fire. Spreadsheet `18BOJmzlO7EoaEhvUsUaqLIZvgrTC1yltUYk_Gz3I3W8` / Apps Script `1_3PDs7CSUsYvjjcXWtYGKjpPd7ekiIsUKKLSwlMtj3ioQKV0ew8TAfPh` / web-app deployment `AKfycbxbfP2Wu6haP4HXUGLk9T4wlW4ixWPsnfZJ51G5wVFTbkhESbT4teJG2xvRowP3BygIkw`. Script Properties `SIM_SSID` + `CYCLE_TRIGGER_TOKEN` set, authorized (protocol step 3b done). **`CARRY_FORWARD_COLD_START_OK` — do not set**: the bench's `Carry_Forward_Store` sheet already carries real C105 carry-forward, so this is not a cold start (see [[DEPLOY_HISTORY]] for the read-back that proved it). Full per-wave trail: [[DEPLOY_HISTORY]] §Current sandbox.

**Live:** PROD **@51** (2026-09-05 02:35, S423) = engine.134 Tasks 4–8 + civic.18 4c/4d over @50 (8 files: `canonNeighborhoodLoader.js`, `updateCrimeMetrics.js`, `updateNeighborhoodDemographics.js`, `updateCivicApprovalRatings.js`, `buildEveningFamous.js`, `buildEveningFood.js`, `ensureCrimeMetrics.js`, `ensureNeighborhoodDemographics.js`; 166 files); @50 = engine.109 the household door over @49 (`godWorldEngine2.js` + `processAdvancementIntake.js`); @49 = engine.141 over @48; @48 = engine.142 over @47; @47 = civic.32 over @46 (= engine.162 + civic.30/civic.31 wave over @45); HELD engine.131 T7 four (`applySportsSeason.js`, `generateCrisisSpikes.js`, `economicRippleEngine.js`, `v3NeighborhoodWriter.js`) stay at base — repo HEAD differs from live by exactly those four. **Six deploys @46–@51 are unsmoked on live** — all bench-proven on live-synced C105 state; the builder's next fire (C106) smokes them together. Proofs: `docs/plans/2026-08-30-hood-identity-remainder-plan.md` §Changelog (S423), `docs/plans/2026-08-29-employment-system-cascade.md` §Changelog 2026-09-04 (S417 wave, S419 civic.32); `docs/plans/2026-08-31-c105-chase-sessions.md` §S-D (engine.142). Full deploy ledger back to @1: [[DEPLOY_HISTORY]] §PROD deploy log.
