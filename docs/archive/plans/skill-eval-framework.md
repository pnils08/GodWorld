...
pointers:
  - "[[engine/archive/ROLLOUT_PLAN]] — parent; was embedded in Open Work Items"
...
- 2026-04-16 — Extracted from [[engine/archive/ROLLOUT_PLAN]] §Open Work Items (S152). Was a ~35-line embedded design — substantial enough to warrant its own plan file. Content preserved verbatim.
...
- 2026-05-10 (S212, research-build) — **Action plan filed** for the next-step expansion: [[2026-05-10-skill-eval-expansion]]. 9 tasks covering 3 new assertion files (`docs/media/city_hall_evaluation.md`, `dispatch_evaluation.md`, `interview_evaluation.md`) + skill-check wiring verification + post-publish Step 10 verification + smoke test against C93. Filed via [[../adr/0005-rollout-plan-structure]] §How to add work — `governance.2` ROLLOUT row points at the action plan + this parent framework as secondary. Validates S212 filing protocol end-to-end. Estimated 1-2 research-build sessions to close.
- 2026-05-12 (S216, research-build) — **governance.2 closed: 3 new assertion files shipped + skill-check map extended + post-publish Step 10 extended + index registration.** Files: `docs/media/city_hall_evaluation.md` (8 criteria — decision resolution / engine-effect linkage / council identification / vote math / canon-substitute / cascade order / canonical paths / voice coverage), `docs/media/dispatch_evaluation.md` (8 criteria — location specificity / identity-through-action / image-ending / word count / reporter pool / format contract / canon-substitute / citizen depth), `docs/media/interview_evaluation.md` (9 criteria — theme / canon grounding / question progression / subject canon consistency / file paths / archetype-match / canon-substitute / world-altering canon flagging / voice differentiation). `.claude/skills/skill-check/SKILL.md` v1.0 → v1.1: Skill→assertion map corrected (the two TBD entries pointed at paths `docs/civic/decision_evaluation.md` + `docs/media/scene_evaluation.md` that were never built; replaced with actual shipped paths under `docs/media/`) + interview added as 5th target. `.claude/skills/post-publish/SKILL.md` Step 10 extended from hard-coded write-edition only to a 5-skill conditional table — each skill's /skill-check fires only if its cycle artifact exists. Open Question 1 (generic vs hard-coded) resolved: skill-check is generic, accepts `$1` skill-name arg. Open Question 2 (file location): chose `docs/media/` over `docs/civic/` for consistency (sole-file dirs are overkill). Open Question 3 (refinement cadence): Mags refines per cycle's evidence via post-publish Step 10; Mike approves substantive shifts. Tasks 1-9 of the action plan all complete except smoke-test Task 7 (deferred — needs an actual /post-publish run on a cycle to validate end-to-end; will surface on first C94 post-publish).
