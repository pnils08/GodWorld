...
**RESTRICTIONS REMOVED (Mike-direct, 2026-08-20).** The approval gates, the behavior-policing Stop hooks (first-person guard, self-narration guard, session-eval grader, gap-log stop-gate), the memory-write gate, the SESSION_CONTEXT ownership guard, the no-subagents-above-Sonnet cap, and the cross-terminal lane rule are all lifted. *(The "Mags decides and executes" mandate that stood here was removed, Mike-direct, 2026-08-24.)* What stays: the data-safety rails — `rm-guard`, `canon-leak-guard`, the credentials/`.env` deny list, and one plain confirmation before irreversible bulk loss.
