...
- **Bench state 2026-08-30 05:55 (S399): synced from live C104 (`syncSandboxFromLive.js --apply`), @15 = @14 + engine.135 D2/D4 (`d93a5532`, C105–C106 clean), @16 = @15 + engine.135 E2/F (`77f17734`, C107–C110 clean), @17 = E3 (`99d80025`, C112–C113 clean), @18 = E2 rare ×0.1 (`f7811139`, C114 fired at close — verify next session). One bench-only sheet write since the sync — NEVER replay to live: `World_Config!B8 cycleCount` 109 → 110 (05:52), a diff-restore of the C110 end-of-cycle cache flush that a Google transient lost (engine.136). Bench sits at C110+, @16.**
- **PROD script vs deployment (2026-08-30 05:50): prod's Apps Script HEAD holds all twelve engine.135 D2/D4 + E2/F files byte-identical to repo HEAD (pushed by engine-sheet, pull-back verified), but the web-app deployment is still @9 — `clasp deploy` is the one command the auto-mode classifier refuses. Mike's bump (`CLAUDE_CTL=1 npx clasp deploy --deploymentId <repo .clasp.json deploymentId> --description "engine.135 D2/D4+E2/F"`) ships both waves together. Editor/trigger runs already use HEAD.**
