...
| engine.120 | WealthLevel dual-writer drift — stale v14.2 formula deleted from `lib/economicLookup.js`, `applyEconomicProfiles.js` no longer writes the column (Task 1 DONE S381). Task 2 backfill SUPERSEDED: live check found 551/940 rows still on v14.2, not 17 — v15 deployed S378 but no cycle has persisted since (C103 predates the deploy; both C104 fires died at 0/28 intents). Column self-repairs on the first clean cycle. | done-pending-archive | engine-sheet | [[../plans/2026-08-18-wealthlevel-dual-writer-drift]] |
