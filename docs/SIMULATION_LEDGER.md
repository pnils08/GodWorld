...
**Simulation_Ledger rows:** 940 / max POP-01055 (S360 live — row-32 canon-drift mint +9; audit prints current count) | **Columns:** 55 (A–BC; **Famous BC added S412** engine.118 — `C<cycle>` when the citizen assumed fame at UsageCount ≥ 25, blank otherwise; permanent, Tier-1 floor, out of decay; **SkillTags BB added S336** employment-living-system — pipe-separated category tags from the 15-field taxonomy holding BOTH truths (current job + trained field), backfilled on 740 Active adults, stamped at settlement/mint, consumed by runCareerEngine v2.7's rehire matcher; **since engine.144 loop 1 the settlement writes the FIELD the kid was aimed at; since engine.145 (S411) the three catalog-only categories (`Trades`, `The Vulnerable`, `2041-Specific`) resolve to a business field through `skillTagField_` — 53 blank adults filled from role 2026-09-02, 2 left blank (not jobs); engine.146 (S411, PROD @21) writes the two truths as two pipe-joined tokens (current, then trained) at both role-change intake sites — token 1 = current job's field, token 2 = trained field when different; 24 disagreeing rows trued the same day**; **LineageId BA** (heritage line linkage); StatusStartCycle AY + HealthCause AZ added S312 engine.52 — activated dormant health-lifecycle wiring in `generationalEventsEngine.js`, which reads both by header name; added to prod AND sandbox; MemoryRegisters added at AX S282 — seams plan B1+B3 substrate, DialState-pattern JSON `{"biases":[...],"unlived":[...]}`, written ONLY by the Phase-9 compressor fold; `.biases` LIVE-in-code S283 Task 6 (bias-intent drain — cap 5, asymmetric reinforce/challenge), `.unlived` LIVE-in-code S283 Task 8 (branch-event capture at fold — cap 3); SMPageId at AW S262; DialState at AV S256, engine.31) | **Sim Year:** 2041
...
**Exits (engine.90, S428):** a citizen whose Status is `deceased` or `Traded` leaves this tab for `Citizen_Archive` — the row verbatim plus exit metadata — once the mover (`Phase11-CitizenArchive`, World_Config `citizenArchiveEnabled=1`) runs. POPIDs are minted from World_Config `popIdHighWater` (`utilities/popIdAllocator.js`), never from this tab's max alone, so an archived POPID is never reissued. A Traded citizen comes back by restore onto this tab under the same POPID (`ReturnedCycle` stamped). Plan: `docs/plans/2026-08-21-citizen-archive.md`.
...
| Col | # | Header | Valid Values | Writers | Readers | S321 Verdict |
|-----|---|--------|-------------|---------|---------|--------------|
| O | 15 | LifeHistory | Long text — accumulated life events. **Minors (age < 18 from BirthYear) receive youth-engine lines only since engine.144 loop 3/4 (S411, PROD @18/@19): the per-citizen adult decks step aside; the household shared moment still reaches them** | All event generators (append each cycle), compressLifeHistory; under 18 → `runYouthEngine_` only (+ the household moment) | buildDeskPackets, queryLedger, citizenContextBuilder, enrichCitizenProfiles | **CAUSAL** (S321 corrects the raw A–O auditor's RECORD call — see §Column Wiring) — `educationCareerEngine.js` `settleAdulthood_` gates the fire-once 18th-birthday settlement on `indexOf('[Adulthood]')`; `generationalWealthEngine.js` `processMoneyLoop_` checks `'crossed six figures'` once-only milestone; `deriveEducationLevels_` checks `indexOf('Graduation')`; `calculateCitizenIncomes_` extracts income band via `extractIncomeBand_` |
| P | 16 | SpouseId | 'POP-NNNNN First Last' (ID + name) | bondEngine `marryCitizens_` (both spouses); `wireFamilyMatch_` (family drip + engine.109 household intake) | spouse-drip tooling | **RECORD** (S321 regen caught live rename CreatedAt→SpouseId — the `marryCitizens_` 'prod no-ops until rollout rename' guard is now live-active; full verdict next audit) |
...
| U | 21 | HouseholdId | `HH-XXXX-XXX` format (`HH-CCCC-Innn` = household intake, engine.109) | householdFormationEngine, seedHouseholds, bondEngine weddings, `formIntakeHouseholds_` | generationalWealthEngine, migrationTrackingEngine, queryFamily | **CAUSAL** — `generationalEventsEngine.js:343-366` birth physics requires household; household reconciliation grouping |
| V | 22 | MaritalStatus | Single / Married / Divorced / Widowed | householdFormationEngine, `wireFamilyMatch_` / `formIntakeHouseholds_` (engine.109) | buildInitiativePackets | **CAUSAL** — `generationalEventsEngine.js:352` birth eligibility; bondEngine marriage/divorce logic |
| W | 23 | NumChildren | Integer | householdFormationEngine, `formIntakeHouseholds_` (engine.109: the count of child rows, never the profile's dice) | — | **CAUSAL** — 3-child cap in `checkBirth_`; `runHouseholdEngine.js:523` family flavor |
...
| Col | # | Header | Valid Values | Writers | Readers | S321 Verdict |
|-----|---|--------|-------------|---------|---------|--------------|
| AF | 32 | EducationLevel | Adults (credential, identity once set): hs-dropout / hs-diploma / some-college / associates / trade-cert / bachelors / masters / doctorate. Minors (<18, restamped from age every cycle — S409 education loop): Pre-K / Elementary / Middle School / High School. Plural vocab ONLY — the engine no longer writes `bachelor` / `graduate` | educationCareerEngine (`deriveEducationLevels_` fill-only, `deriveMinorEducationStage_` minors, `settleAdulthood_` at 18), generationalEventsEngine (`[Graduation]` writes the next credential via `graduationCredential_`), processAdvancementIntake (mint) | buildDeskPackets, migrationTrackingEngine, generationalWealthEngine `EDU_SAVINGS_FACTOR`, bondEngine `bondFitnessOf_`, `settleAdulthood_` parent rank | **CAUSAL** — savings yield 1.0-1.2x; courtship +0.08/+0.15; migration displacement-risk gate; parent rank into the 18th-birthday draw. NOT a job gate and NOT an advancement input (engine.135 E1 removed that; the S321 note here was stale) |
...
| AV | 48 | DialState | JSON 8-dial machine state (engine.31, S256); additive `maneuver: {p, g, a, c}` field since engine.157 (S416) — the cycle's posture memory, written by `runManeuverEngine_` on change, carried through the Phase 9 fold like `chaosExposure`; additive `pressure: {cause: {n, l}}` since engine.201 (S449) — per-cause pressure run written by `emitPressureTag_` (Phase 5), carried by `deserialize_` / `serializeDialState_` | Phase-5 dial engines | 9 Phase-5 engines via `getCitizenDialBands_` | **CAUSAL** — every phase-5 engine reads `getCitizenDialBands_` for frequency multipliers 0.5-1.5x: `runEducationEngine.js:409`, `runNeighborhoodEngine.js:438`, `runRelationshipEngine.js:477`, `runCareerEngine.js:838`, `runHouseholdEngine.js:505`, `runConductEngine.js:176`, `runYouthEngine.js:195`, `generationalEventsEngine.js:442`, `generateGameModeMicroEvents.js:515` |
...
| BA | 53 | LineageId | Text (`LIN-…`, lazy-created; heritage) | `generationalWealthEngine.js` heritage step (founding + inheritance) | heritage step, `heritageTierByPop_`, settlement | **CAUSAL** — line membership; +1/+2 on the 18th-birthday draw |
| BB | 54 | SkillTags | Text, pipe-separated (`current\|trained`, S336/engine.146) | settlement, E3 hire, role-change intake | E3 hiring, hood pay, graduation track (engine.149) | **CAUSAL** — the field a citizen stands in |
| BC | 55 | Famous | Text (`C<cycle>` when assumed, blank otherwise; engine.118 S412) | `applyTierLadderState_` at UsageCount ≥ 25 (the cell — canon on record) | ladder (Tier-1 floor), decay (row frozen), citation pass (hard light dims FameScore instead) | **CAUSAL** — permanence; only the shine moves after |
