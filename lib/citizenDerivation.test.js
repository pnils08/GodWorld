...
{
  assert('simYearFromCycle: the engine formula', cd.simYearFromCycle(52) === 2040 && cd.simYearFromCycle(53) === 2041 && cd.simYearFromCycle(105) === 2042 && cd.simYearFromCycle(156) === 2042 && cd.simYearFromCycle(157) === 2043);
  assert('currentSimYear reads the ledger snapshot cycle', cd.currentSimYear() === cd.simYearFromCycle(cd.currentCycle()) && cd.currentSimYear() >= 2042);
}
