...
| pipeline.66 | Retire the dashboard Chicago page — ChicagoTab still rendered (App.jsx), server.js still parses CHICAGO BUREAU + chicago_c{N} packets; Chicago is canon-retired (Mike-direct 2026-09-06). **CLOSED S429: ChicagoTab deleted, tab/menu/loader/state gone from App.jsx, `chicago` dropped from the hooks/arcs/storylines packet filters + newsroom desk list, legacy `/api/sports` endpoint removed (its only consumer was the tab), unknown `/api/*` paths now 404 instead of hanging. Edition parser keeps CHICAGO BUREAU so the 13 archived editions still parse. Remaining Chicago surface = `buildDeskPackets.js` still writing `chicago_c{N}.json` — engine.116 §Task 4 consumer retirement, not this row.** | done-pending-archive | research-build | [[../plans/2026-08-17-sheet-weight-reduction]] §Task 4 |
