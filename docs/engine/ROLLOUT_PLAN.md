...
| infrastructure.11 | claude-mem observer: refused calls were dropped unretried (200-body error filed unrecoverable). Bundle patched 09-21, retries 2→5, stays free (builder: never paid). Close after a day near-zero `Observer failed`; a plugin update reverts the patch — re-apply per plan §infrastructure.11 | in-progress | engine-sheet | [[../plans/2026-08-20-consolidate-model-calls-on-openrouter]]; `~/.claude-mem/logs/claude-mem-2026-09-20.log` |
