...
// ---------------------------------------------------------------------------
// engine.251 housing stage-3 cohort (plan 2026-09-20-housing-lever §Engine-sheet
// rulings 1-2). The Delivering comparator is hood-scoped over a cohort shape
// {available, tab, cycle, rows}; this builds that shape from Household_Ledger
// rows at read time, storing nothing: per hood, the MEDIAN of
// MonthlyRent*12/HouseholdIncome over active rented households with positive
// finite rent and income. A hood joins the city only with >= minRenters such
// households — a median over one tracked household is a trick (§15) — and the
// thin hoods are named, never guessed at. Mirrored verbatim in the engine as
// civicHousingBurdenCohort_ (parity block in the test).
// ---------------------------------------------------------------------------
// The catalog's derived-expression identifier doubles as the cohort column name, so a
// descriptor names what was measured and the pack's evidence helper keeps refusing it
// until it has a household reader (civicStageEvidence.js guards on [*/]).
var HOUSING_BURDEN_COLUMN = 'MonthlyRent*12/HouseholdIncome';

function housingBurdenCohort(input) {
  var inp = input || {};
  ...
  for (var h = 0; h < hoods.length; h++) {
    ...
    var rec = {}; rec[HOUSING_BURDEN_COLUMN] = Math.round(med * 10000) / 10000;
