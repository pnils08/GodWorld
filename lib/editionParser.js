...
// aren't real titles.
// Shared byline predicate (S265 ES-2, C98 G-W parser fix). A real byline is
// "By <Name>" with an optional `| bureau` or `, Bureau` tail — distinct from
// body prose that opens "By" ("By noon, the tables were set", "By next cycle,
// …", "By C63, a different symptom"). Used by BOTH the article-build byline
// assignment (parseEdition) and the Contract-B bind counter (here) so the two
// never disagree — the C98 throw ("1 byline for 2 CULTURE rows") was exactly
// that disagreement: a loose `^By\s+` assignment captured a prose line the
// tight bind counter then rejected. Accepts every real form observed across
// E9x editions (single-name+pipe "By Anthony | …", honorific "By Sgt./Dr. …",
// comma bureau "By Dr. Lila Mezran, Bay Tribune Health Desk"); rejects all
// observed body-prose "By" openers. Validated against 38 cases.
function looksLikeByline(line) {
  if (!line) return false;
  var b = String(line).trim();
  if (!/^By\s+/.test(b)) return false;
  var rest = b.replace(/^By\s+/, '');
  // Strip a leading honorific so "Dr. Lila Mezran" reads as a name.
  rest = rest.replace(/^(Dr|Sgt|Mr|Mrs|Ms|Rev|Lt|Capt|Gov|Sen|Rep)\.\s+/, '');
  // Split the name part from an optional bureau tail: " | bureau" or ", Bureau".
  var sep = rest.search(/(\s\|\s|,\s)/);
  var hasPipe = /\s\|\s/.test(rest);
  var namePart = sep >= 0 ? rest.slice(0, sep).trim() : rest.trim();
  var bureauPart = sep >= 0 ? rest.slice(sep).replace(/^\s*[|,]\s*/, '').trim() : '';
  var nameTokens = namePart.split(/\s+/).filter(Boolean);
  if (nameTokens.length === 0) return false;
  // Every name token must start uppercase (or be an initial like "M.").
  var allCap = nameTokens.every(function (t) { return /^[A-Z][\w'.\-]*$/.test(t); });
  if (!allCap) return false;
  // A pipe bureau is an unambiguous byline signal — single-token name is fine.
  if (hasPipe) return true;
  // No pipe: require First+Last (≥2 cap tokens); a comma bureau must itself be
  // capitalized so "By Maria Keen, who had waited…" stays prose, not a byline.
  if (nameTokens.length >= 2) {
    if (bureauPart && !/^[A-Z]/.test(bureauPart)) return false;
    return true;
  }
  // Single token, no pipe — only an initial-bearing name qualifies (defensive).
  return /\./.test(nameTokens[0]);
}
...
function bindCanonicalHeadlines(sections, articleTable) {
  ...
  for (var s = 0; s < keys.length; s++) {
    ...
    //
    // Both this bind counter and parseEdition's article.byline assignment use
    // the shared looksLikeByline() predicate (S265 ES-2), so a body-prose "By"
    // line can never be assigned as a byline here and rejected by the counter
    // there — that mismatch was the C98 "1 byline for 2 CULTURE rows" throw.
    var bylineArticles = parsedSection.articles.filter(function (a) {
      return looksLikeByline(a.byline);
    });
...
function parseEdition(filePath) {
  ...
  for (var hc = 0; hc < headerChunks.length; hc++) {
    ...
    for (var h = 0; h < headerLines.length; h++) {
      ...
      // In-world calendar marker line: "Bay Tribune | Cycle 97 | Y2C45 | Fall"
      // → masthead date "Fall · Y2C45". The current edition header format dropped
      // the legacy "Month YYYY" token, which left date='' and made the PDF
      // generator fall back to the real-world clock. Parse the in-world marker
      // instead so the masthead stays on cycle time, never wall-clock. (S265)
      if (!date) {
        var ywMatch = hl.match(/\b(Y\d+C\d+)\b/);
        if (ywMatch) {
          var seasonMatch = hl.match(/\b(Spring|Summer|Fall|Autumn|Winter)\b/);
          var seasonStr = seasonMatch ? seasonMatch[1] : '';
          date = [seasonStr, ywMatch[1]].filter(Boolean).join(' · ');
        }
      }
  ...
  for (var sg = 0; sg < sectionGroups.length; sg++) {
    ...
    // Split content into sub-articles on --- dividers, then again before every
    // `### ` headline (S347). The canonical edition puts a `---` rule between
    // an article's byline and its body, so splitting on `---` alone cuts in the
    // wrong place: every chunk but the first arrives as
    // `<previous article's body>` + `### next headline` + byline. Headline
    // detection then scans the whole chunk, picks up that mid-chunk `###`, and
    // files the previous article's body under the NEXT article's headline —
    // shifting bodies one headline late for the whole section and leaving the
    // section's first article an empty stub. The S244 body-cohesion merge below
    // never fired on those chunks because they DO carry a headline.
    // Splitting before `###` puts the leading body fragment in its own
    // headline-less chunk, which is exactly what that merge already knows how
    // to fold back onto the article it belongs to. Bare `---` scene breaks
    // inside one article carry no `###` and are unaffected.
    var articleTexts = content.split(/^---$/m).reduce(function (acc, chunk) {
      return acc.concat(chunk.split(/^(?=###\s)/m));
    }, []);
    ...
    for (var a = 0; a < articleTexts.length; a++) {
      ...
      for (var j = 0; j < lines.length; j++) {
        ...
        // Byline — shared predicate (S265 ES-2) rejects body-prose "By" lines
        // ("By next cycle, …") that the old loose /^By\s+/ wrongly captured.
        if (!article.byline && looksLikeByline(cl)) {
      ...
      // Body-cohesion merge (S244 ES-1, G-PR-NEW2 structural fix).
      // The canonical edition shape places a `---` rule between the byline and
      // the body, so split(/^---$/m) tears one logical article into a
      // headline+byline block and a headline-less body block. Without this
      // merge the headline block (text = just `### HL` + `By X`, ~120 chars)
      // becomes the article the PDF front-page path renders, and the strip in
      // generate-edition-pdf.js buildArticleHtml removes those two lines —
      // leaving an empty body div. Bodies aren't "length 0"; they're orphaned
      // into separate article objects.
      //
      // A piece with no headline is a continuation (body / scene-break
      // fragment); fold its text back into the preceding real article so the
      // consumer sees the canonical `### HL / By X / body` shape. Consecutive
      // body fragments (long pieces with internal `---` scene breaks) all
      // accrue to the same article. A leading headline-less block with no
      // preceding article stands on its own (defensive — shouldn't occur in
      // canonical editions). The fold keys on absent HEADLINE, not byline: a
      // continuation fragment is defined by having no headline (S265 ES-2
      // tightened byline detection, but headline-absence remains the right key).
      var hasHeadline = !!article.headline;
      var prev = section.articles[section.articles.length - 1];
      if (!hasHeadline && prev) {
        prev.text = prev.text + '\n\n' + article.text;
        // Capture trailing metadata that lives in the body block. Never let a
        // false body-prose byline overwrite the real one already on prev.
        if (!prev.subhead && article.subhead) prev.subhead = article.subhead;
        if (!prev.photographer && article.photographer) prev.photographer = article.photographer;
        if (!prev.namesIndex && article.namesIndex) prev.namesIndex = article.namesIndex;
      } else {
        var isFirst = section.articles.length === 0;
        section.articles.push(article);
        // First pushed article's metadata becomes the section-level metadata.
        if (isFirst) {
