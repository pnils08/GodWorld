 ...
 *   CANON_12          — the core sim neighborhoods in CoreSimRank order (all 22 since civic.21; name kept)
 ...
 *
 * ADR-0016 (engine.99, 2026-08-03): Neighborhood_Map is the truth source for the
 * hood set; MAP_NEIGHBORHOODS is a CACHE of that ledger truth, not an independent
 * authority. scripts/auditHoodDrift.js reconciles it against the live sheet and
 * fails on any diff — edit the sheet first, then this cache. Engine-side (GAS)
 * consumers use phase01-config/canonNeighborhoodLoader.js instead; lib/ never
 * deploys (.claspignore).
 *
...
// S423 (2026-09-05): the sheet has ranked ALL 22 hoods since civic.21 (2026-08-15,
// builder: "we should be using all hoods"); this cache had sat at the old twelve
// for three weeks and auditHoodDrift failed its core reconcile on every run.
// Order = Neighborhood_Map CoreSimRank order (the engine's draw order). The name
// is kept — two live importers (canon-name-check membership, citizenDerivation's
// seeded hood fallback) and the drift audit read it; renaming buys nothing.
const CANON_12 = ['Temescal', 'Downtown', 'Fruitvale', 'Lake Merritt', 'West Oakland', 'Laurel',
  ...
  'East Oakland', 'Baylight District', 'Glenview', 'Dimond', 'Ivy Hill', 'Adams Point',
  'Grand Lake', 'Eastlake', 'Brooklyn', 'San Antonio'];
...
// S423 engine.99 #9 — CHILDREN is now a CACHE of Neighborhood_Map.ChildAreas
// (the truth source for child areas, one column, authored per hood); the drift
// audit reconciles this list against that column. Names that are tracked hoods
// (Adams Point, Grand Lake, Eastlake, Ivy Hill, San Antonio, Dimond, Glenview,
// Brooklyn) were removed from here — they are hoods, not children.
const CHILDREN = ['Lakeshore', 'Maxwell Park', 'Old Oakland', 'City Center', 'Jack London Square',
  'Koreatown-Northgate', 'Koreatown', 'Northgate', 'Montclair', 'Claremont', 'Longfellow',
  'Shafter',
  // engine.99 — Coliseum + Elmhurst: East Oakland sub-areas the sim legitimately
  // speaks (D5 district composition, photo scenes, gender-variance keys). Same
  // class as Montclair/Old Oakland above. 'Jingletown' stays excluded (off-roster).
  'Coliseum', 'Elmhurst',
  // engine.99 Cohort 2 — child areas from the engine's child→parent fold maps
  // (checkForPromotions, commuteFlowEngine): real Oakland sub-geography, same
  // class as Longfellow/Shafter above.
  'Golden Gate', 'McClymonds', 'Prescott', 'Hoover-Foster', 'Brooklyn Basin',
  'Telegraph corridor'];
