...
| civic.30 | Health-beat mismatch (Mike spot-check, S417) — root cause was an empty pool, not the matcher: the civic desk_signal lane carries zero health data. Health record now parsed from the world summary into tiered `health-*` entries; initiative rows demoted to a fallback. Detail in the commit + code comments | done-pending-archive | engine-sheet | `scripts/buildCivicDomainSlice.js` + `.test.js` |
