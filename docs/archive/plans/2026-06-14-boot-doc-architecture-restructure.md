...
**VERDICT (S264, Mike-confirmed): WON'T-BUILD — close stays on the main loop.** The pre-build read corrected the spec above. Two corrections: (1) Step 2 of the close is not just PIN + NEXT — it includes **ROLLOUT row management** (what closed, archive sweep), which is *more* judgment-heavy than the NEXT line. (2) The only mechanical slice is the Session counter, and that is **gov.35's**, not gov.37's — so after gov.35 lands there is no judgment-free residue left to offload. The decisive frame: offload ROI = **volume × context-portability**. The close is **low-volume** (one PIN line + one NEXT line + a few ROLLOUT flips) and **context-resident** (the cheapest closer is the model that just ran the session — it already holds the context a fresh Sonnet agent would reload). A drafter + a mandatory review pass on the single most boot-critical write in the system buys ~0 savings for real next-boot-corruption risk. Consistent with [[../../MEMORY]] `feedback_offload-scripted-jobs-to-helpers` (offload gates on *strict-pattern*; the strict-pattern slice here IS gov.35). **The cost win Mike wanted lives in gov.35.** gov.37 closed `done-pending-archive`. (If ever revisited: the only place economics could flip is the **media** close — it writes a journal = real output volume.)
...
### governance.36 — status (drained from ROLLOUT, 2026-06-26 / S274)

Boot doc architecture restructure — base/worker/on-demand + self-evolve (Mike-directed S259). Reverses governance.5 (CLAUDE.md pointer-only) for the governing-core content: CLAUDE.md becomes grounded (what this is / who we are), not a pointer index — proven need by S259 boot following ZERO of its pointers. Three layers: **base** (CLAUDE.md + identity.md, auto) → **worker** (per-terminal TERMINAL.md, hook-injected) → **on-demand** (MEMORY/ROLLOUT/SESSION_CONTEXT). MEMORY.md (~5,860 tokens, the one still auto-loading) is the real de-auto-load (rollout + session-context already on-demand). **Status: DESIGN, nothing deployed** — grounded CLAUDE.md drafted to working tree, **commit HELD**; old archived. Absorbs **governance.32** (MEMORY split = cascade step 2); siblings governance.26 (ADR-0009 precedent) + governance.35. Orphan-gap constraint: sequence bottom-up (workers absorb canon facts before base strips them; engine-sheet/research-build safe on lean base now, media/civic gated). Open: the seam (CLAUDE.md vs identity.md boundary), the self-evolve mechanism (the unsolved 80% — where a learned gotcha lands), validation signal. **S260: §loop-tightening phase SHIPPED** (Mike-directed "make session-end uniform with boot — a full tight loop") — carried set reduced to `{PIN, NEXT[terminal]}`, boot-read ≡ session-end-write; Shipped block + STATUS narrative + `writeShippedBlock.js`+boundary retired (block frozen-at-S248 = the empirical trigger); per-terminal NEXT + Edition-on-PIN added to hook; `/session-end` v2.3 + `/session-startup` + 4× TERMINAL.md + `sessionEndMechanical.js` conformed; ADR-0009 extended §loop-tightening; governance.35 reduced. Two follow-ons filed → governance.37 (Sonnet closer) + governance.35-remnant (PIN self-derive). Row stays in-progress for the original open Qs (seam / self-evolve).

## Status — item 2 close detail (S356)

Universal-discipline merge shipped 2026-08-05: MEASURE TWICE block emitted by `session-startup-hook.sh` for all registered lanes (the one pre-cognitive principle not already resident via identity.md — anti-guess and don't-build-beyond live there). Three TERMINAL.md principle-restatements (research-build, engine-sheet, media) trimmed to pointers at the boot-emitted block; domain applications untouched per the name-versus-content trap (civic completion checklist, engine cascading-effects procedure, newsroom gen-eval description). MEMORY.md's copy kept deliberately — the harness has swallowed the hook payload before (claude-mem #41715, 2026-07-14), so it is the fallback residency.

## Changelog

- 2026-08-05 (S356, research-build) — Item 2 universal-discipline merge SHIPPED; detail in §Status — item 2 close detail. Row → done-pending-archive.
- 2026-06-16 (S264, research-build) — **gov.37 CLOSED won't-build (Mike-confirmed), same session it was gated open.** Gate met (3 clean closes S261/262/263), flipped `ready`, spec written — then the pre-build read killed the build: Step 2 of the close also carries ROLLOUT row management (judgment-heavy), and the only mechanical slice (Session counter) is gov.35's. Decisive frame: offload ROI = volume × context-portability; the close is low-volume + context-resident, so the cheapest closer is the model that just ran the session. A Sonnet drafter + mandatory review on the most boot-critical write = ~0 savings for real corruption risk. Cost win lives in gov.35; close stays main-loop. Row → `done-pending-archive`. Advisor-reviewed twice (pre-spec + pre-build-kill).
...
| governance.36 | Boot doc architecture restructure — base/worker/on-demand + self-evolve (Mike-directed S259). Reverses governance.5 (CLAUDE.md pointer-only) for the governing-core content: CLAUDE.md becomes grounded (what this is / who we are), not a pointer index — proven need by S259 boot following ZERO of its pointers. Three layers: **base** (CLAUDE.md + identity.md, auto) → **worker** (per-terminal TERMINAL.md, hook-injected) → **on-demand** (MEMORY/ROLLOUT/SESSION_CONTEXT). MEMORY.md (~5,860 tokens, the one still auto-loading) is the real de-auto-load (rollout + session-context already on-demand). **Status: DESIGN, nothing deployed** — grounded CLAUDE.md drafted to working tree, **commit HELD**; old archived. Absorbs **governance.32** (MEMORY split = cascade step 2); siblings governance.26 (ADR-0009 precedent) + governance.35. Orphan-gap constraint: sequence bottom-up (workers absorb canon facts before base strips them; engine-sheet/research-build safe on lean base now, media/civic gated). Open: the seam (CLAUDE.md vs identity.md boundary), the self-evolve mechanism (the unsolved 80% — where a learned gotcha lands), validation signal. **S260: §loop-tightening phase SHIPPED** (Mike-directed "make session-end uniform with boot — a full tight loop") — carried set reduced to `{PIN, NEXT[terminal]}`, boot-read ≡ session-end-write; Shipped block + STATUS narrative + `writeShippedBlock.js`+boundary retired (block frozen-at-S248 = the empirical trigger); per-terminal NEXT + Edition-on-PIN added to hook; `/session-end` v2.3 + `/session-startup` + 4× TERMINAL.md + `sessionEndMechanical.js` conformed; ADR-0009 extended §loop-tightening; governance.35 reduced. Two follow-ons filed → governance.37 (Sonnet closer) + governance.35-remnant (PIN self-derive). Row stays in-progress for the original open Qs (seam / self-evolve). | in-progress | research-build | [[../archive/plans/2026-06-14-boot-doc-architecture-restructure]] (built on [[../plans/2026-05-09-boot-load-audit]] + research [[../research/2026-06-04-boot-orientation-cleanup]] / [[../research/2026-06-05-anthropic-skills-blog]]) |
