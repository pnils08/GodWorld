...
- **Status:** [~] CODE DONE + LIVE @14 S409 (`SCHOOL-OCS` / `Oakland City Schools`; grep clean). **Bench ND floor applied** (`--sheet-id=<bench> --apply`, 22 rows, 0 below floor after, Rockridge held 9/95). **Live ND replay is Mike's to run** (`node scripts/backfillNeighborhoodEducation.js --apply`, default target LIVE) — until then live minors restamp from West Oakland 3 / Fruitvale 3. `backfillNeighborhoodEducation.js` now has FLOOR semantics (never lowers: Rockridge keeps 9/95); dry run 2026-09-01 shows West Oakland 3/62 → 8/91, Fruitvale 3/65 → 8/89, 14 hoods below the band rise, 8 unchanged or floored.
