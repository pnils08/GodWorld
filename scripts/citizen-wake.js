...
function buildVoicePrompts(c, neighbors, sportsLine, lifeArc, textureLine, bondsLine, familyLine, healthLine, pageMemory, cycle, tensionBlock, editionLine, rippleLine, cardBlock, voiceLine, eclLine) {
  ...
  const biasLine = resonance.biasReadback(c.memReg,
    [c.life, neighbors.map((n) => n.name).join(' '), bondsLine, familyLine, healthLine, sportsLine, textureLine, lifeArc].filter(Boolean).join(' '));
...
async function main() {
  ...
  const healthLine = await loadHealthState(c.popId); // engine.101 health slice — current health state reaches the wake (engine.102 W3 export, 87e68f94)
