...
- **S256 (C97): civic cascade clean; all 8 criteria held at source layer.** Mayor took OARI citywide AND bound her administration to per-cycle public reporting (calls, resolution, response time, community feedback), and directed the youth-apprenticeship bill — stalled 4 cycles without a vote — to the floor via Rivers (D5). Transit Hub broke ground in Fruitvale. 21 statement/decision records ingested via Step 3 (18 statements + 3 decisions across 7 offices: crc 3, ind 2, mayor 2, oari 1, opp 4, chief 2, transit-hub 4). **Vote-math / vote-fencing held — no C97 council vote fabricated:** the apprenticeship bill is correctly framed as heading-to-floor (next cycle), not already-voted; the edition fenced all civic with no invented tally. **Council status enum handled correctly:** Crane D6 `recovering` → written statement only, named not-voting; Mobley D9 had no word this cycle (named, no fabricated position); Vega (D4, Council President) lifted his standing OARI reservation and put his position on record. Chief Montez set the OARI/OPD work-split and named downtown's small violence uptick plainly (watch, not fear). Skill-check skipped (Arbiter A; criteria from direct evidence). Evidence: output/civic-voice/*_c97.json + output/city-civic-database/initiatives/{oari,transit-hub,youth-apprenticeship}/decisions_c97.json + production_log_c97.md (## What City Hall Did) + bay-tribune Step 3 ingest 21 memories.
- **S271 (C100): civic cascade produced source cleanly — 32 statement/decision records ingested via Step 3 (opp 5, crc 3, oari 3, stab-fund 3, mayor 2, okoro 2, ind 2, chief 2, transit-hub 2, baylight 1, health-center 1, DA 0).** Mayor convened the Rivers(D5)/Crane(D6) strain working group (C102 deadline) by executive action + put a same-cycle deadline on Okoro, who met it and named Laila Cortez interim apprenticeship-budget owner; Tran (D2) went public on the Piedmont spike and flagged an OARI data gap. **Vote-fencing held — no C100 council vote fabricated:** the working group is mayoral action (no ordinance), the council not implicated, the edition invented no tally. **Source-data items the edition surfaced and Mara flagged for reconciliation (these originate at the civic/engine layer, not editorial invention):** the Stab Fund running count (30 cleared vs ~280 remaining against a 280-total fund) and OARI D7 8:04-vs-6.0-lock both need pinning in world-data before they re-enter canon — a city-hall source criterion: the project agents (Stab Fund, OARI) should emit *reconciled* running totals + a current-vs-target-lock flag, so the figure handed to the newsroom is already canon-consistent. **The C100 grade penalty was an EDITION-layer failure (civic section ran the mechanism with no citizen), NOT a source-layer failure** — the source material existed; the civic desk covered the machinery instead of the people the machinery touches. That's a write-edition/story_evaluation criterion, correctly out of scope here (see §What this skill is NOT graded against). Evidence: ingestCivicWiki C100 (32 memories) + mara_audit_c100.md §Drift Findings + production_log_c100.md.
