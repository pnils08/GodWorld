...
**Next:** civic run = the floor test — latitude where invention *would* be contamination (vote math, faction facts), and the live target is C100's uncovered sentiment/crime event. Graduates to ADR (kill monolith, invert dispatch, recalibrate floor) + a phased build plan handed to engine-sheet for the storage/ingest deltas.
