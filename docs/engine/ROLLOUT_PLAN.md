...
**Status:** Active | **Last Updated:** Session 147 (2026-04-15)
...
2. **Phase 38.1** — Engine ailment detector. **DONE S146 `[engine/sheet]`** — `scripts/engineAuditor.js` + 8 detectors in `scripts/engine-auditor/`. First run against C91 produced 26 patterns (5 high / 21 medium / 0 low); Temescal Community Health Center surfaces as `stuck-initiative` on INIT-005 (§7 acceptance criterion 5 ✅). Deterministic across two runs, 1.1s runtime (budget 30s). Output: `output/engine_audit_c91.json` (includes thin snapshot persistence so cross-cycle diffs work next run). Follow-up `(research-build terminal)`: skill edit per PHASE_38_PLAN §6 — `/engine-review` reads the JSON instead of re-scanning. §10 questions resolved: Initiative IDs = `INIT-XXX`, no native `cyclesInState` column (derived from prior audit JSONs; first-run fallback parses `LastUpdated` crudely — Temescal shows `cyclesInState=88` from date string "3/25/2026", will self-correct once next cycle runs), `lib/sheets.js` reads parallelize via `Promise.all`. Noted skip: `Population_Stats` tab not present in sheet — orchestrator warns + continues.
3. **Phase 38.7 + 38.8** — Anomaly gate + baseline brief auto-generation. **DONE S146 `[engine/sheet]`** — `scripts/engine-auditor/detectAnomalies.js` + `generateBaselineBriefs.js`, wired into the same orchestrator as 38.1. Three output files per cycle now: `engine_audit_c{XX}.json` / `engine_anomalies_c{XX}.json` / `baseline_briefs_c{XX}.json`. Combined C91 runtime 0.9s (§13.2 budget 60s). All three outputs deterministic across two runs (strict diff). **Stock-split fixture (§11.4) passed:** synthetic prior-audit JSON with POP-00001 income inflated 7× (prior=10.5M, current=1.5M, ratio=0.143) flags as anomaly with `triagePath: 'route-to-engine-debug'`, `confidence: 'high'` (fixture injection via `ENGINE_AUDITOR_PRIOR_FIXTURE` env var). Baseline briefs C91: 8 generated (world-event class; other classes deferred per §12.2 because C91's WorldEvents_V3_Ledger uses `EventType: 'misc-event'` uniformly — death/birth/graduation taxonomy not yet present). 5/8 briefs carry promotion hints (63% — §12.5 target was ≥20% ✅). Audit JSON now also persists `citizenIncomes` and `Crime_Metrics` snapshots so next cycle's anomaly detection has the 6-cycle history window for 3σ crime checks. Follow-up `(research-build terminal)`: unified skill edit per PHASE_38_PLAN §6 now covers all three output files in one pass. **Known gap for §13.7 (Beverly Hayes promotion hint):** only triggers when engine emits a citizen-attributed structured event — current EventType taxonomy is single-value `misc-event`. Engine-side fix is not on this spine step; documented here so research/build can flag in the skill-edit pass.
4. **Phase 39.1** — Capability reviewer. **DONE S146 `[research-build]`** — `scripts/capabilityReviewer.js` + 9 assertion modules in `scripts/capability-reviewer/` + declarative `assertions.json`. Replay test against E91 PASSED §8 acceptance #2: reviewer flagged missing Temescal coverage as blocking on `front-page-leads-with-highest-severity-ailment` (cyclesInState=88). 5/9 passed in 0.82s, deterministic across two runs, all 5 categories firing. Skill wrapper `.claude/skills/capability-review/SKILL.md` wired into `/write-edition` Step 3.5 between Compile and Validation+Rhea. **2 grader-only assertions deferred** until Anthropic API key wired (`assertions.json.graderOnlyAssertions`). **Editorial findings to surface:** "cycles" word in narrative copy trips the engine-metrics assertion on 3 articles per the strict newsroom.md rule — needs rule clarification or copy sweep. Canon-names extractor still over-triggers on multi-word capitalized phrases (advisory tier); tightening planned for next pass — require citizen-reference context.
5. **Phase 38.2 + 38.3 + 38.4** — Mitigator check + remedy recommendation + Tribune framing brief. **DONE S146 `[engine/sheet]`** — `scripts/engine-auditor/checkMitigators.js` + `recommendRemedy.js` + `generateTribuneFraming.js` + `mitigatorRegistry.json` + `remedyTemplates.json`. Three enrichers mutate `patterns[]` in the same orchestrator after detectors, before output write. Combined C91 runtime 2.1s (§17.2 budget 60s). Output deterministic across two runs. **§17.4 Temescal end-to-end thread: all 5 sub-criteria pass** — INIT-005 flows `stuck-initiative` → `mitigatorState.gap: mitigator-stuck` (88 cycles in design-phase, effects-not-firing) → `remedyPath.worldSide[0]: { type: advance-initiative, target: INIT-005 }` → `tribuneFraming.storyHandles` populated for civic + culture + letters desks → `suggestedFrontPage: true` → `capabilityHooks: ["covers Temescal", "mentions INIT-005", "cites a Temescal resident"]` (exact strings the plan §16.2 specified). §14.5 #3 partial: 3 of 4 gap values surfaced across C91 (`mitigator-stuck`, `no-mitigator`, `mitigator-firing-but-insufficient`); `remedy-working` will surface once a second audit cycle with real metric deltas runs — the classifier needs prior-audit history to detect positive cross-cycle movement. Follow-ups flagged for `(research-build terminal)` per §16.5: rewrite `/engine-review` and `/sift` skills to consume `mitigatorState` + `remedyPath` + `tribuneFraming` directly instead of re-synthesizing.
6. **Phase 39.2–39.7 + 39.3** — Three-lane scope redesign + Final Arbiter + two-pass hallucination. **DONE S147 `[research-build]`** — all six sub-phases shipped in one session. Commits `1cbd9ba`, `1f40562`, `e3bb393`, `023896f`, `c9d2717`, `e4dbb58`. Rhea narrowed to Sourcing Lane (5 checks, weight 0.3, JSON output contract, RULES.md 431 → 185 lines); cycle-review narrowed to Reasoning Lane (3 checks, weight 0.5); Mara narrowed to Result Validity Lane (3 checks, weight 0.2) via claude.ai system prompt + structured-top markdown output parsed by `maraJsonReport.js`; process/outcome/controllable/uncontrollable fields land on all four lane outputs per `docs/engine/REVIEWER_LANE_SCHEMA.md`; two-pass hallucination detector (`scripts/rheaTwoPass.js`) runs Haiku 4.5 Pass A (text only) and Pass B (text + two-tier canon context) with prompt caching; Final Arbiter agent + `scripts/finalArbiter.js` consume four lane inputs, apply 0.5/0.3/0.2 weights + capability gate as hard halt, emit A/B verdict + blame attribution + publish recommendation (PROCEED / PROCEED-WITH-NOTES / HALT). Pipeline wired into `/write-edition` as Step 4 (Rhea), Step 4.1 (cycle-review), Step 5 (Mara), Step 5.5 (Final Arbiter — publication gate), Step 6 (publish, gated). **E91 replay end-to-end:** capability gate blocks on Temescal miss → Final Arbiter verdict=B, weightedScore=0.799, recommendation=HALT, exit code 1. Two-pass detector caught legit canon drift chain ("Varek, 31" in c91 world_summary vs. ledger BirthYear 2003 → age 38) and surfaced the upstream `/city-hall-prep` bug; fix: `2041 − BirthYear` age rule added to `.claude/rules/newsroom.md`. Spine step 6 **CLOSED**.
7. **Phase 39.9 + 39.8 + 39.10** — Tiered review + reward-hacking scans + adversarial review skill. **DONE S148 `[research-build]`** — three new artifacts: `scripts/tierClassifier.js` (deterministic A/B/C tier assignment per article based on Tier-1 citizens, engine ailments, anomaly flags, section defaults, contested civic signals — tested against C91, deterministic across runs), `scripts/rewardHackingScanner.js` (5 scans: citizen-reuse rate, cherry-picking detection via production-log regen markers, rubric-signal density, rubric-rationale language detection, OOD criteria validation against held-out editions — all clean on C91), `.claude/skills/adversarial-review/SKILL.md` (5-probe devil's-advocate skill: cross-article contradictions, unsourced claims, one-framing civic stats, ambiguous quotes, narrative gaps — runs post-compile pre-capability). Pipeline wired: `/write-edition` Step 3.25 runs all three in parallel after compile. `finalArbiter.js` enriched with `tierDistribution` and `rewardHackingScan` fields in output JSON. E91 replay: tier classifier assigns 4 Tier-A articles (all elevated by front-page/Tier-1-citizen/engine-ailment signals), reward hacking scanner clean (0 flags), Arbiter verdict unchanged (B, HALT on Temescal miss). Spine step 7 **CLOSED**.
...
### Phase 39: Editorial Review Layer Redesign (from MIA + Microsoft UV + Mezzalira) — DONE S147-S148

**S147 closure:** Spine step 6 shipped. The three-lane chain (Rhea Sourcing / cycle-review Reasoning / Mara Result Validity) + capability gate + Final Arbiter is live and wired into `/write-edition`. Two-pass hallucination works with a two-tier canon context.

**S148 closure:** Spine step 7 shipped. Tiered review (`tierClassifier.js`), reward-hacking scans (`rewardHackingScanner.js`), and adversarial review skill (`/adversarial-review`) all built and wired into `/write-edition` Step 3.25. Final Arbiter enriched with tier distribution and reward-hacking scan results. Full Phase 39 (39.1–39.10) is COMPLETE.
...
**39.9 Tiered review — priority-based attention allocation (from Nieman Reports "Automation in the Newsroom", Celeste LeCompte, Sep 2015 — `docs/research/papers/paper5.pdf` [Drive ID: 1slzF0rTo5ND6VWN1EXj-sX8MnyA4ZszH], p.24).** AP's production model: 80 companies get close human review + additional reporting, 220 get editor pass, the rest go direct to wire. Attention tiered to editorial value. Our current Phase 39 runs full Rhea + cycle-review + Mara on every article regardless of stakes. Redesign: classify each article at sift time into review tier based on priority signals (Tier-1 citizen involved, front-page candidate, engine-flagged ailment, anomaly flag from 38.x, contested civic story). Three tiers:
- **Tier A (full review):** All three lanes + capability reviewer + two-pass hallucination + Final Arbiter. Front page, ailment coverage, Tier-1 citizen features, contested votes.
- **Tier B (editor pass):** Rhea sourcing + cycle-review reasoning. Skip Mara if completeness obvious. Neighborhood features, routine council coverage, sports game recaps.
- **Tier C (automated checks only):** Rhea sourcing regex + anomaly flag. No reviewer agents. Baseline briefs (see 38.x), box-score-equivalent coverage, Tier-4 generic citizen scene-setters.

Tier assignment happens once at sift, flows through the pipeline. Saves agent-hours, matches attention to editorial stakes. Prereq: 39.1-39.7 landed so the lanes exist to tier.
...
**41.6 Schema headers wiki-layer integration `(research-build terminal)` — NEW S146.** `schemas/SCHEMA_HEADERS.md` is auto-generated by the Apps Script `exportAndPushToGitHub` flow (refreshed 2026-04-15 after 84 days stale; next refresh on demand). Two small pieces of wiki work, both r-b-side because they touch docs conventions, not engine behavior:
  - **Catalog it in `docs/index.md`.** Currently nothing under `schemas/` is indexed — the 1,349-line canonical column list is invisible to the wiki layer. Needs one index entry pointing at it with a one-line hook ("canonical column definitions, auto-generated, refresh via Apps Script `exportAndPushToGitHub`").
...
7. **38.7 Anomaly-detection gate (from Nieman Reports, `docs/research/papers/paper5.pdf` [Drive ID: 1slzF0rTo5ND6VWN1EXj-sX8MnyA4ZszH] p.21, Netflix stock-split case).** Wordsmith published a false "71% drop" story because its data pipeline had no anomaly flag when the input didn't account for a stock split. Separate concern from ailment detection: ailments are *persistent system states*; anomalies are *sudden unexplained changes* that may indicate bad data, not bad world state. Before sift, the engine-review pass should explicitly flag: citizen income Δ>50% in one cycle, crime stat 3σ from trend, approval-rating flip in one cycle, population shift >10% for a neighborhood in one cycle, any metric that moves more than its historical standard deviation in a single cycle. Sift sees the anomaly and makes one of three calls: (a) cover as story if the anomaly is real and newsworthy, (b) flag as data error and route to engine-build terminal, (c) suppress until verified. Protects against false confident coverage of broken data.

8. **38.8 Baseline brief auto-generation (from Nieman Reports p.30-31, LA Times Homicide Report + Quakebot pattern).** Every engine-registered structured event (citizen death, council vote, initiative milestone, business opening/closing, approval shift >5pts, home sale, birth/graduation) gets an auto-generated baseline brief from the data at engine-review time. Brief includes: what happened, who was involved (POP-IDs + names + roles), where, when (cycle), and the three-layer coverage handle pre-filled (engine cause if any, simulation texture available, user-action connection if applicable). Desk agents at sift see the full baseline-brief set and decide per event: promote to feature coverage (desk rewrites with voice + additional reporting), publish baseline as-is (routine item, Tier C review per 39.9), or suppress. Scales coverage without faking it — we can cover events no real newsroom would staff, without hallucinating details. Paired with 39.9 tiered review: baseline briefs default to Tier C unless promoted.
