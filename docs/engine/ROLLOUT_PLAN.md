...
| engine.120 | WealthLevel dual-writer drift — stale v14.2 formula deleted from `lib/economicLookup.js`, `applyEconomicProfiles.js` no longer writes the column (Task 1 DONE S381, `ee92e5bf`). Scope was 551/940 rows on v14.2, not 17. Cause PROVEN by discriminating test: on rows where the two formulas disagree, script-written rows (EconomicProfileKey set) are 508 v14.2 / 11 v15, while SPORTS_OVERRIDE rows (the branch that never writes WealthLevel) are 29 v15 / 10 v14.2 — the script was last writer and overwrote the engine. Task 2 hand-backfill unnecessary: with the writer gone, the engine's unconditional per-cycle write repairs all rows on the first clean cycle. | done-pending-archive | engine-sheet | [[../plans/2026-08-18-wealthlevel-dual-writer-drift]] |
