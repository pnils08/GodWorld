...
**ES TRACK — S259 reconciliation (research-build, git-verified):** engine-sheet shipped most of Track B in S257; the plan's per-phase `[ ] not started` markers were wholesale stale. Verified state: **ES-3 DONE** (`6a0d904`+`d0a8bbf`); **ES-5 DONE** (`a9cd061`); **ES-2 code DONE + wired** (`e40bb21`+`2361cfc`+`6033c26`+`f987211`) — destructive POP-01021→POP-00781 merge HELD for Mike's go; **ES-1 PARTIAL** (`9b769ce` step 1 only; steps 2+3 open; clasp-deploy deferred post-C98); **ES-4 DONE S259** (steps 3+4 `e690aa2`+`f5b0fbd`; step 1 G-S3 SUPERSEDED by engine.35 Phase 1; step 2 G-S4 tracker-writeback key-name bug fixed + MCP verified). **Genuine ES residual:** ES-1 steps 2+3 + the ES-2 held merge (operator) + ES-1 clasp deploy (rides C98). governance.34 **stays in-progress** for that residual.
...
- **Source gaps:** G-S1, G-S2. **Absorbs/relates:** [[2026-05-30-citizen-lifecycle-fame-system]]. **Status:** [~] PARTIAL — **step 1 DONE S257** (`9b769ce`): `generationalEventsEngine.checkRetirement_` CIV guard (no office-holder/media citizen auto-retires) + AGE_RANGES.RETIREMENT.min 58→68; live cleanup blanked the stray [Retirement] tag on POP-00034. **Step 2 card-display half DONE S259** (`187ce9b`, Node-only, no clasp): citizen cards anchor milestones on the in-world Cycle (`C81 [Death]` from the archive Cycle col) + strip the real-world date from live-O lines — full-population dry-run = zero real-world dates beside any milestone tag; dedup made prefix-robust. **Step 2 engine-side half — CODE DONE S264** (`ecbdca42`, local; clasp deploy PENDING Mike go-call): `applyMilestone_:1075` now writes `var stamp = "C" + cycle` into the col-O *string* (was `yyyy-MM-dd HH:mm`). Measure-twice cleared both col-O consumers (compressLifeHistory v2.0 robust timestamp-optional parsing; card builder prefix-robust by design — its S259 pair). LifeHistory_Log append untouched (structured Timestamp/Cycle cols). **C99-effective, not retroactive** (C98's col-O entries already wrote with timestamps). **Recommendation: bundle the clasp deploy with Step 3** in one clean post-edition window. **Step 3 (baseline-brief office-holder exclusion) OPEN — next-session pickup, do first** — needs a check before any guard: `generateBaselineBriefs.js` reads LifeHistory_Log (not col O), so confirm whether it still emits the retired-Mayor brief now the source roll (step 1) is fixed; C98 evidence — the cycle's 1 retirement is Anna Baker (POP-00471, regular citizen), no office-holder retirement brief surfaced, so step 1 looks effective. Then: deploy Step 2 code + any Step 3 guard together. Election-challenger-from-SL deferred (B) per Mike, working-as-designed stub. Election-challenger-from-SL deferred (B) per Mike, working-as-designed stub.
...
- **Source gaps:** G-S3, G-S4, G-S5, G-P-C97-2. **Status:** [x] DONE S259 — **steps 3+4 DONE S257:** step 3 G-S5 synthesized world-event briefs flagged louder (`e690aa2`); step 4 G-P-C97-2 `buildCitizenCards` failure-dump keys off LIVE World_Config cycleCount (`f5b0fbd`). **Step 1 G-S3 SUPERSEDED S259** by **engine.35 Phase 1** (the storyline-followup gate — non-Oakland-locale + age cap; the full emergence redesign replaces the mechanical "cap recycled" framing per [[2026-06-15-story-seed-deck-engine-emergence]]). **Step 2 G-S4 DONE S259** — root cause was a key-name bug in `buildInitiativePackets.js` (the sole writer of `output/initiative_tracker.json`, MCP `lookup_initiative` Layer-2 source): `init.Domain`/`init.Neighborhoods`/`init.VoteResult` keyed off non-existent sheet columns (live headers are `PolicyDomain`/`AffectedNeighborhoods`/`Outcome` — `toObj` keys by exact header) → domain/neighborhoods always null/[]. Compounded by the file being stale since 2026-04-03 (~C89 — this builder is NOT run every cycle). Fixed the 3 keys + re-ran; verified `lookup_initiative(INIT-007)` now returns workforce/vote-scheduled/neighborhoods/summary (was untracked/None) and INIT-002 dispatch-live/safety/C97-milestones. **Operational note (→ civic.14 pairing):** ensure `buildInitiativePackets.js` runs each cycle (or in city-hall prep) so the tracker JSON doesn't re-stale; the code is now correct but the cadence gap remains.
