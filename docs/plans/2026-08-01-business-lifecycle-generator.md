...
- **The six ghost founders** (Chandrasekaran, Yoon, Nakamura, Tan, Osei, Torres — founders on the Business_Ledger with no ledger row, doctrine 6): **builder decision, not built here** — Generic_Citizens is the only door into the ledger (doctrine 5), so either they enter the waiting room as themselves (Occupation = founder of X) and the drip promotes them, or the names are struck from the cells. Task 10 counts them `unresolved` and never invents a row.
