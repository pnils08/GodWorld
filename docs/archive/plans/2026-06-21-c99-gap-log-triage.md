...
- [x] **ES-3 follow-up (research-build) — extend the cycle-form design to the baylight agent's voice files. DONE S268 (governance.43).** `IDENTITY.md:41` "begins in Q3" → "begins within two cycles"; `LENS.md` "Q3 mobilization" → "gate-1 mobilization", "5 September 15 deliverables"/"Deliverable Tracker" → "gate" form (matches ES-3 relabel), world-history year-anchors dropped ("stood up in 2040" → "stood up"; "doors open in 2046" → "doors open"). Decision on 2040/2046: the agent's own RULES Time Convention is absolute ("No years") and both could bleed into output, so year-dropped while preserving backstory/duration sense (the "5–7 years end-to-end" span on LENS §Political-vs-Project Clock carries the timeline). Durations/cadences (quarterly audits, 5-year time-lapse, 18-month look-ahead) left — spans, not calendar dates. Grep-verified zero Q[1-4]/month/in-prose-year tokens remain. **This plan's sole open thread — plan now fully shipped, ready to archive.**
