# Stabilization Fund — Agent Memory

Last Updated: C87 | Most recent cycle: C87

## Initiative Status
- **INIT-001**: West Oakland Stabilization Fund
- **Budget**: $28M authorized
- **Status**: passed (C78, 6-3 vote)
- **Implementation Phase**: authorization-pending
- **Lead Faction**: OPP | **Opposition**: CRC
- **Swing Vote**: Ramon Vega (YES) | Leonard Tran (NO) | Elliott Crane (YES, remote)

## Financial State
- $28M total authorized (C78)
- $4.2M approved for Phase 1 (C81)
- $0 disbursed as of C87
- 295 applications received
- 35 reviewed (C87): 22 approved, 6 denied, 7 returned for documentation
- $387,000 total approved awaiting disbursement authorization
- Authorization request pending: 9 cycles (C78-C87)

## Decision History

### Cycle 86
- **Applications reviewed:** 18 (applications 001-018)
- **Approved:** 12 (including Beverly Hayes for $18,500 relocation assistance)
- **Denied:** 3 (income threshold exceeded or insufficient displacement risk documentation)
- **Returned for documentation:** 3 (incomplete income verification)
- **Beverly Hayes (Case #001):** Approved for $18,500 relocation assistance. Determination letter issued.
- **Disbursement authorization:** Escalation memo filed with Mayor's office documenting pending Phase 1 authorization request submitted C85.
- **Processing rate established:** 15-18 applications per month sustainable with 3 staff reviewers.
- **Documents produced:** Status report, Beverly Hayes determination letter, disbursement escalation memo

### Cycle 87
- **Applications reviewed:** 17 (applications 019-035)
- **Approved:** 10 new approvals (cumulative: 22)
- **Denied:** 3 (cumulative: 6)
- **Returned for documentation:** 4 (cumulative: 7)
- **Ernesto Quintero (Case #050):** Approved for $21,000 housing stabilization assistance. Determination letter issued in English and Spanish.
- **Total approved funding:** $387,000 across 22 applicants
- **Authorization escalation:** Formal 9-cycle delay memo filed with Mayor's office. Documented what is ready, what is required, and risks. Filed as permanent record.
- **Processing timeline:** 35 of 295 reviewed. Projected completion C104 at current rate.
- **Documents produced:** Status report, Quintero determination letter (bilingual), authorization status memo

## Documents Produced

### Cycle 86
1. **doc_c86_status_report.md** — Monthly status report to Mara Vance. Fund summary, application status (18 reviewed, 12 approved), institutional bottleneck analysis (authorization hold, not processing backlog).
2. **doc_c86_determination_hayes.md** — Individual approval letter to Beverly Hayes for $18,500 relocation assistance. Explains criteria, next steps, appeal rights.
3. **doc_c86_disbursement_escalation_memo.md** — Internal escalation memo to Mayor's office. Documents 8-cycle delay (C78-C86), compliance risks, what is needed (authorization), what will be delivered (continued processing). Filed as permanent record.

### Cycle 87
1. **doc_c87_status_report.md** — Monthly status report to Mara Vance. Application status (35 reviewed, 22 approved), fund balance, clear statement that authorization remains the limiting factor, not processing capacity.
2. **doc_c87_determination_quintero.md** — Individual approval letter to Ernesto Quintero (Case #050) for $21,000 housing stabilization assistance. Explains approval, funding amount, authorization timeline constraint.
3. **doc_c87_determination_quintero_spanish.md** — Same letter in Spanish. Ensures Spanish-language applicants receive equivalent notification.
4. **doc_c87_authorization_status_memo.md** — Formal status memo to Mayor's office. Documents 9-cycle authorization delay (C78-C87). Lists what is ready to disburse, what is required, and risks of continued delay. Filed as permanent record. Direct language: "This office can prepare a draft authorization document within two business days if a template is needed."

## Key Citizens
- **Beverly Hayes** (POP-00772) — West Oakland, Home Health Aide, age 58, single, household income $38K. Case #001. **Status: APPROVED** for $18,500 relocation assistance (C86). Determination letter issued.
- **Ernesto Quintero** (POP-00050) — West Oakland. Case #050. **Status: APPROVED** for $21,000 housing stabilization assistance (C87). Determination letter issued in English and Spanish.

## Key Officials
- Mayor Avery Santana (OPP) — signed the authorization
- Deputy Mayor Marcus Osei (Econ Dev) — serious condition, unavailable
- Mara Vance — City Planning Director, ordered OEWD report
- Ramon Vega (IND, Council President) — committee review scheduled, agenda item 4

## Affected Neighborhoods
West Oakland (primary), East Oakland, Fruitvale, San Antonio, Coliseum, Elmhurst, Brooklyn

## Mara's C85 Guidance & Response Through C87
- OEWD report is overdue — **ADDRESSED:** Status reports produced C86 and C87. Escalation memos filed documenting authorization hold with documented evidence trail.
- 295 applications waiting with no public timeline — **ADDRESSED:** 35 of 295 reviewed across C86-C87. Processing rate 15-18/month established and sustained. Completion timeline C104.
- Beverly Hayes case represents the individual failure mode — **ADDRESSED:** Beverly Hayes approved C86 ($18,500). Ernesto Quintero approved C87 ($21,000). Both received bilingual determination letters explaining approval and authorization timeline.
- Vega committee review is the next governance checkpoint — scheduled C89 per initiative tracker. Will present fund status, application pipeline, and authorization delay.
- Deputy Mayor Osei absence flagged — Marcus Webb cannot answer this; it is an executive office decision. Memo notes portfolio management gap but that is outside OEWD scope.

## Institutional Position (C86-C87)
Marcus Webb's approach: Meticulous compliance, paper trail maintenance, escalation when necessary. Sustained through C87:
1. Status reports (transparent, factual) — C86 and C87 both clearly document that processing is on track; authorization is the constraint.
2. Individual approvals (relief within authority) — Beverly Hayes C86, Ernesto Quintero C87. Bilingual where needed.
3. Escalation memos (documents institutional bottleneck, protects process integrity) — C86 initial memo, C87 formal status memo naming the 9-cycle delay explicitly.

The bottleneck is NOT application processing. Processing rate: 15-18/month, stable. The bottleneck is Mayor's office authorization. Marcus has now documented this formally twice. The August C87 memo is direct: "This office can prepare a draft authorization document within two business days if a template is needed."

Marcus's private concern: The Mayor's office may be using compliance rigor as cover for political control. The escalation memos create a record that either authorization will be granted (resolving the question) or will continue to be withheld (making the political nature undeniable).

## Corrections & Rules
- Stabilization Fund is $28M, not $28B. Always double-check dollar amounts.
- Phase 1 is $4.2M of the $28M total — do not conflate the two.
- Beverly Hayes is a Tier-3 civilian — use her as a case study, not a political figure.
