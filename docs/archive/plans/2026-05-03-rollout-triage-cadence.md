...
updated: 2026-05-25
type: plan
tags: [architecture, infrastructure, archived-retired]
sources:
  - output/production_log_edition_c93_write_gaps.md G-W16 (meta-pattern — HIGHs sit on shelf and compound)
  - docs/engine/archive/ROLLOUT_PLAN.md §Edition Post-Publish (current capture pattern)
pointers:
...
- **Verify:** `grep -c "promoted: C" docs/engine/archive/ROLLOUT_PLAN.md` returns >20
- **Status:** [ ] not started
...
- 2026-05-25 — **RETIRED S235 (governance.6 close).** Plan succeeded at the time — Phases 1-4 shipped S198-S202, script worked against the cycle-promoted-tag scheme of the pre-S212 ROLLOUT structure. **Structurally superseded by S212+S229 architecture:** state taxonomy (`done-pending-archive` makes ship-but-not-archived visible at every ROLLOUT read) + per-terminal sweep ownership + governance.10-class archive sweep cadence (~one sweep per 1-2 closes since §S227 backlog drain) together address the G-W16 compounding-HIGH problem at the rollout-discipline layer rather than the script layer. Empirical proof of obsolescence: `node scripts/rolloutTriage.js <cycle>` now ERRORs with "No (promoted: C<N>, severity: HIGH) tags found" — the data the script needed has dried up because the rollout-discipline solution made the tagging convention redundant. Script preserved in-place at `scripts/rolloutTriage.js` with RETIRED header block (not deleted per SCHEMA §7 archive≠delete). Removed from `sessionEndMechanical.js` step list. References dropped from `.claude/rules/research-build.md` + `.claude/terminals/research-build/TERMINAL.md` + `.claude/skills/session-end/SKILL.md`. Unarchive triggers documented in script header: archive sweep cadence falls behind by >3 sessions OR HIGH ROLLOUT row sits across >3 cycles without movement. Tags shifted `[architecture, infrastructure, complete]` → `[architecture, infrastructure, archived-retired]`.
