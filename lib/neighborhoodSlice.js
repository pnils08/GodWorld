 ...
 *
 * Surfaces housing pressure and median income explicitly so a voice cannot
 * invent struggle the engine never reported (the C95 West Oakland failure mode).
 * S315: trajectory block replaces the gentrification block on Neighborhood_Map.
 */
...
function createSlicer({ ledger = [], neighborhoodMap = [], priorNeighborhoodMap = [], residentCap = 4 } = {}) {
  ...
  function slice(nbhd) {
    ...
    const state = m ? {
      ...
      housingPressure: String(m.HousingPressure || '').trim() || 'none',
      trajectory: String(m.NeighborhoodTrajectory || '').trim() || 'none',
    } : null;
    return { neighborhood: nbhd, residents, state };
  }

  // One-line accuracy handle a voice reads — moved metrics + housing/income
  ...
  function describe(nbhd, s) {
    ...
    parts.push(`trajectory: ${st.trajectory}, housing pressure: ${st.housingPressure}`);
