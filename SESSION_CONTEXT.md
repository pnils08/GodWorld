...
**PIN:** Session: 279→280. S280 (engine-sheet): citizen-event depth step SHIPPED in-tree — central generator (1 of ~20 emitters) rebuilt to reaction-grain, column-conditioned, household-shared texture; pivot decision: content-ledger design (RB writing plan in parallel) before hand-deepening the other emitters. S279 (engine-sheet): no engine work — refused a corrupted MEMORY.md injection. S278 (research-build): research.14 write-back gate cleared; deploy waits on engine-sheet's clasp window. | Day: 161 | Cycle: 100 | Edition: C99/E99 canonized (S265) | engine v3.4
...
**NEXT[engine-sheet]:** Deliberate clasp window: S277 atmospheric expansion + S280 depth stack (all `generateCitizensEvents.js`) → then Task 6/7 cycle verify (perf gate N=4, coverage re-baseline). HOLD other-engine depth until RB's content-ledger plan lands. Detail: plan `2026-06-30-central-generator-atmospheric-expansion.md` changelog + S280 git log. Background: ENGINE_COUPLING_MAP ~26 files left.
