...
| **Undocked_Draw** | 6/cycle | ENGINE, SCRIPT | ENGINE | engine.175 (S438): next cycle's UNDOCKED cast — fire N appends `cast-1..3` + `alt-1..3` for TargetCycle N+1 (`TargetCycle, DrawCycle, Seed, Slot, POPID, Name, BirthYear, Neighborhood, Role, Employer`). Append-only; a cycle with rows is read, never redrawn. The tab is the switch: missing = no draw, show market closed. Orchestrator reads it for `TargetCycle = cycleCount+1` (contract: spacemolt plan (a′)). **LIVE tab created 2026-09-08 19:08 (S438b) after kimi's live mint proof (Desai POP-00946) — 82 tabs; empty until engine.175 reaches PROD with the dials wave, then the first live fire draws cycle N+1's cast and opens the show market.** |
...
| **Business_Archive** | 0 (lazy) | ENGINE | ENGINE | engine.96 Task 7 (S413): the closure ledger — a closed business's full `Business_Ledger` row + `ArchiveReason`, `ExitCycle`, `SourceEventId`, `ClosedCycle`, moved by `Phase11-BusinessArchive` once its stated headcount is 0 and no tracked worker remains (copy → read-back by BIZ_ID → remove). Tab created by a Phase-10 ensure intent at the first closure; BIZ-IDs never reissued (`bizIdHighWater`). Mirrors the Citizen_Archive pattern (engine.90) |
| **Citizen_Archive** | 0 | SCRIPT (`dumpLedger.js` second snapshot, resolver), ENGINE (restore) | ENGINE (`Phase11-CitizenArchive`, flag `citizenArchiveEnabled`) | engine.90 (S428): cold full-row home for citizens who left `Simulation_Ledger` — the 55-column row verbatim at exit + `ArchiveReason`, `ExitCycle`, `SourceEventId`, `LastActiveStatus`, `ReturnEligible`, `SchemaVersion`, `ArchiveNote` (62 cols). Copy → read-back by POPID+ExitCycle → remove; never on the event loop; POPIDs never reissued (`popIdHighWater`). Ensured on the bench S428; live tab + seed via `scripts/ensureCitizenArchive.js --live` on the builder's go, which sets `citizenArchiveTabLive=1`. Plan: `docs/plans/2026-08-21-citizen-archive.md` |
...
| **Health_Cause_Intake** | ~~`healthCauseIntake.js`~~ | **CLOSED 2026-09-09 — no-create, file deleted whole (builder-direct).** The Media-Room-assigns-causes loop was dead code: no live caller, and the engine has written `Hospital_Ledger.Cause` at admission since engine.102 W4. `phase11-media-intake/healthCauseIntake.js` deleted; allowlist entry removed from `scripts/tabReferenceIntegrity.test.js`. The 2026-08-16 Mike-ruled CREATE predates engine.102 W4. `Health_Cause_Queue` remains live. |
...
- **Ghost-tab sweep (infrastructure.6 Track B, 2026-08-16).** Dispositions above — all closed; the last open row (`Health_Cause_Intake`) closed no-create 2026-09-09 (builder-direct) when its dead intake file was retired. Regression guard: `scripts/tabReferenceIntegrity.test.js` (`86f43999`).
