...
5. **Stage to canon — THE WALL (decided S332, mirrors the layer-4 subjective/citable wall).** A gated M–F article is **STAGED / unpublished**: retrievable by the Saturday compile only, tagged `status=staged`/`draft`. It does **NOT** get full canon ingest, and reporters/`sift` must **not** retrieve staged drafts as fact — else Wednesday's reporter builds on Monday's draft that Saturday cuts, a story that never published becomes a premise (the documented DeepSeek-contamination death mode). Only the Saturday-**published** set (Phase 2b) gets full canon ingest. Same wall the quote layer already draws: staging records the *writing*, publication records the *fact*.
   - **Build caveat (name for the executor, don't let a weak model paper over it):** `scripts/ingestEdition.js` / `ingestEditionWiki.js` are **edition-shaped** — read their actual input contract before assuming a `--per-article` flag exists; per-article staging may be a real adaptation, not a flag. And **verify bay-tribune can carry a published/draft distinction** before staging there; if it can't, stage to a separate tag/store the Saturday compile reads.
...
The curatorial half, and **the only place a headless story becomes citable fact.** A Saturday cron where **Mags compiles**: `/sift`-style curation over the week's **staged** articles → pick the top stories → assemble edition → `/post-publish` (which runs the existing **full** canon ingest: `ingestEditionWiki.js` + `ingestEdition.js` + world-data entity records + citizen cards). Curation of the week's staged drafts, not a fresh generation. Publishing here is what respects the S313 freeze: M–F is holding-area accrual, Saturday is the gate. Acceptance: one Saturday run produces a published, fully-ingested edition from the week's staged articles; staged-but-uncompiled drafts never entered canon.
...
- [~] **Publish target under the re-opening freeze** (Phase 2 — AWAITING MIKE CONFIRM S332): proposed default — M–F articles **stage** (holding-area, not live), the **Saturday compile is the publish gate** (Phase 2b). Respects the S313 freeze; scorecard can later relax it. One-word confirm needed before build.
...
- 2026-07-22 — Phase 2 daily writer-wake wiring specced (S332). Assembly chain over live pieces; Step 5 staging wall (M–F stage, Sat publishes) resolves canon-ingest + contamination risk. Publish-target awaits Mike confirm.
