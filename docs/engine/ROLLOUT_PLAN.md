...
1. **Live smoke of PROD @56** at the builder's next fire — eleven deployments (@46–@56) ride it, all bench-proven on live-synced C105. engine.148 expectations: World_Config +6 rows self-armed; 6 'migration wave' ledger rows in the emptiest hoods, women first; feeder adds up to 8 women; 22 Crime_Metrics rows refreshed; no throw on Neighborhood_Map AE–AH (all cells read back). Earlier waves' expectations on their rows.
2. **engine.148 Phase 3** — 7 hood-keyed texture pools + 12 repeated arts/holiday lists → character labels (design in the plan: keep bespoke lines for the twelve, character pools for the ten). Bench re-sync first (C111 carries ~60 bench-only rows).
