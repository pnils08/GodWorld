...
The engine generates a living evening Oakland — 12 neighborhoods with crowd scores, named bars and restaurants, food trends shaped by holidays and economy, safety atmospheres, celebrity sightings. None of it survives serialization. The newsroom has no evening city to report on.
...
The engine generates per-neighborhood dynamics (12 neighborhoods each with their own traffic, retail, nightlife), transit metrics with ridership and alerts, sports neighborhood effects, weather variations by neighborhood, and capacity/congestion data. The newsroom gets 8 city-wide decimal numbers. No neighborhood-level texture, no transit stories, no capacity strain signals.
...
The engine generates per-neighborhood crime data with hotspots, policing capacity, clearance rates, patrol strategies, and reporting signals. It tracks demographic shifts across neighborhoods. The newsroom gets world event descriptions from a sheet but none of the crime analysis, enforcement data, or demographic movement. A reporter covering public safety has no crime data to report from.
...
Phase 4 is partially preserved. World events, arcs, and generational events make it through both the Cycle_Packet and sheets. But: city event details (neighborhood-specific festivals, openings) are only in Cycle_Packet text, not in a sheet. The generationalSummary (who died, who got married, who graduated this cycle) is serialized but the newsroom reads from LifeHistory_Log rows instead, which may miss the structured summary. Faith events are folded into worldEvents but the faith-specific context (crisis response type, community involvement) is lost. Game mode micro-event details don't reach desks.
...
Phase 5 is the most complex phase and has the most data paths to the newsroom, mostly through sheets rather than Cycle_Packet. The critical gaps: namedSpotlights (editorial priority citizens) is in Cycle_Packet but not parsed by buildDeskPackets. storyHooks generated by gentrification, migration, career, education engines are on ctx.summary only — never serialized, never reach agents. Election results are on ctx.summary and written to Election_Log sheet, but buildDeskPackets doesn't read Election_Log. Initiative votes/grants are in Initiative_Tracker sheet which IS read. Bond summary makes it through the sheet. The biggest loss is the accumulated storyHooks — the engine says "this is newsworthy" and the newsroom never hears it.
...
Phase 6 is where the engine says "here's what matters this cycle." The economic summary is well-serialized — that's good. But the intelligence layer is mostly lost: civic load analysis (WHY the system is strained), shock monitoring (WHAT triggered the anomaly and how long it's lasted), per-neighborhood economic states, migration briefs, recurring citizen patterns, transit and faith story signals. The engine flags "shock in Fruitvale, driven by 3 crisis events over 4 cycles" but the newsroom gets "shockFlag: shock-flag" — a boolean with no context.
...
The cycle weight signal makes it through (is this a big week or a quiet one?). But the detailed reason and the compressed narrative summary of what happened this cycle — which would be enormously useful for editors — are lost.
...
Fully serialized: calendar, civic status, cycle signals, population, weather, city dynamics (8 decimals), world events (up to 8), event arcs, texture triggers, named spotlights, relationship bonds summary, generational events, economic status, media climate, weather mood, Chicago satellite.

NOT serialized: nightlife object, evening food, crowd map, evening traffic/safety, crime metrics, neighborhood dynamics, transit metrics, story hooks, civic load factors, shock reasons, migration brief, neighborhood economies, career/education/household event details, faith events detail, cycle summary narrative, recovery state.
