...
- **(a′) `Undocked_Draw` tab contract — PUBLISHED 2026-09-08 (engine-sheet, S438; builder go). Build target for both halves.**
  ...
  - **Status 2026-09-08 (S438b):** engine side CUT — `undockedDrawCast_` (draw + tab append + `S.undockedNextCast`), `casinoResolveUndocked_` aggregated pilot-cycle resolve with cycle guards (before = carry, after = void), placement keys `c<N+1>:<POPID>`, legacy episode-keyed slips still resolve. Unit: casino suite 51 checks green. **BENCH-PROVEN (SANDBOX 0831 @81, C119–C121):** C119 wrote the first draw (6 rows for c120) but the casino threw — the resolve rewrite had dropped `casinoResolveSports_`; fixed `33cf0e61` with a main-entry sports test. C120: draw for c121 + 9 show slips (`credits_sign` ×7, `night_winner` ×2) all against the drawn cast, 13 sports, 0 errors. Seeded 4 bench-only feed rows for c121 (POP-00637 40 and −100 with a mishap; POP-00700 250; POP-00833 windowed). C121 settled 9/9 as predicted before the fire: 637-neg ×3 win, 637-pos loss, 700-pos ×2 win, 833 void-gate, night_winner 700 win / 833 loss; float 250000→246970; draw for c122 landed; 0 errors. **Deploy path:** engine code goes to PROD with the dials wave (not cherry-pickable onto @74); the live `Undocked_Draw` tab is created only when kimi's (1)+(2) can fly a fresh cast. Kimi's (3) can be exercised now against the bench sheet `18BOJmzlO7EoaEhvUsUaqLIZvgrTC1yltUYk_Gz3I3W8` (cast rows for TargetCycle 122 = cycleCount+1).
