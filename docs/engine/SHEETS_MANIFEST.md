...
**S282 drift additions (seams plan):** Simulation_Ledger 49 → 50 cols — `MemoryRegisters` at AX (B1 bias + B3 unlived registers, single DialState-pattern JSON cell; sole writer will be the Phase-9 compressor fold `utilities/compressLifeHistory.js` per seams Tasks 6/8; unread until those land). Determinism caveat for the coming `Event_Content_Ledger` tab (seams Design A): replay fidelity = seed × ledger content, same property as Neighborhood_Map. Also fixed S282: `Reflection_Intake` col H header was blank while the wake writer + compressor drain used it positionally for the affect tag — header `Affect` written, no reader change (drain is positional, `RI_COL_AFFECT=8`).
