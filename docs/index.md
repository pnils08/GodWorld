...
- **[[plans/2026-05-11-civic-7-init-005-investigation]]** — S215 close of civic.7 scope (gap-log G-11). Investigation procedure for INIT-005 Temescal Health Center phase-advance discrepancy — Mayor C92 + Chen-Ramirez C92 both committed to architect-contract execution by C93; engine review C93 shows phase still hasn't moved. Two scenarios (A: commitment slipped / B: writeback bug) need disambiguation via Initiative_Tracker MilestoneNotes read + grep for C93 entries. Decision tree + fix paths per scenario + Phase 3 MilestoneNotes-reader helper (`scripts/readInitiativeMilestoneNotes.js` NEW) wired into city-hall-prep Step 1 to catch future slips at prep-time. Engine-sheet picker-grab; agent-side + skill-text edits cross into research-build as sub-task if Scenario A. *(plan, civic, engine, investigation, active)*
