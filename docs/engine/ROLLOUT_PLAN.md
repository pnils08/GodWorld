...
| pipeline.67 | Oaks beat — Selena Grant + Talia Finch re-beat from the retired Chicago Bureau to two solo Oaks seats (Mike-direct 2026-09-06: "like Nia Rook / Anthony / P Slayer"). **BUILT S429 (research-build): voice files rewritten to the Oaks, `.claude/agents/selena-grant` + `talia-finch` (IDENTITY/LENS/RULES/SKILL), persona-map + approach-map entries, wake packages staged under `_pending-*` keys in `newsroom-wake-packages.json` (loader skips `_` keys), `chicago-desk` agent + skill retired, desk-model-map/index/supplemental/dispatch references re-beat. Neither reporter is on the Simulation_Ledger — HANDED to engine-sheet: mint two MEDIA rows (spec in the S429 dispatch), then rename the `_pending-` keys, fill `assignment.popid`, fill persona-map `popid`. Acceptance = first M–F fanout after the mint wakes both seats off a `TeamsUsed: Oaks` row (opening night is C108).** | in-progress | research-build → engine-sheet (mint) | [[../plans/2026-08-07-anthony-hal-solo-sports-seats]] §Task 6 |
