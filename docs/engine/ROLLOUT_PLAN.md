...
| pipeline.67 | Oaks beat — Selena Grant + Talia Finch re-beat from the retired Chicago Bureau to two solo Oaks seats (Mike-direct 2026-09-06: "like Nia Rook / Anthony / P Slayer"). **BUILT S429 (research-build): voice files rewritten to the Oaks, `.claude/agents/selena-grant` + `talia-finch` (IDENTITY/LENS/RULES/SKILL), persona-map + approach-map entries, wake packages staged under `_pending-*` keys in `newsroom-wake-packages.json` (loader skips `_` keys), `chicago-desk` agent + skill retired, desk-model-map/index/supplemental/dispatch references re-beat. **S430 (engine-sheet): NO MINT — both were already on the ledger (the S429 name scan missed them): Selena Grant = POP-00591 (T2, Chinatown, b.2006, married, RoleType already "Journalist - The Oakland Oaks Beat Reporter"), Talia Finch = POP-00592 (T4, Jack London, b.2012, single; RoleType + CitizenBio re-beat 2026-09-06 to "Journalist - The Oakland Oaks Ground Reporter", read back, snapshot refreshed). Both CitizenBio "Former Chicago Bulls Reporter" — the re-beat was already their story. BACK to research-build: rename the `_pending-` keys, fill `assignment.popid` 00591/00592, fill persona-map `popid`, and align the voice files to the LEDGER facts (Selena is 35 not 31; Talia lives in Jack London).** Acceptance = first M–F fanout wakes both seats off a `TeamsUsed: Oaks` row (opening night is C108).** | in-progress | research-build (activate) | [[../plans/2026-08-07-anthony-hal-solo-sports-seats]] §Task 6 |
