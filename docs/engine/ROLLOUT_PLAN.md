...
4. **Phase 39.1** — Capability reviewer. **PLAN WRITTEN S146** at `docs/engine/PHASE_39_PLAN.md`. 11 assertions across 5 categories (coverage, representation, three-layer, freshness, rubric-fidelity). Deterministic where possible + narrative-grader fallback for the assertions that need judgment (Haiku 4.5, < $0.05 per cycle). Plugs into `/write-edition` as Step 3.5 between Compile and Validation+Rhea — blocking failures (front-page misses the highest-severity ailment, invented citizen names) halt publish. Replay test against E91 as the validating acceptance: reviewer must flag the missed Temescal coverage as a blocking failure. Spine steps 6 + 7 scaffolded with readiness notes; full plans deferred until 39.1 ships and reviewer-output shape is known. **READY TO BUILD `(research-build terminal)`** — mostly code with thin skill wrapper, can build solo unless §11 open questions surface.
