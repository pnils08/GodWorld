...
- **[[plans/2026-05-03-run-cycle-gap-log-surface]]** — S197 Wave 4 meta-gap; **REWRITTEN S198** with all four open questions closed in grill. Engine-sheet "Never create MDs" rule loosened wholesale to align with global no-isolated-MDs rule (broader reach than gap logs alone — per-phase audit notes, schema specs, helper-script docs all become valid). Coder-persona directive locked: hybrid authorship with `scripts/engineCycleAudit.js` doing mechanical detection + engine-sheet adding judgment entries in terse mechanical voice (commit-message style). 8-class engine-fitted taxonomy (phase-skip, writeback-drift, cohort-collision, math-anomaly, determinism-break, phase-ordering, silent-fail, cross-cycle-debt). ROLLOUT triage routing same as other gap logs; cross-session defects go to ENGINE_REPAIR. *(plan, engine, infrastructure, architecture, draft)*
- **[[plans/2026-05-03-rollout-triage-cadence]]** — S197 Wave 4 closes G-W16 meta-pattern (HIGHs sit on shelf and compound — S193 G-R6/R7/R10 → S195 G-W12 recurrence); **READY S198** — all 5 open questions closed via blanket approval. Recs adopted: trailing inline tag `(promoted: C<XX>, severity: HIGH)` / STALE-2C at >2 cycles / in-place `[STALE-2C]` flag / explicit `PARKED: <reason>` override / scan at /session-end. 4 phases ready (design DONE / backfill existing entries / build rolloutTriage.js + wire to session-end / validation). *(plan, architecture, infrastructure, ready)*
