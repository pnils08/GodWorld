...
- **Suggested action:** ~~OPEN~~ **FIXED S409 (`ec6bf5e4`) — root-caused, and it was never the suite.** Fixtures said `CYCLE 900`; real editions say `THE CYCLE PULSE — EDITION N`, so the rater's text regex missed and cycle detection fell to `filePath.match(/(\d+)/)` over the FULL path — a digit was present only when mkdtemp's random six-char suffix contained one (~65%). Reproduced 2/5 passing in isolation. Fixtures now carry the real header, the rater's fallback reads `path.basename` only, `lib/sheets` is lazy-required so dry-run subprocesses skip googleapis, and `run-tests.js` prints exit/signal on a failing file. 6/6 after. Chase plan §S-E.
...
- **Suggested action:** **CLOSED S406, by ruling + one wording fix.** All three readings were arithmetically correct against the live tracker; they answer three different questions and were never in conflict. Verified against the six live rows at C105 (`Initiative_Tracker`, read 2026-08-31).
  - **Approval engine — INTENTIONAL, documented, unchanged.** `classifyInitiativeMotion_` (`updateCivicApprovalRatings.js:745`) reads the *current* phase string and `NextActionCycle`. It never compares against the prior cycle, so **phase advancement is structurally invisible to it by design** — only `complete` pays (`isPerforming_:721`, whose own comment records why: "C103 sat at 95 on those and never built anything"). All six rows were non-complete with `NextActionCycle >= 105`, so all six scored `sitting` at −2 owned = −12; the remaining −1 of the observed −13 is decay. The scoring is not the defect. Ruling pinned in the function's doc comment and locked by 6 regression cases (`civicApprovalCeiling.test.js` P1–P5).
  - **Engine audit — INTENTIONAL, unchanged.** It counts phase-string movement between snapshots. "Advanced" and "not finished" are both true of the same row; that is not a disagreement.
  - **Initiative engine — WORDING DEFECT, FIXED.** `Processed N` printed `S.initiativeEvents.length` — decisions that *resolved*, not rows looked at. At C105 the correct values were: examined 6, in-scope 4 (INIT-002 and INIT-006 are `passed`+`signed`, skipped at :201), resolved 0 — because no row had `VoteCycle === 105` with an open status. "Processed 0" read as "the engine cannot see the tracker" when it saw all six and correctly resolved none. The line is now `civicInitiativeEngine v2.0: Examined N | In-scope M | Resolved K | …`, so scope and resolution can never be conflated again. `Examined 0` is the shape that should alarm.
- **Follow-on opened:** G-PF33 (the cadence seam this chase exposed). No stored civic number was retro-corrected — engine output is canon, fix forward.
- **Pointer:** engine.138
...
### G-PF35: the ledger snapshot can be stamped BACKWARD by a caller working an older cycle

- **Severity:** MED
- **Category:** quiet-pipe
- **What happened:** Found reviewing the working tree at the S407 close, not by a test. `simulation_ledger_snapshot.meta.json` read `"cycle": 104` with a `generatedAt` of 2026-09-01T19:56Z while the engine is canonical at C105. Traced: a civic leg re-run at cycle 104 (the C104 packets, `prep_c104.json` and `production_log_c104.md` are all touched in the same window). `cron-civic-run.js` Step 1.5 tests `meta.cycle !== cycle`, so it read a *newer* snapshot as stale, refreshed it, and handed `104` to `dumpLedger.js`, which stamped it verbatim. Rows were never wrong — 968 either way, straight off the live sheet. Only the stamp regressed.
- **Why it matters:** S329 inverted. That was a stale file claiming to be fresh; this is a fresh file claiming to be stale. The snapshot is rank-0 on `lib/mags.searchDisk`, backs `search_everything` and every `canon-name-check` profile resolution, and every consumer gates on the stamp — so a backward stamp makes the freshest possible snapshot read as stale to the Discord bot, the citizen wakes and the desk runs, each of which then burns a full `Simulation_Ledger` read trying to fix it and gets the same wrong number back.
- **Suggested action:** **CLOSED S407 — the stamp is monotonic even though the rows are not.** `dumpLedger.js` refuses a backward stamp: it still writes the rows (current by construction) but keeps the higher cycle and says so on stderr, naming `lib/getCurrentCycle` for a caller that meant to advance. A null cycle argument (`undockedDraw`'s refresh path) no longer erases a good stamp either — unknown is not newer than known. **Both callers corrected in the same pass**, because the guard alone would have made it worse: civic's post-refresh equality check would have become a hard HALT on a snapshot fresher than it asked for. The real contract is **NOT-OLDER, not equal** — in civic Step 1.5, in `canon-name-check.ensureLedgerSnapshot`, and in the `fresh` flag it returns. Older is the failure the S252/S329 halt exists for; newer is the same live sheet. Snapshot restored to C105, 968 rows, read-back verified. Proven on all three branches: `105` stamps 105; `104` refuses and keeps 105; no argument keeps 105. `ensureLedgerSnapshot(104)` against a C105 snapshot now returns `fresh: true` with no refresh, where before it refreshed on every call and could never match.
- **Pointer:** pipeline.63 (`4bd63509`)
...
- **Suggested action:** ~~OPEN~~ **FIXED S409 (`6239897f`) — the second option.** `lib/sheets.js` `citizenFullName` / `matchCitizensByName` (pure; exact first, substring fallback, returns every hit) / `findCitizensByName` (live). SIMULATION_LEDGER §Identity states the no-Name-column fact. Every engine `indexOf('Name')` site found reads a tab that HAS a Name column (Citizen_Media_Usage, Cultural_Ledger, Community_Programs, Business_Ledger) — the exposed path was ad-hoc queries, and a bare `indexOf` is still not loud; the helper plus the doc is the guard. Chase plan §S-E.
- **Pointer:** engine.138
...
- **Suggested action:** **CLOSED S407.** `buildNeighborhoodTexture.js` now reads `INSTITUTIONS.md` and feeds two things per hood, and the split is the safety property. **IDENTITY** — the hood's own canon paragraph from §Neighborhoods, all 22 of them (4 Tier-1 + 18 in the district-anchor backlog), condensed to the part that describes the PLACE (everything from the council-anchor clause on is canonization bookkeeping) and figure-scrubbed, because handing "$2.1B" and "35,000-seat" to a generator forbidden to print numbers invites the exact leak `metricWarnings` exists to catch. Rendered `IDENTITY (do not print)`, matching the existing GROUNDING convention, with a rule saying it is REGISTER not content: a quiet week in Chinatown should not read like a quiet week in Baylight. **CANON NAMES** — landmarks parsed from `### Tier 1 — Use real names` sections ONLY, routed to a hood by the entry naming that hood. Tier 1 is by definition the safe-to-say set, so the feed *structurally* cannot hand over a Tier-2 name needing a substitute; Tier 2 tables are never read here, which stays the blocklist's job. The rule attached forbids introducing a name as a new subject — a name is not an event. Verified: 22 hoods with identity (4,040 chars total, cheap), 4 with landmarks (KONO → Fox Theater Oakland + Telegraph Avenue; Jack London → Heinold's; Chinatown → OACC; Temescal → Telegraph Avenue), and the deterministic canon sweep run over the entire injected feed returns **0 hits** against the 39-name blocklist.
- **Pointer:** pipeline.63
...
- **Suggested action:** **CLOSED S407 — canon names, and the gate was right.** Settled against the loader, not by preference: `hood` is `{ kind: 'str' }` in `loadEventContentLedger.js:71`, matched against the citizen row's Neighborhood — the canon form. A row gated `hood=lake_merritt` would have been written, looked healthy in the sheet, and never fired for a single citizen. So rejecting it was correct; **rejecting was the wrong remedy.** A slug is unambiguously recoverable and the lines attached to those five gates were good. `normalizeHoodGate` now repairs the gate to canon form and stores the corrected string (normalizing only enough to pass our own check would write a row the engine can never match — the silent version of the same bug), and the drafter prompt gained an enumerated verbatim hood allow-list, since the canon names were already inside the facts payload and the model slugified them anyway. Repairs print as `REPAIRED hood gate: … -> …` and are counted in the report line — the day that count stops falling is the day to go back to the prompt. Genuinely unknown hoods and ANDed double-hood terms (unsatisfiable, not a typo) still reject. All five C105 rejects recover: `lake_merritt`→`Lake Merritt`, `temescal`→`Temescal`, `downtown`→`Downtown`, `rockridge`→`Rockridge`, `fruitvale`→`Fruitvale`. 59/59 in `draftContentRows.test.js`.
- **Pointer:** pipeline.63
...
- **Suggested action:** **CLOSED S407 — both, not either.** (1) The step moved: 5.57 → **5.85**, after Step 5.8, which is what produces the `base_context.json` it folds. Verified no reverse dependency — `buildDeskPackets` does not read `world_state.json`, and neither does anything between the old and new positions; the only readers are `lib/mags.js` and `lib/wakePerception.js`, both cron-side. (2) `buildWorldState.js` now refuses to write on ABSENT canon instead of exiting 0. The distinction is the fix: STALE still degrades-and-flags (the edition pipeline is frozen, so lag is expected), but ABSENT means the chain ran out of order. Every consumer falls back to per-file reads when the fold is MISSING and none of them can tell when it is GUTTED — so writing nothing is strictly safer than writing 5KB. `--allow-no-canon` is the deliberate escape for a bench or fresh clone. Proven on both branches: absent → exit 1, nothing written; absent + flag → 5,484 bytes, exit 0; restored → 55,684 bytes, `canon current`, exit 0. The gate now names the size tell.
- **Pointer:** pipeline.63; `.claude/skills/run-cycle/SKILL.md` §Step 5.85
...
- **Suggested action:** ~~OPEN, unowned~~ **FIXED S409 (`535ff9cf`, undeployed) — both slots inserted at their production positions;** live `safePhaseCall_` lists now diff identical. Mattered more than LOW: `Phase9-DigestSummary` writes `S.compressedLine`/`S.cycleSummary` that `buildCyclePacket_` reads at Phase 10, so the dry-run packet was missing fields. Not unified into one list — a refactor, not a chase fix. Chase plan §S-E.
- **Pointer:** engine.138
...
- **Suggested action:** ~~OPEN, unowned~~ **FIXED S409 (`a94e4dc8`), all three.** (1) `stubEngine.js` write regex now `=(?!=)`; regen removed 27 phantom writer edges across 15 fields — `civicLoad` 3→1, `cycleWeight` 2→1, `shockFlag` 3→1. (2) `writeCycleWeightToDigest_` deleted (0 callers; listed under manifest §9 Deleted). (3) `Riley_Digest` added to manifest §1. Chase plan §S-E.
- **Pointer:** engine.138
...
- **Status (S408, 2026-09-01):** CLOSED as triage — 7 of the 35 were ctxMap false negatives (multi-line closures, Phase-0 direct calls, `typeof` registry dispatch; fixed `cad4f180`/`75657fe6`), the 29 real ones are classified 21 off-cycle / 8 retired / 1 never-wired / 0 fell-out, every one now carries a `@cycle-status` header and ctxMap reports only UNDECLARED files. Delete batch filed as engine.142. Detail: [[../docs/plans/2026-08-31-c105-chase-sessions]] §S-D DONE.
- **Suggested action (original):** OPEN, unowned. **Surfaced, not chased** — deliberately not triaged this session (engine.137 scope guard). The work is one pass classifying each of the 35 as *retired-on-purpose* (delete or annotate) vs *fell-out-of-the-graph* (rewire or file a row). Regenerate the list any time with `node scripts/ctxMap.js | sed -n '/^DEAD FILES/,$p'`.
- **Pointer:** engine.138

### G-PF33: the engine judges a field only the offline civic chain can write

- **Severity:** HIGH
- **Category:** incoherence
- **What happened:** Fell out of the G-PF19 chase. `NextActionCycle` and `ImplementationPhase` are the two fields the approval engine scores initiative motion on (`updateCivicApprovalRatings.js:247,252`), and **nothing inside the cycle advances either of them for an initiative already in implementation.** Full writer set, verified by grep across `phase*/`, `scripts/`, `lib/`: `scripts/applyTrackerUpdates.js` (the offline civic chain, `--apply` only) and `civicInitiativeEngine.js:2757`, which stamps `NextActionCycle = cycle` inside `createInitiative_` at birth. The in-engine reschedule at `civicInitiativeEngine.js:246` fires only for `status='visioning-complete'` **and** `ImplementationPhase='vote-ready'` — and `vote-ready` is likewise chain-written, so INIT-003 (`visioning-complete` / phase `visioning` / next 105) can never move under its own power.
- **Why it matters:** The city's most visible number is on a clock the engine cannot wind. Four of the six live rows carry `NextActionCycle=105`; there is no `close_c105.json` or `gate_c105.json` in `output/cron-civic/` (C105 packs are staged, un-applied), so if the chain does not apply before C106 fires, those four go `105 < 106` → `silence`. The fall is bounded — v1.7 diminishing stacking scores owned silence `[-6,-3,-2,-1]`, so four owned silences cost exactly −12, plus −4 from the two rows still sitting: Mayor 69 → ~53. Not a cliff, and campaign threshold is 40, so no seat is at risk next cycle. But the direction is one-way and the cause is a cadence gap, not a decision anyone made in-world: the engine reads a promise the world had no way to keep.
- **Suggested action:** **FIXED S406 — builder ruled (a), bounded.** `engineClockHold_` (`civicInitiativeEngine.js`, new pure helper) holds an expired clock forward one cycle for a row the engine has no in-cycle path to advance, and records the hold in `Notes` as `[ENGINE-CLOCK n=<used> from=C<origin>]`. Runs after the v1.9 reschedule (so `status`/`voteCycle` are current) and before the vote trigger (so a row the engine is about to resolve is untouched).
  - **Bounded to 3 consecutive cycles, deliberately.** An unbounded hold would erase `silence` altogether, which is a worse defect than the false blame it fixes: a row the chain has abandoned would read healthy forever. After the budget the row is released to silence — because by then it *is* genuinely stalled, and that is the true story. The drain is delayed, never removed.
  - **Self-clearing.** Any chain apply writes a live `NextActionCycle`, the marker is retired and the budget resets to zero. The counter therefore only ever measures consecutive cycles with no civic chain behind them.
  - **Guarded on both columns.** Without `Notes` there is nowhere to bound the grace, so the hold does not engage at all rather than run unbounded.
  - Write is legal: `civicInitiativeEngine.js → Initiative_Tracker` is a sanctioned `own-tab` direct writer in `SHEETS_MANIFEST` §9.
  - **13 regression cases** (`civicApprovalCeiling.test.js` Q0–Q12) covering accrual, origin preservation, exhaustion→silence, re-arm reset, the engine-will-act and unexpired no-ops, and Notes round-trip. 76/76. 0 function collisions across 1198 GAS globals.
- **Builder rulings taken with it (S406):** initiatives stay **hand-fed** — `createInitiative_` stays unwired, revisited only when city-hall seats work a full week autonomously; approval **scoring is unchanged**.
- **Regression cover:** `civicApprovalCeiling.test.js` P6 pins the C106 transition so the seam cannot close silently.
- **Pointer:** engine.138

### G-PF34: the civic system had no way up — three of four approval inputs could only subtract, and the fourth never fired

- **Severity:** HIGH
- **Category:** incoherence
- **What happened:** Builder-direct escalation of the S-A finding. Mayoral approval has **exactly four inputs** and, as wired, **three could only subtract**: negative civic media, decay toward 50 (`-1` above the line, with an explicit `v1.3: no free recovery toward 50` below it), and the ceiling scandal. The fourth — initiative motion — was the only positive path and paid only at `complete`, seven phase-steps down an arc the offline chain advances weekly. **With initiatives stalled there is no restoring force in the system at all.** Two specifics found on the way:
  - **The media arm was not negative-only, it was OFF.** The rule was `rating <= -3 → -2` with no positive arm. Fifteen cycles of live CIVIC ratings — `1, 0, 2, 1, 0, 2, 3, -2, -2, -1, 2, 1, 2, -2, -2`, range **-2..+3** — **never once crossed that threshold.** Across all 86 rated rows, 57 sit in the dead band.
  - **The real inflation bug was still armed.** The v1.3 clamps were written to stop the C103→95 approval pin, and they treated the wrong cause. The pin was never "positives exist" — it was `isPerforming_` returning true for any row parked at `complete`, paying its owner **+3 every cycle, forever**, because the approval engine filters its initiative list on `if (!initName) continue;` and nothing else. That bug survived every clamp and is dormant only because nothing has ever reached `complete`. Adding advancement credit without fixing it would have re-armed the pin the moment anything finished.
- **Why it matters:** Every clamp decision predates the wiring. Coverage ratings wrote nothing between C102 and the G-PF7 fix, and the civic voice→world carrier only landed this session (G-PF18, PROD @12). **No decision in this scoring path has ever seen the system work.** Meanwhile the number is the city's most visible, and it could only fall.
- **Suggested action:** **FIXED S406 (engine.139), builder-direct.** New rule: **positives are EVENTS, negatives are CONDITIONS.**
  - `classifyInitiativeMotion_` takes a fourth argument, `prevPhase`, and splits the terminal state from the transition: **`completed`** (moved into a complete phase) pays +3 owned / +1 once; **`complete-held`** (parked there) pays **0**; **`advanced`** (any non-failing phase change) pays +2 owned / **+1 to a nearby district seat** — the builder named districts explicitly. Not-finished and overdue still drain per cycle (`sitting`, `silence`) because those are conditions, not events.
  - Lifetime yield per initiative is therefore **bounded** — roughly six transitions plus one completion — and structurally cannot pin an official at ceiling. The approval-ceiling scandal still backstops sustained highs.
  - **Media is symmetric and graded inside the range the ratings actually occupy:** `>= +4 → +2`, `>= +2 → +1`, `<= -2 → -1`, `<= -4 → -2`. Scored against the live 15-cycle record the channel totals **+1** — texture, not an inflator. Note the `-3` case softens from `-2` to `-1`; since the old threshold never fired, that is a **no-op against canon**, recorded here rather than smuggled.
  - **Portfolio-width ladders extended** (v1.7's shape, and the missing cap flagged at obs 58092): `sitting` owned `[-2,-1,-1]` and `advanced` owned `[2,1,1]` are deliberate mirrors — a cycle where everything moves is worth +4, a cycle where everything stalls costs -4. `silence` keeps its heavier `[-6,-3,-2,-1]` curve because overdue is worse than unfinished.
  - **Carrier:** `previousCycleState.initiativePhases`, written at Phase 5 from the tracker read the approval engine already does, serialized at Phase 9 (`finalizeCycleState_` v1.9). Gated on the blob being **exactly one cycle old** so a stale carry-forward (bench replay, skipped restore) cannot re-detect an old transition and pay twice. Absent it, nothing reads as a transition — the conservative direction.
- **Counterfactual on real cycles (mayor, owns all six):** C104, where four initiatives advanced, scored **-13**; under engine.139 it is `+4 advanced, -3 sitting, -1 decay` = **0**, holding at 82. C105, where nothing moved, scored **-13**; it is now `-4 sitting, -1 decay` = **-5**. The number becomes volatile instead of monotonic — which is the point.
- **Cover:** 22 cases (P1-P13, R1-R6, plus F5/F6 rewritten off the old contract). **91/91.** 0 collisions across 1198 GAS globals. `ctxMap` reports `initiativePhases` CONNECTED.
- **Proving note:** `advanced` **cannot be proven by a plain bench fire** — no chain runs on a bench, so no phase ever changes and the transition path never fires. A standard two-cycle wave proves only the degraded path and the sitting ladder. The wave needs a **hand-staged phase edit on the bench tracker between fires** (bench-only writes are sanctioned), with the C104 counterfactual above as the expected shape.
- **Pointer:** engine.139
...
- 2026-08-31 — **G-PF34 FIXED** (S406, engine.139, builder-direct): civic scoring re-wired on "positives are events, negatives are conditions". Advancement and completion pay once, parked-at-complete pays nothing (defusing the still-armed C103→95 pin), media goes symmetric inside its live range, sitting and advanced get mirrored width ladders. 91/91.
- 2026-08-31 — **G-PF19 design note superseded.** The G-PF19 *ruling* stands — the three systems were never in conflict. Its design conclusion ("scoring untouched; do not add advancement credit without a builder ruling") was reopened and replaced the same session by G-PF34: the builder ruled that the v1.3 clamps were set while the media channel was dark and no decision ever saw the wired system.
- 2026-08-31 — **G-PF33 FIXED** (S406, engine-sheet): bounded engine clock hold — 3 cycles of cover for a row only the offline chain can advance, then release to silence. Builder ruled initiatives stay hand-fed and approval scoring stays as-is. 13 cases, 76/76.
- 2026-08-31 — **G-PF14 worsened, still S-E's.** `rateEditionCoverage.test.js` now fails **standalone** at 1/16 ("Could not detect cycle number from file"), not just under the full suite as originally logged. Proven pre-existing: identical 1/16 with S406's changes stashed. Diagnosis in the original entry is stale — it is no longer intermittent.
- 2026-08-31 — **G-PF19 CLOSED** (S406, engine-sheet, chase session S-A). All three civic readings verified correct against the six live rows; two ruled intentional and documented in code, the third fixed as a wording defect (`Processed N` → `Examined / In-scope / Resolved`). No stored civic number retro-corrected. 6 new regression cases, 63/63. **G-PF33 opened** from the same trace — the engine scores `NextActionCycle`/`ImplementationPhase`, and only the offline civic chain can write them.
...
- **G-EC57 [judgment]** — engine.163: Citizen_Media_Usage `Context` (the writer-wake packet stem, model id and all — an ops dedupe key) reached LifeHistory_Log `EventText` verbatim via the Advancement_Intake1 J note: 11 live `Advancement` rows carry `deepseek-deepseek-chat` / `packet-v2_…` filenames in a citizen's life record (C104–C105). Not in SL col-O (0), not rendered anywhere (0). Fixed at the seam (`describeUsageContext_`, mediaRoomIntake.js; bench @66 C110/C111 clean — note reads "a Bay Tribune sports desk piece, C109"). The 11 historical rows are a many-row sheet write, builder's go. Note: "Anthropic shuttles" event texture is library-s325 authored content, not this defect — 46 rows match a naive grep, only the 11 Advancement rows are real. Anthropic is canon (INSTITUTIONS §stack companies: the software behind the city's systems, Civis Systems' model layer) — not a leak. **Scrub DONE S431:** 11 rows rewritten via lib/sheets updateCell, read-back 11/11, 0 Advancement rows carry a stem.

- **G-EC58 [judgment]** — POP-01083 Antone Bautista (Relief Pitcher / Oakland A's, minted S403 at C105) carries BirthYear `2--6` — a hand-mint typo the audit never flagged (`Number('2--6')` is NaN, and `by &&` skipped it; guard closed S431). No roster/feed source carries his age (As_Roster row has no Year/Age, MLB_Game_Intake and Oakland_Sports_Feed have no row); peers POP-01080–01082 are b.2010–2012. Sports is Paulson's domain — the year is his to state; not derived.
