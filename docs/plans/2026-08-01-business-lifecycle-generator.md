...
- **Mechanism:**
  1. **Field:** tally `skillTagField_` over every living member's SkillTags tokens (both truths; athlete/coach/scout resolve to nothing — Paulson's domain) with `roleFieldOf_(RoleType)` as the fallback per member; the most common field wins, a tie goes to the staker's own; a line with no field (Kelley alone) takes the hood's most common business field on the Business_Ledger; none → Small Business.
