...
**Documented failure of the current pipeline (the proof case).** C100 fired a population-wide sentiment anomaly across ~20/21 neighborhoods (engine.38 G-EC33 sentiment-uniformity watch) PLUS at least one abnormal crime spike (engine-flagged anomalies, `output/engine_anomalies_c100.json`; claude-mem 33970 "Edition 100 Published Without Covering Either Engine-Flagged Crime Anomaly"). The edition said **nothing** about either. The template desks covered their lanes (sports, civic — which do affect citizens) and walked past the cycle's largest actual event — the one *most of the population lived*. A desk charged to "find the story that matters to the people here," looking at that data, cannot miss it, because the people didn't. This is the bug-is-event principle and the dispatch retool, both validated by a real omission.
