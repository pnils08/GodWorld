...
| T5 | **Varied-provocation question bank** — seeded pick from 10–20 provocation-types per daypart, each latching a real engine signal (self / block / city-sports / media / people / likes-inferred). Replaces the fixed user-prompt; fixes vector-2 (convergent *mode*). Conditions the research.21 convergence signal. §T5 Design above. | research-build (design DONE S273) → engine-sheet (build) | none (wake-side, seeded/frozen) |
...
## T5 Design (S273) — varied-provocation question bank (the vector-2 fix)

**Decision (Mike-approved S273).** The wake's user-prompt stops being one fixed question ("write a private reflection on the small things") and becomes a **deterministically-seeded pick from a bank of 10–20 provocation-types per daypart**, each latching onto a *real engine signal* the citizen already perceives. This is the **user-prompt** twin of T2/T1a/T1c's **system-prompt** enrichment: those vary the *answerer*, T5 varies the *question*. Fixes **vector-2** — the single shared task that channels every citizen into the same *mode* of thought even when their inputs differ. Mike's framing: citizens "wake with the context and be **prompted differently from others in the same wake cycle**" → variety AND broader engine-output coverage (each provocation-type pulls a different signal, so across many wakes more of the engine's output gets painted).

### The bank (grouped by latch-target — each pulls a real signal)

| Category | Latches onto | Example provocation |
|---|---|---|
| **Self** | LifeHistory arc / latest milestone (T1a′) · a dial tension | "something from your past has been on your mind — what, and why now?" |
| **Block / world** | T2 neighborhood texture · a hood event | "what did you notice on your own block this week?" |
| **City / sports** | A's feed (T1b) · a city event · the construction site | "the city's buzzing about something — what's your take, as someone who lives here?" |
| **Media / leisure** | the cycle's evening-media list · a local spot | "you caught a show / passed a place tonight — what stuck?" |
| **People** | a co-resident · a relationship | "someone's been on your mind — what's going on between you?" |
| **Likes / dislikes** | *inferred* from dials + occupation + history (see gap) | "something small delighted or annoyed you today — sit with it." |

### Mechanism

- **Selection: deterministic seed** = hash(`popId` + `cycle` + `daypart`) → pick one type → fill its template with the citizen's *actual* signal values. **Frozen per wake** (determinism-safe, input-side, same discipline as all perception). Varied across citizens / cycles / dayparts — so two citizens woken the same cycle get *different* provocations, and the same citizen gets a different one next cycle.
- Each type is a **short, specific** provocation, not an essay prompt — a latch, not a frame.
- **Phase-1 gate unchanged** — only the classified tag reaches the engine; the provocation is wake-side input.

### Honest gap (carry from S273)

"Likes / dislikes" + per-citizen **media attribution** ("a show *they* watched") imply *preferences/attribution we don't model* — media + events are city-wide, not tagged to a person. **Infer-light now** (derive from dials / occupation / neighborhood / relationships); flag a real **preference signal** as a future engine add (not built here). Don't fake per-citizen attribution.
...
### research.19 — status (drained from ROLLOUT, 2026-06-26 / S274)

**Citizen perception & immersion access layer** (S269 Mike-direct — "what allows a citizen to immerse in a world?"). The INPUT side of the citizen loop (research.14 owns reflection/write-back; this owns what a citizen *perceives*). **Baseline audit:** live wake is thin (disposition + last-5 LifeHistory + 3 names) with two holes — **writes a memory it never reads** (amnesia) + **no world beyond the block** (live wake dropped research.13's world-summary/sports/memory pieces). 4-ingredient immersion frame (continuity / world-larger-than-self / relationships / stakes) → **4 access tiers:** T1 own-page read-back (bounded+guarded) + sports feed + neighborhood-state (now); T2 world-summary→lived-particulars **translation build**; T3 read-the-Pulse guarded pilot (needs research.17 filter); T4 ask-the-citizen = research.12 Layer 3. **Self-pressure-test (advisor PENDING — was overloaded):** page-readback AMPLIFIES fabrication (NOT contamination-free — the page holds the S264 "shady Greg" invention), T2 is a real build not free reuse, page growth needs bounding (Phase-C scale), immersion amplifies the write-back (couples to engine.38 B4 cap + affect re-audit), T3 needs a real canon filter not a prompt. Guardrails: lived-particulars-not-aggregates, C92 containment, scale-honesty, determinism. **ADVISOR PASS DONE S270.** Killed T1a-as-prose-read-back (AP-1: research.17 is a *salience* filter, not a canon-truth validator — never guarded the fabrication risk; AP-2: prose read-back compounds C92, rolling summary doesn't save it). **Reframe: T1a pivots to canon-anchored self-state read-back (LifeHistory rollup + dial trajectory) — ungated AND contamination-free** (fabrication lives in generated prose, not canon self-history; LifeHistory already in wake per CV-1). **T1a + T1b now buildable, ungated, off the C100 wall** (engine-sheet build); research.17 demoted to a Tier-3-only dependency. T1b pre-pipe: confirm Oakland_Sports_Feed canon-clean. **T1a + T1b BUILT S270 (engine-sheet, `f17d110a`, wake-side `scripts/citizen-wake.js`, no clasp).** T1a = canon-anchored self-state read-back: "Your life so far" milestone arc sourced from **LifeHistory_Log** (canonical per-citizen event history — inline col O is compressed so milestones live in the Log; NOT prose, per AP-reframe) + a **dormant** dial-trajectory hook (current==base until the write-back drain deploys; surfaces a shift only on real movement, never fabricates). T1b = "Around Oakland" A's slice from `Oakland_Sports_Feed` (TeamsUsed-filtered A's-only, valid record only, narrative not FanSentiment aggregate). T1b pre-pipe DONE (spot-check canon-clean: GodWorld roster + fictionalized opponents). Measure-twice: dry-run prompt dump caught empty-arc (inline vs Log) + conflated-sports (Oaks/A's) pre-commit, both fixed + re-verified on POP-00001. **T2 DESIGN DONE S273** (shared generative per-neighborhood texture digest `output/neighborhood_texture_c{XX}.md`, frozen post-world-summary, wake reads hood block — mirrors T1b seam; generator = batched OpenRouter call + blocklist sweep, source-grounding bounds fabrication; build handoff + acceptance criteria in plan §T2 Design). **T1a′ filed S273** (production-verify finding: T1a's arc is an aggregate `"N advancement events"` that never surfaces in 15 post-upgrade reflections — translate to named latest milestone from LifeHistory_Log, canon-anchored/ungated, rides T2 build). **T1c UN-DEFERRED S273 (Mike — overturns AP-2 gate):** prose read-back is UNGATED (page = subjective memory; self-read-back never crosses the publication wall; "shady Greg" = continuity not contamination), size-bounded for tokens only — closes the original amnesia hole, cheapest immersion fix, coexists with shipped T1a. **T2 + T1a′ BUILT S272 (engine-sheet, concurrent same-day)** — `beda8fa7` (`buildNeighborhoodTexture.js` v1.0.0 + wake reader) + `0110597f` (T1a′ count→named milestone) + `fb73e4e2` (T2 contentless-event guard). **C100 artifact live** `output/neighborhood_texture_c100.md` — design validated first run (lived-particulars, zero metrics, source-grounded, 13/21 quiet-week guard holding). T1a′ surfaced **ENGINE_REPAIR Row 32** (ARC EventText is bookkeeping → starves milestone-name quality; engine-sheet follow-up). Full-stack dry-run verified POP-00001. **T5 DESIGN DONE S273** (varied-provocation question bank — seeded pick from 10–20 provocation-types/daypart, each latching a real engine signal; fixes vector-2 / convergent-mode; conditions research.21 signal; §T5 Design in plan + engine-sheet handoff). **T1c BUILT S272** (`b3841e4a`, bounded own-page prose read-back, `lib/citizenPage.js` + wake reader, no clasp) **+ T5 BUILT S273** (`1a226492`, `lib/provocationBank.js` + `selectProvocation` + 5 distinct dayparts live, no clasp) — wake-side immersion stack now BUILT through T5. **NEXT: ENGINE_REPAIR Row 32** (engine-sheet — ARC EventText is bookkeeping, starves T1a′ milestone-name quality) + **T3** (read-the-Pulse, gated research.17) + **T4** (research.12 Layer 3 = ask-the-citizen). **Cost PINNED (research.21 OQ6):** ~$0.0004/wake; even all-900/day ≈ $11/mo — throughput is signal-gated, NOT cost-gated.

## Changelog

- 2026-06-26 (S273) — **T5 designed (Mike: "write it").** Varied-provocation question bank: the wake's fixed user-prompt becomes a deterministically-seeded pick from 10–20 provocation-types per daypart, each latching a real engine signal (self / block / city-sports / media / people / likes-inferred). The user-prompt twin of T2/T1a/T1c (vary the *question*, not just the answerer); fixes vector-2 (convergent *mode*) and conditions the research.21 convergence signal. Honest gap: likes/dislikes + per-citizen media attribution unmodeled → infer-light, flag preference-signal as future engine add. Added §T5 Design + build-row + engine-sheet handoff (seeded `selectProvocation`, acceptance criteria). Coupling: variety pays off only with throughput volume (research.21 §scale; cost = non-constraint per OQ6).
...
| research.19 | **Citizen perception & immersion access layer** (S269 Mike-direct — "what allows a citizen to immerse in a world?"). The INPUT side of the citizen loop (research.14 owns reflection/write-back; this owns what a citizen *perceives*). **Baseline audit:** live wake is thin (disposition + last-5 LifeHistory + 3 names) with two holes — **writes a memory it never reads** (amnesia) + **no world beyond the block** (live wake dropped research.13's world-summary/sports/memory pieces). 4-ingredient immersion frame (continuity / world-larger-than-self / relationships / stakes) → **4 access tiers:** T1 own-page read-back (bounded+guarded) + sports feed + neighborhood-state (now); T2 world-summary→lived-particulars **translation build**; T3 read-the-Pulse guarded pilot (needs research.17 filter); T4 ask-the-citizen = research.12 Layer 3. **Self-pressure-test (advisor PENDING — was overloaded):** page-readback AMPLIFIES fabrication (NOT contamination-free — the page holds the S264 "shady Greg" invention), T2 is a real build not free reuse, page growth needs bounding (Phase-C scale), immersion amplifies the write-back (couples to engine.38 B4 cap + affect re-audit), T3 needs a real canon filter not a prompt. Guardrails: lived-particulars-not-aggregates, C92 containment, scale-honesty, determinism. **ADVISOR PASS DONE S270.** Killed T1a-as-prose-read-back (AP-1: research.17 is a *salience* filter, not a canon-truth validator — never guarded the fabrication risk; AP-2: prose read-back compounds C92, rolling summary doesn't save it). **Reframe: T1a pivots to canon-anchored self-state read-back (LifeHistory rollup + dial trajectory) — ungated AND contamination-free** (fabrication lives in generated prose, not canon self-history; LifeHistory already in wake per CV-1). **T1a + T1b now buildable, ungated, off the C100 wall** (engine-sheet build); research.17 demoted to a Tier-3-only dependency. T1b pre-pipe: confirm Oakland_Sports_Feed canon-clean. **T1a + T1b BUILT S270 (engine-sheet, `f17d110a`, wake-side `scripts/citizen-wake.js`, no clasp).** T1a = canon-anchored self-state read-back: "Your life so far" milestone arc sourced from **LifeHistory_Log** (canonical per-citizen event history — inline col O is compressed so milestones live in the Log; NOT prose, per AP-reframe) + a **dormant** dial-trajectory hook (current==base until the write-back drain deploys; surfaces a shift only on real movement, never fabricates). T1b = "Around Oakland" A's slice from `Oakland_Sports_Feed` (TeamsUsed-filtered A's-only, valid record only, narrative not FanSentiment aggregate). T1b pre-pipe DONE (spot-check canon-clean: GodWorld roster + fictionalized opponents). Measure-twice: dry-run prompt dump caught empty-arc (inline vs Log) + conflated-sports (Oaks/A's) pre-commit, both fixed + re-verified on POP-00001. **T2 DESIGN DONE S273** (shared generative per-neighborhood texture digest `output/neighborhood_texture_c{XX}.md`, frozen post-world-summary, wake reads hood block — mirrors T1b seam; generator = batched OpenRouter call + blocklist sweep, source-grounding bounds fabrication; build handoff + acceptance criteria in plan §T2 Design). **T1a′ filed S273** (production-verify finding: T1a's arc is an aggregate `"N advancement events"` that never surfaces in 15 post-upgrade reflections — translate to named latest milestone from LifeHistory_Log, canon-anchored/ungated, rides T2 build). **T1c UN-DEFERRED S273 (Mike — overturns AP-2 gate):** prose read-back is UNGATED (page = subjective memory; self-read-back never crosses the publication wall; "shady Greg" = continuity not contamination), size-bounded for tokens only — closes the original amnesia hole, cheapest immersion fix, coexists with shipped T1a. **T2 + T1a′ BUILT S272 (engine-sheet, concurrent same-day)** — `beda8fa7` (`buildNeighborhoodTexture.js` v1.0.0 + wake reader) + `0110597f` (T1a′ count→named milestone) + `fb73e4e2` (T2 contentless-event guard). **C100 artifact live** `output/neighborhood_texture_c100.md` — design validated first run (lived-particulars, zero metrics, source-grounded, 13/21 quiet-week guard holding). T1a′ surfaced **ENGINE_REPAIR Row 32** (ARC EventText is bookkeeping → starves milestone-name quality; engine-sheet follow-up). Full-stack dry-run verified POP-00001. **T5 DESIGN DONE S273** (varied-provocation question bank — seeded pick from 10–20 provocation-types/daypart, each latching a real engine signal; fixes vector-2 / convergent-mode; conditions research.21 signal; §T5 Design in plan + engine-sheet handoff). **T1c BUILT S272** (`b3841e4a`, bounded own-page prose read-back, `lib/citizenPage.js` + wake reader, no clasp) **+ T5 BUILT S273** (`1a226492`, `lib/provocationBank.js` + `selectProvocation` + 5 distinct dayparts live, no clasp) — wake-side immersion stack now BUILT through T5. **NEXT: ENGINE_REPAIR Row 32** (engine-sheet — ARC EventText is bookkeeping, starves T1a′ milestone-name quality) + **T3** (read-the-Pulse, gated research.17) + **T4** (research.12 Layer 3 = ask-the-citizen). **Cost PINNED (research.21 OQ6):** ~$0.0004/wake; even all-900/day ≈ $11/mo — throughput is signal-gated, NOT cost-gated. | T1a/T1b/T1c + T2 + T1a′ + T5 BUILT (wake-side); T3/T4 future-gated | research-build (design DONE) → engine-sheet (build DONE through T5) | [[../plans/2026-06-23-citizen-perception-immersion-layer]] |
