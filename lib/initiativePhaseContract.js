...
const INTERVENTION_CATALOG = {
  ...
  'housing-program': {
    label: 'Housing stabilization or rent relief program',
    policyDomain: 'housing', type: 'vote', playable: true, disburses: true,   // engine.255: spends BudgetRemaining onto flagged Household_Ledger rows (Task 4); playable after the Task 7 pair (bench 0908 @98, C109–C113: Delivering at C113, control hoods flat)
    effectChannel: 'S.initiativeDisbursement → applyHousingDisbursement_ (fill-to-buffer grants onto the engine\'s own flagged active rented households; the whole tranche leaves BudgetRemaining)',
    stage3Metric: { tab: 'Household_Ledger', column: 'flagged/cohort', direction: 'down', scope: 'hood' },   // engine.255 Task 6: flagged-at-vote cohort share still flagged or lost
  },
};
...
// engine.255 Task 1 — the budget columns (engine mirror INITIATIVE_BUDGET_COLUMNS_).
// `Budget` stays the display string; these carry the number the engine spends.
const BUDGET_COLUMNS = ['BudgetTotal', 'BudgetRemaining', 'LastDisburseCycle'];

// engine.255 Task 9 (builder call 6, 2026-09-22): a seat-proposed row carries a
// budget inside its domain's band, or it stands as `blocked: no-budget`. One
// closed table, Node-side only (the engine never mints); the numbers are the
// builder's to set — these are engine-sheet's first values, sized so a seat's
// proposal is a fraction of the live rows ($12.5M–$230M) and a district can
// afford to lose one. Domains absent here are not proposable (not playable).
const BUDGET_BANDS = {
  health:    { min: 5000000,  max: 100000000 },
  transit:   { min: 20000000, max: 500000000 },
  education: { min: 1000000,  max: 50000000 },
  housing:   { min: 5000000,  max: 50000000 },
};

// MIRROR of phase05-citizens/civicInitiativeEngine.js parseBudgetMoney_ — body text-identical.
...
function stageCatalogByDomain() {
  ...
  Object.keys(INTERVENTION_CATALOG).forEach(function (key) {
    ...
    out[e.policyDomain] = {
      ...
      disburses: e.disburses === true,   // engine.255
    };
  });
  return out;
}
...
// ---------------------------------------------------------------------------
// engine.255 Task 6 — the housing stage-3 cohort, rebuilt as the FLAGGED-AT-VOTE
// household cohort (plan 2026-09-22-initiative-budget-disbursement §Task 6).
// At the vote the freezer names every household the engine's own stress rule
// flags, per hood; a hood joins the city only with >= minFlagged of them; every
// member hood reads 1. At each later observation the same households are judged:
// relieved (active, rented, in the hood, no longer flagged), still flagged, or
// lost (dissolved, moved out, gone) — a loss is never an improvement (§3, §8) —
// and the hood reads (flagged + lost) / members, direction DOWN. A hood share
// would have opened for the control arm (crisis rows dissolve out of both
// numerator and denominator); this cannot. The member IDs ride the baseline
// descriptor (`members`), attached by baselineWithMembers after stageBaselineFrom.
// Flag rule mirrors householdFormationEngine.detectHouseholdStress_ (0.40 warning,
// 12-month savings buffer) — three-way parity in the test. Mirrored verbatim in
// the engine as civicHousingFlaggedCohort_.
// ---------------------------------------------------------------------------
// A derived identifier, never a literal header: the pack's evidence helper keeps
// refusing it (civicStageEvidence.js guards on [*/]) until Node has a household reader.
var HOUSING_FLAGGED_COLUMN = 'flagged/cohort';
var HOUSING_FLAG_WARNING = 0.40;        // == householdFormationEngine RENT_BURDEN_WARNING
var HOUSING_FLAG_BUFFER_MONTHS = 12;    // == householdFormationEngine / migrationTrackingEngine SAVINGS_BUFFER_MONTHS

function housingFlaggedCohort(input) {
  var inp = input || {};
...
// Attach the frozen member IDs to a freshly built descriptor (Household_Ledger
// only; fixed-canon tabs carry none). Pure; returns the same descriptor object.
function baselineWithMembers(descriptor, cohort) {
...
function cohortForBaseline(cohort, baseline) {
  var co = cohort;
  ...
  if (b.members && typeof b.members === 'object' && Array.isArray(co.households)) {
    var judged = housingFlaggedCohort({ rows: co.households, cycle: co.cycle, minFlagged: 1, resolveHood: co.resolveHood, members: b.members });
...
module.exports = {
  ...
  stageBaselineFrom,
  deliveryEdge,
  HOUSING_FLAGGED_COLUMN,
  HOUSING_FLAG_WARNING,
  HOUSING_FLAG_BUFFER_MONTHS,
  housingFlaggedCohort,
  baselineWithMembers,
  cohortForBaseline,
  deliveryHoldStep,
  stallClock,
