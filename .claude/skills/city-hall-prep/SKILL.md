...
**Engine-primacy carve-out on the override (S258 RB-4, closes G-PREP9).** ESCALATION overrides the *absence-of-statement* default, but it does NOT override **engine-primacy**: a Mara ESCALATION cannot force a voice to speak to a subject that is **absent from the initiative tracker**. Forcing it would manufacture civic source material for a program the engine doesn't carry — the contamination loop (edition fabrication → civic canon). C97 case: Mara escalated Okoro/Webb/Ramos on the `$14M Retail Recovery` (not in the tracker) and a `KONO blackout` (a prior *edition* story with no live engine signal) — both correctly dropped, the voices not forced. Rule: if the ESCALATION subject has no tracker row AND no live engine signal this cycle, engine-primacy wins — do not assign the topic, log `ESCALATION declined — subject not in tracker/engine (engine-primacy)`.
...
6. **Baseline briefs** (S256 RB-3, feeds the City This Cycle digest). `output/baseline_briefs_c{XX}.json` — `.briefs[]` carries this cycle's synthesized world-events / citizen-life-events / initiative-milestones, each tagged `.neighborhood` with `.facts.neighborhoodState` deltas (sentiment / retailVitality / eventAttractiveness / crimeIndex). This is the living-city texture the engine surfaced THIS cycle that Mara's directive is structurally blind to. Read alongside world_summary; it is the per-voice neighborhood-pulse source for Step 3.
...
**Read the LATEST entry, not the current cycle's (G-PREP1 timing fix, S256).** This auto-investigate runs at prep — *before* `/city-hall` Step 6 writes this cycle's MilestoneNote — so a `C{XX}` entry structurally cannot exist yet. The disambiguation question is "has the initiative shown recent documented progress that the engine flag missed?" — answered by the **most recent MilestoneNotes entry present** (normally `C{XX-1}`, written by last cycle's city-hall). Checking literally for `C{XX}` always finds none and mis-fires every initiative into Scenario A. Read the latest stamped entry instead. (If a `C{XX}` entry somehow already exists — a stopgap/manual write — it is the latest, and this still works.)

- **The latest MilestoneNotes entry names concrete recent progress (advanced phase / completed deliverable) that the engine flag contradicts** → engine auditor likely false-flagged (Scenario C — engine lag/cold-start walked priors). Drop the ailment from the topic-assignment surface; note "engine auditor false positive verified via MilestoneNotes" in the production log. Don't burn voice cycles re-litigating phase advances that happened.
- **The latest entry predates the flag or shows no recent progress (the initiative genuinely hasn't moved)** → real signal (Scenario A — commitment slipped, or B — writeback bug). Surface the highlighted history to the owning voice's topic assignment with the no-recent-progress note ("no milestone since C{XX-N} — voice owes a commitment-status update").
- **The latest entry documents a reason that contradicts the engine flag** (e.g., "delayed pending council approval") → real but reframed. Surface to topic assignment with the contradicting note attached so the voice agent sees the documented reason and can respond against it.
...
Runs at Step 1, before Step 2 builds topic assignments. The auto-investigation prevents a cycle of voice work being wasted on phantom-stuck initiatives like INIT-005 was in C93. Plan + Scenario C trace: [[archive/plans/2026-05-11-civic-7-init-005-investigation]].
...
### Step 1.5: Verify the ledger snapshot is fresh (S252, reworked S329)

The snapshot refresh now runs on the cycle path (run-cycle Step 5.56) — this step **verifies** it instead of writing it. Read `output/simulation_ledger_snapshot.meta.json` and check `"cycle"` equals {XX}. If it matches, log the actual meta line (cycle + rowCount + generatedAt). If it does NOT match, run `node scripts/dumpLedger.js {XX} --quiet`, then **read the meta back** and log what it says.

Never log this step as "refreshed" from a command exit or a remembered count — the C101 run (S329 finding) logged "913-row snapshot refreshed" while the file sat untouched since Jul 10, and every Discord persona went blind to ten days of canon. The log line for this step must be a quote of the freshly-read meta, nothing else.
...
- **Mayor (and any citywide voice — Chief, DA): the citywide digest.** From `world_summary_c{XX}.md` — pull §City State, §Three-Cycle Trends (C{XX-2}–C{XX}), §Evening Texture, §World Events, and the medium/high §Engine Review Findings. 3–6 lines: what the city *feels like* this cycle, not what an initiative is doing.
...
- **A `## MAYOR'S DECISIONS THIS CYCLE` block (S258 RB-4, closes G-PREP8).** Prep must NOT pre-script the Mayor's call. That block is injected at *runtime* by `/city-hall` Step 3 *after* the Mayor agent actually runs — pre-scripting it at prep makes reacting voices rubber-stamp a decision the Mayor's own agent never made (C97: first-pass Chief/CRC/IND packets carried her INIT-007 calls as settled). Reacting packets give the live situation only; the voices hear the Mayor's real output at runtime.
...
- 2026-06-14 — v1.7 (S258, research-build closing C97 triage RB-4 G-PREP8 + G-PREP9). **G-PREP8 — no pre-scripted Mayor's-decisions block:** "does NOT include" now forbids a `## MAYOR'S DECISIONS THIS CYCLE` block in prep packets; /city-hall injects the Mayor's *actual* runtime output at its Step 3 (line 151) — prep pre-scripting it made reacting voices rubber-stamp a decision the Mayor agent never made (C97 first-pass Chief/CRC/IND). **G-PREP9 — engine-primacy carve-out on ESCALATION-override:** the S229 ESCALATION-override rule gained a carve-out — ESCALATION cannot force a voice to speak to a subject absent from the initiative tracker with no live engine signal (C97: $14M Retail Recovery + KONO blackout — both non-tracker, correctly declined); prevents the edition-fabrication→civic-canon contamination loop. Source gaps: `output/production_log_run_cycle_c97_gaps.md` §G-PREP8, §G-PREP9. Plan: [[../../../docs/plans/2026-06-13-c97-gap-log-triage]] RB-4.
