...
4. (Engine, post-land) Phase-9 compressed digest reports real `migration`/`economy` values — **PROVEN on bench @7 / C110**: `S.demographicDrift` survives Phase 8 as an object, so `applyCompressionDigestSummary.js:155-157` reads real values instead of `0`/`'stable'` off a string. `saveV3NeighborhoodMap_` emitting non-'Stable' labels — **PARTIAL, and it exposed a pre-existing scale mismatch (see below)**: `Neighborhood_Map.DemographicMarker` went `Stable` x18 + `Civic pressure zone` x4 → `Inflow surge` x17 + `First Friday arts walk zone` x1 + `Civic pressure zone` x4. The label is unpinned from always-'Stable', but it is now pinned to always-'Inflow surge'.

   **Scale mismatch — decide before this wave goes to PROD (engine-sheet, 2026-09-10).** `v3NeighborhoodWriter.js:249-262` reads `S.demographicDrift.migration` and thresholds it at ±5 / ±20 / ±35. That field is the city's **net migration count** (1306 at C110, clamped ±5000 by `applyDemographicDrift.js:95-96`) — it clears +20 every cycle, so every hood reads 'Inflow surge' forever. The correctly-scaled field is `S.migrationDrift` (21 at C110) — already the writer's own documented fallback at `:250`, and almost certainly what the ±5/20/35 thresholds were written against. The clobber fix did not cause this; it uncovered it (while `demographicDrift` was a string, `driftNum` was forced to 0 and the thresholds never ran). It matters because the label is consumed downstream: `scripts/buildCivicVoicePackets.js:381` feeds `DemographicMarker` into civic voice packets. **Not fixed here** — what the marker should measure is a design call, not a one-liner.

   **Builder question 2026-09-10, and the answer the data gives.** Asked: use `migrationDrift` for the labels; are we not tracking a hood-level migration number; and is a city-wide metric the right thing for coverage when civic covers hoods and the office that runs them.

   - **`migrationDrift` is the field the thresholds were written for.** `applyMigrationDrift.js:44` documents `ctx.summary.migrationDrift` as numeric −50..+50; the writer's ±5/20/35 bands sit inside that range exactly. Repointing `driftRaw` fixes the *scale* bug in one line.
   - **But it does not fix the structural one.** `baseDemoLabel` is computed ONCE at `v3NeighborhoodWriter.js:249-262`; the hood loop does not start until `:325`. Every hood receives the same label by construction — only `getDemographicMarkerV35_` overrides (holiday, First Friday, arc, shock) vary it. Swapping `migration` → `migrationDrift` only changes *which* single city number all 22 hoods share. `DemographicMarker` is a per-hood column driven by a city-wide input.
...
2. **`Health_Cause_Queue` in the beat tabs — RULED: keep one cycle, then drop.** Builder 2026-09-09. After the C107 dump confirms the queue contributes nothing live, remove it from `SEAT.tabs` in `buildHealthSlice.js` and from `BEAT_TABS` in `scripts/dumpBeatTabs.js`. (The tab itself stays live in the sheet.)
3. **Hospital capacity constant mismatch — DONE, but the engine half was the inert half (corrected by engine-sheet 2026-09-10).** Builder ruled the sheet key wins. `phase10-persistence/buildCyclePacket.js`: the hardcoded `HOSPITAL_CAPACITY = 40` placeholder is deleted; census `load` now divides by `hospitalBaseCapacity` (100, World_Config) as surfaced by `applyDemographicDrift_` (W2b `hospitalConfig`), with a warn-logged 100 fallback.

   **Review finding:** that engine-side value reaches no live consumer. `ctx.summary.hospitalCensus` has **zero** engine readers (`buildCyclePacket.js:1017` is the only write), and the one place its `load` surfaced — the `Hospital:` line inside the packet's `--- POPULATION ---` section — has been filtered out of `Cycle_Packet` since C103 by the S328 W2b `KEEP_SECTIONS` allowlist (`buildCyclePacket.js:767`). The hospital load figure anyone actually reads is the world summary's, and `scripts/buildWorldSummary.js:1338` still divided by a hardcoded 40 — carrying a comment claiming it "matches persistHospitalLedger_", which it no longer did. **Fixed at `e358293c`**: `buildWorldSummary` now loads `World_Config` (tolerantly, like the other optional tabs) and divides by `hospitalBaseCapacity`, warning and falling back to 100 rather than re-introducing a constant. Both halves now bind at the one key. Talk-back behavior unchanged — it already bound at 100.
4. **Stale twin city number — DONE, but it publishes nothing (corrected by engine-sheet 2026-09-10).** `buildCyclePacket.js`'s packet `IllnessRate` line now computes the fresh post-attractor `S.demographicDrift.illnessRate` (fallback to the old field). **Review finding:** the line is computed and then dropped — `--- POPULATION ---` is not in `KEEP_SECTIONS` (`buildCyclePacket.js:767`), so it has not reached `Cycle_Packet` since C103. Verified on the bench: packets C103–C110 carry 7 sections / ~1.2KB, C85–C102 carried 22–24 / ~7–13KB. The change is correct and harmless, and becomes live the moment POPULATION is re-admitted to the keep list — but it is NOT a shipped fix to a published number, and should not be proven against the packet. `utilities/cycleModes.js:328` was NOT repointed: it is `buildCycleChecksum_`, whose output is stored in `Cycle_Seeds` and compared verbatim by `compareReplayOutput_` on replay — changing the formula would false-MISMATCH every historical cycle replayed with new code. A checksum is not a published number; ruling scoped to the publication. Builder to confirm.
5. **Orphan write — RULED: KEEP.** Builder 2026-09-09, verbatim: "i use this to authorize apps script since I know nothing reads it." `S.demographicDriftSummary` stays as an inert authorization marker.
...
- 2026-09-09 (kimi) — Rulings 2–5 answered and executed: queue drops from the beat path after one confirming cycle (2); census load divides by `hospitalBaseCapacity` — placeholder 40 deleted from buildCyclePacket.js (3); packet IllnessRate repointed to the fresh `S.demographicDrift.illnessRate`, checksum deliberately untouched (4); `demographicDriftSummary` kept as the builder's Apps Script authorization marker (5). hospitalTalkback 24, illnessEnvelope 29, employmentEnvelope 18, patternShockRelative 9, stink-scanner, buildWorldSummary 234 — all PASS; stub maps regenerated (1357 functions).
- 2026-09-10 (engine-sheet, S441) — Review + bench landing. **One defect found and fixed:** ruling 1's whole-file deletion orphaned `scripts/archiveHeritageResolve.test.js`, which loaded `healthCauseIntake.js` into its VM namespace — the suite died ENOENT, taking six live-code heritage assertions down with the retired one (`62838898`; 14 passed, 0 failed after). **Two claims corrected:** rulings 3 and 4 are both inert as shipped (see above) — the real capacity defect was on the Node side and is fixed at `e358293c`. **Bench:** HEAD `e358293c` staged to SANDBOX 0908 @7 (script version 8) alongside engine.96 T12 owner door reel-2 `68dc5a66`; pull-back verified 169 files, `healthCauseIntake.js` absent from the remote, both markers present. **C110 fired clean** — `ok:true`, 143s, 0 Engine_Errors, cycleCount 110. Acceptance criterion 4 proven (see above). Criterion for ruling 3 had **no signal** this fire (Hospital_Ledger 0 open rows → load 0 under both 40 and 100); criterion 1 (stale-slice drop) still waits on the next Monday fanout at an unchanged cycle. Kimi's scripts half re-verified empirically off `output/cron-compare/health_slice_c106.json`: 4 citizens, 7 facts, `HEALTH-SLICE-2`, zero queue rows with `StatusStartCycle <= 0`/`Processed`, no Osei/Crane/Lee.
