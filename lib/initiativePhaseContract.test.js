...
(function () {
  ...
  ok(keys.length >= 7, 'catalog: present and non-empty (' + keys.length + ' keys)');
  ...
  ok(JSON.stringify(unplayable) === JSON.stringify(['economic', 'safety', 'sports', 'workforce']), 'catalog: safety has no lever; economic, workforce and sports too weak to open their gate (builder ruling b); no housing entry — no housing program in the sim (builder 2026-09-23)');
  ...
  // Job 2 (builder rulings 2026-09-25): no menu. The pack lists every proposal
  // category (not the delivering catalog's keys), each with a band; the
  // non-deliverable ones are marked, never hidden.
  var menu = slice.loadCategoryMenu();
  var cats = Object.keys(C.PROPOSAL_CATEGORIES);
  ok(menu.available === true && menu.list.length === cats.length && cats.every(function (c) { return mint.POLICY_DOMAINS.indexOf(c) >= 0 && !!C.BUDGET_BANDS[c]; }), 'categories: every category is a mintable policy domain with a budget band, all listed in the pack');
  ok(cats.indexOf('housing') < 0 && !C.BUDGET_BANDS.housing && !/housing|-program|-service|-project/.test(menu.text), 'categories: no housing category or band, and no old menu key reaches the pack (builder 2026-09-25)');
  ok(menu.list.filter(function (p) { return !p.canDeliver; }).map(function (p) { return p.key; }).sort().join() === 'economic,environment,safety,sports,workforce', 'categories: the pack marks exactly the categories with no playable Delivering gate');
})();
...
// Initiatives in the World Job 3 — build duration (builder 2026-09-25).
(function () {
  var fs = require('fs');
  var path = require('path');
  var root = path.resolve(__dirname, '..');
  var engineSrc = fs.readFileSync(path.join(root, 'phase05-citizens/civicInitiativeEngine.js'), 'utf8');
  var E = new Function('Logger', engineSrc + '\nreturn {' +
    'INITIATIVE_BUILD_COLUMNS_: INITIATIVE_BUILD_COLUMNS_, ensureInitiativeBuildColumns_: ensureInitiativeBuildColumns_,' +
    'getCivicBuildCycles_: getCivicBuildCycles_, civicStageStep_: civicStageStep_, civicBuildOpenStep_: civicBuildOpenStep_,' +
    'applyCivicStageStep_: applyCivicStageStep_, CIVIC_BUILD_DOMAINS_: CIVIC_BUILD_DOMAINS_, CIVIC_BUILD_PHASE_: CIVIC_BUILD_PHASE_' +
    '};')({ log: function () {} });
  var seedsSrc = fs.readFileSync(path.join(root, 'phase01-config/engine94SheetContract.js'), 'utf8');
  var seeds = eval(seedsSrc.match(/var ENGINE213_CONFIG_SEEDS = (\[[\s\S]*?\n\]);/)[1]);
  var config = {};
  seeds.forEach(function (s) { config[s[0]] = s[1]; });

  ok(JSON.stringify(E.INITIATIVE_BUILD_COLUMNS_) === JSON.stringify(C.BUILD_COLUMNS), 'build: engine column list == lib BUILD_COLUMNS');
  ok(E.CIVIC_BUILD_DOMAINS_.every(function (d) { return Object.prototype.hasOwnProperty.call(config, 'civicBuildCycles_' + d); }), 'build: every build domain has a seeded World_Config dial');
  ok(config.civicBuildCycles_health === 8 && config.civicBuildCycles_education === 8 && config.civicBuildCycles_transit === 10 && config.civicBuildCycles_sports === 12 && config.civicBuildCycles_environment === 6, 'build: seeds carry the builder-ruled weeks (health 8, education 8, transit 10, sports 12, environment 6)');
  ok(['economic', 'workforce', 'safety'].every(function (d) { return E.getCivicBuildCycles_({ config: config }, d) === 0 && !('civicBuildCycles_' + d in config); }), 'build: funds and programs open at stand-up — no dial, 0 weeks');
  ok(Object.keys(C.PROPOSAL_CATEGORIES).every(function (c) { return E.CIVIC_BUILD_DOMAINS_.indexOf(c) >= 0 || ['economic', 'workforce', 'safety'].indexOf(c) >= 0; }), 'build: every proposal category is either a build category or an open-at-stand-up one');
  var threw = false; try { E.getCivicBuildCycles_({ config: {} }, 'health'); } catch (e) { threw = true; }
  ok(threw, 'build: a missing dial for a build domain throws, never a silent zero');
  ok(C.isCanonical(E.CIVIC_BUILD_PHASE_) && C.intensityOf(E.CIVIC_BUILD_PHASE_) > 0, 'build: the site phase is canonical with a positive ripple');
  var hd = fs.readFileSync(path.join(root, 'phase02-world-state/applyInitiativeImplementationEffects.js'), 'utf8').match(/var HEALTH_DELIVERING_PHASES = \{([\s\S]*?)\};/)[1];
  ok(hd.indexOf("'" + E.CIVIC_BUILD_PHASE_ + "'") < 0, 'build: a site treats nobody — the build phase is not a health delivering phase');

  // the step: a build category stands up as a site with an opening Cycle
  var base = { status: 'passed', mayoralAction: 'signed', stage: 'Funded', phase: 'vote-ready', lastStageChangeCycle: 110, lastWorkCycle: 110, cycle: 111 };
  var mk = function (o) { var x = {}; Object.keys(base).forEach(function (k) { x[k] = base[k]; }); Object.keys(o).forEach(function (k) { x[k] = o[k]; }); return x; };
  var s8 = E.civicStageStep_(mk({ policyDomain: 'health', buildCycles: 8 }));
  ok(s8.stage === 'Standing' && s8.phase === 'construction-active' && s8.opensCycle === 119, 'step: a clinic funded C110 and worked stands up C111 as a site opening C119');
  var s0 = E.civicStageStep_(mk({ policyDomain: 'economic', buildCycles: 0 }));
  ok(s0.phase === 'operational' && !s0.opensCycle, 'step: a no-build category stands up open, no opening Cycle');

  // the open step
  var ob = { stage: 'Standing', status: 'passed', mayoralAction: 'signed', phase: 'construction-active', buildCycles: 8 };
  var mo = function (o) { var x = {}; Object.keys(ob).forEach(function (k) { x[k] = ob[k]; }); Object.keys(o).forEach(function (k) { x[k] = o[k]; }); return x; };
  var w = E.civicBuildOpenStep_(mo({ opensCycle: 119, cycle: 118 }));
  ok(w && w.open === false && w.stamp === false, 'open: before its opening Cycle a site stays a site');
  ok(E.civicBuildOpenStep_(mo({ opensCycle: 119, cycle: 119 })).open === true, 'open: at its opening Cycle it opens');
  var leg = E.civicBuildOpenStep_(mo({ opensCycle: '', voteCycle: 80, lastStageChangeCycle: 108, cycle: 109 }));
  ok(leg.stamp === true && leg.opensCycle === 88 && leg.open === true, 'open: INIT-005 shape — a legacy site counts from its C80 vote, stamps C88, opens at C109 (builder 2026-09-25)');
  var leg2 = E.civicBuildOpenStep_(mo({ opensCycle: '', voteCycle: '', lastStageChangeCycle: 108, cycle: 109 }));
  ok(leg2.stamp === true && leg2.opensCycle === 116 && leg2.open === false, 'open: a legacy site with no vote counts from its stage change');
  ok(E.civicBuildOpenStep_(mo({ phase: 'stalled', opensCycle: 100, cycle: 120 })) === null, 'open: a stalled site never opens — the untended clock wins, revival restores the site');
  ok(E.civicBuildOpenStep_(mo({ phase: 'operational', opensCycle: 100, cycle: 120 })) === null && E.civicBuildOpenStep_(mo({ stage: 'Funded', opensCycle: 100, cycle: 120 })) === null && E.civicBuildOpenStep_(mo({ mayoralAction: 'vetoed', opensCycle: 100, cycle: 120 })) === null, 'open: only a voted Standing site is on the build clock');

  // the requirement: no delivery judged on a site
  var r = C.stageRequirement({ stage: 'Standing', phase: 'construction-active', policyDomain: 'health', opensCycle: 119, metricMoved: true });
  ok(r.blocked === 'building' && r.clears === false && r.moveThatClears === 'work' && /opens at C119/.test(r.text), 'requirement: a Standing site is blocked `building`, cannot deliver on noise, says when it opens');

  // the applier end to end: Funded -> site -> open, carry for the Phase-2 detector
  var hdr = ['InitiativeID', 'Name', 'Status', 'MayoralAction', 'ImplementationPhase', 'PolicyDomain', 'LastUpdated', 'VoteCycle', 'Stage', 'StageBaseline', 'LastStageChangeCycle', 'LastWorkCycle', 'LastWorkSeat', 'PriorPhase', 'StageHold', 'OpensCycle', 'MilestoneNotes'];
  var ix = function (n) { return hdr.indexOf(n); };
  var sIx = { stage: ix('Stage'), lastStageChange: ix('LastStageChangeCycle'), lastWork: ix('LastWorkCycle'), status: ix('Status'), mayoralAction: ix('MayoralAction'), phase: ix('ImplementationPhase'), policyDomain: ix('PolicyDomain'), lastUpdated: ix('LastUpdated'), id: ix('InitiativeID'), name: ix('Name'), priorPhase: ix('PriorPhase'), hold: ix('StageHold'), opens: ix('OpensCycle'), voteCycle: ix('VoteCycle'), milestone: ix('MilestoneNotes') };
  var ctx = { summary: {}, now: 'NOW', config: config };
  var row = ['INIT-020', 'Clinic', 'passed', 'signed', 'vote-ready', 'health', 'then', 110, 'Funded', '', 110, 110, '', '', '', '', 'C110: funded'];
  E.applyCivicStageStep_(ctx, row, sIx, 111);
  ok(row[ix('Stage')] === 'Standing' && row[ix('ImplementationPhase')] === 'construction-active' && row[ix('OpensCycle')] === 119 && ctx.summary.initiativeEnginePhaseMoves['INIT-020'] === 'vote-ready', 'applier: stand-up writes the site phase and OpensCycle, carries the phase it left');
  row[ix('LastWorkCycle')] = 117;
  var ctx2 = { summary: {}, now: 'NOW', config: config };
  ok(E.applyCivicStageStep_(ctx2, row, sIx, 118) === false && row[ix('ImplementationPhase')] === 'construction-active', 'applier: a tended site waits out its build — no write');
  var ctx3 = { summary: {}, now: 'NOW', config: config };
  ok(E.applyCivicStageStep_(ctx3, row, sIx, 119) === true && row[ix('ImplementationPhase')] === 'operational' && ctx3.summary.initiativeEnginePhaseMoves['INIT-020'] === 'construction-active', 'applier: at C119 it opens — phase operational, construction carried so the Phase-2 detector sees the advance');
  ok(row[ix('MilestoneNotes')] === 'C110: funded\nC119: opened — construction complete, open to the public.', 'applier: the open appends one milestone line in the world\'s words, prior notes kept');
  ok(E.applyCivicStageStep_({ summary: {}, now: 'NOW', config: config }, row, sIx, 120) === false, 'applier: an open clinic does not open twice');
  var legacy = ['INIT-005', 'Temescal', 'passed', 'signed', 'construction-active', 'health', 'then', 80, 'Standing', '', 108, 108, '', '', '', '', ''];
  var ctx4 = { summary: {}, now: 'NOW', config: config };
  E.applyCivicStageStep_(ctx4, legacy, sIx, 109);
  ok(legacy[ix('MilestoneNotes')] === 'C109: opened — construction complete, open to the public.', 'applier: a blank note gets just the open line');
  ok(legacy[ix('OpensCycle')] === 88 && legacy[ix('ImplementationPhase')] === 'operational' && ctx4.summary.initiativeEnginePhaseMoves['INIT-005'] === 'construction-active', 'applier: the live INIT-005 row stamps OpensCycle 88 and opens at the C109 fire');

  // self-arm
  var calls = [];
  var sheet = { getLastColumn: function () { return 41; }, getMaxColumns: function () { return 41; }, insertColumnsAfter: function (a, n) { calls.push(['insert', a, n]); }, getRange: function (r, c, h, w) { return { setValues: function (v) { calls.push(['set', r, c, h, w, v[0].join()]); } }; } };
  ok(E.ensureInitiativeBuildColumns_(sheet, ['x']) === true && calls.map(function (c) { return c.join(','); }).join('|') === 'insert,41,1|set,1,42,1,1,OpensCycle', 'self-arm: OpensCycle appended once at the end of a full grid');
  ok(E.ensureInitiativeBuildColumns_(sheet, ['x', 'OpensCycle']) === false, 'self-arm: present column, no write');

  // the gavel cannot phase a voted staged row
  var ntw = require('../scripts/applyTrackerUpdates').normalizeTrackerWrite;
  var g = ntw({ ImplementationPhase: 'operational' }, { Stage: 'Standing', ImplementationPhase: 'construction-active' }, 109);
  ok(g.updates.ImplementationPhase === undefined && g.warnings.some(function (x) { return /phased by the engine/.test(x); }), 'gavel: a Standing site cannot be stamped open by the Sunday chain');
  var gp = ntw({ ImplementationPhase: 'vote-scheduled' }, { Stage: 'Proposed', ImplementationPhase: 'announced' }, 109);
  ok(gp.updates.ImplementationPhase === 'vote-scheduled', 'gavel: a Proposed row keeps its vote-scheduling phases');
  var gl = ntw({ ImplementationPhase: 'operational' }, { Stage: '', ImplementationPhase: 'construction-planning' }, 109);
  ok(gl.updates.ImplementationPhase === 'operational', 'gavel: a legacy unstaged row (Baylight) is still the gavel\'s');
})();
