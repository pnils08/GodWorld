...
**Current bench pointer:** SANDBOX 0908 (`1FFpUs98L0wrEcfAaQy6-Ir2xYiCq9baCc0SzqzAMrtM`) web app **@108 = `12289fdb`** (open writes a milestone line), **re-synced from live C108 2026-09-25 ~01:05**, sheet at **C114** bench-only, never replay (C109 ran @106 in the propagation window; C110–C111 on @107 = the Job 3 proof; INIT-005 hand-reset to a site opening C113 = bench-only fixture; C112–C114 on @108). A hung fire GET holds the script lock until its execution ends — later GETs return `a cycle is already running`; retry, don't re-sync. Before that @106 = `578c2237` (== PROD @128), sheet at C115 (C115 on @106 engine.259); before that @105 = `02a50b75`, sheet at **C114** bench-only (housing removal proven through C113 on @104, C114 on @105 258d, 0 new errors), never replay. Before that **@102 = `17daa559` (== PROD @125)**, re-synced from live C108 2026-09-23 ~18:05 then C109 @100 (kimi 4, `71bfc8c8`), C110 @101 (engine.258 `ced4b17c`), C111 @102 (258b) — all ok:true, 0 Engine_Errors; sheet at **C111** bench-only, never replay. Before that @99 = `bb15f9c9` (== PROD @124), sheet **RE-SYNCED FROM LIVE C108 2026-09-22 ~20:20** then fired C109–C113 on the @98 experiment (the Task 7 pair: Delivering at C113) and C114 on @99, with `civicHousingCohortMinFlagged` hand-set 1 — sheet at **C114**, bench-only, never replay; re-push HEAD + re-sync before any unrelated proof. Earlier today: @93–@97 (lever off, budget columns, first grant, dirty pair), INIT-001 hand-retagged housing to mirror live, bench-only since the sync, never replay. Before that @92 = BENCH-ONLY EXPERIMENT BUILD #2 (main `655b3001` + engine-mirror housing `playable:true` patched in the staged copy — NOT on main; engine.251 reader proof, Delivering reached C126)**, sheet at **C126** bench-only, never replay — re-synced from live C108 2026-09-22 ~08:00, fixture `INIT-951` (housing, West Oakland) walked blank → Proposed → Funded → Standing, `civicHousingReliefEnabled` 1 and `civicDeliverMargin_housing` 0.01 hand-set (mechanism proof). **Main HEAD is ahead of the bench build; push HEAD (and re-sync from live) before any unrelated proof.** Before that @90 = `31606eb6`, @87 = `78eb6935` (== PROD @116). Before that @85 = `7ba27e95` (== PROD @115), sheet at C120. Before that **@84 = `a342400d`**, sheet at **C117** (bench-only, never replay — fixtures `INIT-901` / `INIT-902`, `INIT-902` at Delivering with hold obs 116). Before that **@83 = `411b9e69` (== PROD @112, web app read back @99)**, sheet at **C116** (bench-only, never replay — fixtures `INIT-901` / `INIT-902`, tracker at 38 columns, `INIT-902` at Delivering); re-sync from live before the next cut. Before that **@82 = `f78561d0` (== PROD @111, web app read back @98)** — re-synced from live C108 2026-09-21 ~17:52, then sheet at C112 with fixtures `INIT-900` / `INIT-901`. Before that **@80 = `9da4d5cc` (== PROD @110, web app read back @97)** — **RE-SYNCED FROM LIVE C108 2026-09-21 ~02:05 after the @110 proof** (82 tabs, 55,424 rows, read-back OK; tracker back to 31 columns, fixture `INIT-900` gone), sheet at **C108**; proof trail [[DEPLOY_HISTORY]] §PROD @110. Before that **@79 = `3500a370` (== PROD @109)** — re-synced from live C108 2026-09-21, sheet at **C110** (bench-only, never replay; [[DEPLOY_HISTORY]] §PROD @109). Before that **@78 = `a92e84a6` (== PROD @108 HEAD push)** — sheet at **C122** (bench-only, never replay — INIT-005 stalled again; [[DEPLOY_HISTORY]] §PROD @108). Before that **@77 = `95b67423` (== PROD @107 HEAD push)** — sheet at **C121** (bench-only, never replay — tracker INIT-003 `construction-active` + `ProposingOffice COUNCIL-D4`, INIT-005 stalled then revived; [[DEPLOY_HISTORY]] §PROD @107). Before that **@76 = `2b855aaa` (== PROD @106 HEAD push)** — sheet at **C119** (bench-only, never replay — incl. `Initiative_Tracker` INIT-003 → `construction-planning`, INIT-005 → `stalled`; [[DEPLOY_HISTORY]] §PROD @106). Before that **@75 = `d5738907` (== PROD @105)** — sheet at **C116** (bench-only, never replay — incl. four C116 feed rows at sheet rows 76–79; [[DEPLOY_HISTORY]] §PROD @105). Before that **@74 = `b5cb18fb` (== PROD @104)**, C115. Before that **@73 = `c04c4caa` (== PROD @103)**, C112–C114. Before that **@62 = `a1f973a3` (== PROD @101)** — sheet at **C111** (synced from live AFTER the engine.241 recalibration, then C108–C111 fired clean; bench-only, never replay; readouts `output/engine-sheet/2026-09-19-bench-c108-readouts.md`, [[DEPLOY_HISTORY]] §PROD @101). Before that **@56 = `b58d8a2b` (== PROD @100)** — sheet at **C117** (C116–C117 moment-trigger proof, bench-only rows; [[DEPLOY_HISTORY]] §PROD @100). Before that @54 = engine tree `b2607ebe` — sheet at C115 (2026-09-18: engine.202 WeekRecord proof fired ok:true 149 s, 0 failed, one planned Engine_Errors row; trail in [[DEPLOY_HISTORY]] §PROD @98. **BENCH-ONLY, NEVER REPLAY: bench `Oakland_Sports_Feed` U1 header + rows 224–225 (C115) and everything C115 wrote.**) Before that @52 = repo `44cf056f` (engine.210) — sheet at C114 (S465: C114 fired ok:true 143.6s, 132 phases, 0 failed, `Engine_Errors` empty; order proven on the deployed run via the fire response's `timing.timings` — `Phase2-SportsSeason` idx 8 before `Phase2-SeasonalWeights` idx 9; `Neighborhood_Map.SportsSeason` `playoffs` 22/22; pull-back byte-identical on both changed files. **HEAD has since moved past the benched SHA by a comment-only diff in `applySportsSeason.js` (`0cbb837d`) — a pull-back byte-check against HEAD is not a bench-identity check.** **BENCH-ONLY, NEVER REPLAY: the `Oakland_Sports_Feed` row at C114 — A's / playoffs / game-result, Notes `S465 BENCH-ONLY engine.210 proof - NEVER REPLAY` — and everything C114 wrote.** The bench feed's last real row is C107, so without that row the fire takes the `oakland-feed-empty` branch and exercises none of the cut). Was @51 = repo `0a8013fc` (PROD @96) — sheet was at C113 (S463: @47 C109 engine.231, @48 C110 + @49 C111 engine.232, @50 C112 engine.190+233, @51 C113 engine.192+232b — all 0 errors). Bench World_Config carries the 4 `school*` keys (live self-arms them at its next fire). Bench-only since the S462 resync: C108–C111 rows and memory, never replay. Was @46 = `47063de6` + DIAG229 bench-only synthetic flood at C108 (never shipped); sheet at C108 after the sixth typed live-C107 resync of 2026-09-15 (S462). Earlier today: @39 225, @40 226, @42 228b proofs; @43–@45 died at the Apps Script 6-minute cap on afternoon latency (engine.230 fixed the per-row Advancement writes; `Diag_Fire` tab on the bench holds those runs' phase progress). The `/exec` fire response comes back as Google's "Page Not Found" interstitial after long runs — read the ring and World_Config, not the response; a run that dies at the cap leaves World_Config.cycleCount one behind every other tab (engine.136 stall signature) → resync.
