...
Three of the four commercial-sim patterns this design draws on are **already present** — re-derived from first principles, which is a good sign the model is sound:
- **Dwarf Fortress — bounded memory slots.** The spine (8/10 slots, strongest-keeps, promote-to-core).
- **Crusader Kings 3 — temporal decay + traits-multiply-event-impact.** Present as the 0.95/cycle decay + the salience **relevance-multiplier** (R biases how hard an event lands).
- **Victoria 3 — pressure accumulation → threshold trigger → structural change.** Present as the **conduct momentum loop** (transgression memories accumulate → threshold → dark core → biases generation → more transgressions). "No one is safe" is a Victoria-3 pressure accumulator pointed at one citizen.
- **RimWorld — adaptation factor (the UNTAPPED one).** Philosophy: *expect bad outcomes, engineer recovery* — when a colony spirals, the engine increases positive events to pull it back. **Our bound gates *individual* runaway, but at the *city* level "bad begets bad" has no floor.** Counterweight to flag: when city-level `Crime_Metrics` spikes, the engine should lean positive (opportunities / interventions / community wins) — a real city's resilience. → **engine.32 / population-balance consideration** (not engine.31 scope, but the safety valve the dark side needs).
