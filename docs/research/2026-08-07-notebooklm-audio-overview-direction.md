...
- 2026-08-07 (S358, 51b) — **Archive-query canon hook landed** (Mike-direct, same session): `cron-desk-run.js` `nlmCanonBrief()` — every M–F desk wake's canon-research step now queries the published-record archive notebook per assignment (angle + citizens + hood), saves the cited answer as `output/cron-compare/<stem>nlm-canon.md` (a legal ref the depth validator accepts), and injects it line-numbered into the candidate floor. Non-blocking: missing config/CLI/auth-rot degrades to the grep floor. `canonResearch.nlmBrief` rides angle.json + story doc §2b for audit. Answers Mike's "enforce canon search at article build / cron-hooked canon call attached to desk slice" — the cron assembling the slice is the caller, not Sheets (NLM auth stays droplet-side).
