...
**Simulation_Ledger rows:** 904 (S256 live count via `buildCitizenCards.js` ledger read) | **Columns:** 48 (A–AV; DialState added at AV S256, engine.31 live deploy) | **Sim Year:** 2041
...
**Canonical column definitions:** `schemas/SCHEMA_HEADERS.md` under `## Simulation_Ledger` — auto-generated header list, refreshed via Apps Script `exportAndPushToGitHub`. Live sheet is **48 columns** as of S256 — the A–AT table below covers 46; column AU (Gender, confirmed S146) and column AV (DialState, added S256 — engine.31 per-citizen 8-dial JSON machine state; TraitProfile col R is the derived face) are not yet in §Column Reference. SCHEMA_HEADERS will report 48 after next `exportAndPushToGitHub` refresh. When the sheet drifts from this doc, SCHEMA_HEADERS is ground truth. *(Phase 41.6 backlink, S156.)*
