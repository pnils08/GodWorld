...
| pipeline.57 | lived-context basis systemically unreachable — no builder populates `packet.exposure.evidence`. Correct per ADR-0017, needs a real evidence-source design pass. **CLOSED S431 (research-build): the addressable source already existed upstream — `story.ref` (`buildWorldSummary.js` desk_signal entries: a specific `sheet row(s)` citation, a `StorylineId`, a feed cycle — never a bare label) was being computed and carried into `signal.src`/`known` claims but discarded before it reached `exposure`. Fix: `evidenceFor(candidate, story)` in `livedExperiencePacket.js`, wired into `buildReportPacket`'s `exposure.evidence` — populated for any candidate NOT reached through the `neighborsFromLedger` proximity fallback (the only two `why` values that mean pure hood-proximity: `same-hood-ledger`/`ledger-resident`; everything else — `assignment` or a desk's own label like `feed-names` — traces to the same `story.ref` citation and counts). Guarded: no evidence when `story.ref` is empty/the literal `'assignment'` placeholder (editor-only stories with no engine event), or when the candidate has no resolvable pop. `livedExperiencePacketV2.js` (the LIVE contract — all 22 packages run v2) clones V1's packet and merges into `exposure` without dropping the new key, so the fix reaches production through the same path it already used. Verified: `hasLivedEvidence`/`validateInterviewOutput` now correctly accept `lived-context` basis for an evidenced candidate and reject it for a proximity-only one (full roundtrip test); smoke-tested against a real live C105 desk_signal anomaly (POP-00136, a genuine `Neighborhood_Map` row citation). 6 new assertions in `livedExperiencePacket.test.js`, 4 in `livedExperiencePacketV2.test.js`(+the interview-contract roundtrip) — all adjacent suites (V1, V2, interview-contract ×2, wake-packages) still green. **Deferred, not this cut:** several desk-slice builders attach their own `why` labels (Jax `stink-pool`/`bond-hop from interview pool`, P Slayer `feed-names`) that this fix already treats as addressable by construction (anything but the two proximity markers) — that inclusion is verified correct for the civic/business paths traced here but each desk's own `why` semantics haven't been individually audited; flag if a citizen ever claims `lived-context` off a label that turns out NOT to trace to a real `story.ref` event.** | done-pending-archive | research-build | [[../adr/0017-typed-lived-experience-packets]] §1 |
