...
- **Landed 2026-09-06 (S428, engine-sheet).** `utilities/popIdAllocator.js` (`nextPopIdLocked_`, `persistPopIdHighWater_`, pure `popIdNext_` core) + `lib/sheets.js` (`getPopIdHighWater` / `setPopIdHighWater` / `nextPopIdNumber` — the Node half `require`s the same pure core, so the two artifacts cannot drift). Six cycle-path minters now call `nextPopIdLocked_(ctx)`; the mark persists at `Phase10-PopIdHighWater` (before `Phase10-CommitLedger`, both entry points) through the World_Config cache flush the cycleCount bump uses. Node minters (`integrateFaithLeaders`, `integrateCelebrities`, `seedYouthBalance`, `ingestFemaleCitizensBalance`, both engine57 backfills, `canon3_backfill_t9`) fold the mark in and persist after their append; `ingestPublishedEntities` only prints. Drift from this plan found on landing: `processIntakeV3.js` no longer exists (nothing to convert); `getMaxPopId_` had zero callers → deleted; `maintenance/repairSimulationLedger.js` is claspignored Apps-Script menu code that mints from a row count — left as is, not deployed. `scripts/popIdAllocator.test.js` (8 cases, incl. a source scan that fails if any cycle-path file formats a POPID from a local counter).
...
- **Landed 2026-09-06 (S428).** `utilities/resolveCitizen.js` (pure core over header+rows arrays) + `lib/resolveCitizen.js` (requires the core, supplies both tabs; absent tab = empty archive) — `scripts/resolveCitizen.test.js` 13/13 runs the same fixtures through the clasp-loaded core and the Node export. `dumpLedger.js` writes `output/citizen_archive_snapshot.jsonl` + `.meta.json` when the tab exists; live run proved the skip path (tab absent, flag not 1 → active snapshot written, `archiveRowCount: null`, exit 0). `canon-name-check.js` `loadRows()` unions the archive snapshot (rows tagged `_archived`). `buildCitizenCards.js --from-archive` = the archive card set ONLY — newest snapshot per POPID, a POPID back on Simulation_Ledger skipped (its live row owns the card); positional A–BC, header-checked; no `--apply` here. (First cut was a union; corrected the same night per §Consumer classification — a union under `--apply` is the 964-card burst and double-cards a restored citizen.) **Deferred:** dashboard Layer 1b (`dashboard/server.js` carries another lane's uncommitted hunk tonight; a path-specific add would sweep it) and `queryLedger.js` / `healthCauseIntake.js` resolver hops (Commit 12).
...
- **Landed 2026-09-06 (S428).** `archiveCitizenExits_` in `utilities/archiveCitizenExits.js`, wired at both entry points after `Phase11-BusinessArchive`; `requireTab_` inside the flag gate; header contract asserted before any move (SL header + meta = archive header, else throw, nothing moved); one block append → read-back → `deleteRows` on contiguous runs bottom-up (a failure leaves Simulation_Ledger intact rather than half-rewritten); skips malformed POPIDs, any POPID with an Active twin, and any POPID that appears more than once on the sheet (`duplicate-in-batch` — no row of it moves, so the (POPID, ExitCycle, ArchiveReason) key stays unique; live has 0 duplicates today); same-cycle re-fire dedupes on the same key; mark bump via `persistPopIdHighWater_` rides the end-of-cycle cache flush (Phase 11 runs before the `finally` flush at both entry points). `CITIZEN_ARCHIVE_DIAG` → fire JSON `citizenArchive`. `scripts/archiveCitizenExits.test.js` 9/9. Guards checked, not added: `updateHeritage_` MembersList miss is `continue` (L1571); `householdFormationEngine` HeadOfHousehold writes are Household_Ledger rows, never an SL mint; bond sites `continue` on a missing person. **Wave A bench proof (C108 on @61, Commits 1–4, flag 0): 8 mints POP-01107…01114, `popIdHighWater` 1106 → 1114 persisted, 0 Engine_Errors, archive empty.**
...
- **Landed 2026-09-06 (S430).** `citizenArchiveLatestByPop_(ctx, slHeader)` in `utilities/archiveCitizenExits.js` is the one read-side view: newest exit per POPID, re-shaped by column name to the caller's ledger header (a ledger that grew a column after the exit still indexes), `getSheetByName` only (absent tab → `{}`), cached on `ctx` for the cycle. `updateHeritage_`: a MembersList member missing from the ledger is served from the archive — keeps its place in MembersList and the generation chain (`rowOrArchived` in `genDepth`), never living, never in TotalNetWorth (`archivedPop` guard beside `living()`), counted in `results.archivedMembers`; a member on neither tab is dropped and counted in `results.membersUnresolved` with a Logger line; a child or spouse of an archived line member still inherits the line. `getCitizenWealth_` miss → snapshot NetWorth/WealthLevel (`archived: true`); `findHouseholdSurvivors_` miss → the snapshot's HouseholdId. Nothing archived is ever pushed onto `ctx.ledger`. Commit 12's GAS half rode along: `healthCauseIntake.js` names an archived miss (`POPID archived (deceased C108), HealthCause not written`). `scripts/archiveHeritageResolve.test.js` 15/15 (narrower re-ordered archive header, two-snapshot newest-wins, tab-absent no-throw); householdReconcile 105/105 unchanged.
...
- **Landed 2026-09-06 (S430, Node half + GAS half).** Node: `scripts/queryLedger.js citizen <popid|name>` on a ledger miss resolves through `lib/resolveCitizen.js` and serves the newest exit snapshot labelled `archived: true` + `exit` + `archiveHistory` (bench-proven on POP-00026 traded-away C107; a true miss still exits 1); `scripts/auditSimulationLedger.js` gains an **Archive integrity** block — Citizen_Archive rows/max vs `popIdHighWater`, duplicate POPIDs on the ledger, POPIDs above the mark, POPIDs on both tabs classified `restored` / `pending-re-archive` / `GHOST-deceased-back-on-ledger` / `on-both-not-return-eligible` (JSON `archiveIntegrity`; live: 0 rows, mark 1083, 0/0/0; bench: 54 rows, mark 1098, 0/0/0); `dashboard/server.js` `/api/citizens/:popId` Layer 1b serves the newest `Citizen_Archive` snapshot flagged `_archived` before the cycle-TSV fallback (PM2 restarted; live active + miss unchanged). GAS: `healthCauseIntake.js` — see Commit 6. `dashboard/server.js` was clean of the other lane's hunk by then, so the Commit 4 deferral closed here too.
...
- **Landed 2026-09-06 (S430) on the stated default — Open Q5 stays open, nothing SchoolQuality-shaped was written.** Repair: the 6 Traded rows' `MigrationIntent` cells cleared on live (dry-run → apply → read-back; the S380 Task-2 class, which had covered retired/deceased only) and the writer-side fix that S380 left open — `migrationTrackingEngine.js` skips `traded` rows in the intent writer and the three risk/alert/hook loops, so the cells stay clear until the move. `ArchiveNote`: `citizenExitDefects_` + `citizenArchiveNote_` in `utilities/archiveCitizenExits.js` — one pure classifier over the row's own cells, used by the mover (`citizenArchiveRow_` stamps `defects-at-exit: <classes>`) and by the Commit 7 inventory (one rule). CareerStage spelling and SchoolQuality are named in the note, never repaired at exit (engine.82 / engine.117 own them; a returning player carries the note back). Zero deletes. Inventory after: 49 of 53 carry a defect (44 spelling, 35 SchoolQuality, 1 employer), 4 clean. Bench @65 C109: an organic death (POP-00473) left with `defects-at-exit: schoolQualityUnusable`, 0 errors. PROD @63. Tests: `citizenExitDefects.test.js` 7/7, mover 10/10, migration 43/43.
...
- **Landed 2026-09-06 (S428).** `restoreCitizenPlan_` (pure planner in `utilities/archiveCitizenExits.js`, clasp-deployable, no cycle-path caller yet — nothing on the cycle path sets a traded POPID Active, so the operator is the only caller and Node is the surface that can be round-tripped on the bench) + `scripts/restoreCitizen.js --popid --cycle --sheet [--apply] [--live]` (flip when the Traded row is still on SL; append the latest ReturnEligible snapshot with `Status=Active`, `ReturnedCycle`, `LastUpdated`, health cells cleared, `MigrationDestination` cleared for a canon hood, a `[Return]` LifeHistory line, one LifeHistory_Log row; read-back = exactly one Active row; archive rows retained). Fail-loud: deceased (SL or latest exit), missing everywhere, duplicate SL rows, non-Traded SL states, layout drift (SchemaVersion wider than SL, or any renamed column inside the snapshot width). `sportsFeedWriter.js` call-up on a POPID missing from SL now says "returns through restore, not a roster action". Ripple row deferred (no source hash on the exit; LifeHistory_Log carries the return). `scripts/archiveCitizenRestore.test.js` 7/7; COL_MAP `ReturnedCycle` names its first writer.

### Commit 10 — Sandbox proving (including clone of eligible rows)

- **Landed 2026-09-06 (S428, SANDBOX 0831 @61/@62, C108–C111).** Bench seeded from live C105 + engine.119 proofs (SL 991 rows, max POP-01106). Sequence and read-backs:
  - **Wave A — C108 on @61 (Commits 1–4, flag 0):** 8 mints POP-01107…01114, `popIdHighWater` 1106 → 1114 persisted through the cache flush, 0 Engine_Errors, archive empty.
  - **Wave B — C109 on @62 (Commit 5, `citizenArchiveEnabled=1`):** fire JSON `citizenArchive {candidates 56, archived 56, skipped 0, remaining 950}`; Citizen_Archive 56 rows = 48 `traded-away` (ReturnEligible TRUE) + 8 `deceased` (FALSE), every row `ExitCycle 109` / `SchemaVersion 55`, A–BC verbatim (spot check POP-00026 Allen Lopez, Status cell `Traded` preserved as `LastActiveStatus`); Simulation_Ledger 950 rows, 0 duplicate POPIDs, no archived POPID left on it; Retired / `active` / hospitalized rows untouched; 0 Engine_Errors; mark 1121 (7 more mints).
  - **Restore — `scripts/restoreCitizen.js --popid=POP-00026 --cycle=109 --sheet=<bench> --apply`:** dry run printed the plan, apply appended the snapshot with `Status=Active`, `ReturnedCycle 109`, `MigrationDestination` cleared (Fruitvale is canon), read-back exactly one Active row, archive row retained; a second `--apply` is a no-op; `POP-00331` (deceased) is refused.
  - **After the restore — C110 + C111 (two cycles advanced in the window; one fire was this terminal's, research-build confirmed it was not theirs, no cron targets the bench — source unidentified; both clean, 0 Engine_Errors):** POP-00026 still Active with `ReturnedCycle 109`, one archive snapshot (not re-archived); two natural deaths at C111 (POP-00431, POP-00901) archived at `ExitCycle 111`; archive 58 rows; SL 965; mark 1137. The steady-state mover works on organic exits, not only the backfill batch.
  - **Builder's editor proof on a fresh C105 re-sync (02:49–02:59, execution logs on Drive):** C106 at flag 0 — whole stack @46–@63, 0 errors, carry-forward cold path recovered from the sheet ring, `popIdHighWater → 1091`, archive no-op line; C107 at flag 1 — `54 archived, 0 skipped, 929 remain` (48 traded-away + 6 deceased, one line per POPID, no mismatch), 0 errors, mark → 1098, SL 929 / 0 duplicates, archive 54 rows `ExitCycle 107` / `SchemaVersion 55`. Trail: DEPLOY_HISTORY §Current sandbox.
  - Bench-only after this proof (never replay to live): World_Config `popIdHighWater` / `citizenArchiveEnabled=1` / `citizenArchiveTabLive`, the Citizen_Archive tab and its 58 rows, the 56-row Simulation_Ledger drop, POP-00026's restore row and LifeHistory_Log line, mints POP-01107…01137, C106–C111 memory.
...
- 2026-09-06 (engine-sheet, S430, later) — **Commits 7, 12, 6 landed; PROD @62.** Inventory: 53 live candidates, 0 relational, defects measured (§Commit 7). Commit 12 Node half live on the loop side (queryLedger / audit / dashboard). Commit 6 + the intake miss path bench-proven on SANDBOX 0831 @64 C108 (0 errors, 7 lines rewritten intact) and pushed PROD @62 (pull-back 0 / 169 / 0). Remaining: Commit 8 (Open Q5 = SchoolQuality; recommended default = clear the 6 Traded-row `MigrationIntent` cells as the S380 Task-2 class, `ArchiveNote` the rest, no CareerStage/SchoolQuality writes), then Commit 11 = the builder's flag flip after his live C106 fire.
- 2026-09-06 (engine-sheet, S430) — **Live landed (Commits 2 + 3 live halves, code PROD @61).** `ensureCitizenArchive.js --seed=1083 --apply --live`: World_Config `popIdHighWater` 1083 / `citizenArchiveEnabled` 0 / `citizenArchiveTabLive` 1, `Citizen_Archive` 62-col header, read back exact (81 tabs). `dumpLedger.js` wrote both snapshots (968 / 0). `preMortemScan` SAFE (scan 7 now 0 missing). PROD @61 = the 11-file engine.90 tree, pull-back 0 differing / 169 / 0 tests. Mover is live-wired but off; smoke at flag 0 = `popIdHighWater` climbs with the builder's C106 mints + `citizenArchive` no-op line. Next in order: Commit 7 inventory, Commit 6, Commit 12; Commit 8 waits on Open Questions 5 + 8.
...
- 2026-09-06 (engine-sheet, S428, close-out) — Post-proof true-ups: `--from-archive` corrected to archive-only; in-batch duplicate guard in the mover; collision audit 0 (1,3xx names) after all six commits; `ctx.cache.queueWrite` has no Simulation_Ledger caller (grep empty), so Phase-11 `deleteRows` cannot shift a queued cell. **PROD push gated, not waiting on approval:** `preMortemScan` scan 7 reports `Citizen_Archive` MISSING on live as CRITICAL (exit 1) because `requireTab_` sites are found by literal, not by flag gate — the /deploy pre-flight blocks until the live ensure runs. The ensure is the builder's sheet write; the code push follows it the same hour. Bench moved to @63 (Commit 9 planner + these fixes; no fire needed — no caller).
- 2026-09-06 (engine-sheet, S428) — **Commits 4, 5, 9 landed; Commit 10 bench proving complete** (C108–C111 on SANDBOX 0831 @61/@62: allocator wave, 56-row move, same-POPID restore, two clean cycles after). Live is untouched: the remaining live steps are (1) `node scripts/ensureCitizenArchive.js --sheet=<live> --seed=1083 --apply --live` (mark 1083 + empty tab + `citizenArchiveTabLive=1`, flag stays 0), (2) PROD deploy of the code (mover is a no-op at flag 0), (3) Commit 7 dry-run inventory, (4) Commits 6/8/11/12 per the PR order — Commit 11's live flag flip is the builder's call after 7 and 8.
