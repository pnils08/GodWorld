...
| engine.162 | ENGINE-clock gate inverted — money + education half SHIPPED undeployed (fb588641): 5 sites cut, CIVIC/MEDIA rejoin the economy, GAME still out via `isSportsLayerRow_` (Direction pt 12); E1 career derivation deliberately kept. Live effect 13 raises, 0 lowered. OPEN: the $100k GAME intake floor — no live engine path writes ClockMode='GAME', door unknown, back to builder | in-progress | engine-sheet | [[../plans/2026-08-29-employment-system-cascade]] §Direction fourth pass |
...
| civic.31 | Campaign challengers have a third door the builder did not describe: `mintOutOfTownChallenger_` invents a person from two name arrays + a hash when Generic_Citizens has no fit, and `pickCampaignChallenger_`'s contract is "always return a challenger". Conflicts with SIM_DOCTRINE 5 (GC is the ONLY door), 9 (they arrive AS THEMSELVES or not at all) and 1 (no quotas). Never fired on live — trigger is approval <40, floor is Ashford 46. Builder ruling needed, not an engine call | needs-info | engine-sheet | `phase05-citizens/updateCivicApprovalRatings.js:1159` + [[../SIM_DOCTRINE]] |
