...
- **S256 neighborhood-roster alignment: BUILT (committed `e9f06c8`), NOT DEPLOYED.** Live set-difference found 205 citizens in untracked hoods — citizen-residence (CANON_12) and the map writer roster had drifted. Added Lake Merritt (98)/Uptown (89) + KONO profile (12, was in roster but profileless → silently skipped every cycle = ES-2 G-S6 structural) + Baylight District (INIT-006 Coliseum redevelopment, forward-looking) to `phase08 v3NeighborhoodWriter.js`; reconciled `lib/canonNeighborhoods.js` (MAP_17→MAP_NEIGHBORHOODS, 21). Live Neighborhood_Map stays 17 rows until **phase08 deploys at C97** (new rows populate the cycle after). 6 stray col-T ledger values reassigned to canon LIVE (0 strays remain). **Districts left blank for Lake Merritt/Uptown/Baylight by design** (writer canon-only map; auditor flags blanks) — needs canon district anchors in INSTITUTIONS §Neighborhoods (operator/civic decision). **REMAINING — half-built, Mike-flagged S256:** the roster+profile add is NOT the full wiring. The 4 new hoods must also be (a) given council districts in `lib/districtMap.js` + the writer's `NEIGHBORHOOD_DISTRICT_MAP` (paired with the INSTITUTIONS canon-anchor decision), and (b) audited into the ~30 files carrying hardcoded hood literals. Triage from the S256 scope grep: engine.33 CORE is data-driven (neighborhoodPulseMap / loadNeighborhoodState / runConductEngine / buildCityEvents / citizenDialMap = 0 hardcoded → already generic); the work is the hardcoded surfaces — `utilities/ensureNeighborhoodDemographics.js`, `phase02 applyWeatherModel.js` + `applyCityDynamics.js` (cluster defs), phase05 citizen engines (household/career/neighborhood/civic). Own-session audit, each clasp-gated. **Incident (repaired):** first stray-reassign pass wrote to col U (HouseholdId) not T — `updateRangeByPosition` startCol is 0-indexed, passed 1-based; caught by post-write verify, HouseholdId restored from Household_Ledger (invariant validated), redo guarded on column-letter assertion. Deploy: rides the C97 clasp window with engine.31/.33.
