...
- **(a′) `Undocked_Draw` tab contract — PUBLISHED 2026-09-08 (engine-sheet, S438; builder go). Build target for both halves.**
  ...
  - **Status 2026-09-08 (S438b):** engine side CUT — `undockedDrawCast_` (draw + tab append + `S.undockedNextCast`), `casinoResolveUndocked_` aggregated pilot-cycle resolve with cycle guards (before = carry, after = void), placement keys `c<N+1>:<POPID>`, legacy episode-keyed slips still resolve. Unit: casino suite 50 checks green. Bench next.
