...
### Phase 39: Editorial Review Layer Redesign (from MIA + Microsoft UV + Mezzalira) — 39.1–39.7 + 39.3 DONE S147; 39.8/9/10 REMAIN

**S147 closure:** Spine step 6 shipped. Full detail in `ROLLOUT_ARCHIVE.md` (S147 entry) and `docs/engine/archive/PHASE_39_PLAN.md`. The three-lane chain (Rhea Sourcing / cycle-review Reasoning / Mara Result Validity) + capability gate + Final Arbiter is live and wired into `/write-edition`. Two-pass hallucination works with a two-tier canon context (authoritative sheet rows first, derived docs second). Remaining work below: 39.8 reward-hacking scans + OOD validation, 39.9 tiered review, 39.10 adversarial review skill. Those three constitute spine step 7.
...
Source: `/tmp/hermes-agent/hermes-already-has-routines.md` + `docs/engine/archive/ROLLOUT_PLAN.md` current scheduled agents (Mara sync, code review, bay-tribune audit, local repo hygiene, supermemory tagger).
