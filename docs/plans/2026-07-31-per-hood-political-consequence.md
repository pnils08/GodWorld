...
- **Status:** [x] SHIPPED 2026-08-02 (engine-sheet) — `phase02-world-state/commuteFlowEngine.js` (new, wired `Phase2-CommuteFlows` at both entry points, before its consumers). All three consumers wired: ridership weighted by inbound commuters (+30% cap), disruption `affectedHoods` expanded to upstream hoods (≥3 workers), daytime retail/traffic lift in `applyCityDynamics` (+12%/+8% cap). `scripts/testCommuteFlows.js` — 20 assertions incl. determinism; mutation-tested ×3. Coverage is honest: unresolved employers counted in `S.commuteFlowStats`, never guessed onto the diagonal
...
- **Status:** [x] SHIPPED 2026-08-02 (engine-sheet) — migration publishes `S.relocationPressureDeltas`; `neighborhoodTrajectoryEngine` (the column's only writer) folds them into its existing pressure computation, one `relocation-pressure` ripple row per batch, consume-and-clear. One-cycle lag is structural (Phase5-Trajectory runs BEFORE Phase5-MigrationTracking) and honest: people move, then the block feels it. `scripts/testHousingSupplyResponse.js` — 12 assertions, mutation-tested ×2. **Found + fixed while testing:** at 1dp rounding the −0.05 departure delta rounded away entirely, so a hood losing one household per cycle never eased — pressure now rounds to 2dp (no consumer assumed 1dp)
...
- 2026-08-02 (engine-sheet) — Track C Tasks 9–10 SHIPPED: commute matrix + 3 consumers, housing-supply response. All builds done; live-cycle proof pends next engine fire.
